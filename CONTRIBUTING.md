...
- Redis 7+
- **On Windows:** Docker Desktop with the WSL2 backend, and a bash for the
  `.sh` gates — WSL2 or Git Bash. See
  [`DEPLOYMENT_GUIDE.md` → Windows hosts](DEPLOYMENT_GUIDE.md#windows-hosts)
  and set `core.autocrlf input` **before** cloning.
...
# 2. Bring the stack up — see README.md Quick Start for the steps in full
...
git clone https://github.com/<your-fork>/atom-network-platform.git
cd atom-network-platform
Copy-Item .env.example .env; Copy-Item backend\.env.example backend\.env
docker network create certagent_cert-net
docker compose up -d --build
docker compose exec backend python seed.py
```
...
fix(guardrails): correct PII regex for national-ID formats
...
```powershell
# PowerShell — the hygiene gate has no PowerShell port; run it through a bash
wsl bash scripts/ci/hygiene-check.sh           # or from Git Bash
docker compose run --rm backend pytest
cd frontend; npm run lint; npm run build       # `&&` is PowerShell 7+ only
```

#### If you run pytest any other way, read this first

`docker compose run --rm backend pytest` works because compose mounts the repo
and supplies the environment. Reach for a bare `docker run` against the backend
image and you will get **forty failures that are not real**. All three causes
are silent and all three look like code defects:

| symptom | cause |
|---|---|
| ~15 failures with `FileNotFoundError` on a `/backend/...` path | Those tests resolve the repo root as `Path(__file__).parents[3]`. Inside the image the tests live at `/app/tests/...`, so that expression is `/`. They can only run from a checkout — mount the repo and run from `<repo>/backend`. |
| ~16 failures/errors in `tests/docgen/` and `test_prompt_snapshot` | `app/docgen/config.py` calls `ensure_dirs()` at **import** time, and the gitignored `backend/.env` sets `ARTIFACTS_DIR` to a developer's host path. A path that does not exist in the container becomes an `ImportError` for every test that transitively imports docgen. Pass `-e ARTIFACTS_DIR=/tmp/artifacts`. |
| collection errors: `ModuleNotFoundError: No module named 'psycopg'` | Using a `postgresql+psycopg://` DATABASE_URL. The image ships the default driver; the app's own URL is plain `postgresql://`. Forcing the psycopg3 dialect asks for a package that is not installed. |

The working invocation, if you need one outside compose:

```bash
docker run --rm \
  -e DATABASE_URL='postgresql://u:p@localhost:5432/d' \
  -e ARTIFACTS_DIR=/tmp/artifacts \
  -v "$PWD:/repo" -w /repo/backend \
  --entrypoint sh atom-network-platform-backend:latest \
  -lc 'python -m pytest tests/ -q'
```

Two tests still fail under it and are environment-bound, not defects:
`test_cert_assertions` needs the 55-schema XSD corpus, and
`test_a2a_client_protocol_switch` needs `test-partner.local` to resolve past the
SSRF guard.

`&&` between commands needs PowerShell 7; on Windows PowerShell 5.1 use `;`,
which runs the next command regardless of the previous one's exit code — check
each result rather than trusting the chain.
...
- **Do not add domain knowledge to `core/`.** Anything that knows about a
  specific ecosystem belongs in a domain pack. `backend/tests/core/` asserts
  this for the core modules themselves; `app/prompts/**` is NOT yet covered by
  that check, so prompt text is on you to review.
