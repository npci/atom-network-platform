...
AtOM — **Agentic Orchestration & Messaging** — is a full-stack,
...
- ✅ **Certification** — an A2A conversation with a partner's certification agent,
  driving wire-level test cases through simulators
...
- **An LLM provider key.** The stack starts without one, but every AI feature
  fails on authentication. Set the key matching your `LLM_PROVIDER` —
  `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`, `AINXT_API_KEY`, or
  none at all for a local `ollama`. Step 2.
...
The stack is Linux containers and runs unchanged; the shell differs. Two
prerequisites before the Quick Start, both covered in full in
[`DEPLOYMENT_GUIDE.md` → Windows hosts](DEPLOYMENT_GUIDE.md#windows-hosts):

- **Docker Desktop with the WSL2 backend**, given ≥8 GB RAM and 15 GB disk.
- **`git config --global core.autocrlf input` before cloning.** The
  entrypoint scripts are `#!/bin/sh`; checked out with CRLF they make the
  containers exit with `exec /docker-entrypoint.sh: no such file or
  directory` — naming a file that is right there. `.gitattributes` pins
  `*.sh` to LF, and this covers the rest.

Cloning inside a WSL2 distro lets you use every command below verbatim, and is
the recommended path. Working from PowerShell also works — the guide carries a
PowerShell variant of every block, and a Windows-only troubleshooting table.
...
```bash
# backend/.env
LLM_PROVIDER=anthropic          # anthropic | openai | gemini | ainxt | ollama
ANTHROPIC_API_KEY=sk-ant-...

# ...or pick a different provider and set that one's key instead:
# LLM_PROVIDER=openai
# OPENAI_API_KEY=sk-...
```
...
Prints the generated password once. Set `ADMIN_EMAIL` and `ADMIN_PASSWORD` in
`backend/.env` beforehand to choose your own; re-running is a no-op if the user
exists.

`exec` uses the environment the container was created with, so if you edit
`backend/.env` *after* `docker compose up -d`, run `docker compose up -d
backend` to recreate — or pass it inline with `docker compose exec -e
ADMIN_PASSWORD='<yours>' backend python seed.py`. Otherwise `seed.py` silently
falls back to a generated password. Only `seed.py` reads these; the running
backend does not.
...
```powershell
# PowerShell — $(pwd) is ${PWD}, and the continuation is a backtick
docker compose run --rm -v "${PWD}/backend/scripts:/app/bscripts" backend `
  python /app/bscripts/reset_password.py <admin-email> --password '<new>'
```
...
The two `.sh` gates need a bash and have no PowerShell port. From Windows, run
them through `wsl bash scripts/ci/...` or Git Bash — or skip them locally and
let CI report. `pytest` runs in the container and is identical everywhere.
...
wiki/                 Architecture, workflow phases, retrieval, A2A wire
```
...
- [`CONFIGURATION.md`](CONFIGURATION.md) — settings, environment variables, and
  domain pack selection
- [`DEPLOYMENT_GUIDE.md`](DEPLOYMENT_GUIDE.md) — bring-up and operational runbook
...
- **Output quality is measured, not assumed** — but the golden-output suite
  ships with two captured cases, covering two of the 73 tracked prompts. The
...
| **Windows:** `exec /docker-entrypoint.sh: no such file or directory` | CRLF line endings on a `#!/bin/sh` script. See [Windows hosts](DEPLOYMENT_GUIDE.md#windows-hosts). |
| **Windows:** nginx cannot bind port 80 | IIS or a Hyper-V reserved range owns it. Same section, plus a full [Windows-only table](DEPLOYMENT_GUIDE.md#windows-only). |
...
AtOM is a single-vendor open-source project: copyright is held collectively by
the contributors, but one sponsoring organisation employs the maintainers and
makes the final call. See [GOVERNANCE.md](GOVERNANCE.md) for the full governance
model, maintainer list, and decision-making process.