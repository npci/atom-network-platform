...
Five. `LLM_PROVIDER` accepts `anthropic`, `openai`, `gemini`, `ainxt` or
`ollama`, and defaults to `anthropic`. (`claude` is still accepted as a
backward-compatible alias for `anthropic`.) Every call goes through
`backend/app/core/llm.py`, which dispatches on that setting; agents never call a
vendor SDK directly.

`ainxt` is not a hosted service you can sign up for — it is the adapter for an
OpenAI-compatible gateway you run yourself, pointed at by `AINXT_BASE_URL`.

Switching provider is a one-line change for text generation, but it is **not**
true that every feature works on every provider. The matrix below is the honest
answer; read it before choosing.

### Provider capability matrix

All five providers are first-class for text generation. Two capabilities are
built on the Anthropic `/v1/messages` tool-use protocol and are reachable only
on providers that speak it.

| Capability | `anthropic` | `openai` | `gemini` | `ollama` | `ainxt` |
|---|:---:|:---:|:---:|:---:|:---:|
| Text completion (`call_llm`) | ✅ | ✅ | ✅ | ✅ | ✅ |
| Streaming (`stream_llm`) | ✅ | ✅ | ✅ | ✅ | ✅ |
| Structured output | ✅ | ✅ | ✅ | ✅ | ✅ |
| Document generation, retrieval, evaluation | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Agentic code generation** (native tool loop) | ✅ | ❌ | ❌ | ❌ | ⚠️ |
| **Figure/diagram understanding** (vision) | ✅ | ❌ | ❌ | ❌ | ⚠️ |

⚠️ `ainxt` supports the tool loop and vision **only** when
`AINXT_COMPAT_MODE=anthropic`. In its default `openai` mode it behaves like
`openai` in this table.

Structured output uses forced tool-use where the protocol supports it and
prose-JSON parsing everywhere else; both paths are tested.

**What happens when a capability is missing.** Agentic code generation refuses
at preflight, naming the provider and the setting to change — before it clones a
repository or builds a workspace. Vision logs one warning per process and skips
the figures, so an upload still succeeds but embedded diagrams do not reach the
pipeline and the generated document is correspondingly thinner.

**Output budgets are clamped per model, not per vendor.** Agents request an
ambitious `max_tokens`; `app/core/tokens.py` resolves the real cap for the
active model (falling back to the provider's) and clamps to it. A model with a
16k output cap no longer hard-fails on an agent written against a 64k one.
Context windows resolve the same way — add unlisted models to
`_MODEL_CONTEXT_WINDOW` so budgeting stays accurate.

Embeddings are a separate concern and currently require Ollama regardless of
`LLM_PROVIDER`.
