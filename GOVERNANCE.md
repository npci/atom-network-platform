...
**Single-vendor open source.** Copyright is held collectively by the
contributors as "The ATOM Authors" (see `LICENSE`), but the project is not
governed collectively: one sponsoring organisation employs the maintainers and
makes the final call on scope, architecture and releases. The maintainers are
listed in [CODEOWNERS](CODEOWNERS), which is the authoritative answer to "who
decides".
...
governance and the enforced governance were different things.
