...
On Windows the interpreter is `python`, not `python3` — or generate it inside
the container, which needs no host Python at all:
...
| **LLM provider** | `LLM_PROVIDER`, per-provider keys, model routing | `anthropic` \| `openai` \| `gemini` \| `ainxt` \| `ollama`; default `anthropic` |
...
`LLM_PROVIDER` selects one of five providers, and all five are first-class for
text generation. Two capabilities are built on the Anthropic `/v1/messages`
tool-use protocol and are reachable only on providers that speak it — the
platform fails fast and names the setting to change rather than discovering
this mid-run.
...
active model (falling back to the provider's) and clamps to it, so an agent
written against a 64k-output model does not hard-fail on a 16k one. Context
windows resolve the same way — a model absent from `_MODEL_CONTEXT_WINDOW`
falls back to its **provider's** window, not to any one vendor's. Add unlisted
models to that map to keep budgeting accurate.
