...
> `backend/app/a2a_common/` — every code below was read out of the middleware
> that emits it, not transcribed from an older document.
...
| **mTLS** | Bearer JWT **and** a pinned client certificate | High-assurance partners |
...
editable source, and `MANIFEST` vendors each of them into a single tree in this
repository — `backend/app/a2a_common`. The partner platform holds its own copies
in its own repository, where this gate cannot see them. Editing a vendored copy
is wasted work: the sync overwrites it, and the hygiene gate fails on drift
meanwhile.
...
- The protocol these layers carry, and how the wire code is shared:
