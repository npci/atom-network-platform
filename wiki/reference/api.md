...
Re-derive the figures below when the source changes, and update the stamp:
    grep -rhoE '@router\.(get|post|put|patch|delete)\("[^"]+"' \
      backend/app/api/*.py | sort -u                        # route list
-->
# HTTP API reference

> **Derived** from `the FastAPI OpenAPI schema`, against alembic head `0141_rename_remaining_columns`.
> Hand-derived from the source tree — re-derive and re-stamp when it changes
> (see the comment at the top of this file).

349 operations across 37 tags, taken from the running application rather than from the router source -- so anything mounted conditionally appears exactly as it is actually served.
...
| `GET` | `/api/admin/a2a-logs/{message_id}/integrity` | Check A2A Message Integrity |
...
| Method | Path | Summary |
|---|---|---|
| `GET` | `/api/cert/flows/{cflow_id}/report` | Cert Flow Report |
| `GET` | `/api/cert/flows/{cflow_id}/rounds/{run_number}/diff` | Cert Round Diff |
...
| Method | Path | Summary |
|---|---|---|
| `GET` | `/api/integration-testing/exchanges` | List Exchanges |
| `DELETE` | `/api/integration-testing/{partner_id}/{alias}/{target_path}` | Tunnel Exchange |
| `GET` | `/api/integration-testing/{partner_id}/{alias}/{target_path}` | Tunnel Exchange |
| `HEAD` | `/api/integration-testing/{partner_id}/{alias}/{target_path}` | Tunnel Exchange |
| `OPTIONS` | `/api/integration-testing/{partner_id}/{alias}/{target_path}` | Tunnel Exchange |
| `PATCH` | `/api/integration-testing/{partner_id}/{alias}/{target_path}` | Tunnel Exchange |
| `POST` | `/api/integration-testing/{partner_id}/{alias}/{target_path}` | Tunnel Exchange |
| `PUT` | `/api/integration-testing/{partner_id}/{alias}/{target_path}` | Tunnel Exchange |
...
| `POST` | `/api/changes/{change_id}/phase-b/test/trigger` | Trigger Uat Tests |
| `GET` | `/api/changes/{change_id}/phase-b/triage/latest` | Get Latest Triage |
| `POST` | `/api/changes/{change_id}/phase-b/triage/run` | Run Phase B Triage |
...
| `POST` | `/api/changes/{change_id}/partners/{partner_id}/cert/dispatch` | Dispatch Certification |
...
| `GET` | `/api/changes/{change_id}/partners/{partner_id}/cert/txns` | Cert Txns |
...
| Method | Path | Summary |
|---|---|---|
| `POST` | `/api/sim/execute` | Execute |
| `GET` | `/api/sim/packs` | List Packs |
| `POST` | `/api/sim/packs` | Store Pack |
| `POST` | `/api/sim/packs/build` | Build From Change |
| `GET` | `/api/sim/packs/capabilities` | Capabilities |
| `DELETE` | `/api/sim/packs/{pack_ref}` | Withdraw Pack |
| `GET` | `/api/sim/packs/{pack_ref}` | Get Pack |
| `GET` | `/api/sim/packs/{pack_ref}/diff` | Diff Pack |
| `GET` | `/api/sim/packs/{pack_ref}/effective` | Effective Pack |
| `POST` | `/api/sim/packs/{pack_ref}/publish` | Publish Pack |
...
| `POST` | `/api/agentic/runs/{run_id}/decide-schema-amendment` | Decide Schema Amendment |
| `POST` | `/api/agentic/runs/{run_id}/decide-tsd-approval` | Decide Tsd Approval |
