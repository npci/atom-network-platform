...
Re-derive the figures below when the source changes, and update the stamp:
    grep -rhoE '__tablename__ = "[^"]+"' backend/app/models/*.py \
      | sort -u | wc -l                                     # table count
-->
# Data model

> **Derived** from `SQLAlchemy `Base.metadata``, against alembic head `0141_rename_remaining_columns`.
> Hand-derived from the source tree — re-derive and re-stamp when it changes
> (see the comment at the top of this file).

95 tables at alembic head `0141_rename_remaining_columns`.
...
| `payload_sha256` | `VARCHAR(64)` | yes |  |
| `hmac_signature` | `VARCHAR(64)` | yes |  |
| `hmac_key_version` | `INTEGER` | yes |  |
...
| `tsd_version_locked` | `INTEGER` | yes |  |
| `created_at` | `DATETIME` | no |  |
| `updated_at` | `DATETIME` | no |  |
...
| `coldstore_path` | `VARCHAR(500)` | no |  |
| `original_size_bytes` | `INTEGER` | yes |  |
| `compressed_size_bytes` | `INTEGER` | yes |  |
| `compressed_at` | `DATETIME` | no |  |
| `ready_for_archive` | `BOOLEAN` | no |  |
...
| `requested_action_from_authority` | `TEXT` | yes |  |
| `payload` | `JSON` | yes |  |
| `created_at` | `DATETIME` | no |  |
...
|---|---|---|---|
| `id` | `VARCHAR(36)` | no | PK |
| `cflow_id` | `VARCHAR(64)` | no |  |
| `run_number` | `INTEGER` | no |  |
| `case_id` | `VARCHAR(100)` | no |  |
| `variant_id` | `VARCHAR(36)` | yes | FK -> `cert_request_variants.id` |
| `api_message_id` | `VARCHAR(36)` | yes | FK -> `api_messages.id` |
| `api_field_id` | `VARCHAR(36)` | yes | FK -> `api_fields.id` |
| `field_path` | `VARCHAR(1000)` | yes |  |
| `assertion_kind` | `VARCHAR(20)` | no |  |
| `expected` | `JSON` | no |  |
| `origin` | `VARCHAR(20)` | no |  |
| `wire_format` | `VARCHAR(20)` | no |  |
| `authority_data` | `JSON` | yes |  |
| `created_at` | `DATETIME` | no |  |
| `updated_at` | `DATETIME` | yes |  |
...
| Column | Type | Null | Key |
|---|---|---|---|
| `cflow_id` | `VARCHAR(64)` | no | PK |
| `change_request_id` | `VARCHAR(36)` | no |  |
| `partner_id` | `VARCHAR(36)` | no |  |
| `phase` | `VARCHAR(30)` | no |  |
| `current_round` | `INTEGER` | no |  |
| `history` | `JSON` | no |  |
| `halted_reason` | `VARCHAR(200)` | yes |  |
| `created_at` | `DATETIME` | no |  |
| `updated_at` | `DATETIME` | yes |  |
...
| `run_number` | `INTEGER` | no |  |
| `case_id` | `VARCHAR(100)` | no |  |
| `variant_id` | `VARCHAR(64)` | no |  |
| `api_message_id` | `VARCHAR(36)` | yes | FK -> `api_messages.id` |
| `initiator` | `VARCHAR(10)` | no |  |
| `wire_format` | `VARCHAR(20)` | no |  |
| `input_data` | `JSON` | yes |  |
| `fixture_ref` | `VARCHAR(200)` | yes |  |
| `expected` | `JSON` | no |  |
| `strategy` | `VARCHAR(30)` | no |  |
| `covered_rules` | `JSON` | yes |  |
| `is_negative` | `BOOLEAN` | no |  |
| `fault_key` | `VARCHAR(200)` | yes |  |
| `provenance` | `JSON` | yes |  |
| `created_at` | `DATETIME` | no |  |
| `updated_at` | `DATETIME` | yes |  |
...
| `dispatched_by` | `VARCHAR(20)` | yes |  |
| `previous_run_id` | `VARCHAR(36)` | yes |  |
| `fix_notification_message_id` | `VARCHAR(36)` | yes |  |
| `coverage` | `JSON` | yes |  |
| `pack_ref` | `VARCHAR(120)` | yes |  |
| `pack_id` | `VARCHAR(80)` | yes |  |
| `authority_mode` | `VARCHAR(20)` | yes |  |
| `partner_mode` | `VARCHAR(20)` | yes |  |
...
| `description` | `TEXT` | no |  |
| `authority_resolution_text` | `TEXT` | yes |  |
...
|---|---|---|---|
| `id` | `VARCHAR(36)` | no | PK |
| `exchange_id` | `VARCHAR(64)` | no |  |
| `direction` | `VARCHAR(10)` | no |  |
| `alias` | `VARCHAR(100)` | no |  |
| `method` | `VARCHAR(10)` | no |  |
| `path` | `VARCHAR(1000)` | no |  |
| `query` | `VARCHAR(1000)` | yes |  |
| `status` | `INTEGER` | yes |  |
| `error_code` | `VARCHAR(40)` | yes |  |
| `request_bytes` | `INTEGER` | no |  |
| `response_bytes` | `INTEGER` | no |  |
| `elapsed_ms` | `INTEGER` | yes |  |
| `dropped_headers` | `JSON` | yes |  |
| `correlation_id` | `VARCHAR(64)` | yes |  |
| `cert_context` | `JSON` | yes |  |
| `created_at` | `DATETIME` | no |  |
| `updated_at` | `DATETIME` | yes |  |
...
| `cert_agent_partner_id` | `VARCHAR(50)` | yes |  |
| `status` | `VARCHAR(9)` | no |  |
| `agent_card_url` | `VARCHAR(1000)` | yes |  |
| `metadata` | `JSON` | yes |  |
| `protocol_version` | `VARCHAR(20)` | no |  |
| `jwt_signing_secret` | `TEXT` | yes |  |
| `signing_secret` | `TEXT` | yes |  |
...
| `previous_jwt_signing_secret` | `TEXT` | yes |  |
| `previous_signing_secret` | `TEXT` | yes |  |
| `secret_rotated_at` | `DATETIME` | yes |  |
| `signing_secret_version` | `INTEGER` | no |  |
| `created_at` | `DATETIME` | no |  |
| `updated_at` | `DATETIME` | yes |  |
...
| `phase_b_run_id` | `VARCHAR(36)` | no | FK -> `phase_b_runs.id` |
| `build_run_id` | `VARCHAR(36)` | yes | FK -> `build_runs.id` |
| `uat_test_run_id` | `VARCHAR(36)` | yes | FK -> `uat_test_runs.id` |
| `report` | `JSON` | yes |  |
| `walkthrough` | `JSON` | yes |  |
| `created_by` | `VARCHAR(36)` | yes |  |
| `created_at` | `DATETIME` | no |  |
...
| `regulatory_compliance` | `TEXT` | yes |  |
...
|---|---|---|---|
| `id` | `VARCHAR(36)` | no | PK |
| `pack_ref` | `VARCHAR(120)` | no |  |
| `pack_id` | `VARCHAR(80)` | no |  |
| `target` | `VARCHAR(500)` | no |  |
| `response_status` | `INTEGER` | yes |  |
| `echoed_pack_id` | `VARCHAR(80)` | yes |  |
| `published_by` | `VARCHAR(200)` | yes |  |
| `published_at` | `DATETIME` | no |  |
...
| `pack_id` | `VARCHAR(80)` | no |  |
| `change_request_id` | `VARCHAR(36)` | yes |  |
| `base_pack_ref` | `VARCHAR(120)` | yes |  |
| `engine_min` | `VARCHAR(20)` | no |  |
| `requires` | `JSON` | yes |  |
| `content` | `JSON` | no |  |
| `coverage` | `JSON` | yes |  |
| `status` | `VARCHAR(20)` | no |  |
| `created_by` | `VARCHAR(200)` | yes |  |
