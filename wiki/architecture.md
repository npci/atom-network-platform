...
> **Verified at:** alembic head `0141_rename_remaining_columns`, commit `e116cea`
...
>
> The routing table is checked against `nginx/nginx.dev.conf.template`, which is
> what the compose file mounts. The prefixes below are the defaults: the template
> is rendered at container start with `AUTHORITY_CONTEXT` (default `a2a`) and
> `PARTNER_CONTEXT`, so a deployment that overrides either sees different paths.
>
...
|---|---|---|
| `/` | redirect to `/a2a/` | The bare host lands on the UI |
| `/a2a/` | platform UI | |
...
| `/a2a/.well-known/` | platform backend | The card under the UI prefix, rewritten to the root |
| `/a2a-rpc/` | platform backend | The A2A JSON-RPC wire, unprefixed by design |
| `/api/a2a/` | platform backend | The partner-facing A2A REST surface, unprefixed by design |
...
- **The wire code is generated, not copied.** `hmac_signer.py`, `protocol.py`,
  `executor_base.py` and the two integration-tunnel modules have exactly one
  editable source, `packages/a2a-core/`. Within this repository each lands in
  exactly **one** tree, `backend/app/a2a_common` (see
  [the wire](a2a-wire.md)). Editing a copy is wasted work: the next sync
  overwrites it and the hygiene gate fails on drift meanwhile. Every service
  hashes the same wire bytes, so a one-sided edit silently breaks signatures
...
- **The workflow phases and the security layers in depth** — they have their own
  pages: [workflow phases](workflow-phases.md),
