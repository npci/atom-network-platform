...
progress and readiness, negotiation, and the certification lifecycle. The first
29 — Part A, Part B and the `echo` utility — are the frozen contract; the
remaining eight extend it, and `is_ext()` is what keeps the drift test from
comparing them against the frozen appendix. Two are certification witnessing,
three are the negotiation-round lifecycle (`revision_in_progress`,
`round_opened`, `round_closed`), and three arrived with the integration-testing
tunnel (`http_exchange_request`, `http_exchange_response`,
