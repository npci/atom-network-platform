...
"""Generate the LLM system-prompt catalogue from the prompt files.
...
file-backed prompts:
  ...
  * FULL TEXT for prompts under app/prompts/
...
Output lands in wiki/reference/, alongside the other generated reference pages,
and is committed: `--check` compares the committed copy against a fresh build,
so a prompt change that skips regeneration fails the hygiene gate rather than
shipping a catalogue that disagrees with what the platform sends.
...
APP = BACKEND / "app"
PROMPTS = APP / "prompts"
OUT = ROOT / "wiki" / "reference" / "prompt-catalogue.md"
# Prefix that turns a repo-relative path into one relative to OUT's directory,
# because check-links.sh resolves markdown links from the file they appear in.
OUT_TO_ROOT = "../.."
...
def build() -> str:
    ...
    for p in files:
        ...
        lines.append(f"| [`{rel}`]({OUT_TO_ROOT}/backend/app/prompts/{rel}) | {owners} | "
    ...
    lines.append(
        "\nPrompts assembled inside functions are out of scope by design. A "
        "prompt built from local variables has no stable identity to catalogue "
        "and no body that exists before the call runs; listing the call site "
        "without the text would be an index of things the reader cannot read.\n"
    )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    if "--check" in sys.argv:
        ...
        if current != text:
            print(f"{OUT.relative_to(ROOT)} is stale — regenerate it "
