...
# `syft` on PATH.
#
# Output: artifacts/sbom/*.cdx.json  (CycloneDX 1.6 JSON)
#
# artifacts/ is gitignored: an SBOM is a build output, valid only for the
# closure it was generated from. A committed one is a claim about dependencies
# that stops being true at the next bump, and nothing checks it.
#
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUT_DIR="${REPO_ROOT}/artifacts/sbom"
FROM_IMAGES=false
FROM_VENV=false
case "${1:-}" in
...
echo "    When you submit these, attach a VEX document alongside them. An SBOM"
echo "    lists what is present; it cannot say which findings are exploitable"
echo "    here. Without that analysis every disputed CVE and every advisory with"
echo "    no fix available reads as an open vulnerability."
