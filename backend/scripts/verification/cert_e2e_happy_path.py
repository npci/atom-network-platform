...
# Element names and enum members here are SELF-CONTAINED, not wire values: the
# pack this script publishes at step [2] is built from these very registry rows,
# and step [4] validates against that pack. Nothing outside this file — no
# simulator, no stored corpus — reads them. They may be renamed, but the sample
# XML, the xpaths, the enum and the step-[4] request bodies must move together.
msg = ApiMessage(id=generate_uuid(), api_name="ReqDispute", direction="request",
                 sample_xml='<ReqDispute><Head ver="2.0"/><Txn type="DISPUTE"/></ReqDispute>',
...
db.add_all([
    ...
    ApiField(id=generate_uuid(), message_id=msg.id, position=2, xml_tag="type",
             ...
             mandatory="Y", enum_values=["DISPUTE", "PRE_ARB"],
             introduced_by_change_id=CHG),
])
db.commit()
...
# `initiator` is PINNED: "npci" is the authority role token the harness defaults
# to (cert_pack_run.py) and what the network pack declares as
# `cert_roles.authority`. Renaming it routes the round down the partner leg.
...
check("CertRun stamped with modes", (run1.authority_mode, run1.partner_mode) == ("simulator","simulator"))
...
reply = asyncio.run(runtime.handle(db, body='<ReqDispute><Head ver="2.0"/><Txn type="DISPUTE"/></ReqDispute>',
