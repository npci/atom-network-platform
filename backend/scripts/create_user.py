...
Usage:
    cd /opt/atom-platform/app/backend
    ...
    python scripts/create_user.py --email alice@example.com --password 'NewPass!' --update

    # Generate a random password (printed to stdout once, never again):
    python scripts/create_user.py --email bob@example.com --name Bob --role tech_lead
