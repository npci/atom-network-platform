...
not a test double.
    ...
    ./scripts/dispatch_certification.py <change_id> <partner_id> [--role ROLE]
                                        [--advance] [--modes authority=..,partner=..]
...
def main() -> int:
    ...
    ap.add_argument("--role", default="",
                    help="the role the partner certifies for (scopes the case "
                         "set to that actor's workbook sheet). Accepted values "
                         "are pack-defined — they must match a sheet in the "
                         "ACTIVE domain pack's workbook, e.g. LENDING_LIBRARY "
                         "/ BORROWING_LIBRARY. Naming one domain's roles here "
                         "would send an operator on a different pack looking "
                         "for a sheet that does not exist.")
    ap.add_argument("--advance", action="store_true",
                    help="walk the assignment to CERTIFYING first")
    ap.add_argument("--modes", default="",
                    help="authority=simulator|application,partner=simulator|application")
