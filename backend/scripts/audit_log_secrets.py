...
#
# MUST stay index-aligned with log_buffer._LOG_REDACTIONS. This tuple was one
# entry SHORT — the 5th pattern (credentials in a URL with no `//`) was added to
# log_buffer without a label here, so it reported "unknown-secret" and
# test_every_runtime_pattern_has_a_human_label failed.
_PATTERN_LABELS = (
    "Bearer token",
    "Authorization / API-key / internal-token header",
    "API key / secret / token / password assignment",
    "Credentials embedded in a URL",
    "Credentials in a URL without '//'",
    "Secret in JSON / dict form",
    "Cookie or Set-Cookie header",
    "Bare JWT",
    "Bare vendor API key (sk-ant- / glpat- / ghp_ / AKIA…)",
    "SSH / PEM private key block",
