...
Scripts for the NLLN library-loan fixture repo. The fixture repo is
**operator-supplied** — this repository ships no URL for it and the scripts
assume no default checkout path. Clone your own and export the location before
running either script:

```
git clone <YOUR-FIXTURES-REPO-URL> /path/to/nlln-fixtures
export NLLN_FIXTURES_DIR=/path/to/nlln-fixtures
```

With `NLLN_FIXTURES_DIR` unset both scripts fail immediately with that
instruction rather than churning against a path that does not exist.
