...
# $NLLN_FIXTURES_DIR is a checkout of YOUR OWN fixture repo. There is
# deliberately NO default — the fixture repo is operator-supplied, so an unset
# variable is a hard failure rather than seven checks failing against a path
...
if [ -z "$FIXTURES" ]; then
    ...
    echo "        This sample reads an OPERATOR-SUPPLIED fixture repo; it ships"
    ...
    echo "TESTS: total=0 passed=0 failed=0 skipped=0"
    exit 1
fi
