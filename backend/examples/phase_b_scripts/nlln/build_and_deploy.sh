...
#
# Fixture location: $NLLN_FIXTURES_DIR, a checkout of YOUR OWN fixture repo.
# There is deliberately NO default — the fixture repo is operator-supplied, and
# a wrong guess here would build the wrong contract. Unset => hard failure.
#
...
if [ -z "$FIXTURES" ]; then
    echo "[ERROR] NLLN_FIXTURES_DIR is not set."
    echo "        This sample builds an OPERATOR-SUPPLIED fixture repo; it ships"
    echo "        with no repository URL and no default checkout path."
    echo "        Clone your fixture repo and point the variable at it:"
    echo "            git clone <YOUR-FIXTURES-REPO-URL> /path/to/nlln-fixtures"
    echo "            export NLLN_FIXTURES_DIR=/path/to/nlln-fixtures"
    echo "BUILD FAILURE"
    exit 1
fi
...
[ -d "$FIXTURES" ] || fail "NLLN_FIXTURES_DIR points at a directory that does not exist: $FIXTURES"
