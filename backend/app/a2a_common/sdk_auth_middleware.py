...
Part of the A2A security hardening. This middleware also carries mTLS
fingerprint pinning, CIDR allowlist enforcement and audit metadata
population; HMAC envelope verification lives in the separate outer
middleware (`sdk_hmac_middleware`). Each check is one block; the order
below is fixed to fail fast on the cheapest checks first.
...
class AuthContext:
    ...
    everything that gets persisted on the `A2AMessage` audit row —
    caller_ip, jwt_sub/jwt_iat/jwt_exp, and client_cert_fingerprint
    for mTLS partners.
    """
...
class SdkAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if any(path.startswith(p) for p in self._skip_paths):
            # No auth on agent-card / well-known. A future change may also
        ...
        try:
            ...
            # The refresh-token columns are irrelevant here; this check
            ...
            # ── mTLS enforcement for mTLS-tier partners ──────────────
            ...
            if getattr(partner, "tls_tier", "jwt") == "mtls":
                ...
                if not presented:
                    return _err(
                        401, "mtls_required",
                        "mTLS-tier partner must call via the mTLS ingress (:8443).",
                    )
            ...
            # `partner.rate_limit_rps` has existed for a long time as an
            # "override hook" (see the field's own comment in
            ...
            audit = {
                "caller_ip":          request.headers.get("x-real-ip")
                                       ...
                                       or (request.client.host if request.client else None),
                "jwt_sub":            partner_id,
                # No `iat` in the JWT today. Use the
                ...
                # Populated from nginx's X-Client-Cert-Fingerprint
                # header for mTLS partners.
...
def _ip_in_cidrs(ip: str | None, cidrs: list[str]) -> bool:
    """True iff `ip` falls inside any CIDR in `cidrs`. Used by the
    CIDR allowlist check above.
