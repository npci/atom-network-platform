...
Part of the A2A security hardening. Runs as an ASGI raw middleware
...
@dataclass
class HmacAuditMeta:
    """Closes T2 (non-repudiation). Populated by
...
class SdkHmacMiddleware:
    """ASGI middleware. Verifies X-Auth-Signature on POST/PUT/PATCH (the
    superseded X-NPCI-Signature spelling is still accepted on inbound);
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        # A8 (architecture review: "gateway-only security" is a prohibited
        # anti-pattern, and size limits are required on every inbound
        # request/message, independent of any edge proxy). Reject an
        # oversized body BEFORE it is fully buffered
        ...
        if not secret:
            ...
            # — "configuration validation failures" / "signature failures"
            # MUST be structured telemetry, not just a plain warning log.
        ...
        # Pass the WHOLE header map to `verify`, not a three-key extract.
        #
        # `verify._h()` looks up each name and then its superseded `X-NPCI-*`
        # spelling, which is what lets a counterparty that has not deployed the
        # rename still authenticate. Pre-filtering to the canonical names
        # defeated that entirely: for a request carrying only `X-NPCI-*` the
        # extract held the canonical KEYS with None VALUES, and the legacy names
        # were not in the dict at all, so the fallback had nothing to find and
        # every such request was rejected `missing_envelope_headers`.
        #
        # This is the only verify() call site on this side, so the inbound
        # dual-accept was dead in practice while reading as implemented — and it
        # is what blocked the partner platform, which still signs `X-NPCI-*`,
        # from returning ANY certification result.
