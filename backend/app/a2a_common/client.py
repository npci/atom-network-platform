...
# Observability headers, EMITTED on every outbound call. Both names are
# LEGACY WIRE SPELLINGS and are pinned: unlike the HMAC envelope headers, these
# never got a dual-emit rollout, and a partner's nginx/WAF/APM rules key on the
# literal string. Renaming them here would silently stop correlating deliveries
# in someone else's log pipeline, with nothing in this repo failing.
#
# Built from `hmac_signer.LEGACY_HEADER_PREFIX` so the namespace is retargetable
# in exactly one place, alongside the envelope headers that share it. Giving
# them the same dual-emit treatment is the correct eventual fix; it is a wire
# change and belongs in its own coordinated rollout, not a rename sweep.
# The neutral spellings, ALWAYS emitted. The superseded pair below is emitted
# alongside them while `A2A_EMIT_LEGACY_HEADERS` is on — the same switch that
# governs the HMAC envelope headers, so an operator flips one variable and every
# legacy header stops together.
...
async def send_a2a_message(
    ...
    *,
    ...
    Args:
        ...
        hmac_secret: Optional HMAC envelope secret. When set,
                     attaches `X-Auth-Timestamp / -Nonce / -Signature`
                     headers signing the actual HTTP request body. The
                     superseded spelling (`hmac_signer.LEGACY_HEADERS`) is
                     emitted alongside them and still accepted inbound. None
        ...
        correlation_id: A12 (architecture review Advisory #18, promoted to
                     High) — sent as
                     the `LEGACY_HEADER_CORRELATION_ID` HTTP header, in ADDITION to
        ...
        change_id:   Same rationale, sent as `LEGACY_HEADER_CHANGE_ID` — lets an
    ...
    # request without touching the payload — this is what makes "globally
    # unique correlation/execution IDs" propagated "across sync calls"
    # actionable at the transport layer, not just inside application logs.
    #
    # Dual-emitted since the coordinated rollout: the neutral `X-Auth-*` name is
    # ALWAYS sent, the superseded one rides the same `A2A_EMIT_LEGACY_HEADERS`
    # switch as the envelope headers. Additive, so a partner whose nginx/WAF/APM
    # rules key on the old literal keeps working until they retarget and flip it.
    _legacy = emit_legacy_headers()
    if correlation_id:
        headers[HEADER_CORRELATION_ID] = correlation_id
        if _legacy:
            headers[LEGACY_HEADER_CORRELATION_ID] = correlation_id
    if change_id:
        headers[HEADER_CHANGE_ID] = change_id
        if _legacy:
            headers[LEGACY_HEADER_CHANGE_ID] = change_id

    # Outbound HMAC signing — register an httpx event hook that
    ...
    async with httpx.AsyncClient(
        ...
        # Admin A2A logs UI — capture the response artifact
