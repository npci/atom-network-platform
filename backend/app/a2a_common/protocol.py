...
# The agent id this platform stamps on an outbound message when the caller
# names no more specific agent (see `services/a2a_client.py`).
PLATFORM_AGENT_ID = "authority.platform.v1"
...
LEGACY_PLATFORM_AGENT_ID = "npci.platform.v1"
...
def is_platform_agent(value: str | None) -> bool:
    """True for the current platform agent id and the superseded one."""
    return (value or "").strip() in (PLATFORM_AGENT_ID, LEGACY_PLATFORM_AGENT_ID)
...
class Direction(str, enum.Enum):
    # WIRE-PINNED VALUES. The member names are neutral; the strings are the
    # spellings a separately-deployed counterparty compares against and that sit
    # in stored `a2a_messages` rows, so they cannot be renamed from this side.
...
class ErrorCode(str, enum.Enum):
    ...
    response so a partner's SOC can alert on the string without log parsing.

    The code VALUES are wire-pinned for that reason — partners already have
    detection rules keyed on these exact strings — so the ones that read as
    partner-specific (`bank_identity_mismatch`, `bank_unreachable`) keep their
    spelling even though the member names around them are neutral."""
...
PII_CLASSIFICATION_RATIONALE: dict[A2ATaskType, str] = {
    ...
    _T.CERT_CASE_RESULT: "Carries per-case request/response bodies from real "
                         "production-shaped traffic (see api/phase_c.py::cert_txns) — "
                         "subject identifiers, account references and contact "
                         "numbers are the expected content, not an accident.",
    ...
    _T.CERT_TEST_PREPARATION: "Partner-supplied test data descriptions; test "
                              "accounts and subject identifiers are the point "
                              "of the message.",
    ...
    _T.HTTP_EXCHANGE_REQUEST: "Carries an ENCAPSULATED HTTP request verbatim — "
                              ...
                              "run the body IS live production traffic. This is the "
...
# --- accept-first aliases (coordinated two-phase rollout) ---
# Neutral spellings ACCEPTED on parse; the legacy values are still what is
# EMITTED, because the cert rig and partner SOC rules match them byte-for-byte.
# Flip emission only after those consumers read the neutral pair; acceptance
# landing first is what lets that happen without a coordinated deploy.
_DIRECTION_ALIASES = {
...
def _direction_missing(cls, value):
    return _DIRECTION_ALIASES.get(value)


def _error_code_missing(cls, value):
    return _ERROR_CODE_ALIASES.get(value)


Direction._missing_ = classmethod(_direction_missing)
ErrorCode._missing_ = classmethod(_error_code_missing)
