...
Part of the unified A2A SDK refactor — these are the handler
...
and the SDK executor. With the legacy router gone, the SDK
...
def process_counter_proposal(
    ...
    # lives at payload['payload'] with the newer wrapping; tolerate both.
    ...
    open_authority_cp = db.scalars(
        select(CounterProposal).where(
            ...
            CounterProposal.originator == ORIGINATOR_AUTHORITY,
        )
    ).first()
...
def process_counter_decision(
    ...
    the open authority-originated CounterProposal (`ORIGINATOR_AUTHORITY`,
    a legacy wire spelling — see models/phase_c.py) by its wire id and
...
def process_blocker(
    ...
    for b in blockers:
        ...
        row = Blocker(
            ...
            requested_action_from_authority=requested_action_from(b),
            payload=b,
        )
        db.add(row)
        db.flush()
        try:
            ...
            record_linked_message(
                ...
                # escalations into the same inbox as spec negotiation, so a partner reporting
...
def _normalize_direction(value: str) -> CertDirection:
    """Map an inbound `direction` wire value onto the enum.

    Accepts the superseded spellings too — see
    `models.phase_c.LEGACY_CERT_DIRECTION_ALIASES` for why the catch-all below
    makes that mandatory rather than merely polite.
    """
    from app.models.phase_c import LEGACY_CERT_DIRECTION_ALIASES

    s = (value or "").lower()
    if s == "partner_to_authority":
        return CertDirection.PARTNER_TO_AUTHORITY
    if s in LEGACY_CERT_DIRECTION_ALIASES:
        return LEGACY_CERT_DIRECTION_ALIASES[s]
...
def process_cert_fix_notification(
    ...
    if decision.action is cert_loop.LoopAction.DISPATCH:
        ...
        _modes = cert_modes.RunModes(
            authority=getattr(_last, "authority_mode", None) or cert_modes.SIMULATOR,
...
def _partner_reportable(row: CertTestResult) -> bool:
    """May a partner report WRITE this row?
    ...
    * a row a previous partner report already wrote — so a re-delivery, a
    ...
    if not isinstance(response, dict):
        return False
    return bool(response.get("not_reported")) or response.get("reporter") in ("bank", "partner")
...
def process_cert_case_result_report(
    ...
    """A partner-ORIGINATED cert_case_result.
    ...
    Anything that is not a partner-originated report (no reporter, an echo of an
    ...
    if reporter not in ("bank", "partner") or not case_id:
        message.status = "completed"
        db.commit()
    ...
    if run is None:
        message.status = "completed"
        db.commit()
        logger.warning("CERT_CASE_RESULT(partner): change=%s case=%s — no run to attach",
                       change_id, case_id)
        return f"Partner-reported result for {case_id} received (no run to attach)"
    ...
    # previous partner report wrote) are its to speak for.
    ...
    if row is None and rows:
        ...
        return (f"Partner-reported result for {case_id} refused: the case was "
    ...
    if override_reason:
        ...
        logger.warning("CERT_CASE_RESULT(partner): change=%s case=%s reported 'passed' "
    ...
    message.status = "completed"
    db.commit()
    logger.info("CERT_CASE_RESULT(partner): change=%s case=%s -> %s (run %s)",
                change_id, case_id, internal.value, run.run_number)

    # The join decides whether this was the LAST awaited case — off the
    ...
    return f"Partner-reported result recorded for {case_id}: {wire or 'unknown'} (run {run.run_number})"
