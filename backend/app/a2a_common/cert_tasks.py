...
(AtOM A2A Protocol Specification, Part B / Appendix B.B).

COPY of `app/services/cert_agent/tasks.py`, which is canonical for these
builders. This copy exists so the admin cert-trigger surface has spec-faithful
templates without importing from `services/`. Pure stdlib dict-builders.

Keep the two in step: the 14 task-type constants must stay identical, and no
gate checks it — see the canonical module's header.
...
# The config fields the Authority asks the partner to populate (spec §cert_config_request).
...
def cert_config_request(
    *,
    ...
    """The Authority requests the partner's certification configuration (§cert_config_request)."""
    return {
...
# ── Partner → Authority (builders provided for tests / simulating the partner) ─
...
def cert_config_submission(
    *,
    ...
    """Partner responds with its configuration (§cert_config_submission).
...
def cert_test_preparation(*, case_data: dict) -> dict:
    """Partner declares per-case data + readiness (§cert_test_preparation).
...
def cert_verdict_dispute(
    *,
    ...
    """Partner disagrees with a verdict (§cert_verdict_dispute)."""
    return {
        "case_id": case_id,
        "attempt": attempt,
...
def cert_waiver_request(
    *,
    ...
    """Partner cannot/will not pass a case in this release (§cert_waiver_request)."""
    return {
        "case_id": case_id,
        "category": category,
        "reason": reason,
...
def cert_fix_notification(
    *,
    ...
    """Partner resolved real_defect verdicts; requests re-run (§cert_fix_notification)."""
    return {
...
def cert_case_result(
    *,
    case_id: str,
    attempt: int,
    reporter: str,                   # the wire's initiator tokens — see
                                     # cert_agent/execution.py::WIRE_INITIATOR
