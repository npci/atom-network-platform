    ...
    *,
    ...
    Args:
        ...
        task_store:       Optional `TaskStore`. Defaults to `InMemoryTaskStore()`.
                          A SQLAlchemy-backed store is available for
                          durability — see `task_store_db`.
        ...
        auth_middleware:  Optional Starlette middleware CLASS (not instance)
                          to wrap the sub-app with. The Authority side
                          passes `SdkAuthMiddleware` here; cert-agent and
                          the partner backend pass their own. None = no auth
                          (current behaviour for any caller that hasn't
                          migrated yet).
        hmac_middleware:  Optional ASGI middleware CLASS that enforces the
                          X-Auth-Signature envelope (the legacy
                          X-NPCI-Signature spelling is still accepted). Wrapped
