...
Zero business logic is duplicated here — this module is glue. Once the
legacy POST endpoint is decommissioned, the handler functions will
move into this file (or stay where they are with a thin shim).
...
Auth note: we look up the partner by `partner_api_key` in
the message data — same lookup the legacy `CurrentPartner` dependency
does, just from a different envelope. A later change replaces this with
...
def _receipt_payload(*, task_id: str, status: str, task_type: str,
    ...
    the HTTP status under `response.status`, never at top
    level.
    """
...
class AuthorityAgentExecutor(AgentExecutor):
    ...
    async def execute(
        ...
        # partners simultaneously — tracked separately as a phased plan).
        ...
        # validation; a2a_strict_envelope_validation_enforce gates the
        # phased cutover plan. Fail-open in TWO senses: (a) a schema mismatch never blocks
        ...
        if getattr(settings, "a2a_strict_envelope_validation", True):
            try:
                ...
                try:
                    ...
                    # (the "excessive logging of sensitive or large payload
                    # data" anti-pattern).
                    _errs = str(ve)[:800]
                    logger.warning(
        ...
        # Security hardening: SdkAuthMiddleware validates the
        ...
        try:
            ...
            # The audit columns are populated from `audit_meta`
            # set by SdkAuthMiddleware: caller_ip, jwt_sub/iat/exp, mTLS
            # fingerprint.
            # T2 (non-repudiation) — SdkHmacMiddleware
            ...
            # T1 (at-rest integrity) — hash the EXACT
            ...
            # is processed instead of acked-and-dropped.
                ...
                if message.status == "completed":
                    ...
                    # Optional per-case overrides keyed by tc_id.
                ...
                if (
                    ...
                    # Optional per-TC overrides on this lifecycle path too
                ...
                # A `reporter: "bank"` result is THE result for
            ...
            # Persist the response body alongside the request
            # so the admin A2A logs UI can render both halves of the
            # round-trip.
            # String handlers keep the four-key receipt byte for byte;
            ...
            logger.exception(
                "a2a_executor_error task_type=%s change=%s",
                task_type_str, change_id,
            )
            # Error-message exposure (CWE-209): this handler wraps DB commits and the process_* dispatch,
            ...
            # Also stash the error in `response_body` so the
    ...
    async def cancel(
        ...
        no-op. This gets wired when long-running cert triage moves
