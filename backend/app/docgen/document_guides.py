...
# that authenticates/fulfils what the initiating participant requests — the
# settlement network's Clearing Participant, a library network's Lending Library.
...
# How this authority numbers a published reference. "OC No." is one
# authority's Operating Circular scheme; another numbers policy circulars,
# directives or bulletins entirely differently, and the model copies whatever
# format string it is handed onto the letterhead of a real-looking document.
_CIRCULAR_REFERENCE_FORMAT = _prompt_block(
    "circular_reference_format", "[ORG]/[DEPT]/[REF No.]/[YYYY-YYYY]"
)

# The sign-off convention of the issuing organisation. "Yours Sincerely," +
# "SD/-" is one country's public-sector officialese; a pack that says nothing
# gets a plain closing rather than another jurisdiction's house style.
_CIRCULAR_SIGNOFF_CONVENTION = _prompt_block(
    "circular_signoff_convention",
    "a courteous closing line",
)

# The data-protection regime the compliance section must map the change onto.
# Jurisdictional: one deployment answers to the DPDP Act, another to the GDPR,
# a third to no statutory regime at all. Naming one in the blueprint made every
# domain's TSD assert a legal framework it may not be subject to.
_PRIVACY_REGIME_LABEL = _prompt_block("privacy_regime_label", "Data Privacy")
    ...
    "sections": [
        {
            ...
            "prompt_instruction": (
                ...
                f"{_CIRCULAR_REFERENCE_FORMAT}, and the issue date flush-right. "
        ...
        },
        {
            ...
            "level": 1,
            "render_style": "body",
            # "the issuer" here meant the body ISSUING the circular, but it is
            # the same word as a domain's Issuer participant role, and the model
            # cannot tell which is meant from one sentence — circulars came back
            # attributing the directive to a participant. "The issuing
            # authority" is unambiguous in either reading.
            "content_instructions": (
                "Explain the current state, the ecosystem gap, and why the issuing authority is issuing "
                "this directive. Keep it factual and vendor-neutral."
            ),
            "prompt_instruction": (
                "Describe the current state, identify the limitation or opportunity, and briefly state what the "
                "issuing authority has decided to do in response. Single paragraph, 3-5 sentences."
            ),
            "include_table": False,
            "include_diagram": False,
            "diagram_type": "flowchart",
        ...
        },
        {
            ...
            "prompt_instruction": (
                f"Close with {_CIRCULAR_SIGNOFF_CONVENTION}, then the authorizing official's name, "
...
PRODUCT_NOTE_BLUEPRINT = {
    ...
    "sections": [
        ...
        },
        {
            ...
            "level": 1,
            "render_style": "body",
            "content_instructions": "Describe the transaction scope, high-level system changes, supported modes, end-to-end transaction journey, and roles/responsibilities table for the transaction execution flow.",
...
BRD_BLUEPRINT = {
    ...
    "sections": [
        ...
        {
            "section_key": "changes_authority_setting",
        ...
        {
            "section_key": "changes_authority_transaction",
            ...
            "content_instructions": (
                ...
                "platform routing changes, response handling. "
                "Minimum 2 paragraphs + ≥ 4 numbered requirements."
            ),
            "prompt_instruction": (
            ...
            "diagram_type": "sequence",
            "diagram_description": (
                "Authority platform processing changes for the new flow"
            ),
        },
        ...
        # ── Failure, Reversal & Idempotency ───────────────────────────────
        {
            ...
            "content_instructions": (
                ...
                "their servicing participant, raise dispute). "
        ...
        # ── Regulatory & Compliance Hooks (regulatory closure — split) ────
        {
            ...
            "content_instructions": (
                ...
                f"(c) {_PRIVACY_REGIME_LABEL} Mapping — [Provision, This Change's Trigger, "
            ...
            "prompt_instruction": (
                ...
                f"{_PRIVACY_REGIME_LABEL} rows name the specific provision or clause they map to "
                "(e.g. a consent clause, a lawful-basis clause), not a generic description."
            ),
            "include_table": True,
            "table_fallback_profile": "process_steps",
            "include_diagram": False,
        ...
        # ── Annexure B — Error Code Reference ─────────────────────────────
        {
            ...
            "content_instructions": (
                ...
                "Customer Next Step (retry / contact their servicing participant / raise dispute / cancel)]. "
            ...
            "prompt_instruction": (
                ...
                f"'contact the {_SERVICING_ROLE}'s customer care', 'wait 30 minutes and retry')."
            ),
            "include_table": True,
            "table_fallback_profile": "process_steps",
            "include_diagram": False,
        ...
        # Lighter tiers consolidate per-layer: authority / participant / servicing role × 1 each.
        # This avoids forcing a 6-section split when a typical feature only
        # touches 2-3 of the underlying combinations.
        {
            "section_key": "changes_authority_merged",
            ...
            "content_instructions": (
                ...
                "shared-library element, or authority-side routing/validation. "
                ...
                "no shared-library change, no platform behaviour change; the feature is participant-internal'). Render NO "
            ...
            "diagram_type": "sequence",
            "diagram_description": (
                "Authority-side processing of the new feature, end to end"
            ),
        },
        {
...
#       Changes sections (authority / participant / servicing role, one each). No state machine
...
#       (authority×{Setting,Tx}, participant×{App,Tx}, servicing×{Auth,Tx}). State machine,
...
BRD_TIER_COMPACT_KEYS = {
    ...
    "changes_authority_merged",         # merged Authority section (Setting + Transaction)
    "failure_reversal",
...
BRD_TIER_STANDARD_KEYS = BRD_TIER_COMPACT_KEYS | {
    ...
    "dispute_management",
    # PINNED, not leakage: `changes_psp_merged` / `changes_issuer_merged` (and
    # the `changes_psp_*` / `changes_issuer_*` granular keys below) are
    # `section_key` VALUES. They are written into every stored document plan
    # (docgen/models.py `SectionPlan.section_key`) and matched back on re-render,
    # repair and validation (docgen/document_validator.py), so renaming them
    # orphans the sections of every document generated before the rename. The
    # domain-facing part — the heading and the instructions — is already
    # pack-driven via `_SERVICING_ROLE`; only the key stays.
    "changes_psp_merged",          # merged participant-systems section
    "changes_issuer_merged",       # merged servicing-role section
...
BRD_TIER_COMPREHENSIVE_KEYS = {
    ...
    "changes_authority_setting",
    "changes_authority_transaction",
    "changes_psp_app",
