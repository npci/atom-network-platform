...
def _build_default_plans() -> dict[str, dict]:
    ...
    dn_ = f"{dn} " if dn else ""  # "Network " or "" — keeps sentences readable either way
    ...
    # See document_guides._CIRCULAR_SIGNOFF_CONVENTION: "Yours Sincerely," +
    # "SD/-" is one jurisdiction's public-sector house style, not a universal
    # one. Same pack block, so both circular paths sign off identically.
    signoff = prompt_block("circular_signoff_convention", "a courteous closing line")
    return {
        "BRD": {
            ...
            "sections": [
                ...
                {"heading": "6. User Journey", "level": 1, "content_instructions": "Describe the end-to-end user journey from initiation through authentication, routing, fulfilment, and notification. Cover happy path and error paths.", "include_table": False, "include_diagram": True, "diagram_type": "sequence", "diagram_description": f"{dn_}transaction lifecycle sequence diagram"},
        ...
        "TSD": {
            ...
            "sections": [
                ...
                {"heading": "8. Integration Points", "level": 1, "content_instructions": f"Document all external integration touchpoints, including the {authority} platform, participant back-office systems, risk/abuse detection, and the notification service. Include SLA per integration.", "include_table": True, "include_diagram": False, "diagram_type": "flowchart", "diagram_description": ""},
        ...
        "Circular": {
            ...
            "sections": [
                ...
                {"heading": "Signature Block", "level": 1, "content_instructions": f"Close with {signoff}, then the authorising official's name, designation, and department.", "include_table": False, "include_diagram": False, "diagram_type": "flowchart", "diagram_description": ""},
            ],
        },
    }
...
# Domain-specific role responsibilities (e.g. which participant owns identity
# checks, record validation and settlement) come from the ACTIVE
# PACK, appended to the static rules file at the seam — prompt files are static
# (the loader does no interpolation), so per-domain lines cannot live inside the
# .md. A pack that supplies no ownership block gets only the generic layer model.
...
# still read "No new Authority wire codes": the writer prompt handed the model
# payments-network ecosystem knowledge no matter which pack was active, and the
# model used the vocabulary it was given. There is no doc type for which another
# ecosystem's participants are the right context, so there is no longer a second
# path.
#
...
# Wire-schema namespace guidance for generated diagrams. Domain-specific (the
# payments network's xmlns prefix binding, NLLN's default-namespace binding),
# so it comes from the pack; an empty block means the diagram prompt simply
# carries no namespace rule.
_SCHEMA_NAMESPACE_NOTE = prompt_block("docgen_schema_namespace_note", "")

# Who a Product Note is written FOR. The two PN prompts named one ecosystem's
# participant classes outright, which is the thing `app/prompts/README.md` §3
# forbids: a library-network Product Note told the model it was writing for
# banks and PSPs. Same block the canvas / deep-researcher / cluster-router
# prompts already resolve.
_ECOSYSTEM_ACTORS = prompt_block("ecosystem_actors", "the ecosystem's participants")
...
# The third Envisaged-Changes stakeholder (8.3) is domain-specific: a settlement
# network names its Clearing Participant, a library network names the Lending Library.
# Resolved from the pack so the planner never plans a foreign ecosystem's
# section.
...
_PN_SYSTEM_PROMPT = render_prompt(
    ...
    SECTION_SCHEMA=_SECTION_SCHEMA, DOMAIN_KNOWLEDGE=_PACK_DOMAIN_NOTES, COMMON_JSON_RULES=_COMMON_JSON_RULES,
    ECOSYSTEM_ACTORS=_ECOSYSTEM_ACTORS,
)
...
def plan_document(state: dict) -> dict:
    ...
    try:
        ...
        if proposals:
            ...
            # OMIT what the caller did not supply rather than inventing it. These
            # lines are labelled AUTHORITATIVE, so a default here is asserted to
            # the model as fact — and "Auth Method: network PIN / Transaction
            # Limit: ..." is a PAYMENTS fact. Under a library or logistics pack
            # it told the model a loan renewal has a PIN and a transaction limit.
            # A missing value is silence, not a payments default.
            _lines = [
                "\n\nAUTHORITATIVE TECHNICAL SPECIFICATIONS (derived from authority documentation):",
                f"APIs: {_json.dumps(proposals.get('apis', []), indent=2)}",
            ]
            for _label, _key in (("Auth Method", "auth_method"),
                                 ("Transaction Limit", "transaction_limit")):
                _value = proposals.get(_key)
                if _value:
...
def _generate_single_diagram(llm: ChatOllama, spec: dict, output_dir: str, llm_content=None) -> tuple[str, str, dict]:
    ...
    plantuml_system = (
        ...
        "- For ACTIVITY diagrams: every activity step MUST end with a semicolon, e.g. :Process Request;\n"
...
def _flow_spec_anchor(fs: dict | None) -> str:
    ...
    When the plan carries party_flows (the evidence-cited end-to-end routes), the
    anchor renders from THOSE: every involved entity becomes a labelled participant
    box and every hop a numbered arrow (1, 2, …) — the Authority's flow-diagram
    style the BRD's Transaction Flow section requires."""
    ...
    try:
        ...
        for spec in specs:
            if flow_anchor and spec.get("diagram_type") in ("sequence", "flowchart", "activity"):
                # The blueprint's canned caption names the generic end-to-end route and can
                # CONTRADICT the approved flow (e.g. a "…→ Receiving Participant" caption on a
                # read-only enquiry that never touches that side). Stamp the caption from the
                ...
                #
                # The PREFIX is pack data: "Transaction flow:" was stamped onto every
                # sequence/flowchart/activity figure of every generated BRD and TSD in
                # every domain, including ones whose figures are not transactions.
                if pf_entities:
                    spec["caption"] = (
                        prompt_block("flow_diagram_caption_prefix", "Flow")
                        + ": " + " → ".join(pf_entities)
                    )
...
def _write_section(
    ...
    shared_blob = (
        ...
        + (
            ...
            f"fulfilment timeline, or compliance claim that has corpus support above MUST "
    ...
    from app.core.prompt_blocks import segments_for_prompt_cache
    system_segments = segments_for_prompt_cache(
...
def write_content(state: dict) -> dict:
    ...
    try:
        ...
        def _write_one(idx_section: tuple[int, dict]) -> tuple[int, dict]:
            ...
            # (Reverts the 2026-06-05 "Authority BRDs are technical artefacts" change so the
        ...
        # Normalize SCREAMING_SNAKE network API placeholders (REQ_FOO/RESP_FOO) to canonical
