...
# The docgen pipeline's ecosystem/message/flow reference block: this domain's
# participants, message types and the ReqTransfer stage flow.
#
# It USED to live at `prompts/docgen/agents/pipeline/common_network_domain.md`,
# where the doc-type-specific prompts loaded it directly — which is why a
# library-domain TSD still read like a payments spec. Those callers are gone
# (docgen/agents/pipeline.py now reads `docgen_generic_domain_notes` from
# whatever pack is active), leaving a 44-line UPI reference sheet sitting in the
# GENERIC prompts tree with one caller: this line. It is pack content, so it now
# ships with the pack. `base=` points the loader at this directory; without it
# the loader's containment check correctly refuses a path outside `prompts/`.
_DOCGEN_GENERIC_DOMAIN_NOTES = load_prompt("domain_notes.md", base=Path(__file__).resolve().parent)
