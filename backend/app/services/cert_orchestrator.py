...
Flow when partner declares ready with role + test_data:
  1. Resolve partner.cert_agent_partner_id (mapping the Authority's partner →
     cert-agent partner short-code). Skip with structured warning if missing.
...
from app.services.cert_roles import authority_role, partner_role
...
def _flow_for(change: ChangeRequest) -> str:
    ...
    Without this, a CR title like '"Smart Split & Auto-Collect"'
    derives to SMART_SPLIT_AUTO_COLLECT, while the docgen-produced
    cert_test_cases doc carries feature_name='Smart Split & Auto-Settle
    Certification Annexure' → SMART_SPLIT_AUTO_SETTLE_CERTIFICATION_ANNEXURE.
...
# the derived flow; unmatched flows leave the product checklist blank for
# Authority ops to complete in the downloaded doc (the cert is partner-stamped manually
...
def _build_signoff_meta(
    ...
    return {
        "handle":               partner.cert_agent_partner_id or "",
        ...
        "authority_mode":            (modes.authority if modes else _modes.SIMULATOR),
...
def _filter_partner_test_data(role: str, test_data: dict) -> dict:
    """Keep only fields relevant to the role. Prevents an initiating-side
    payload from accidentally overwriting the receiving side's addressing
    handles etc.
...
async def orchestrate_cert_run(
    ...
    `test_data_per_case` is a `{tc_id: {field: value, ...}}`
    ...
    try:
        ...
        if not partner or not change:
            ...
            return summary
        participant_id = (partner.cert_agent_partner_id or "").strip()
        if not participant_id:
            summary["skipped"] = True
            summary["skip_reason"] = (
                f"partner '{partner.name}' has no cert_agent_partner_id mapping — "
        ...
        logger.info(
            "cert_orchestrate.start change=%s partner=%s participant=%s flow=%s role=%s prefix=%r",
            change_id, partner_id, participant_id, flow, role, prefix,
        )
        ...
        async with httpx.AsyncClient(timeout=180.0, headers=_headers) as http:
            ...
            # one click, the Authority side seeds the simulator + patches + runs.
            if total_in_flow == 0:
                logger.info(
                ...
                # "Smart Split & Auto-Settle Certification Annexure"
                # while the CR title is just "Smart Split &
                # Auto-Collect"). cert-push uses the artifact's name to derive
            ...
            if assignment:
                set_status(
                    ...
                    reason=f"Cert run started for flow {flow} (participant={participant_id})",
                )
            ...
            #   Phase A: every TC initiated by the authority role runs through
            #            cert-agent via the deterministic REST path (as before).
            #   Phase B: every TC initiated by the partner role runs via the
            #            bank-agent self-test endpoint so the request genuinely
            #            originates from the partner side.
            ...
            authority_tc_ids = [
                ...
                if (t.get("initiated_by") or authority_role()).upper() != partner_role().upper()
            ]
            partner_tc_ids = [
                t["tc_id"] for t in matching
                if (t.get("initiated_by") or "").upper() == partner_role().upper()
            ]
            tc_ids = [t["tc_id"] for t in matching]  # legacy var; kept for mock path + persistence
            summary["authority_tc_count"] = len(authority_tc_ids)
            summary["partner_tc_count"] = len(partner_tc_ids)
            ...
            async def _direct_rest_run(subset_tc_ids: list[str], phase_label: str) -> dict:
                ...
                cert-agent's /api/llm-agent/run lets the LLM pick test cases
                via tools, but the model has been observed to end_turn
                early without polling get_run_status — returning empty
                results even while the simulator continues processing TCs
                ...
                init = await http.post(
                    ...
                    json={"bank_id": participant_id, "feature": f"{flow}-{phase_label}"},
                )
                ...
                run = await http.post(
                    ...
                    json={"bank_id": participant_id, "run_id": rid,
            ...
            async def _partner_agent_run(subset_tc_ids: list[str]) -> dict:
                ...
                partner side; bank-agent then forwards to cert-agent via A2A
                ...
                """
                bank_url = getattr(_app_settings, "partner_agent_url", "") or "http://bank-agent:8003"
                ...
                # Prefix names the side this run originates on, from the pack's
                # own partner token — it was the literal "BANK-" and so read as
                # a payments run id in every domain. Falls back to a neutral
                # word when the pack declares no partner role.
                _prefix = (partner_role() or "PARTNER").upper()
                rid = f"{_prefix}-{change_id[:8]}-{_uuid4().hex[:6].upper()}"
                ...
                try:
                    ...
                    logger.warning(
                        "cert_orchestrate.partner_phase_failed partner_url=%s err=%s",
                        bank_url, exc,
                    )
            ...
            if getattr(_app_settings, "mock_cert_run", False):
                ...
                if authority_tc_ids:
                    authority_batch = await _direct_rest_run(authority_tc_ids, authority_role())
                else:
                ...
                summary["authority_run_id"] = authority_batch.get("run_id")
                summary["authority_passed"] = authority_batch["passed"]
                summary["authority_failed"] = authority_batch["failed"]

                # Phase B — partner-initiated batch via bank-agent self-test.
                # Fires after Phase A reaches terminal. Demo policy:
                # continue even if Phase A had failures (the partner batch
                # still adds signal). If you'd rather block on Phase A
                # failures, gate the call on authority_batch["failed"] == 0.
                if partner_tc_ids:
                    partner_data = await _partner_agent_run(partner_tc_ids)
                else:
                    partner_data = {"run_id": None, "passed": 0, "failed": 0,
                                 "elapsed_ms": 0, "results": []}
                logger.info(
                    "cert_orchestrate.phase_b_done change=%s partner_pass=%d partner_fail=%d",
                    change_id, partner_data["passed"], partner_data["failed"],
                )
                summary["partner_run_id"] = partner_data.get("run_id")
                summary["partner_passed"] = partner_data["passed"]
                summary["partner_failed"] = partner_data["failed"]

                # Stitched run_data — combined results, the authority-phase
                # run_id as primary for back-compat (downstream cert_run persistence
                # uses one run_id; the per-phase ones live in summary).
                run_data = {
                    "run_id": authority_batch.get("run_id") or partner_data.get("run_id"),
                    "passed": authority_batch["passed"] + partner_data["passed"],
                    "failed": authority_batch["failed"] + partner_data["failed"],
                    "elapsed_ms": authority_batch.get("elapsed_ms", 0) + partner_data.get("elapsed_ms", 0),
                    "results": list(authority_batch.get("results", [])) + list(partner_data.get("results", [])),
                }
            ...
            init_by_tc = {
                t["tc_id"]: (t.get("initiated_by") or authority_role()).upper()
                for t in matching
            }
            for r in results:
                ...
                direction = (
                    ...
                    if init_by_tc.get(tc_id) == partner_role().upper()
            ...
            # 7) Ship results back to the partner over A2A so they
            #    can see pass/fail per TC alongside the cert lifecycle
            #    stepper. Wire payload mirrors what cert-engine sends
            #    inbound (CERT_TEST_RESPONSE handler on the Authority side), so the
            ...
            try:
                ...
                #
                # Stamp each wire row with `initiated_by` and
                # the phase it ran in (authority batch or partner batch) so the
                ...
                for r in results:
                    ...
                    wire_results.append({
                        ...
                        "status":        status_w,
                        # Transaction id + sent timestamp ride along so the
                        ...
                        "initiated_by":  init_by_tc.get(tc_id_w, authority_role()),
                ...
                _authority_phase = {
                    "run_id":  summary.get("authority_run_id"),
                    "total":   summary.get("authority_tc_count", 0),
                    "passed":  summary.get("authority_passed", 0),
                    "failed":  summary.get("authority_failed", 0),
                }
                _partner_phase = {
                    "run_id":  summary.get("partner_run_id"),
                    "total":   summary.get("partner_tc_count", 0),
                    "passed":  summary.get("partner_passed", 0),
                    "failed":  summary.get("partner_failed", 0),
                }
                results_payload = {
                    "cert_run_id":     cert_run.id,
                    ...
                    # Per-phase split so the partner can see
                    # which side drove which batch and whose failures
                    # they're looking at.
                    #
                    # DUAL-EMIT. This payload is ours — no spec pins it — but a
                    # partner reading it deploys separately, so the old keys
                    # ride along for one release while the neutral ones become
                    # the documented pair. REMOVE `npci`/`bank` once every
                    # counterparty in the estate reads `authority`/`partner`
                    # (they mirror the internal `summary["authority_*"]` /
                    # `summary["partner_*"]` names, which is the point).
                    "phases": {
                        "authority": _authority_phase,
                        "partner":   _partner_phase,
                        "npci":      _authority_phase,
                        "bank":      _partner_phase,
                    },
                }
                ...
                #    completion_signed_off_at on the Authority side and flips
                ...
                if all_passed:
                    ...
                    signoff_payload = {
                        ...
                        # Per-phase breakdown on the certificate
                        # so the partner's signoff doc shows the
                        # per-phase split too.
                        ...
                        "signoff_message": (
                            ...
                            f"test cases passed "
                            f"(authority {summary.get('authority_passed', 0)}/"
                            f"{summary.get('authority_tc_count', 0)}, "
                            f"partner {summary.get('partner_passed', 0)}/"
                            f"{summary.get('partner_tc_count', 0)})."
                        ),
                    }

                    # Render the Certification Result certificate and
                    ...
                    try:
                        ...
                        fname = f"Certification_Result_{participant_id}_{summary['run_id']}.docx"
                ...
                # Non-fatal — partner can still pull via the Authority's admin tools.
                logger.warning(
...
async def orchestrate_cert_run_precert_engine(
    ...
    a2a_messages); config + per-case test data come FROM the partner over A2A. The engine
    (connector/provisioner) still drives the REAL precert → simulator → precertdb. Selected by
    ...
    try:
        ...
        flow = _flow_for(change)
        participant_id = (partner.cert_agent_partner_id or settings.precert_engine_psp_org_id or "").strip()
        logger.info(
            "precert_engine.start change=%s partner=%s participant=%s flow=%s role=%s subset=%s",
            change_id, partner_id, participant_id, flow, role, settings.precert_engine_subset,
        )
        ...
        # config + per-case test data come FROM the partner over A2A, results + verdicts go back.
        from app.services.a2a_client import send_task_to_partner
        cflow_id = f"CFLOW-{change_id[:8]}-{partner_id[:8]}"
        ...
        # 1) CONFIG — ask the partner for its configuration.
        ...
        participant = (cfg.get("psp_org_id") or partner.cert_agent_partner_id
                       or settings.precert_engine_psp_org_id or "").strip()

        # ── PROVISION from the partner's own config, then resolve.
        # Per spec Appendix B, cert_setup_notification fires once the Authority "validated
        # the config, provisioned the simulator, mapped the suite" — so the partner's
        ...
        # already-onboarded partner changes nothing.
        def _provision():
            ...
            return provision_from_config(
                ...
                defaults={"psp_org_id": participant},
            )
        provisioned = await asyncio.to_thread(_provision)

        # Fall back to the READ-ONLY resolve when the partner sent nothing usable, so a
        ...
        def _resolve():
            ...
            participant_cfg = prov.config_of(participant)
            if participant_cfg is None:
                return None, None, []
            subset = settings.precert_engine_subset
            assigned = prov.subsets_of(participant_cfg.partner_org_id)
            if assigned and subset not in assigned:
                subset = assigned[0]
            scope = NfiniteConnector(db=db_cfg).cases_in_subset(subset) if subset else []
            return participant_cfg, subset, scope

        if provisioned is not None:
            participant_cfg, subset, scope = (
                provisioned.participant, provisioned.subset, provisioned.cases)
            logger.info("precert_engine.provisioned participant=%s subset=%s changes=%s",
                        participant_cfg.participant_org_id, subset, provisioned.changes)
        else:
            participant_cfg, subset, scope = await asyncio.to_thread(_resolve)
        if participant_cfg is None or not scope:
            summary["skipped"] = True
            summary["skip_reason"] = f"participant {participant} not onboarded / empty scope"
            return summary
        ...
        #    values the Authority owns) — all read straight from precertdb.
        ...
        # The wire's own side tokens, from the module that owns that boundary —
        # `cert_agent.execution` maps internal authority|partner onto them.
        _wire_authority, _wire_partner = wire_initiator("authority"), wire_initiator("partner")
        ...
        initiator_of = {c["case_id"]: (c.get("initiator") or _wire_authority)
                        for c in case_list}
        ...
        # both-sides change, but the simulator address has to be on the
        # wire before partner-initiated execution can exist at all.
        ...
        setup_msg = await _say(_proto.A2ATaskType.CERT_SETUP_NOTIFICATION,
            {"summary": f"{_onboarded}. Assigned test suite '{subset}' comprising {len(case_ids)} test case(s).",
             ...
             # An alias declaration, never a raw URL — the partner's
             # stack calls its OWN tunnel ingress with this name.
             # The MODE selects which catalogue entry that name is —
             ...
             "simulator": simulator_block(
                 alias=_modes.alias_for(
                     "authority", _run_modes.authority,
        ...
        # With the tunnel enabled, partner-initiated cases
        ...
        # partner-reported cert_case_result messages, which replace the
        ...
        if settings.integration_testing_enabled:
            _partner_owned = {tc for tc in case_ids
                              if reporter_for(initiator_of.get(tc, _wire_authority))
                              == _wire_partner}
        if _partner_owned:
            await _say(_proto.A2ATaskType.CERT_EXECUTION_START, {
                "summary": (f"Begin partner-initiated execution: "
                            ...
                            f"cert_case_result (reporter={_wire_partner})."),
        ...
        for tc, cg in scope:
            if tc in _partner_owned:
                # The partner's report will replace this row (see
                ...
                results.append({"test_case_id": tc, "status": "ERROR",
                                "error_message": "not_reported — partner-initiated "
                                                 "case awaiting the partner's report"})
                continue
            prep = test_data_reply.get(tc) or {}
            _initiator = initiator_of.get(tc, _wire_authority)
            # Spec (cert_test_preparation): `ready` on an authority-initiated case is what
            # PERMITS execution; on a partner-initiated one it is ignored. Absent means
            # execute — a partner that sends no flags (as ours does today) must still
            ...
            if not is_ready(prep, initiator=_initiator):
                executions = [(None, "SKIP", {"test_case_id": tc, "status": "SKIP",
                                              "error_message": "partner has not declared this case ready"})]
            else:
                # `case_data` minus its control key becomes the request overrides —
                # connector._fire merges them last, so the partner's values win.
                partner_overrides = {k: v for k, v in prep.items() if k != "ready"}
                executions = []
                ...
                for _variant in (variants_by_case.get(tc) or [None]):
                    ...
                    try:
                        r = await asyncio.to_thread(
                            conn.run_case,
                            CaseSpec(participant, subset, settings.precert_engine_cert_name,
                                     tc, cg, overrides=overrides))
            ...
            for _variant, st, row in executions:
                results.append(row)
                # Spec: cert_case_result is bidirectional — the partner reports its own
                # (partner-initiated) cases. The authority executes against the simulator,
                # then the partner formally reports the outcome (its reply renders on the
                # partner→authority leg).
                _reporter = reporter_for(_initiator)
                _wire = wire_status(st)
                _cr_summary = (f"Partner-initiated case {tc} executed against the simulator ({_wire}); awaiting partner report."
                               if _reporter == _wire_partner else f"Test case {tc}: {_wire}.")
                ...
                if _fails:
                    ...
                    await _say(_proto.A2ATaskType.CERT_WAIVER_DECISION,
                        {"summary": f"Waiver granted for test case {tc}, subject to risk and product sign-off.",
                         ...
                         # Spec splits these: `category` is the enum the partner waives
        ...
        # With partner-owned cases dispatched, the run is NOT terminal —
        # it awaits their partner-reported results (or the suite deadline). The
        ...
        cert_run = CertRun(
            ...
            cflow_id=cflow_id,
            run_number=_round,
            # A run awaiting partner results stays RUNNING; the join
            ...
            authority_mode=_run_modes.authority,
        ...
        for r in results:
            ...
            tc_status = (
                ...
                # deliberately-skipped case, both for the partner-report upsert
            ...
            db.add(CertTestResult(
                ...
                status=tc_status,
                # The join tells a genuine execution ERROR from a
                # partner case still awaited by this marker; the partner's
        ...
        if (not _awaiting and assignment
            ...
            # Never certify while partner cases are unreported — a
        ...
        # NOT while awaiting partner results — a mid-suite results
        ...
        try:
            ...
            results_payload = {
                ...
                "external_run_id": summary["run_id"],
                "feature_name": change.title or "",
                "flow": _flow_code, "bank_id": participant_id, "role": role,
            ...
            if all_passed:
                ...
                signoff_payload = {
                    ...
                    "external_run_id": summary["run_id"],
                    "feature_name": change.title or "",
                    "flow": flow, "bank_id": participant_id, "role": role,
                ...
                try:
                    ...
                    signoff_payload["signoff_filename"] = (
                        f"Certification_Result_{participant_id}_{summary['run_id']}.docx"
                    )
                    signoff_payload["signoff_sha256"] = _hashlib.sha256(raw).hexdigest()
                    signoff_payload["signoff_size_bytes"] = len(raw)
                except Exception as exc:  # noqa: BLE001
