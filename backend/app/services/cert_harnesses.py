...
vocabulary. Keeping it inside the default domain's pack made the platform's only
domain-neutral certification engine unreachable for every other domain.

`CertAgentHarness` and `PrecertEngineHarness` stay genuinely domain-bound
IMPLEMENTATIONS (participant_id, the Nfinite precert stack) — but the registry that
...
class CertAgentHarness:
    """Certification via the cert-agent service (REST). Domain-bound in practice:
    the orchestrator it drives requires `partner.cert_agent_partner_id`."""
...
class PrecertEngineHarness:
    ...
    Drives the real precert → partner-simulator → precertdb path, with the whole
    exchange carried as a signed A2A conversation with the partner. Domain-bound.
    """
