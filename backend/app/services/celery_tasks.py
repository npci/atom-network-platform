...
# Scheduling timezone. Was `Asia/Kolkata`, which silently shifted every
# calendar-anchored sweep for a deployment outside India. UTC is the only
# defensible default for a platform with no home country; a deployment that
# wants its own local time sets CELERY_TIMEZONE. Nothing in `beat_schedule`
# below is crontab-anchored today — every entry is a plain interval — so this
# changes no cadence, only what the logs and `celery inspect` report.
_TIMEZONE = os.environ.get("CELERY_TIMEZONE", "UTC")

# The app's main name. Renaming it was checked rather than assumed: every task
# in this module (and in scheduled_ingest) registers with an EXPLICIT `name=`,
# so nothing is auto-named off the main name, and the worker commands name
# their queues explicitly. An already-queued task still resolves after this.
celery_app = Celery(
    "atom",
...
celery_app.conf.update(
    ...
    # makes this isolation effective in production.
    task_routes={
...
celery_app.conf.beat_schedule = {
    ...
    # Outbound A2A delivery retry: every 2 minutes, re-send messages to partners whose
    # delivery failed and whose backoff has elapsed. Without this, one transient network
    # blip meant the partner never received its Product Kit and nobody was told. Backoff
    # schedule and attempt cap live in app/services/a2a_client.py.
    "a2a-delivery-retry": {
        "task":     "a2a.retry_failed_deliveries",
        "schedule": 2.0 * 60.0,                   # 2 min
    },
    # The suite-join deadline sweep: finalize cert runs whose partner-
    ...
    # A2A security hardening — daily key-age scan. Logs a
    ...
    # Agentic codegen — recovery sweep. Reclaims agentic
...
def retry_failed_deliveries_task(self, limit: int = 50):
    ...
    never received the message, nothing retried, and the only trace was a log line.
    ...
    try:
        ...
        # Tunnelled exchanges are excluded — a replayed tunnelled POST
...
def scan_partner_secret_ages_task(self, max_age_days: int = 90):
    ...
    occasional false positive is fine. Per-secret rotation
