...
"""Simulator-or-application mode, chosen per side, per run.
...
of a simulator, INDEPENDENTLY — four postures, all legitimate:
    ...
    authority=simulator  partner=simulator    the wiring works (the demo)
    authority=simulator  partner=application  the partner's generated code behaves
    authority=application partner=simulator   this platform's application behaves
    authority=application partner=application full integration
...
class RunModes:
    authority: str = SIMULATOR
    partner: str = SIMULATOR

    @property
    def any_application(self) -> bool:
        return APPLICATION in (self.authority, self.partner)
    ...
    def evidence(self) -> str:
        ...
        if self.authority == APPLICATION and self.partner == APPLICATION:
    ...
    def as_dict(self) -> dict:
        return {"authority_mode": self.authority, "partner_mode": self.partner}
...
def resolve(requested: Mapping[str, Any] | None = None, *,
    ...
    """
    req = requested or {}
    # Dual-key read on the authority side. `_one` is deliberately loud about a
    # bad VALUE, but a MISSING key resolves to SIMULATOR — so a `test_data` blob
    # still spelling `npci_mode` runs the simulator and then reports "both ends
    # were simulators" as evidence. That is precisely the silent downgrade `_one`
    # exists to prevent, reintroduced through the key rather than the value.
    # Stored A2A payloads keep the old spelling (nothing rewrites payload JSON),
    # so this outlives the partner platform being updated.
    authority_mode = req.get("authority_mode")
    if authority_mode is None:
        authority_mode = req.get("npci_mode")
    return RunModes(authority=_one("authority", authority_mode),
...
def auto_dispatch_allowed(modes: RunModes, *,
    ...
    if permitted:
        return True, ("application mode auto-dispatch explicitly permitted by "
                      "configuration")
    sides = [name for name, value in (("authority", modes.authority),
