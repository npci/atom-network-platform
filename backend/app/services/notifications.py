...
when an outbound A2A message to a partner failed, the ONLY trace was a `logger.error(...)`
that nobody watches, and the change quietly proceeded as if the partner had been told.
...
def notify_delivery_failure(db, message, partner, *, context: str = "A2A message") -> int:
    """Raise an alert that an outbound message to a partner was not delivered.
...
def notify_round_silently_closed(
    ...
    otherwise never notice a partner went quiet and any implicit acceptances
