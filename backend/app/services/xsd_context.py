...
2. ``build_involved_xsd_bundle`` — resolves the schemas *involved in the flow*
   ...
   KB chunks (existing_xsds / authority_xml_spec categories).
...
def _msg_tokens(text: str) -> list[str]:
    """Wire-message tokens (network pack: ReqTransfer, RespAuthDetail, …) as they appear in
...
def _kb_schema_content(name: str, db: Session) -> tuple[str, str]:
    """Reassemble an ingested schema (KB existing_xsds / authority_xml_spec chunks)
