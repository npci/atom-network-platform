...
"""Egress: an inbound A2A exchange → a local HTTP call.
...
rides home through the executor's receipt merge.

Mirror of `atom-partner-platform .../integration_testing/egress.py` (the
...
1. **Resolve locally, never from the payload.** `resolve_alias` takes an alias
   ...
   alias is a hard rejection with no fallback.
