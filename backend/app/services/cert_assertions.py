...
# A pattern longer than this is a registry defect regardless; the bound matters
# because catastrophic backtracking needs room to nest, and 200 chars is far more
# than any XSD facet in the corpus uses.
_MAX_PATTERN_CHARS = 200
# Values are partner-supplied. Backtracking cost grows with INPUT length as well
# as with pattern shape, so bound both.
_MAX_PATTERN_INPUT_CHARS = 4096
# Nested quantifiers — `(a+)+`, `(a*)*`, `(a+){2,}` — are the shape that turns a
# linear match into an exponential one.
_NESTED_QUANTIFIER = re.compile(r"\([^()]*[+*}][^()]*\)\s*[+*]|\([^()]*[+*]\)\s*\{")


def assert_pattern(expected: Mapping[str, Any],
                   values: Sequence[str]) -> AssertionOutcome:
    """Match `values` against the registry's `pattern_rule`.

    SECOND-ORDER ReDoS is the risk, and the reason for the guards below.
    `pattern_rule` is not a constant: it is registry data, writable via
    `PATCH /api/api-registry/fields/{field_id}` and also derived from `xs:pattern`
    facets during XSD ingest. The values it runs against come out of a
    partner-submitted wire payload. So a pattern stored once — `(a+)+$` fits
    easily inside the column's 500 chars — makes every later certification run
    hang a worker on a partner-chosen input.

    A rejected pattern is a `_skip`, not a `_fail`, matching how a non-compiling
    pattern is already treated: the fault is in the stored rule, not in the
    partner's message, and failing their certification for it would be wrong.
    """
    raw = str(expected.get("pattern_rule") or "")
    if len(raw) > _MAX_PATTERN_CHARS:
        return _skip(f"pattern is {len(raw)} chars (max {_MAX_PATTERN_CHARS}) "
                     "— registry defect")
    if _NESTED_QUANTIFIER.search(raw):
        return _skip(f"pattern {raw!r} has a nested quantifier and is rejected as "
                     "catastrophic-backtracking-prone — registry defect")
    try:
    ...
    if not values:
        return _skip("field absent — presence is occurrence/mandatory's question")
    for v in values:
        if len(v) > _MAX_PATTERN_INPUT_CHARS:
            return _fail(f"value is {len(v)} chars, above the "
                         f"{_MAX_PATTERN_INPUT_CHARS}-char pattern-match ceiling")
