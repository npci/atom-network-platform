...
async def _pump(stream, queue: asyncio.Queue) -> None:
    ...
    *silence* (to emit a heartbeat) without cancelling the generator, which
    ...
    try:
        ...
        # Error-message exposure (CWE-209): this string lands in `run.lines`, which `to_dict()` returns
...
async def start_background_run(core_branch: str, app_branch: str,
    ...
    async def _drive():
        # Resource release — `with blog:` (rather than a manual `.close()` in
        ...
        with blog:
            try:
                ...
                run.status = "error"
                # Error-message exposure (CWE-209): `note` is serialised by to_dict() and returned by
