...
def build_kit_envelope(
    ...
    *,
    ...
    `change_summary` is the partner-facing "what changed since the
    ...
    # Promo/explainer videos: the SCRIPT is an internal authority-side artifact (the basis
...
async def dispatch_kit_to_partners(
    ...
    *,
    ...
    for assignment in assignments:
        ...
        # — so a partner that never got the kit was recorded as having received it, and its
    ...
    # Count DELIVERIES, not attempts. `results` holds one row per partner we
    # tried, including the ones that refused — reporting len(results) as
    # "delivered"/"notified" told the operator the opposite of what the ERROR
    # line above says, and did it with an HTTP 200. The state machine already
    # gates on `delivered` (see :790); the counter has to as well, or a kit
    # every partner rejected reads as a clean ship.
    delivered_count = sum(1 for r in results if r["delivered"])
    logger.info(
        "Kit dispatch (%s): change=%s delivered=%d failed=%d skipped=%d",
        mode, change_id, delivered_count, len(results) - delivered_count, skipped,
    )
    return {
