...
Parses the platform's real network schemas (workspace repo clones or the KB
...
every ``.xsd`` in the folder (network messages include ``network-common.xsd``), then
...
def _ns_prefix_for(xsd_text: str, tns: str | None) -> str:
    ...
    WHY THIS EXISTS. `build_sample_xml` used to emit a hardcoded prefix
    while taking the URI from the schema, so every generated sample read
    `<pay:ReqMemberVerify xmlns:pay="http://nlln.in/schema/v1">` — a prefix from
...
def _expand_element(node: ET.Element, reg: TypeRegistry, depth: int = 0,
    ...
    if ct is not None:
        ...
        # Authority convention: container tag rows read Datatype=Alphabetic, Length=Fixed value.
...
# ---------------------------------------------------------------------------
# Authority-style hierarchical tag numbering (deterministic)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Deterministic sample XML (enums inline, Authority style)
# ---------------------------------------------------------------------------
...
def build_sample_xml(msg: MessageSpec) -> str:
    ...
    prefix ships to the reader as fact: never pair a fixed prefix with a dynamic URI.
    There is likewise no default URI — a schema with no targetNamespace gets no
    `xmlns` rather than another ecosystem's.
    """
    root = msg.root
...
def ingest_from_dir(db, xsd_dir: Path, source: str = "xsd_parse",
    ...
    # ReqCheckStatus.xsd) — first wins within a run (sorted file order, so the
...
def derive_involved_api_names(*texts: str) -> list[str]:
    ...
    (network: Req*/Resp*/Ack). A pack that declares none yields [] — "no messages
