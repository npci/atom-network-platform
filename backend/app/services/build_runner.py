...
# start with one of a handful of hardcoded prefixes, so any stack whose services
# are named otherwise silently reported "no services started" — including the
...
[INFO] Downloaded from internal-nexus: network-protocol-3.1.0.jar (788 kB at 2.1 MB/s)
[INFO]
...
● network-core.service - Network Core Switch
   Active: active (running); 0s ago
   Main PID: 18472 (java)
$ systemctl restart network-2-0
● network-2-0.service - Network v2.0 Edge Service
   Active: active (running); 0s ago
   Main PID: 18519 (java)
...
async def _run_demo_build(
    ...
    *,
    ...
    logger.info(
        "Build+deploy SIMULATED (demo mode) started: change=%s core=%s app=%s",
...
# module matched by AGENTIC_VERIFY_SKIP_MODULES (e.g. `*legacy-switch*,*hsm-proxy*`) is
...
def _do_local_build(build_key: str, change_id: str, repos: list[tuple], *, reuse: bool) -> dict:
    ...
    # Improper Resource Shutdown or Release (CWE-404) — `blog.close()`
    ...
    with diag.open_build_log(change_id, utcnow().strftime("%Y%m%d-%H%M%S")) as blog:
        blog.write(f"=== Network local build + mock deploy — change={change_id} workspace={build_key} ===")
...
async def _run_local_build(
    ...
    *,
    ...
    logger.info(
        "Build LOCAL started: change=%s mode=%s workspace=%s repos=%s core=%s app=%s",
...
async def run_build_and_deploy(
    ...
    *,
    ...
    logger.info(
        "Build+deploy started: change=%s mode=%s host=%s core=%s app=%s script=%s",
    ...
    try:
        blog.write(f"=== Network build+deploy — change={run.change_request_id} build_run={build_run.id} ===")
