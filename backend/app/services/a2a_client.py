...
Part of the unified A2A SDK refactor. The legacy hand-rolled POST
...
Auth model (post the A2A security hardening):
  ...
  cert_engine partner — Bearer JWT obtained via the `/a2a/auth`
                        ...
                        CertAgentAuthMiddleware verifies.
  ...
  Partners with `jwt_signing_secret` NULL — no Authorization header
                        ...
                        until the partner deploys inbound
                        validation).
"""
...
from app.a2a_common.protocol import (
    PLATFORM_AGENT_ID, SENDER_AUTHORITY, make_envelope,
)
...
# T3 — partner IDs for which the tls_verification_disabled
...
# `PLATFORM_AGENT_ID` — the agent id this platform stamps on outbound messages
# when a caller names none — now lives in `a2a_common/protocol.py` alongside
# `SENDER_AUTHORITY` / `CERT_ORCHESTRATOR_AGENT_ID` and their LEGACY_* twins,
# which is where every other wire identity on this protocol is declared.
...
def _record_attempt(message: A2AMessage) -> None:
    ...
    try:
        ...
        # A tunnelled exchange is NEVER scheduled for retry — replaying
...
def _agent_card_url(partner: PartnerAgent) -> str:
    ...
    path prefix that deployment uses (`https://partner.example.com/a2a`).
    Either way, the card lives at `{endpoint_url}/.well-known/agent-card.json`.

    Before the security hardening this helper hardcoded a
...
def partner_verify(partner: PartnerAgent):
    ...
    if not verify_on:
        # T3 ("No telemetry when ssl_verify=False is used
...
async def resend_message(db: Session, message: A2AMessage) -> A2AMessage:
    ...
    try:
        # T4 — per-partner bulkhead around the actual
...
async def send_task_to_partner(
    ...
    *,
    ...
    the MIDDLE LAYER of the integration-tunnel budget, which must
    ...
    # adds around it).
    #
    # Protocol v1: the envelope now carries protocol_version +
    ...
    # the exact conversation. Single-emit — no legacy documents[] aliases.
    ...
    wire_data: dict = make_envelope(
        ...
        agent_id=agent_id or PLATFORM_AGENT_ID,
    ...
    # Security hardening: every partner with a configured
    ...
    # Audit: stamp jwt_sub when we attached a Bearer (we know
    # the partner ID, which IS the sub claim). caller_ip
    ...
    send_kwargs = dict(
        ...
        data=wire_data,
        auth_header=auth_header,
        # Outbound HMAC envelope. When set, send_a2a_message
        # registers an httpx event hook that adds X-Auth-Signature
    ...
    try:
        ...
        # S6 (outbound dependency response validation "applies to LLM
        # providers and partner replies alike").
        ...
        message.error_code = _error_code_for(exc)
        # Record the failure shape on response_body too so
    ...
    # Alert a human on any non-delivery. Previously this was logger.error only, so a partner
