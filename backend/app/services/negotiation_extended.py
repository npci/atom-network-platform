...
async def classify_and_cluster_cp(
    ...
    # clustering so the PM can see patterns across partners (e.g. 3 partners
...
async def _auto_resolve_cp(cp: CounterProposal, disposition: str, db: Session,
    ...
    if accepted:
        ...
        # was told, so the PM had no signal that a partner had been turned down automatically.
        try:
...
def finalize_negotiation(
    ...
    # requirements were added) was converted straight to ACCEPTED — a partner deviation from
    ...
    for cp in open_cps:
        if cp.brd_classification == BRDClassification.MANDATORY_VIOLATION.value:
            ...
            if cp.brd_classification in (None, "", BRDClassification.UNCATEGORIZED.value):
                _unchecked += 1
            cp.status = CounterProposalStatus.ACCEPTED
            cp.resolution_text = "Negotiation finalized — specs locked by the authority's PM."
        cp.resolved_at = now
        cp.resolved_by = actor_user_id
...
def collect_round_outcomes(change_request_id: str, db: Session) -> list[dict]:
    """Consolidate the closing round's outcomes across all partners.
