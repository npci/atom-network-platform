...
* **Two-sided by class (Gate 3).** Each side executes ONLY its own class:
  ...
  partner-initiated ones are announced to the partner (`cert_setup_notification`
  ...
  stays RUNNING and `cert_join` finalizes it on the partner's reports or the
...
* **Modes are resolved per run, never inherited** (I-6b/§3.6.4, the rules in
  `cert_modes`), and BOTH postures execute. `authority=simulator` calls the
  in-process pack simulator. `authority=application` DRIVES this platform's
...
# This module is the PACK-DRIVEN harness — the one path that must carry no
# ecosystem's vocabulary. The `initiator` tokens it puts on the wire come from
# `cert_agent.execution.wire_initiator`, which owns that internal->wire
# boundary, rather than being spelled out here.
...
def _demo_catalogue(db: "Session", change_id: str) -> list[dict]:
    ...
    return [
        {"case_id": f"TC_{msg.api_name}", "api": msg.api_name,
         "initiator": _WIRE_AUTHORITY, "expected_status": "PASS",
...
def _fire_authority_trigger(*, case_id: str, variant, cflow_id: str,
    ...
    return fire_trigger(
        ...
        cert_context={"cflow_id": cflow_id, "cert_attempt": run_number,
                      "initiator": _WIRE_AUTHORITY, **modes.as_dict()},
...
async def _announce(db: "Session", *, change_id: str, partner_id: str,
    ...
    Publishes `simulator.endpoint` as an ALIAS selected by this run's
    authority-side mode, with `?pack=` binding the contract this round is
    graded against — so the partner-initiated direction certifies against the
    same pack as the authority-initiated one.
    ...
    case_list = [
        {"case_id": c.get("case_id"), "api": c.get("api"),
         "initiator": c.get("initiator") or _WIRE_AUTHORITY,
    ...
    # `send_task_to_partner` ALWAYS RETURNS and never raises on a delivery
    # failure — its own docstring (a2a_client.py:427) says the row comes back
    # with status 'delivery_failed' for "SDK exception (auth, transport,
    # server)". The two broad `except` guards below were written for
    # exactly that case and could never fire for it, so the honest degradation
    # this function's docstring promises was unreachable: a partner we could
    # not reach at all still came back as `awaiting_partner`, HTTP 200, and the
    # round sat waiting for reports until the suite deadline turned them into
    # `not_reported` — 24h later, for something knowable at dispatch.
    # Found live: Meridian Airways' card resolves to a compose-internal host,
    # both sends failed with ERRNO -3, and the dispatch still reported
    # awaiting_partner on 12 cases. Third instance of the F-22/F-23 root cause
    # (a delivery result reported by return value, read as if by exception).
    def _delivered(msg) -> bool:
        return getattr(msg, "status", None) == "delivered"

    try:
        _setup_msg = await send_task_to_partner(
            partner=partner,
            ...
            cert_attempt=run_number)
        if not _delivered(_setup_msg):
            logger.error(
                "sim_pack round %s r%d: cert_setup_notification NOT DELIVERED "
                "(status=%s) — running this side's class only rather than "
                "awaiting reports that cannot arrive",
                cflow_id, run_number, getattr(_setup_msg, "status", "unknown"))
            return set()
    except Exception:
        logger.exception(
    ...
    # The START SIGNAL for the partner-initiated half. The setup
    ...
    try:
        _start_msg = await send_task_to_partner(
            partner=partner,
            task_type=_proto.A2ATaskType.CERT_EXECUTION_START,
            payload={
                # Partner-visible prose. `reporter=` names the WIRE token, so
                # it is interpolated rather than written out — a literal here
                # would tell a partner in any domain to report as a bank.
            ...
            db=db, change_request_id=change_id, cflow_id=cflow_id,
            cert_attempt=run_number)
        if not _delivered(_start_msg):
            # Deliberately NOT the same degradation as the setup failure above.
            # There, the partner never learned the scope, so awaiting them is
            # meaningless and we drop their class. Here they DO hold the scope
            # and could still self-start or be re-driven, so the cases stay in
            # the partner class and expire at the deadline as `not_reported` —
            # which is the honest record this function's docstring describes.
            # The difference that matters is that the operator is now TOLD, at
            # dispatch, instead of inferring it from a silent 24h wait.
            logger.error(
                "sim_pack round %s r%d: cert_execution_start NOT DELIVERED "
                "(status=%s) — the partner knows the scope but was never told "
                "to begin; their cases will expire at the suite deadline as "
                "not_reported (the honest record)",
                cflow_id, run_number, getattr(_start_msg, "status", "unknown"))
    except Exception:
        logger.exception(
...
async def run_round(
    ...
    *,
    ...
    if modes.authority == cert_modes.APPLICATION and not settings.cert_trigger_url:
        ...
        return {
            ...
            "detail": ("authority_mode=application needs `cert_trigger_url` — the "
    ...
    run = CertRun(
        ...
        authority_mode=modes.authority, partner_mode=modes.partner,
    )
    ...
    for case_id, variants in sorted(scope.variants_by_case.items()):
        for variant in variants:
            if variant.id in partner_owned:
                # The partner's report replaces this row (see
                ...
                db.add(CertTestResult(
                    ...
                    actual_response={
                        ...
                        "reason": "partner-initiated case awaiting the partner's "
                                  "report",
                    },
                    pack_ref=graded_by.pack_ref, pack_id=graded_by.pack_id,
                    authority_mode=modes.authority, partner_mode=modes.partner,
            ...
            if modes.authority == cert_modes.APPLICATION:
    ...
    # A run still owed partner reports stays RUNNING with no
