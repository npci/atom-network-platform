...
# The domain descriptor comes from the active pack; under the default network pack
...
def _warn_vision_unavailable(provider: str, n_figures: int) -> None:
    """Warn once per process that figures are being dropped, and why."""
    global _vision_unavailable_warned
    if _vision_unavailable_warned:
        logger.info("vision unavailable on provider %r — %d figure(s) skipped", provider, n_figures)
        return
    _vision_unavailable_warned = True
    logger.warning(
        "vision is not available on LLM provider %r, so %d figure(s) in this upload were "
        "NOT described and their content will not reach the pipeline. Diagrams in uploaded "
        "documents are silently ignored. Set LLM_PROVIDER=anthropic (or ainxt with "
        "AINXT_COMPAT_MODE=anthropic) to enable figure understanding.",
        provider, n_figures,
    )
...
async def describe_document_images(path: Path, ext: str, *,
    ...
    from app.core.llm import call_llm_vision, get_provider, supports_vision
    # Fail-open is right for a single bad figure, but it used to also swallow
    # "this provider has no vision at all" — the operator saw a successful
    # upload, zero captions, and a worse BRD, with only a per-figure warning as
    # evidence. Say it once, plainly, and skip the doomed calls.
    if not supports_vision():
        _warn_vision_unavailable(get_provider(), len(prepped))
        return []
