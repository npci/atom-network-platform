...
def _initiator_for(case_id: str, partner_prefixes: set[str]) -> str:
    ...
    The two wire tokens come from `cert_agent.execution.wire_initiator`, which
    owns that boundary — restating them here is how one of them drifts."""
    for prefix in partner_prefixes:
        if case_id.startswith(prefix):
            return wire_initiator("partner")
    return wire_initiator("authority")
...
def case_catalogue_for_change(db: "Session", change_id: str,
    ...
    cases, exactly as an initiating participant on the network does not run the
    receiving participant's cases; the switch's and other participants' sheets
    belong to their own runs.
    A case under a role's sheet is that role's to execute (the partner-side
    initiator token for a non-authority role), overriding the id-prefix
    heuristic.
    ...
    for i, m in enumerate(headings):
        ...
        executed_by_partner = bool(case_role) and not is_authority_role
        if executed_by_partner:
            initiator = wire_initiator("partner")
        else:
        ...
        if outcome == "success":
            ...
            # ...unless this is the PARTNER's case and the steps say the
            # partner's OWN answer is a successful one.
            #
            # `found[0]` is the first code anywhere in the block, and a
            # partner-sheet case narrates two actors: the partner answers, then
            # the authority reacts. The first code is therefore routinely the
            # AUTHORITY's rejection — asserted against the PARTNER's response.
            #
            # Observed on 8ebc2b48's Operator sheet, TC_4: the operator is
            # specified to answer "overall standing SUCCESS but with
            # hangarLocation set to an empty string", and ComplianceService then
            # raises E008. The operator was failed for not emitting E008 — a
            # code it is never supposed to send. Correct behaviour graded as a
            # defect.
            if executed_by_partner:
                own = _partner_own_response(
                    block, role_labels.get(case_role or ""), code_pattern)
                if own == "SUCCESS":
                    expected_rc = "SUCCESS"
...
# The workbook's per-case sections. Only TEST STEPS is read by
# `_partner_own_response`, and that scoping is load-bearing — see its docstring.
_TEST_STEPS = re.compile(r"\*\*TEST\s+STEPS\*\*", re.IGNORECASE)

# Clause boundaries. Semicolons and em-dashes matter as much as full stops: the
# workbook routinely puts the partner's answer and the authority's reaction in
# one sentence joined by either.
_CLAUSE_SPLIT = re.compile(r"(?<=[.;])\s+|\s+—\s+")

# Words meaning the partner's OWN answer was not a clean success. Deliberately
# broad: a false negative here just leaves the existing derivation alone, while
# a false positive would assert SUCCESS on a case whose partner really is
# supposed to answer with an error.
_NOT_A_SUCCESS = ("failure", "suspended", "times out", "timed out", "timeout",
                  "not airworthy", "no resp", "rejected", "unable",
                  "zero or null", "aborted")


def _partner_own_response(block: str, partner_label: str | None,
                          code_pattern) -> str | None:
    """"SUCCESS" when the TEST STEPS say the PARTNER itself answers successfully.

    Returns None when the steps never say what the partner returned — the
    honest answer for a case like a double-timeout, where the partner by
    definition sends nothing. None leaves the caller's derivation untouched, so
    this can only relax an assertion the steps contradict, never invent one.

    READS **TEST STEPS** ONLY, AND THAT IS THE WHOLE TRICK. An earlier version
    read the entire block and was withdrawn after a dry run against a real
    workbook: DESCRIPTION prose is COPY-PASTED between cases with different
    outcomes. On 8ebc2b48's Operator sheet, TC_2 (a FAILURE-standing case) and
    TC_3 (a double timeout) both carry TC_1's happy-path description verbatim —
    "the Operator responds with a SUCCESS status and cyclesRemaining greater
    than zero" — so a whole-block reader concluded SUCCESS for both and would
    have broken two passing cases to fix one. TEST STEPS are written per case
    and do not share that defect.

    Scoped further to the CLAUSE naming the partner, because the codes in a
    partner-sheet case live in the AUTHORITY's clauses — reading whole steps is
    how they leak back into the partner's expectation.
    """
    if not partner_label:
        return None
    label = partner_label.strip().lower()
    if not label:
        return None
    m = _TEST_STEPS.search(block or "")
    if not m:
        return None
    for clause in _CLAUSE_SPLIT.split(block[m.end():]):
        low = clause.lower()
        if label not in low:
            continue
        if "return" not in low and "respond" not in low:
            continue
        # A code inside the partner's OWN clause is the partner's to emit —
        # leave those alone; they are exactly the cases this must not touch.
        if code_pattern and code_pattern.findall(clause):
            return None
        if any(w in low for w in _NOT_A_SUCCESS):
            return None
        return "SUCCESS"
    return None
