...
Provisions a participant into precertdb, runs each certification case against the live precert
switch (precert -> the partner simulator -> the simulator's transaction log table), and applies
a pure pass/fail rule to the response code the partner's own system actually returned.
...
cert-agent <-> partner-agent services.
...
__all__ = [
    ...
    "NfiniteProvisioner", "ParticipantConfig",
    "run_certification",
]
