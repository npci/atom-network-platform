...
Thin orchestration over provisioning + connector: provision the partner, resolve the cases in the
...
from .provisioning import ParticipantConfig, NfiniteProvisioner
...
def run_certification(
    ...
    *,
    ...
    """Provision `participant`, run `cases` (or every case mapped to `subset`) and return a run_data dict.
    ...
    robustness. `provision=False` skips the DB writes (for a re-run against an already-set-up partner).
    """
    prov = NfiniteProvisioner(db)
    if provision:
        prov.configure_participant(participant)
        if cases:
            prov.assign_scope(participant, cert_name=cert_name, subset=subset, cases=cases)
    ...
    for tc, cg in scope:
        ...
        try:
            r = conn.run_case(
                CaseSpec(participant.participant_org_id, subset, cert_name, tc, cg),
                timeout_s=timeout_s)
