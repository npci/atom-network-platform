...
# (stored as regexes, not synthesized from labels, so the network pack's hand-tuned
# negative lookaheads survive byte-for-byte); `primary_effect_operations` names
# the operations that make an FR a "financial" flow. A pack that declares none
...
def _tagging_vocab(pack_key: str) -> tuple[dict, dict, frozenset]:
    ...
    from app.core.domain.contract import (
        operation_patterns_of, party_patterns_of, primary_effect_operations_of,
    )
    ...
    return op_patterns, party_patterns, primary_effect_operations_of(pack)
