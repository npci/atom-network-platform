...
# Domain error code pattern: should be digits like 00, U02, etc. — not HTTP 4xx/5xx
...
def check_error_code_table_present(artifact: dict) -> list[str]:
    ...
    if "error code" not in text.lower() and "error-code" not in text.lower():
        return [
            ...
            "Network integrations require explicit error codes."
        ]
    return []
...
def check_at_least_one_source(artifact: dict) -> list[str]:
    ...
    return [
        ...
        "(authority documentation, circular, etc.) before advancing."
    ]
...
# ── Cross-artifact grounding checks ─────────────────────────────────────────
#
...
# (`error_code_pattern` in the pack YAML) rather than one domain's literal baked
# in here. The network pack declares the identical regex, so its verdicts are
# unchanged. A domain that declares no shape gets the check SKIPPED — see
# `_valid_error_code_pattern` — because applying one ecosystem's alphabet to another
...
def check_domain_error_codes_are_valid(artifact: dict) -> list[str]:
    ...
    The NAME is pinned: it is referenced by id from the eval contracts
    (`contracts.py`) and from the `CHECKS` registry below, and those
    references are validated at import time. Rename all three together or
    not at all.
    """
...
def check_no_http_codes_as_domain_errors(artifact: dict) -> list[str]:
    ...
    section (these are not valid domain codes and confuse partner systems)."""
    tech_spec = _get_named_artifact(artifact, "tech_spec_document")
    text = (tech_spec.get("content") or tech_spec.get("text") or "")
    if not text:
        return []
    ...
    if bad:
        return [
            ...
            "Replace with the domain's error codes (U##, Z#, RB, XT, XD, YB, YC, YD)."
        ]
    return []
