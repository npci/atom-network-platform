...
def read_critic_config(db: Session, checkpoint_id: CheckpointId) -> dict:
    ...
    if not provider:
        ...
        generator_provider = (getattr(settings, "llm_provider", "anthropic") or "anthropic").lower().strip()
        if cross:
...
def _pick_cross_provider(generator_provider: str) -> str:
    ...
    Falls back to the generator's own provider when no other provider is
    actually USABLE. That last word is the fix: the preference table used to be
    consulted blind, so an ainxt-only or openai-only deployment was routed to
    `anthropic` for every critic call — a provider it has no key for, and in
    the ainxt case one the network may not even be able to resolve. Now a
    preference is only honoured when its API key is configured.
    """
    preference = {
        "claude":    ("openai", "gemini", "anthropic"),
        "anthropic": ("openai", "gemini"),
        "openai":    ("anthropic", "gemini"),
        "ainxt":     ("anthropic", "openai", "gemini"),
        "ollama":    (),   # local-only env: nothing else to fall back to
    ...
    key_for = {
        ...
        "ainxt":     getattr(settings, "ainxt_api_key", ""),
    }
    for candidate in preference.get(generator_provider, ()):
        if (key_for.get(candidate) or "").strip():
            return candidate
    logger.info(
        "eval critic: no configured cross-provider for generator %r — grading with the "
        "generator's own provider. Set another provider's API key for independent eyes.",
        generator_provider,
    )
