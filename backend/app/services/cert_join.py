...
"""The suite join — harness-agnostic, beside the dispatch seam.
...
next partner report or deadline sweep picks up exactly where the process died.
...
`not_reported` marker the orchestrator stamps at dispatch. A partner report
...
def deadline_for(run: CertRun):
    ...
    window (migrations 0133–0135 stay reserved for the simulator/tunnel columns).
...
async def finalize_run(db: "Session", run: CertRun, *, reason: str) -> dict:
    ...
    a COMPLETED run is left untouched, so a partner report racing the deadline
...
async def check_and_finalize(run_id: str) -> dict:
    """Partner-report hook: finalize iff nothing is pending. Opens its own
...
def _join_managed(run: CertRun) -> bool:
    ...
    marker (still awaiting) or a partner-reported row (was awaiting). A run with
    neither belongs to a synchronous harness and is out of bounds."""
    for r in run.results:
        response = r.actual_response
        if isinstance(response, dict) and (
                response.get("not_reported")
                or response.get("reporter") == wire_initiator("partner")):
            return True
    return False
