...
Field-level, not just API-level. "this API changed" is not reviewable; "the
enum on <api>/<element>/@type lost one of its values" is. So APIs are compared
by name, fields within them by path, and each changed field reports its changed
