...
the earlier reconciliation pass never caught. Known divergences the builders
here fix:

  * ``cert_setup_notification`` omits the ``simulator`` block entirely, so the
    partner is never told where to send partner-initiated cases (spec: the URL
    the partner's stack calls for partner-initiated cases).
...
NOTHING CALLS THIS YET. This change vendors + tests the layer; the cert path is
rewritten around it later. Keep this file a pure mirror of the
...
def cert_case_result(
    *,
    case_id: str,
    attempt: int,
    reporter: str,                   # a `wire_initiator()` token
