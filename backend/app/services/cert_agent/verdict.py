...
A certification test case says what response it EXPECTS from the partner; the partner's
system returns what it ACTUALLY did. This decides PASS or FAIL by comparing the two.
...
LLM. It reads the two-part response shape every domain here uses — a result category plus a
response code — so a test expecting {result: SUCCESS, code: "00"} passes only if the partner
...
class ResultStatus(str, Enum):
    ...
    FAILURE = "FAILURE"   # declined, with a domain-defined reason code
    DEEMED = "DEEMED"     # the authority acted even though the partner stayed silent
...
class Expectation:
    """What the test case expects the partner to return."""
...
class ResponseOutcome:
    """What the partner's system actually returned."""
    result_status: ResultStatus
    code: str = ""
