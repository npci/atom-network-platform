...
"""Turn the partner's ``cert_config_submission`` into a real precert environment.

This is the step the spec calls for and the platform never did. Per Appendix B,
``cert_setup_notification`` fires once "the Authority validated the config, provisioned
the simulator, mapped the suite, generated credentials" — but the cert path only
ever READ precertdb and skipped the whole run when the partner was absent
(``skip_reason="participant … not onboarded / empty scope"``), which surfaced nowhere
because the orchestrator is fire-and-forget.

So: config arrives -> onboard the partner -> map the subset it asked for -> and only
...
from app.services.precert_engine.provisioning import ParticipantConfig, NfiniteProvisioner
...
class ProvisionResult:
    ...
    changes: list[str]                # what configure_participant / assign_scope actually did
    requested_subset: str | None      # what the partner asked for, before any fallback


def participant_config_from_submission(cfg: dict, *, defaults: dict | None = None) -> ParticipantConfig | None:
    """Map a ``cert_config_submission`` payload onto precert's ParticipantConfig.

    Accepts BOTH shapes on purpose. The key names quoted below are the
    PARTNER'S spelling on the wire, kept verbatim because they are what
    actually arrives; the config this returns uses neutral field names.
    ...
    A later revision moves the partner to the nested shape; until then the flat one is
    ...
    unaffected. If credential-block encryption is ever driven from this, that
    stops being true.

    Returns None when the payload carries too little to identify a participant.
    """
    cfg = cfg or {}
    d = defaults or {}
    ...
    # The SUBMISSION KEYS below are the partner's wire contract, not ours — the
    # counterparty deploys separately, so they stay exactly as sent. This
    # function is the boundary: everything it returns uses ParticipantConfig's
    # neutral field names.
    participant_org_id = pick(cfg.get("psp_org_id"), ident.get("participant_code"),
                              d.get("psp_org_id"))
    if not participant_org_id:
        return None
    ...
    # "8443.0", which precert would store as the partner's switch port and then fail to
    ...
    return ParticipantConfig(
        partner_name=pick(cfg.get("bank_name"), ident.get("bank_name"), fallback="Participant"),
        partner_code=pick(cfg.get("bank_code"), d.get("bank_code")),
        partner_org_id=pick(cfg.get("bank_org_id"), ident.get("org_id"), d.get("bank_org_id")),
        partner_routing_code=pick(cfg.get("bank_ifsc"), ident.get("ifsc")),
        participant_name=pick(cfg.get("psp_name"), fallback="Participant"),
        participant_org_id=participant_org_id,
        participant_code=pick(cfg.get("psp_code"), ident.get("participant_code")),
        handler=handler,
        credential_length=pick(cfg.get("mpinlength"), fallback="6"),
        partner_server_ip=pick(cfg.get("bank_server_ip"), net.get("host")),
        partner_server_port=pick(cfg.get("bank_server_port"), port),
...
def catalogue_cases(db_cfg: dict, subset: str) -> list[tuple[str, str]]:
    ...
    try:
        cur = con.cursor()
        # The catalogue table lives in the cert agent's database, not ours, so
        # its name is deployment config rather than a constant here.
        from app.core.config import require_external_table, settings
        table = require_external_table(
            settings.cert_agent_testcase_table,
            "CERT_AGENT_TESTCASE_TABLE", "the cert agent")
        cur.execute(
            "SELECT u.name, u.certgroup FROM tbl_subset_testcase st "
            f"JOIN {table} u ON u.id = st.testcase_id "
...
def provision_from_config(
    ...
    *,
    ...
    """Onboard the partner and map its requested suite. Idempotent.

    `configure_participant` and `assign_scope` both no-op on rows that already exist, so
    re-running a cert for an already-onboarded partner changes nothing — which is
    what keeps this safe to put in the middle of a live conversation.

    Subset resolution: the partner's ``requested_subset`` wins (the spec makes the
    subset the partner's choice, not the Authority's). An unknown one falls back to
    ...
    Returns None if the payload cannot identify a participant, or if the resolved subset
    has no cases to map.

    HSM cert seeding removed: this used to copy a shared RSA public key into
    ...
    `participant.hsm_file_b64` is written empty, same as
    `participant_config_from_submission` already leaves it.
    """
    participant = participant_config_from_submission(cfg, defaults=defaults)
    if participant is None:
        logger.warning("cert_setup: config submission carried no psp_org_id — cannot onboard")
        return None

    prov = NfiniteProvisioner(db_cfg)
    changes = prov.configure_participant(participant)
    ...
    if requested and not subset_exists(db_cfg, subset):
        logger.warning(
            "cert_setup: participant %s requested unknown subset %r — falling back to %r",
            participant.participant_org_id, requested, default_subset)
    ...
    changes += prov.assign_scope(participant, cert_name=cert_name, subset=subset,
                                 cases=[tc for tc, _ in cases])
    logger.info("cert_setup: participant=%s subset=%s cases=%d changes=%s",
                participant.participant_org_id, subset, len(cases), changes)
    return ProvisionResult(participant=participant, subset=subset, cases=cases,
...
def simulator_block(*, alias: str, protocol_version: str | None = None,
    ...
    ``endpoint`` is an **alias declaration**
    (``a2a://<alias>``), never a raw URL. A URL on the wire is an address for
    the partner to dial straight into the Authority's test estate — the exact shape the
    ...
    e.g. ``network-1.0`` / ``nlln-1.0``); a literal here would put one
    ecosystem's protocol label on every domain's wire. A caller with a real
    protocol registry can still pass its own string.
