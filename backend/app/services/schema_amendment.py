...
UNCONDITIONAL refusal it created a deadlock with no exit:
    ...
    Phase A added an enumeration value to a schema, reusing a token another
    constant had already claimed — so every message carrying it would be
    classified as the wrong thing downstream. The reviewer correctly demanded
    a different token. The code agent could not make that edit. The reviewer
    re-issued the same demand every round; the agent escalated to a human seven
    times over seventeen hours and never shipped.
