...
class CircularDocxBuilder(DocxBuilder):
    ...
    def add_letterhead(self, reference: str = "", date: str = "") -> None:
        # Issuer name and rail name come from the ACTIVE PACK. Both were
        # literals, so every .docx this platform generated — in every domain —
        # carried one clearing house's name in bold at the top. An empty
        # letterhead is the right last resort: a document with no issuer is
        # obviously incomplete, one carrying somebody else's name is wrong only
        # to a reader who already knows. Same posture as `add_footer()` below.
        from app.core.domain.registry import prompt_block

        org_name = (prompt_block("authority_full_name", "")
                    or prompt_block("authority", "")).strip()
        if org_name:
            p = self.doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        domain_name = prompt_block("domain_name", "").strip()
        if domain_name:
            p2 = self.doc.add_paragraph()
            p2.alignment = WD_ALIGN_PARAGRAPH.CENTER
            r2 = p2.add_run(domain_name)
