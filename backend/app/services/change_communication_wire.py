...
after per-partner size-gating) to the **v1.1 canonical wire payload**.
...
_NULL_BUSINESS_FIELDS = (
    ...
    # WIRE-PINNED: `npci_contact` is a contract §7 field NAME. It is emitted as
    # null in every domain, but the counterparty reads the key, so renaming it
    # here would drop a field from the envelope rather than change any wording.
    "npci_contact",
)
