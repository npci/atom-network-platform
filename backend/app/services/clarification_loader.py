...
def _recover_parties_multi_select(entry) -> list[str]:
    ...
    option `id` (which the question builder set to the domain's own
    canonical PARTY key). Malformed JSON → empty list (safe default).
    """
...
def _recover_signal_value(entry) -> str | None:
    ...
    ("yes"/"no", "low"/"high", and the pack's own compliance tiers), so we map
    the chosen label back to its option id. Case is preserved — compliance
    ...
    Shape:
        {
            ...
            "compliance_sensitivity": str,               # <pack compliance tier>
