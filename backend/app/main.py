...
# T8 — record every successful mutating request that passed
...
# Clickjacking defence-in-depth. Unconditional (not prod-only,
# unlike HSTS) since framing is a risk in every environment.
#
# SCOPE — read this before treating it as the app's clickjacking/CSP answer:
# this middleware decorates responses that pass through FastAPI, i.e. /api/*
# JSON, which is not a framable target in the first place. The page an attacker
# would actually frame is the SPA document, served off disk by the frontend
# container, which never reaches this code.
#
# An earlier version of this comment asserted that
# frontend/nginx.conf.template "already sets these on the SPA's own responses".
# It did not — the add_header block had been lost when that file was
# reconstructed, so the SPA shipped with no X-Frame-Options and no CSP at all
# while this comment implied otherwise. The headers now exist there (including
# a real script-src, which this layer cannot provide). Verify that file
# directly; do not infer the browser surface's posture from here.
@app.middleware("http")
...
# Correlation ID — one id per request, logged and returned.
#
# The 500 handler below deliberately returns a fixed, detail-free body (that is
# the right call: no stack traces to clients). The cost was that a
# user-reported failure could not be tied to its log line at all — "it broke"
# and a log file with no shared handle. An opaque UUID leaks nothing while
# making the join possible.
#
# Registered as middleware so the id exists for the whole request, not only on
# the error path; `request.state.correlation_id` is available to any handler
# that wants to log it, and every response carries it.
@app.middleware("http")
...
async def _unhandled_exception_handler(request: Request, exc: Exception):
    cid = getattr(request.state, "correlation_id", None) or "-"
    logger.exception("Unhandled exception on %s %s [correlation_id=%s]",
                     request.method, request.url.path, cid)
    from fastapi.responses import JSONResponse
    # The id is the ONLY thing added to the body — still no exception text, no
    # stack, no paths. It is what the user quotes and what greps the log.
    return JSONResponse(
        status_code=500,
        content={"detail": "An internal error occurred", "correlation_id": cid},
...
# router (above) until registered partners are flipped over via the
# `protocol_version` field on PartnerAgent. The legacy router is decommissioned after that.
#
...
try:
    ...
    # HMAC envelope verification (X-NPCI-Signature etc.).
    ...
    try:
        _a2a_sub_app, _a2a_card_routes = build_a2a_components(
            ...
            # In-memory store for now — a later SDK refactor swaps
        ...
        logger.info(
            ...
            "(HMAC envelope + Bearer JWT enforced via the inbound middlewares)"
        )
...
def _seed_authority_policy_from_file() -> None:
    """First-boot seed of the policy singleton row. No-op if the table already
    has a row.

    The file is the bootstrap source; the DB row is the runtime source of truth
    once seeded. Admins maintain the row via the Admin UI thereafter. Re-seeding
    (after a wipe) just requires deleting the row and restarting.

    Which file, and why it may be the pack's rather than the operator's:
    `app.core.authority_policy_seed`.
    """
    from app.core.authority_policy_seed import authority_policy_seed_path
    from app.core.database import SessionLocal
    from app.models.authority_policy import AuthorityPolicy

    path = authority_policy_seed_path()
    db = SessionLocal()
    try:
        ...
        if path is None:
            logger.info(
                "No authority policy seed found (checked %s and the active "
                "pack's data/ directory) — leaving the row empty; an admin can "
                "paste content into the Admin UI",
                settings.authority_policy_path,
            )
            content = ""
        else:
...
async def on_startup():
    ...
    # Opt-in (AUTO_MIGRATE_ON_STARTUP, default off) — apply pending migrations
    # before validation runs, so the drift check below has nothing to report
    # rather than refusing the boot. Only useful in the dev hot-reload stack,
    # where a model edit reloads this process but nothing re-runs alembic; see
    # the setting's comment in config.py for why it must stay off elsewhere.
    if getattr(settings, "auto_migrate_on_startup", False):
        try:
            from app.core.db_migrate import upgrade_to_head
            applied = upgrade_to_head()
            logger.info("Auto-migrate: schema at head%s",
                        f" (applied {applied})" if applied else " (nothing to apply)")
        except Exception:
            # Deliberately non-fatal: the drift check immediately below is what
            # decides whether the resulting schema state may serve traffic. A
            # failure here that DID leave the schema behind will be reported
            # there, with the revisions named; one that did not is not worth
            # taking the process down for.
            logger.exception("Auto-migrate failed — falling through to schema validation")

    # A7 / S1 — fail-fast security config validation.
    # Runs AFTER load_db_overrides() so an operator's
    ...
    from app.core.llm import get_model as _get_model
    logger.info("LLM config: provider=%s model=%s prompt_cache_enabled=%r",
                settings.llm_provider, _get_model(), settings.prompt_cache_enabled)
    ...
    # Fire one cheap embed call so Ollama loads the model
