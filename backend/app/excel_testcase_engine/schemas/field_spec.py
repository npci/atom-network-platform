...
Source of truth: `authority_specs/message_fields.json`. Every value is
...
class FieldSpec(BaseModel):
    ...
    `name` is the wire-format field name exactly as the domain's schema spells
    it (e.g. "requestId", "expiryTimestamp") — case-preserving. `legs` lists
    which message legs of the primary API this field appears in.
    """
...
class FieldCatalog(BaseModel):
    ...
    `authority_specs.get_field_specs(api)` rather than reading the raw dict
