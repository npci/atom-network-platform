...
one test stub per ``DeclineRow`` (see ``decline_expander``).
...
- ``owning_entity`` is the party that PRODUCES the failure; ``observing_entity``
  is the party that must HANDLE it. The two drive the role-sheet split.
...
class DeclineRow(BaseModel):
    ...
    id:                str                 # stable handle, e.g. "DCL-INI-001"
    api:               str                 # the primary API this failure occurs on
    owning_entity:     str                 # who FAILS (e.g. "Initiating Participant")
    observing_entity:  str = ""            # who must HANDLE it (e.g. "Receiving Participant")
...
class ExcludedCandidate(BaseModel):
    ...
    candidate: str                         # e.g. "Authorisation ceiling exceeded (T28)"
    reason:    str                         # e.g. "feature has no authorisation leg"
