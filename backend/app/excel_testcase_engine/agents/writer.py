...
network grounding are all removed. The writer sees only the current change's
...
from ._runtime import parse_json_response, render_prompt
...
def _writer_system() -> str:
    """The writer system prompt, with the initiator tokens filled from the pack.

    Same contract as `planner._planner_system`, and missed when that one was
    written: hard rule 7 tells the model which literal to put in
    `txn_initiated_by`, and that literal is a wire value the certification agent
    matches character for character. Frozen in the markdown it named one
    ecosystem's tokens whatever pack was active, and `WorkbookPlan`'s validator
    rewrites an unrecognised token to the PARTNER role rather than raising — so
    a non-payments pack silently lost every authority-initiated case with
    nothing in the logs.

    `DETAILS_BLOCK_EXAMPLE` is the same class of bug one step softer. The
    worked example under "details_block" spelled out one ecosystem's roles,
    message names and currency verbatim, so a library-domain run was shown
    a payment as the model of a
    well-formed one-line behaviour and copied its vocabulary into prose the
    TSD never authorised. The neutral default names no roles at all rather
    than borrowing another domain's.
    """
    ...
    return render_prompt(
        ...
        DETAILS_BLOCK_EXAMPLE=prompt_block(
            "testcase_details_block_example",
            "the initiating role sends the request; the response returns success",
        ),
    )
...
async def _llm_call(
    ...
    system = [SystemBlock(text=_writer_system(), cache=True)]
...
async def _lint_and_repair(
    ...
    own response_code fields (for error codes). No canonical network set.
    """
