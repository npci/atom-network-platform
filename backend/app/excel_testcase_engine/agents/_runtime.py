...
def render_prompt(name: str, **values: str) -> str:
    """Load a prompt from this package and fill its `{{NAME}}` placeholders.

    Delegates to `app.core.prompts.render_prompt` rather than re-implementing
    substitution, so this package inherits that function's two-way contract:
    every placeholder in the template must be supplied, AND every supplied name
    must appear in the template. The second half is the one that earns its
    keep — it turns a renamed placeholder into an error instead of a prompt
    that ships with a literal `{{...}}` in it.

    Needed because these prompts state WIRE VALUES back to the model: the
    initiator tokens the certification agent matches character for character.
    Hardcoded in the markdown, they instructed the model to emit one
    ecosystem's tokens regardless of which pack was active.

    NOT lru_cached, unlike `load_prompt` above: the values come from the active
    domain pack, which is chosen per process by an environment variable and is
    swapped between tests.
    """
    from app.core.prompts import render_prompt as _render

    return _render(name, base=PROMPTS_DIR, **values)
