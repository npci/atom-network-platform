...
whatever the stubs declare, not from a bundled network catalog.
"""
...
def _step_lint_defects(
    ...
    plan itself declares — no canonical network catalog.
    """
    ...
    severity_map = {
        ...
        # WARNING, not critical, deliberately. The generator cannot invent a
        # code the BRD never named, so failing the build on it would block kits
        # whose source genuinely is silent. It must be VISIBLE — an ungradeable
        # failure case is how a partner gets marked wrong for correct behaviour
        # — but the person who can fix it is the BRD author, not this pipeline.
        "failure_without_response_code": "warning",
