...
def _check_failure_terminus(stub: TestCaseStub, steps: str, code_allow: set[str], issues: list[StepIssue]) -> None:
    ...
    if not stub.response_code:
        # A Failure case that names NO code anywhere is the one this rule most
        # needs to catch, and it used to be the one case that returned early —
        # the check bailed out precisely when there was nothing to check
        # against, so the defect passed as clean.
        #
        # What it costs downstream: the workbook ships with no
        # `_Response code:_` marker for that case, and the certification
        # grader has nothing authoritative to assert. `cert_catalogue` then
        # falls back to scraping the first error code out of the narrative,
        # and on a partner sheet that code is usually the AUTHORITY's
        # rejection rather than the partner's own answer — so the partner is
        # failed for behaving correctly. Change 8ebc2b48's Operator sheet
        # reached a live cert run with 8 cases and 0 markers this way.
        #
        # `planner.py` is deliberately instructed to leave response_code empty
        # when the BRD names no code, and nothing downstream fills it in — the
        # repairer only APPENDS a code it already has. So this is the only
        # place the gap can be reported rather than silently shipped.
        issues.append(StepIssue(
            test_id=stub.test_id,
            code="failure_without_response_code",
            message="Failure case carries no response_code, so nothing can "
                    "assert what it should return.",
            fix_hint="Name the error code this scenario produces in the BRD, "
                     "or set response_code on the stub. A failure case with no "
                     "code cannot be graded.",
        ))
        return
...
def lint_plan(
    ...
    canonical network union). Callers derive them from the TSD interface_spec
