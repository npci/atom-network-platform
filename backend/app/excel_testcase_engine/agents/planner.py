...
network specs (apis / error codes / scope ownership / coverage matrix / XSD diff)
...
from ._runtime import (
    load_prompt, parse_json_response, render_prompt, retry_message as _retry_message,
)
...
# error codes (network: U09, T27, GC, ZM, ZH). Extracted from the BRD text so the LLM sees
...
# hand-copied domain lists drifted: only the planner knew PSP1).
...
def _planner_system() -> str:
    """The planner system prompt, with the initiator tokens filled from the pack.

    The prompt tells the model which literal to put in `txn_initiated_by`, and
    that literal is a wire value the certification agent matches exactly — so
    it has to come from the active pack rather than be frozen in the markdown.

    The `STUB_EXAMPLE_*` values fill the worked `TestCaseStub` JSON a few lines
    below that rule. Only `txn_initiated_by` was ever templated, so the schema
    example around it still read `"apis": ["ReqTransfer","RespTransfer"]`,
    `"api_type": "Pay"` and one ecosystem's role in `"entities"` on every pack — and a
    schema example is the strongest instruction in a prompt. A library-domain
    run planned "Pay" cases against payment APIs the TSD never declared, then
    the writer's API-name rule dutifully carried them into the workbook.
    """
    from app.core.domain.registry import prompt_block
    from app.excel_testcase_engine import domain_vocab
    from app.services.cert_roles import authority_wire, partner_wire

    # Derived, not re-declared: a pack that already names its request/response
    # pair and its cert defaults should not have to repeat them for one example.
    apis = domain_vocab.default_apis() or ["<primary Req API>", "<primary Resp API>"]
    api_type = prompt_block("cert_default_api_type", "") or "Message"
    entities = [e.strip() for e in prompt_block("cert_default_entities", "").split(",")
                if e.strip()] or ["<the sheet's role>"]
    prefix = prompt_block("testcase_stub_example_prefix", "TC")

    return render_prompt(
        "planner.md",
        AUTHORITY_INITIATOR=authority_wire(),
        PARTNER_INITIATOR=partner_wire(),
        STUB_EXAMPLE_TEST_ID=f"{prefix}_1",
        STUB_EXAMPLE_APIS=", ".join(f'"{a}"' for a in apis),
        STUB_EXAMPLE_API_TYPE=api_type,
        # One entity, not the whole list: the example is showing the SHAPE of
        # the field, and a two-element list reads as "always name both parties".
        STUB_EXAMPLE_ENTITIES=f'"{entities[0]}"',
        STUB_EXAMPLE_SCENARIO=prompt_block(
            "testcase_stub_example_scenario", "Successful happy path"),
        STUB_EXAMPLE_PAIR_ID=f"{prefix}_{api_type.lower()}_1",
    )
...
async def _plan_skeleton(brief: EnrichedBrief) -> WorkbookPlan:
    ...
    system = [
        SystemBlock(text=_planner_system(), cache=True),
        SystemBlock(
            text=(
                "## Skeleton-only mode\n"
...
def _format_stubs_user_message(
    ...
    # The two tokens are the pack's WIRE spellings, not literals. The system
    # prompt has resolved them from the pack for a while; this instruction —
    # which is the one that actually tells the model what to WRITE into the
    # field — was still naming one ecosystem's pair, on the line immediately
    # after a correctly pack-resolved authority label.
    #
    # The failure was silent by construction: the model dutifully emits the
    # literal it was shown, `normalize_role()` does not recognise it under any
    # other pack, callers read None as "unknown initiator" and leave the field
    # empty, and the whole run groups under the upload default. Nothing raises.
    # That is the exact failure mode `workbook_plan.TestCaseStub` documents as
    # the reason these tokens come from `cert_roles:` at all.
    # Omitted entirely when the pack declares no cert roles — a domain with no
    # certification body has no two sides to attribute a first message to, and
    # interpolating the empty tokens would ask the model to choose between two
    # empty backticks. An absent instruction leaves `txn_initiated_by` empty,
    # which every downstream reader already treats as "unknown initiator".
    _partner_tok, _authority_tok = partner_wire(), authority_wire()
    initiated_by_instruction = (
        ...
        f"all other parties): `{_partner_tok}` when this role fires the "
        f"first message of the case, `{_authority_tok}` when the simulator "
        "fires first toward this role — see the hard rule in your system "
        "prompt.\n"
        if sheet.layout in ("C1", "C2") and _partner_tok and _authority_tok
        else ""
    )
    ...
    if brd_codes:
        codes_hint = (
            ...
            f"Do NOT fall back to canonical "
            f"{domain_vocab.authority_label()} codes.\n"
        )
