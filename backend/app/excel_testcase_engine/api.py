...
#   - the .xlsx — Authority-format workbook, only the engine renders it.
...
def _serve_companion(db, change_id: str, current_user, *, kind: str, mime: str, ext: str):
    ...
    """
    # A cert workbook can carry sensitive network flow detail — gate the download to
...
async def download_cert_testcase_xlsx(
    ...
    """Download the most-recently-completed cert_test_cases Authority workbook.
...
async def download_cert_testcase_json(
    ...
    # authenticated user (a cert workbook can carry sensitive network flow detail).
    _access_check(db, change_id, current_user)
...
def _validate_xlsx_bytes(data: bytes) -> list[str]:
    ...
    try:
        ...
        # Error-message exposure (CWE-209): an ImportError message can disclose interpreter/filesystem
    ...
    try:
        ...
        # Error-message exposure (CWE-209): openpyxl raises a bare KeyError for a zip that is not a
    ...
    # this is an Authority-format pack. Absence isn't fatal — but flag it so the
...
def _xlsx_to_markdown(data: bytes, *, max_cell_chars: int = 4000) -> str:
    ...
    for sheet in wb.worksheets:
        ...
        # Assume the first row is a header (real Authority packs always have one).
        headers = [
...
# because real Authority packs vary in casing/naming across archetypes A/B/C and
...
_COL_SYNONYMS = {
    ...
    # INGEST-ONLY back-compat: "psp as" and friends are the network's historical
    # column names and stay accepted forever, because they are printed in
    # workbooks that already exist. The canonical field is `role_as` (see
    # `schemas/workbook_plan.TestCaseStub`); the active pack's own
    # `role_as_label` (e.g. "Library as") is matched in `_canonical_field()`
    # below.
    "role_as":          ("psp as", "psp role", "psp acting as", "role as"),
...
def _canonical_field(header: str) -> str | None:
    ...
    # maps to `role_as` alongside the network's historical spellings.
...
def _detect_header_row(rows: list[list], *, scan_depth: int = 6) -> tuple[int, dict[int, str]]:
    ...
    Real Authority annexure workbooks often have decorative rows at the top —
