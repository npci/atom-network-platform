...
half-dozen hardcoded payment-network copies they used to be. Every function degrades
...
txn_initiated_by` stays on the pack's `cert_roles:` wire tokens
(cert-agent matches those exactly);
...
def feature_name_default() -> str:
    """Workbook/feature-name fallback (network pack: "Network Test Cases")."""
...
def default_apis() -> list[str]:
    ...
    engine defaults (network pack: ReqTransfer, RespTransfer). Empty when
    undeclared."""
    return _csv_block("default_apis")


def default_response_api() -> str:
    """The response half of `default_apis` (network pack: RespTransfer); "" when
    none."""
...
def role_sheet_names() -> list[str]:
    ...
    block when declared (packs usually declare short role forms rather than
    full participant titles), else the cert-party labels."""
    return _csv_block("workbook_role_sheets") or party_labels()


def summary_role_headers() -> list[str]:
    """Per-role column headers on the summary sheet, taken from the active
    pack's `summary_role_headers` block, falling back to the cert-party labels."""
    return _csv_block("summary_role_headers") or party_labels()


def scope_titles() -> list[str]:
    """Distinct certification scopes declared by the pack's `cert_vocabulary`,
    Title-cased for display. Empty when the pack scopes no roles."""
    out: list[str] = []
...
def suspect_role_phrases() -> list[str]:
    ...
    pack's `step_linter_role_phrases` block ("|"-separated — a pack declares
    one when its workbooks use more role spellings than its cert parties
    enumerate), else the cert-party labels plus participant labels."""
...
def exempt_role_phrase() -> str:
    """A phrase allowed in steps without a DETAILS entity entry (network pack:
    "Network / Authority" — the rail itself); "" when the pack declares none."""
    return prompt_block("step_linter_exempt_phrase", "")


def cred_subtypes() -> str:
    """The domain's credential subtypes for auth steps (network pack:
    "NETWORKPIN / MPIN / OTP"); "" when the domain has no credential-block
    concept, which disables the cred-consistency lint rather than borrowing
    the payment network's."""
