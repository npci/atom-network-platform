...
6. **`assumptions`** — one short line per non-obvious inference (e.g. "treated the fulfilment agent as a distinct party; the TSD names it only in the sequence diagram"). Keep it short. Never record a role you added to satisfy an archetype — do not add such roles at all.
