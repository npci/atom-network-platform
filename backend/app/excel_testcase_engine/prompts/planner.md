...
The user message carries the enriched brief and, in stubs mode, extracts from the BRD and TSD. **The BRD and TSD are the ONLY sources of truth.** Do not use canonical network knowledge, historical packs, or your own memory of what "typical" authority cases look like.
...
```json
{
  "test_id": "{{STUB_EXAMPLE_TEST_ID}}",
  "apis": [{{STUB_EXAMPLE_APIS}}],
  "api_type": "{{STUB_EXAMPLE_API_TYPE}}",
  "entities": [{{STUB_EXAMPLE_ENTITIES}}],
  "scenario_summary": "{{STUB_EXAMPLE_SCENARIO}}",
  ...
  "pair_id": "{{STUB_EXAMPLE_PAIR_ID}}",  // groups Success/Failure siblings or Req/Resp legs
  "txn_initiated_by": "{{PARTNER_INITIATOR}}",  // {{PARTNER_INITIATOR}} | {{AUTHORITY_INITIATOR}} — who fires the first message on THIS sheet
...
4. `coverage_tag` is a short slug identifying the scenario intent. Common values: `happy_path`, `timeout`, `decline`, `neg_ack`, `duplicate`, `invalid_identifier`, `limit_exceeded`. Free-form — a TSD-authored scenario name is welcome, and preferred wherever the TSD names the scenario itself.
5. `txn_initiated_by` is `"{{PARTNER_INITIATOR}}"` when the sheet's role fires the first message of the case, `"{{AUTHORITY_INITIATOR}}"` when the Authority simulator fires first. Only meaningful on C1/C2 sheets. Use those two spellings exactly, including their casing — they are wire values that the certification agent matches character for character.
