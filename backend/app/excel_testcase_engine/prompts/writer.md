...
There is no third source. Do NOT reach for canonical network grounding, historical cert packs, or general authority knowledge to fill gaps — leave the prose thinner rather than invent domain content the BRD/TSD didn't authorise.
...
7. **`txn_initiated_by`** — emit the WIRE TOKEN verbatim: `{{AUTHORITY_INITIATOR}}` or `{{PARTNER_INITIATOR}}`. These two strings come from the active pack's `cert_roles:` (see `services/cert_roles.py`) — cert-agent matches them exactly, and any other spelling silently mis-groups the run across a trust boundary. `{{AUTHORITY_INITIATOR}}` means the flow starts from the authority simulator toward the sheet's role; `{{PARTNER_INITIATOR}}` means the sheet's role fires first. The workbook renders a friendly per-domain label separately, so do not substitute one here.

## details_block, description_block, steps_block — what goes in each

- **details_block** — one-line testable behaviour ("{{DETAILS_BLOCK_EXAMPLE}}"). Under 200 chars where possible.
