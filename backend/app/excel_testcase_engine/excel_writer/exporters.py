...
def to_markdown(plan: WorkbookPlan) -> str:
    ...
    for sheet in plan.sheets:
        ...
        for tc in sheet.test_cases:
            ...
            lines.append("```")
            # The case's EXPECTED RESPONSE CODE, as a field rather than as
            # something a reader has to infer from the prose above.
            #
            # This used to be emitted only for failures carrying a non-"00"
            # code, so a whole sheet could reach a grader with no stated
            # expectation at all — 8ebc2b48's Operator sheet had 8 cases and 0
            # markers. `cert_catalogue` then fell back to scraping the first
            # error code out of the narrative, and on a partner sheet that code
            # is usually the AUTHORITY's rejection, not the partner's answer.
            # The partner was failed for behaving correctly.
            #
            # Success is now stated explicitly ("00" is the success code and was
            # being suppressed as if it meant "unset"). A failure whose code the
            # planner never captured still emits nothing — that is honest, and
            # it is the remaining gap: `planner.py` is instructed to leave
            # `response_code` empty when the BRD names no code, and no later
            # stage fills it in. The repairer only APPENDS a code it already
            # has.
            if tc.expected_status and tc.expected_status.strip().lower() == "success":
                lines.append("_Response code: `SUCCESS`_")
            elif tc.response_code and tc.response_code != "00":
          ...
          "test_cases": [
            {
              ...
              "approval_type", "initiator_handle", "counterparty_handle",
              "scenario_summary", "expected_status",
              "response_code"?,                # omitted when empty
              "coverage_tag", "scope",
              "txn_initiated_by", "role_as",
        ...
        for tc in (sheet.test_cases or []):
            try:
                entry: dict = {
                    ...
                    # Renamed off one ecosystem's vocabulary; `TestCaseStub`
                    # still ACCEPTS the old spellings on the way in (see its
                    # `validation_alias`), but the exported contract emits the
                    # domain-neutral names.
                    "initiator_handle": str(getattr(tc, "initiator_handle", "") or ""),
                    "counterparty_handle": str(getattr(tc, "counterparty_handle", "") or ""),
                ...
                from app.services.cert_roles import partner_wire
                _pw = partner_wire()
                entry["txn_initiated_by"] = str(getattr(tc, "txn_initiated_by", _pw) or _pw)
                entry["role_as"] = str(getattr(tc, "role_as", "") or "")
...
_REQUIRED_CASE_KEYS = (
    ...
    "initiator_handle", "counterparty_handle", "scenario_summary",
    "expected_status", "coverage_tag", "scope",
    "txn_initiated_by", "role_as", "rendered",
)
