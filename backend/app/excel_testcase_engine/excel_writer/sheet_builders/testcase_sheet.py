...
def _display_scope(tc: TestCaseStub) -> str:
    ...
    hardcoded "v2.0" default printed a payments version number on library-loan
    workbooks.
    """
...
def _display_initiator(tc: TestCaseStub) -> str:
    ...
    `tc.txn_initiated_by` carries the pack's wire tokens because cert-agent
    matches those exactly (see the field's note in workbook_plan). Only the
    rendered label is per-domain, so an NLLN workbook reads "NLLC" where a
    payment-network one reads its own authority label, while both push the
    identical value on the wire.
    """
    from app.core.domain.registry import prompt_block
    raw = (tc.txn_initiated_by or "").strip()
    from app.services.cert_roles import authority_wire, partner_wire
    if raw.upper() == authority_wire().upper():
        return prompt_block("initiator_authority_label", raw)
    if raw.upper() == partner_wire().upper():
...
def _row_for_layout(layout_key: str, index: int, tc: TestCaseStub) -> list[object]:
    ...
    if layout_key == "C2":
        return [index, tc.test_id, details, tc.role_as, description, _display_scope(tc),
                _display_initiator(tc), tc.expected_status, steps]
    if layout_key == "C3":
