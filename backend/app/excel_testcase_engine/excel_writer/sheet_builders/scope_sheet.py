...
"""Build the gateway-style scope sheet."""
...
def build(ws: Worksheet, plan: WorkbookPlan) -> None:
    ...
    # The success row is pack content too, not a "universal" row. `00` is the
    # ISO-8583/payments success code — universal only inside that family, and
    # meaningless on a library-lending workbook, which was printing it as the
    # one guaranteed entry of its response-code table. A pack that declares no
    # success code simply has no such row.
    from app.core.domain.registry import prompt_block
    from app.excel_testcase_engine import domain_vocab

    success = prompt_block("scope_sheet_success_code", "")
    if success:
        code, _, desc = success.partition("|")
        if code.strip():
            ws.append([code.strip(), desc.strip() or "Success"])
