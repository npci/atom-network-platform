...
def _log_env(suffix: str, default: str) -> str:
    """Read `TCE_LOG_<suffix>`, falling back to the legacy `UPI_LOG_<suffix>`.

    These are the only two env var NAMES in the tree that carried an ecosystem
    in them — visible to anyone reading the deployment guide or the output of
    `env`, on a platform that is not that ecosystem's. Renamed to TCE_ (the
    test-case engine, which is the thing they configure).

    The legacy name is still honoured because an env var is a deployment's
    contract, not ours to break silently: a running install that sets
    UPI_LOG_LEVEL keeps working, and only stops if that fallback is removed in
    a later release with a note.
    """
    return os.getenv(f"TCE_LOG_{suffix}") or os.getenv(f"UPI_LOG_{suffix}") or default
...
def configure_logging(level: str | int | None = None, *, json_logs: bool | None = None) -> None:
    ...
    resolved_level = level or _log_env("LEVEL", "INFO")
    ...
    if json_logs is None:
        json_logs = not sys.stdout.isatty() or _log_env("JSON", "") == "1"
