...
"""XSD schema graph + deterministic element-index diff.
...
def diff_schema(old_content: str, new_content: str) -> XsdDiff:
    ...
    structure. A change to a *referenced* type (``type="Address"``) surfaces as a
    MODIFIED on ``Address`` itself; consumers that reference it are found via the
