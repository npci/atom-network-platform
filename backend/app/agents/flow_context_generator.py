...
"""Index-time API FLOW-MAP generator (reuse-first).

Builds a per-repo ``flow_context`` row: a map of how the system's APIs compose
into transaction flows — which existing API carries the ACTUAL business effect
versus the metadata/initiation/status APIs, plus multi-leg sequences. Runs at
index time (parallel to ``module_context``) off the already-indexed facts
(module entry points + XSD message types), with ONE cheap LLM labelling pass.
The reuse-first approach gate pulls this so the agent reasons over a ready flow
map instead of rediscovering it each run.

Deliberately generic — it never names a specific API; the LLM discovers the
primary leg from the repo's own entry points. What that leg IS, though, is a
domain fact ("money movement (the debit/credit leg)" in a payments network,
something else everywhere else), so the label comes from the pack. Fail-soft +
gated: a flag-off, no-facts, LLM hiccup, or parse error just writes nothing and
...
def _primary_leg_label() -> str:
    """What this domain calls the API that carries the real business effect.

    Read at call time, not import time: the prompt is built per index run, so
    there is nothing to gain from baking it in, and a call-time read keeps a
    pack swap honest.
    """
    from app.core.domain.registry import prompt_block

    # Article included: the value slots into "which API carries <X>" and
    # "why this is <X>", so a bare noun phrase would read wrong in both.
    return prompt_block("flow_primary_leg_label", "the real business effect")
...
def _build_prompt(facts: dict) -> str:
    ...
    leg = _primary_leg_label()
    return (
        "Modules:\n" + mods + "\n\nAPI entry points (handlers/endpoints):\n" + eps
        + "\n\nXSD message types: " + xsds + "\n\n"
        "Map how these APIs compose into transaction flows. Identify which API carries "
        + leg + ", versus the metadata/initiation/status APIs "
        ...
        ' "transaction_apis": [{"api": "name", "why": "why this is ' + leg + '"}],'
