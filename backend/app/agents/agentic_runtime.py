...
* drive :func:`call_messages_api_tools` (S5) with tools+system caching;
...
from app.core.llm import call_messages_api_tools, tool_result_block

logger = logging.getLogger("app.agentic")

_MUTATING = {"edit_file", "create_file", "delete_file"}

# Per-request output ceiling for the truncation-retry's doubled budget. An
# unclamped double past the model's limit 400s — converting a recoverable
# truncation into a run failure. This used to be a flat 64_000 (the Claude
# Sonnet 4.x cap) applied on EVERY provider, which is the same failure in the
# other direction on a model whose cap is lower: gpt-4o stops at 16_384.
# `model_max_output_tokens` resolves the real cap per model, falling back to
# the provider's, so the retry stays inside what the model will accept.
...
class RuntimeResult:
    ...
    # The structural-intel tokens this batch queried
...
def _perturbed_for_retry(messages: list[dict], note: str) -> list[dict]:
    ...
    forces temperature=0 (AiNxt injects it for sonnet-4-6/haiku):
    greedy decoding reproduces the same bad turn verbatim. Changing the input is the
...
async def _pin_progress_summary(messages: list[dict], system, model, agent_name: str, assistant_content) -> bool:
    ...
    _fold_instr(convo)
    try:
        turn = await call_messages_api_tools(system=system, messages=convo, tools=[], model=model,
        ...
        try:
            ...
            turn = await call_messages_api_tools(system=system, messages=convo2, tools=[], model=model,
                                           max_tokens=settings.agentic_compact_summary_max_tokens,
                                           agent_name=(agent_name or "agent") + ":compact")
            summary = (getattr(turn, "text", "") or "").strip()
    ...
    *,
    ...
    while i < cap:
        ...
        if settings.agentic_turn_recovery:
            ...
            _retry_tokens = min(out_tokens * 2, model_max_output_tokens(model))
        ...
        # cache counters to 0 — trusting that poisoned this
