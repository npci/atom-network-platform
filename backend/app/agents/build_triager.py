...
"""Build-failure triager (verify tier).
...
production build. Static scoping (what to look at) is safe; static suppression
...
#   [ERROR] /app/clone/igw/src/main/java/Foo.java:[42,10] cannot find symbol
...
def module_name(path: str) -> str:
    ...
    (``/app/clone/igw/src/...``) or repo-relative (``igw/src/...``) — both → ``igw``."""
    p = (path or "").replace("\\", "/")
