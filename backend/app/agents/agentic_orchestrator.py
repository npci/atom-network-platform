...
def _ws_id(run: AgenticRun) -> str:
    """The run_id whose on-disk clone holds this run's working tree.
...
def _render_analysis_plan(ta: dict | None, fp: dict | None, flow: dict | None) -> str:
    ...
    parts: list[str] = []
    # BINDING DIRECTIVES first — the critical decisions (state-changing legs/atomicity/…)
    # are the contract the implementer may NOT re-decide. Rendering them at the very top is
    # the fix for the observed drift class: a ratified 'per-participant' decision living only
    # in assumptions was silently re-decided into a consolidated one two runs in a row.
...
def _tsd_approval_gate(db, run: AgenticRun) -> bool:
    """ADR-0005 — the TSD binding-contract gate. Called ONLY at a
...
def _new_api_flow_gaps(ta: dict | None, flow: dict | None) -> list[str]:
    """Message schemas the plan INTRODUCES whose party route is ABSENT from flow_spec.
...
async def _phase_code(db, run: AgenticRun, art: dict, model) -> None:
    ...
    intel_acc: set = set()           # Structural-intel tokens across ALL rounds
    ...
    art["change_set"] = cs
    # The FULL cross-round intel-token set (not just this
    ...
    # In-loop deterministic self-correction — a cheap compile→fix BEFORE the authoritative
...
async def _inloop_self_correct(db, run: AgenticRun, art: dict) -> None:
    ...
    Reuses the dormant self-correction module: compile the agent's touched files with the
...
async def _phase_review(db, run: AgenticRun, art: dict, model) -> bool:
    ...
    # behaviour edits, undeclared consequential legs, promised-but-unbound config keys) fed by the ratified plan.
    ...
    if getattr(settings, "agentic_contract_gate", True):
        try:
            ...
            # measured independently of the enforce decision — see A18 and
            # ADR-0004.
    ...
    # Static DI-WIRING gate (context-load) — deterministic, LLM-free. The verify phase never
    ...
    # Cross-module analysis gate (deterministic, LLM-free) —
    ...
    # TSD-derived test coverage gate (shadow-first) — extracts checkable
...
def _is_transient(e: Exception) -> bool:
    ...
    a network blip, API timeout/5xx, rate limit, or a dropped connection that the run
    can pick up from once it clears. Real logic/build errors are NOT transient."""
...
# Vendor tokens that mark an error as coming FROM the LLM backend rather than from
# git or the workspace. One entry per provider `llm.normalize_provider` accepts:
# this list previously held "anthropic" alone, so an outage on any of the other
# four providers fell through to "internal_error" and never showed up as
# `llm_unavailable` in ops triage. Keep in step with `app.core.llm`.
_LLM_VENDOR_MARKERS = ("anthropic", "openai", "gemini", "googleapis", "ollama", "ainxt")


def _error_code(e: Exception) -> str:
    """Coarse failure category for ops triage (stored on the run). Lets an operator
    tell 'LLM rate-limited' from 'git clone failed' from 'verify timed out'
    ...
    if (any(k in name for k in ("apiconnection", "apitimeout", "overloaded", "internalserver",
                                "serviceunavailable"))
            or "overloaded" in s
            or any(v in s for v in _LLM_VENDOR_MARKERS)):
...
async def _step(db, run: AgenticRun, art: dict, model) -> None:
    ...
    if phase == P.PENDING.value:
        ...
        if not proposal:
            ...
            # Party-route coverage (deterministic, advisory): every NEW message schema the
                ...
                emit_event(db, run.id, "plan_flow_coverage",
                           {"missing": _fgaps,
                            "action": ("⚠ New message(s) planned but NOT routed in the flow spec: "
                                       ...
                                       "flow. Reopen the plan to add the route, or "
...
def _git_push_branch(run_id: str, repo_id: str, branch: str, base_sha: str,
    ...
    if adapter.run_command(rd, ["git", "diff", "--cached", "--quiet"]).exit_code != 0:
        # Git author on every agent commit — durable in the customer's history.
        _run_git(rd, "-c", "user.email=agentic@atom.local", "-c", "user.name=ATOM Agentic",
