...
def _wire_entity_in_code(checkouts: list, name: str):
    ...
    for d in checkouts:
        try:
            ...
            # `--` terminator, matching the `git clone` call sites in this file
            # and api/code_indexing.py. `base` is harvested from conflict
            # evidence / LLM text by a `[\w-]+\.xsd` pattern, and `[\w-]+`
            # permits a LEADING dash — without the terminator a value like
            # `--exclude-standard.xsd` reaches git as a flag rather than a
            # search term.
