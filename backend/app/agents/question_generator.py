...
questions in domain voice ("Should we cap the transfer amount at X?" rather
...
# be the canonical (and only) copy of the network's party/operation/risk/compliance
...
# question hint — was hardcoded "RBI-mandated / PMLA-touched" for the network.
    ...
    if gap_key == "scope_error_codes":
        ...
        return [{
            ...
            "text":       (
                ...
                '`{"<operation>_failure": ["<CODE>"], ...}` — '
                "leave blank if only generic codes (INVALID INPUT, TIMEOUT, etc.) apply. "
...
# Domain vocabulary supplied by the active domain pack, not hardcoded
# (genericisation sweep). `_pack`/`_AUTHORITY` are already resolved above.
