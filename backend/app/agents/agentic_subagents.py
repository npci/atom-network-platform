...
"""XSD-Discovery + Code-Change subagents.
...
# canonical-term examples are pack content. The network pack's bytes are unchanged
# (prompt-snapshot-verified).
_DOMAIN_NAME = _PB("domain_name", "platform")
# The central body every participant routes through. `central_switch_label` was
# the original key name and is payments vocabulary ("switch"); the neutral name
# is `authority_platform_label`. Both are read — new name first — so packs that
# already declare the old key keep working unchanged.
_SWITCH_LABEL = _PB("authority_platform_label",
                    _PB("central_switch_label", "the platform"))
...
# The leg of a flow where the change actually takes effect. In a payments
# network that is the debit/credit leg; in every other domain it is whatever
# API carries the real business effect, so the noun is pack content.
_PRIMARY_LEG_LABEL = _PB("primary_leg_label", "the primary business-effect leg")
# The dimensions a plan's `critical_decisions` list MUST cover. Ordering,
# atomicity and idempotency are platform truth; "settlement_model" and
# "money_movement_legs" are one ecosystem's, so the list is pack content and
# the default names only the generic ones.
_CRITICAL_DECISION_DIMENSIONS = _PB(
    "critical_decision_dimensions",
    "state_changing_legs (enumerate every leg that changes state and who is "
    "affected), ordering_model")
# A worked example of a well-formed binding directive. Neutral by default —
# a payments example here would put debits and credits in every domain's plans.
_DIRECTIVE_EXAMPLE = _PB(
    "critical_decision_directive_example",
    "Each child request applies its own share of the state change; no "
    "consolidated aggregate record may be generated.")
# Worked examples of invariant tests for binding directives. Same reasoning:
# "sum of merchant credits equals the original total" is a payments invariant.
_INVARIANT_TEST_EXAMPLES = _PB(
    "invariant_test_examples",
    "the sum of the parts equals the original whole (conservation), a "
    "participant's share is applied exactly once")
...
# A citable, priority-ordered conflict-resolution rule for the
...
_CODE_PREFACE = (
    ...
    "To orient quickly, consult flow_context (the flow map — which API carries "
    + _PRIMARY_LEG_LABEL + ") and module_context for any module you intend to touch: these are "
...
def _tests_clause() -> str:
    ...
    if settings.agentic_write_unit_tests or getattr(settings, "agentic_require_feature_tests", False):
        return ("\nWrite or extend JUnit tests covering the behaviour you changed — and for any new "
                ...
                "INVARIANT TESTS FOR BINDING DIRECTIVES: for EVERY state-changing / atomicity / "
                "ordering directive in the plan's BINDING DIRECTIVES, write at least one test that "
                "asserts the invariant itself — e.g. " + _INVARIANT_TEST_EXAMPLES + ", "
...
# Reuse-first decision pass: map the existing flows, decide where this
...
_PROPOSE_PREFACE = (
    ...
    "to discover (a) which existing API actually carries " + _PRIMARY_LEG_LABEL + " — the leg where "
    "the real state change lands — versus (b) the metadata/initiation/status APIs around it, and "
    "(c) how multi-leg flows are composed. That leg almost always belongs in the EXISTING "
    ...
    "fit-into-existing-flow option that names the specific existing API/flow it rides (e.g. route "
    + _PRIMARY_LEG_LABEL + " through the existing transaction flow, adding only a thin "
    "initiation/eligibility meta "
    ...
    "read_file'd this run (the handler/flow carrying " + _PRIMARY_LEG_LABEL + " + a consumer or message). "
...
# Phase-A (XSD stage) propose pass: the question is ONLY about the network's XML schemas, asked
...
# Refine-loop guardrail: when the human requests changes to the generated XSDs,
...
class ChangeSet:
    ...
    # Structural-intel tokens queried this batch
...
def build_system_segments(ctx: ContextPack, preface: str) -> list[dict]:
    ...
    return segments_for_prompt_cache([
        (preface, True),
...
_ANALYSIS_PREFACE = (
    ...
    "decision is CRITICAL and a PM must own it (value movement, compliance, a partner-visible "
    ...
    "imperative sentence the implementer must obey, e.g. '" + _DIRECTIVE_EXAMPLE + "')}. "
    "If you cannot source a touched dimension from the "
    ...
    "that hop (what the user does, what " + _SWITCH_LABEL + " decides"
...
async def run_approach_proposal(db, *, run_id: str, ctx: ContextPack, intent: str = "",
    ...
    if scope == "xsd":
        ...
        ask = ("Decide what changes (if any) the network XML schemas need — verify against the code "
...
def _code_resume_messages(transcript: list[dict], followup: str) -> list[dict] | None:
    ...
    a ~4KB string summary (the root non-convergence gap: the fixer must be the
...
async def run_code_change(db, *, run_id: str, ctx: ContextPack, xsd_scope: XsdScope | None = None,
    ...
    _decisions = (
        ...
        "the code, call ask_decision — NEVER silently choose an interpretation for a state-changing "
        "leg, atomicity, or ordering. Do NOT use it for code mechanics you can decide, and "
