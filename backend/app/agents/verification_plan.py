...
"""Runtime-owned VerificationPlan + deterministic hard gates.
...
def _skip_patterns() -> list:
    ...
    ReDoS / regex injection (CWE-1333) — this used to build the
    ...
    `.*igw.*` but `(a+)+$` becomes the literal string `\\(a\\+\\)\\+\\$`
...
def run_plan(db, run_id: str, plan: list[VerificationStep], *, touched_modules: set[str],
    ...
    try:
        ...
        vlog = None
    # Improper Resource Shutdown or Release (CWE-404) -- vlog.close()
