...
A bundle is the portable Agent-Skill shape: a
...
# Every path COMPONENT of an archive member must match this. The leading class
# excludes "-" deliberately: a member name reaches argv as the script operand of
# `python3 <path> <target>`, and `shlex.split` turns a leading dash into an
# OPTION — "-c<code>.py" makes argv[1] a `python3 -c` payload. `..`/absolute
# rejection alone does not cover that, and `_validated_invocation` cannot either,
# because such a member IS a declared script and so passes its `in scripts` test.
# Charset containment at the extraction boundary is what actually closes it.
_SAFE_MEMBER_COMPONENT = re.compile(r"^[A-Za-z0-9_][A-Za-z0-9_.-]*$")
...
def _reject_member_name(name: str) -> str | None:
    ...
    if any(p in (".git",) for p in parts):
        return ".git content"
    for p in parts:
        if p and not _SAFE_MEMBER_COMPONENT.match(p):
            return (f"unsafe path component {p!r} — each component must match "
                    f"{_SAFE_MEMBER_COMPONENT.pattern} (no leading '-', no spaces "
...
def classify_file(path: str) -> str:
    ...
    if ext in _ASSET_EXT:
        return "asset"
    # Rules-as-data (semgrep ymls, a SAST rule CSV catalog, PATTERNS tables):
    # csv included — commercial SAST tools ship their rule catalog as a .csv,
    # and those catalogs run to tens of thousands of rows.
...
def _validated_invocation(invocation: str, script_path: str, scripts: set[str],
    ...
    # Belt-and-braces against option injection: `_reject_member_name` already
    # refuses a member whose name could be read as a flag, so a dash here means
    # the invocation string itself supplied one rather than naming a script.
    # Checked BEFORE `in scripts`, because a bundle-declared script named like a
    # flag would otherwise satisfy that test.
    if target_script.startswith("-"):
        problems.append(
            f"{script_path}: invocation's script operand {target_script!r} is an "
            "option, not a path")
        return invocation
...
def validate_exec_manifest(exec_manifest: dict | None, bundle: "ParsedBundle") -> dict:
    ...
    for path in sorted(scripts - declared):
        # Undeclared scripts get a synthesized invocation. It still goes through
        # `_validated_invocation` — the declared branch above is not the only way
        # a bundle-controlled `path` reaches argv, and skipping the gate here left
        # the synthesized string as the one unchecked route into `shlex.split`.
        out["scripts"].append({"path": path, "role": "generator", "scope": "repo",
                               "invocation": _validated_invocation(
                                   f"python3 {path} {{target}}", path, scripts, problems),
