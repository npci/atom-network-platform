...
``emit_event`` is the single entry point both the state machine and the
phase bodies (tool calls, commands, edits, gates) call.
"""
...
#
# This was a hand-maintained mirror of `app.core.log_buffer._LOG_REDACTIONS`
# with a "keep them in sync" comment beside it. It had drifted in BOTH
# directions: this copy carried SSH-key / AKIA / certificate patterns that
# log_buffer lacked, while log_buffer carried a userinfo fix (and later the
# JSON-form, cookie, bare-JWT and vendor-key patterns) that this copy lacked —
# and this is the ONLY scrubber on `AgenticEvent.payload`, which api/agentic.py
# returns to clients. A comment cannot enforce synchronisation; an import can.
# The key/certificate patterns moved into log_buffer so both sinks now get them.
from app.core.log_buffer import _LOG_REDACTIONS as _REDACTIONS
