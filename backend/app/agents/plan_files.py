...
def _msg_tokens(text: str) -> list[str]:
    """Wire-message names (the network: ReqTransfer, RespBalEnq, …) as they appear in schema
...
def touched_message_stems(ta: dict | None, flow: dict | None = None) -> list[str]:
    """Wire message names (ReqX/RespX) the plan's TECHNICAL surface touches — the
    input to the party-flow requirement. Detection is by message TOKEN, not file
    name: network schemas commonly bundle many messages in one file
    (network-meta.xsd), so
