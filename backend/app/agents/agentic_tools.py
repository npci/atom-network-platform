...
def edit_file(ctx: RunContext, repo_id: str, path: str,
    ...
    # Guarded post-edit syntax check: if the edit turns a previously CLEAN Java parse
...
def _verify_evidence(ctx: RunContext, evidence: list) -> tuple[list, list]:
    ...
    selected repo) vs UNVERIFIED. The network codebase is large and confusing, so a reuse-vs-
...
def propose_approach(ctx: RunContext, summary: str, options: list,
    ...
    be grounded in the real code (the network's flows are not guessable). If too few claims are
    ...
    if len(verified) < 2:
        ...
        return ("propose_approach NOT recorded — your reuse-vs-new decision is not grounded in code. "
                ...
                "(find_existing_xsd, grep for the transaction leg, read_file them), "
...
def propose_plan(ctx: RunContext, summary: str, functional_plan: dict,
    ...
    # mandate was skipped in practice): every network message the plan's technical surface
...
def symbol_graph(ctx: RunContext, repo_id: str | None = None, symbol: str = "") -> str:
    ...
    indexed calls/called_by graph. Cheapest tier, no live process.
...
def callers(ctx: RunContext, symbol: str = "", repo_id: str | None = None) -> str:
    ...
    (one repo, def+callers), this sweeps ALL selected repos, which is where a multi-repo
    network change breaks: the schema/type lives in core, the callers in the app repo. Advisory (the
...
def find_existing_xsd(ctx: RunContext, repo_id: str | None = None, query: str = "") -> str:
    ...
    # Namespace spelling-variant awareness: if the query names a known schema
    # namespace (in any spelling), also match its sibling spellings so reuse
    # doesn't silently fail across the example.org / www.example.net split (§7.4 —
    # those two hosts are the real namespace URIs, quoted verbatim).
...
_DOMAIN_DOC_CATEGORIES = (
    "rbi_guideline", "network_product_doc", "past_brd", "api_spec", "xsd",
    "authority_circular", "authority_error_code", "authority_tsd", "authority_product_note",
    "authority_faq", "authority_product_deck", "authority_xml_spec", "api_design_knowledge",
)
...
def domain_docs_search(ctx: RunContext, query: str, top_k: int = 8) -> str:
    ...
    try:
        from app.rag.retrieval import retrieve
        chunks = retrieve(query, ctx.db, top_k=k,
                          categories=list(_DOMAIN_DOC_CATEGORIES), use_reranker=True)
    except Exception as e:  # noqa: BLE001
    ...
    lines = ["[domain_docs — official documents, SUPPORTING evidence only: a doc may CONFIRM "
...
def gov_bash(ctx: RunContext, command: str, timeout_s: int | None = None) -> str:
    """Governance stages only: a sandboxed shell inside the governance
    ...
    transcript of every command and result. Network stays off (sandbox
...
TOOL_SCHEMAS: list[dict] = [
    ...
    },
    {
        ...
        "description": "Surface a decision a human must make: a BINDING DIRECTIVE conflicts with "
                       ...
                       "NEVER silently choose an interpretation for a state-changing leg, atomicity, "
                       "or ordering; NEVER use this to reopen or re-litigate a ratified "
        ...
        "input_schema": {"type": "object", "required": ["summary", "options", "evidence"], "properties": {
            "evidence": {"type": "array",
                "description": "≥2 items grounding the reuse-vs-new call in code you READ. Each: "
                               ...
                               "(e.g. the handler carrying the transaction leg, a message/XSD, a consumer). "
    ...
    },
    {
        ...
        "description": "Index-time API FLOW MAP for a repo, with key-based retrieval: omit `flow` "
                       "to get the INDEX (summary + which APIs carry the actual transaction "
                       "leg vs the meta APIs + the list of flow NAMES); call again with "
