...
def _default_entities() -> list[str]:
    ...
    first domain it saw. For the network pack this yields the same five entities the old
...
# Domain nouns come from the active pack; under the default network pack the
...
# The critic's priority list is the one instruction that decides WHICH gaps get
# surfaced, so leaving it hardcoded made the adversarial pass hunt for payment
# reversals in every domain — a library run was told to prioritise "credit-side
# timeouts" and "beneficiary-bank declines", neither of which exists there, and
# spent its one pass on them instead of unanswered recalls.
_CRITIC_PROMPT = render_prompt(
    "agents/decline_designer/critic_prompt.md",
    PRIORITY_FAILURE_MODES=prompt_block(
        "decline_priority_failure_modes",
        "timeouts that leave an uncertain outcome needing reconciliation, "
        "counterparty-side declines, replay/idempotency, limit boundaries"),
)
...
def _entity_prefix(entity: str) -> str:
    """Short stable prefix from an entity name, e.g. 'Receiving Partner' -> 'RPA'."""
