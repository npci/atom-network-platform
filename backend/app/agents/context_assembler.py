...
"""Context-Assembler — builds the scoped, stale-aware ContextPack.
...
def _format_flow(r, flow: str | None = None) -> str:
    ...
    if flow:
        ...
        if r.transaction_apis:
            parts.append("transaction (primary-effect) APIs:\n" + "\n".join(
...
def _redact_doc_sections_for_llm(
    ...
    """T6 — filter PII out of document content before it can
    ...
    always-high-signal mobile/PIN patterns.
    ...
    try:
        ...
        if count:
            # Structured security telemetry. Never logs the redacted
            # VALUES, only the count.
            logger.info(
