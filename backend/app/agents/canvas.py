...
"""
    ...
    # Preserve [N] citations from upstream research_report.
    ...
    # was extremely tight — production observed 33k+ char canvas outputs
    # (~8k tokens) cut mid-section. 24K is generous for any plausible canvas
    # length; stream_llm clamps it to the active model's real output cap.
...
def generate_canvas_docx(change_title: str, canvas_content: str) -> bytes:
    """
    Generate a .docx Product Canvas that mirrors the Authority's Build Framework grid.
    ...
    run.bold = True
    run.font.size = Pt(13)
    run.font.color.rgb = RGBColor(26, 60, 94)  # brand navy
