...
from app.models.phase_c import A2AMessage, requested_action_from
...
# system prompts here are module-level constants). Under the default network pack
...
Produce a single JSON object with these fields:
  ...
  - escalation_target: when action is "escalate", STRICTLY one of "risk" / "infosec" / "tech".
      "risk"    — financial / fraud / regulatory-risk exposure.
...
async def resolve_message(
    *,
    ...
    if not policy_content:
        logger.warning("resolve_message: authority policy content is empty")
...
async def auto_resolve_background(
    ...
    try:
        ...
        if "counter" in _tt.lower():
            ...
            # Dual-key read: `inner` is the RAW stored payload, so historical
            # rows and the not-yet-updated partner both use the old spelling.
            _requested_action = requested_action_from(inner)
            if _requested_action:
                _parts.append(f"Requested from {_AUTHORITY}: {_requested_action}")
            message_text = "\n".join(_parts)
        ...
        # ── Doc-impact (Step 5) ───────────────────────────────────────────
        # When the Authority's answer is a real reply (not a holding response while we
        ...
        # consumed by the round-close consolidation. When a change is
