...
- Fail-open: on any LLM error or missing input, return every canonical
  ...
  gets a blank recommendation. `source="fallback_all_parties"` marks that
  path for observability. It emitted `"fallback_all_four"` until the
  four-party model stopped being a platform assumption; that spelling sits
  in already-stored `change_request_context` rows, so it is still ACCEPTED
  on read and normalised to the new one — dual-accept, single-emit.
...
# carrying their own hardcoded copy (was 3+ separate network-only literals).
...
# The pre-genericisation spelling of `SOURCE_FALLBACK`. Kept ACCEPTED (never
# emitted) because this field is serialised into `change_request_context`, so
# rows written before the rename still have to validate. Same shape as
# `models/phase_c.py`'s LEGACY_CERT_DIRECTION_ALIASES.
LEGACY_SOURCE_ALIASES = {"fallback_all_four": "fallback_all_parties"}

SOURCE_FALLBACK = "fallback_all_parties"
...
class PartyInferenceResult(BaseModel):
    ...
    # `fallback_all_four` is read-only legacy — see LEGACY_SOURCE_ALIASES.
    source: Literal["llm", "fallback_all_parties", "fallback_all_four",
                    "no_signal"] = "llm"

    @field_validator("source", mode="before")
    @classmethod
    def _normalise_source(cls, v):
        return LEGACY_SOURCE_ALIASES.get(v, v)
...
_SYSTEM = (
    ...
    "or metadata-only change may involve only a subset — infer the subset from the "
...
def _fallback_all_parties(reason: str) -> PartyInferenceResult:
...
async def infer_parties(
    ...
    """Infer the network's canonical parties in-scope for a change.

    Fail-open contract: never raises. On any error returns the all-parties
    ...
    if not (enhanced_prompt or "").strip():
        return PartyInferenceResult(
            parties_in_scope=list(_CANONICAL_PARTIES),
            rationale="No prompt provided — defaulting to all canonical parties.",
            confidence="low",
            source="no_signal",
        )
    ...
    try:
        ...
        return _fallback_all_parties(f"LLM error: {type(exc).__name__}.")

    if not isinstance(data, dict):
        return _fallback_all_parties("LLM returned non-object response.")
    ...
    try:
        ...
        return _fallback_all_parties("LLM response failed schema validation.")
