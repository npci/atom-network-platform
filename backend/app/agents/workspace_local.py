...
"""Per-run workspace: clone layout, leasing, reset, and GC.
...
def materialize_files(run_id: str, repo_id: str, files: list[dict]) -> int:
    ...
    the shared workspace was GC'd before code generation started."""
    rd = repo_dir(run_id, repo_id)
    n = 0
    for f in files or []:
        ...
        if not path or content is None:
            continue
        p = (rd / path).resolve()
        # `is_relative_to`, not a string prefix. `path` comes from the LLM-built
        # handoff (`xsd_files[].path`), and a string prefix treats a SIBLING
        # directory whose name merely begins with the repo dir's name as
        # contained — "/w/repo-evil/x".startswith("/w/repo") is True, so a write
        # escaped the checkout. Path-aware containment compares components.
        # `agentic_tools._resolve` already does it this way.
        if not p.is_relative_to(rd.resolve()):
            continue
...
def _scrub_before_removal(d: Path) -> None:
    """T10 — strip platform-written credentials from a
