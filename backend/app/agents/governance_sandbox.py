...
def run_shell(command: str, *, cwd: Path, ro_dirs: list[Path] | None = None,
    ...
    """Sandboxed shell for the governance review agent: run one
    ...
    transcript of every command and result.
...
def _xml_item(el) -> dict:
    """Best-effort finding record from a scanner-XML element (SAST `<Result>`,
...
def _parse_xml(res: ScriptResult, contract: dict, output_path: Path) -> None:
    """Count findings in XML scanner output (SAST results documents with
    ...
    try:
        ...
        return
    # Default to the per-finding element common SAST XML uses; authors override via findings_parse
