...
_STAGE_PREFACES = {
    "ea": (
        "You are the ENTERPRISE ARCHITECTURE (EA) governance reviewer for the Authority's network "
    ...
    "infosec": (
        "You are the INFORMATION SECURITY (InfoSec) governance reviewer for the Authority's network "
        "platform. You are NOT the author of this change. Verify the change complies with "
...
def _bundle_review_extras(skills, run=None) -> tuple[str, list]:
    ...
    sandboxed ``bash`` tool so a portable skill works without product-specific changes, with the concrete
    ...
    if run is not None:
        ...
        note += (
            ...
            "SKILL.md documents it: actually RUN each command and read its real output. "
            f"Do not describe, simulate, or assume what a step would print.{slot_note} Layout:\n"
...
async def _phase_gov_fix(db, run: AgenticRun, art: dict, model) -> None:
    ...
    preface = (f"You are the {meta['label']} FIXER for the Authority's network platform — a surgical editor. "
