...
counter-proposal using the hybrid approach:
  ...
  - Everything else → the LLM evaluates against BRD text
...
# Identity nouns come from the active domain pack; under the default network pack
...
def _redact_partner_text(text: str, *, field_name: str,
                         task_type=None) -> str:
    """T6 — strip PII from PARTNER-AUTHORED free text before
    it is embedded in a prompt sent to an external LLM provider.

    This is the exact surface classified as PII-POSSIBLE: a partner's
    justification / counter-proposal payload is free text the platform does
    not control, which may reference an account
    holder, a mobile number, or an addressing handle. It is prose, not a
    ...
    **Tier 2 interaction (`task_type`).** `pii_redaction_freetext_enabled`
    ...
    try:
        ...
        if count:
            # Structured telemetry; never logs the redacted values
            # themselves, only the count.
            logger.info(
...
async def _ai_tolerance_check(
    ...
    """Ask the LLM whether the request is within tolerance for this requirement."""
...
async def _ai_mandatory_violation_check(
    ...
    """Ask the LLM whether the counter violates any mandatory requirement.
