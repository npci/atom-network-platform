...
# Placeholder-data conventions for generated click-through prototypes.
#
# This block used to be four real organisation brands, two live addressing-handle suffixes, a
# currency symbol and a country dialling code, hardcoded — so EVERY prototype
# this agent generated, in every domain, was instructed to render one
# ecosystem's branding. It also directly contradicted the paragraph a few lines
# above it in the same prompt, which forbids reproducing "an organisation's
# registered mark" on the grounds that a real brand cannot be redistributed with
# a review artifact. A real organisation's name in a screenshot is exactly that.
#
# The default names no organisation. A pack that wants its own conventions
# declares `kit_prototype_placeholder_data`.
# Audience and language guidance for consumer-facing video scripts.
#
# Was "a general Indian audience", "reference everyday Indian contexts" and
# "translatable to Hindi" — hardcoded, so every promo script this platform
# generated was written for one country's viewers whatever the deployment. The
# default names no country or language; a pack states its own.
_AUDIENCE_LOCALE = prompt_block(
    "kit_audience_locale",
    "Write for a general consumer audience — everyday contexts, not technical "
    "jargon. Keep sentences short and free of idioms so the voiceover "
    "translates cleanly into the deployment's local languages.",
)
_PROTOTYPE_PLACEHOLDER_DATA = prompt_block(
    "kit_prototype_placeholder_data",
    "Use placeholder data that is obviously synthetic: invented organisation "
    "names (\"Northwind\", \"Meridian\", \"Acme\") suffixed with whatever this "
    "ecosystem calls its participants, identifiers in this domain's format but "
    "with reserved/example values, and documentation-range phone numbers. Never "
    "a real organisation's name, mark, or live identifier.",
)
...
# the network, nllc_circular_refs for NLLN, ...).
_REGULATOR_KEY = (
...
- **Mobile number input**: dialling-code prefix box (light grey, fixed width) + text input combined.
...
- **Dropdown** (participant selector etc.): looks like a text input but with a chevron on the
...
_VIDEO_DOCTYPE_INTENT = {
    "promo_video": (
        ...
        "emotionally engaging, locale-appropriate, translation-friendly, no jargon. "
