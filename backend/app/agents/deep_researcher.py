...
Given an enriched prompt, performs a three-part research:
  ...
  3. Regulatory Compliance — applicable guidelines from RAG + general compliance analysis
...
# Citation rules appended to the system prompt when enforcement flag is on.
# _CITATION_RULES is extracted to `app.agents.citations.GENERATE_RULES`
...
# Domain vocabulary supplied by the active domain pack (genericisation sweep).
...
async def stream_research_turn(
    ...
    """
    # Classify the feature into the domain taxonomy; drives analogue_queries in retrieval
    ...
    # When citation enforcement is on, replace the context with a
    ...
    # our Layer-1 detection never fired. 32K leaves comfortable headroom for
    # thorough reports; `call_llm`/`stream_llm` clamp it down to whatever the
    # ACTIVE model's real output cap is, so this stays portable across providers
    # instead of encoding one vendor's 64K ceiling.
    #
