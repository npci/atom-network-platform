...
Output format:
    ...
    <<FILE: com/example/network/service/SomeService.java>>
    <full file content>
    <<END_FILE>>
...
# are pack content. For the network pack the assembled prompt is byte-identical to the
...
def parse_files_from_output(
    ...
    correct CodeRepo:
      ...
      1. Strict marker form (system-prompt-prescribed):
         <<FILE: [Network Core] src/main/.../Foo.java>>
         <content>
         <<END_FILE>>

      2. Fenced-code form with path comment as first line:
         ```java
         // [Network Core] src/main/.../Foo.java
         package ...
         ```
    ...
    # The `FILE:` / `PATH:` prefix is optional — frontier models
    ...
    # `FILE:` / `PATH:` prefix optional (matches frontier-model phrasing).
    if not files:
...
# Max directories rendered per repo in the file tree. With the network's two big
...
# a blueprint expansion run 40-60KB each; combined that's ~100KB
...
# the BRD blueprint documented in document_guides.py +
...
# Generic across every domain: these name sections that are narrative or
# governance framing rather than implementable behaviour.
_DOC_SKIP_KEYWORDS_BASE = (
    "executive summary",
...
def _doc_skip_keywords() -> tuple[str, ...]:
    """The SKIP list, plus whatever this domain names its own governance
    sections.

    The regulator and its published references are domain facts — the network
    pack contributes "rbi", "dpdp", "npci circular". Hardcoding them meant
    every other domain carried three headings that can never match while its
    OWN regulator's sections were never skipped at all. One newline-separated
    keyword per line; absence simply leaves the generic base list.
    """
    from app.core.domain.registry import prompt_block

    extra = (prompt_block("doc_skip_keywords_extra", "") or "").splitlines()
    return _DOC_SKIP_KEYWORDS_BASE + tuple(
        k for k in (line.strip().lower() for line in extra) if k)
...
def _compress_doc_for_code_change(
    ...
    skip_keywords = _doc_skip_keywords()
...
def _build_impact_block(db: Session, tech_spec: str, brd: str) -> str:
    """Thin compatibility wrapper around the shared helper.
...
def _build_system_prompt(
    ...
    for q in queries:
        # Broaden retrieval beyond Java to all code-source categories.
    ...
    # Production BRDs from a blueprint expansion can run 40-60KB
