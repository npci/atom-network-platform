...
"""Deterministic XSD namespace canonicalization.
...
— one host for the majority of schemas and a second for a handful (e.g.
``ApiName.xsd``, ``TransactionResult.xsd``). Which spellings those actually ARE
is pack data (`schema_namespaces:`); the pack shipped here declares generic
placeholders, NOT any real ecosystem's namespaces. ``find_existing_xsd`` matches
...
def _normalize(ns: str | None) -> str:
    ...
    (``example.org`` vs ``example.net``) are genuine and handled by the explicit
