...
# authority (was a hardcoded network-only set; genericisation sweep).
# For the network pack this evaluates to the exact prior literal set, byte-for-byte.
...
def _operation_aliases() -> dict[str, str]:
    """Loose operation spelling -> canonical operation key.

    The party half of this remap became `party_aliases_of(pack)` for exactly
    this reason; the OPERATION half stayed hardcoded, so every domain carried
    a payments table ("reversal" -> "debit_reversal") that either matched
    nothing or, worse, remapped its own word onto an operation it does not
    have. Pack data now: one `alias: canonical_key` per line. Empty when
    undeclared — an unrecognised operation is then dropped, never remapped
    through another ecosystem's synonyms.
    """
    out: dict[str, str] = {}
    for line in (prompt_block("operation_aliases", "") or "").splitlines():
        alias, _, canonical = line.partition(":")
        alias, canonical = alias.strip().lower(), canonical.strip()
        if alias and canonical:
            out[alias] = canonical
    return out


_FC_OPERATION_ALIASES = _operation_aliases()
...
def _coerce_feature_criteria(items: list, max_items: int = 10) -> list[dict]:
    ...
    for it in items:
        ...
        # Normalise whitespace/hyphens to underscores so "Issuer Party",
        # "issuer-party" and "ISSUER_PARTY" all hit the same alias key.
        ...
        if operation not in _FC_OPERATIONS:
            operation = _FC_OPERATION_ALIASES.get(operation, "")
