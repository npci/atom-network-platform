...
_REVIEW_PREFACE = (
    ...
    # Same priority order the code agent is held to, so a
...
async def run_review(db, *, run_id: str, ctx, change_set, xsd_scope=None, intent: str = "",
    ...
    # `get_model()` (not get_model(provider="anthropic")) — the hardcoded
    # provider meant that under llm_provider=gemini/ollama/openai the reviewer
    # silently resolved settings.claude_model and then failed deep in the SDK.
    # The active provider's own model is the right default; the cross-vendor
    # guard below only applies when an operator deliberately names a different
    # vendor's id.
    model = (reviewer_model or (settings.agentic_reviewer_model or "").strip()
             or get_model())
    if str(model).strip().lower() != str(get_model()).strip().lower() \
            and "claude" not in str(model).lower():
        ...
        # provider (llm_provider=anthropic, or ainxt in openai-compat mode) the id passes the
        # guard above and then hard-fails deep in call_messages_api_tools / at api.anthropic.com
