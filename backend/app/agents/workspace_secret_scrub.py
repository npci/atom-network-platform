...
"""Workspace credential scrubbing — closes threat T10.
...
repo hygiene:
    ...
    post-clone scrub (`workspace_local.set_remote`) rewrites
...
_SECRET_RULES: tuple[tuple[str, re.Pattern], ...] = (
    ...
    # LLM provider keys — ALL five supported providers, not one. This scrubber
    # runs over the agent's workspace before a commit is pushed, so covering
    # only `sk-ant-` left a Gemini or OpenAI deployment with no protection
    # against its own key landing in a generated file.
    ("anthropic-key", re.compile(r"sk-ant-api03-[A-Za-z0-9_\-]{20,}")),
    ("openai-project-key", re.compile(r"sk-proj-[A-Za-z0-9_\-]{20,}")),
    ("openai-key", re.compile(r"\bsk-[A-Za-z0-9]{32,}\b")),
    # Google AI Studio keys are fixed-length and GET-parameter friendly, so
    # they land in URLs and logs readily.
