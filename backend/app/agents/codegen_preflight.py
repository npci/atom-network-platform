...
def check_dependencies(*, include_worker: bool = True, include_build: bool = False) -> list[str]:
    ...
    prov = normalize_provider(settings.llm_provider)
    key_for = {
    ...
    # 5. ...and that provider must be able to drive the tool loop the agentic
    # runtime is built on. Without this check the run passes preflight, clones
    # the repository, sets up a workspace, and only then dies on the first LLM
    # turn — with an error aimed at a developer, not at the operator who set
    # LLM_PROVIDER. Fail here, before any of that work.
    from app.core.llm import supports_native_tool_loop
    if not supports_native_tool_loop(prov):
        problems.append(
            f"The LLM provider '{prov}' does not support the native tool loop that agentic "
            "code generation requires. Set LLM_PROVIDER=anthropic, or LLM_PROVIDER=ainxt with "
            "AINXT_COMPAT_MODE=anthropic. Document generation, retrieval and the review gate "
            "work on every supported provider — see the capability matrix in CONFIGURATION.md."
        )
