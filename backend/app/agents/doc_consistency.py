...
_SYSTEM = (
    ...
    "  - A RATIFIED VALUE the plan FIXES that the doc reproduces in a DIFFERENT FORM — a "
    "quantity/limit (plan 'max 500 items per batch' vs doc vague/other number), an enum-valued "
    "field (plan type=FULL subType=DELTA vs doc 'Full' or subType 'initial|rotate'), a specific "
    "error code, or "
    "a state name — SAME meaning but WRONG spelling/casing/number → severity 'blocker', kind "
    ...
    "sets a 500-items-per-batch cap and the doc never states it) → severity 'blocker', kind "
...
async def check_doc_against_plan(*, doc_kind: str, doc_content: str, plan_contract: str) -> dict:
    ...
    system = segments_for_prompt_cache([
        (_SYSTEM, True),
