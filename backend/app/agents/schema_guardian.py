...
"""Deterministic XSD reuse-vs-create guardian.
...
@dataclass
class ReuseFinding:
    element: str        # element-index key, e.g. "element:ReqTransfer" / "complexType:Address"
    ...
    if conflict or cross_ns:
        ...
        if conflict:
            verdict.reasons.append(f"{conflict} element(s) conflict with an existing definition — do not "
                                   "fork a shared type; needs human/partner sign-off")
        if cross_ns:
