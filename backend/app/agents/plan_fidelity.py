...
# Both lists below drive a NEVER-AUTO-DOWNGRADE decision: a finding they match
# stays blocking no matter what the corroboration pass says. The generic terms
# are platform truth — every domain has security, compliance and half-done-leg
# findings. The DOMAIN's own words for them ("settlement", "sgf", "debit") are
# pack data: hardcoding one ecosystem's meant every other deployment ran this
# classifier without its own regulator, its own risk function, or its own name
# for a leg, and quietly auto-downgraded exactly the findings a human must see.


def _term_alternation(base: tuple[str, ...], pack_key: str) -> str:
    """`base` plus the active pack's extra terms, as a regex alternation.
    Comma-separated in the pack; absence just leaves the generic base."""
    from app.core.domain.registry import prompt_block

    extra = [t.strip() for t in (prompt_block(pack_key, "") or "").split(",")]
    return "|".join(list(base) + [t for t in extra if t])


# Partial / location-specific claims (one leg done, the other not): the symbol appearing SOMEWHERE can't
# refute these, so they are NEVER auto-downgraded — a real "one leg drops it" stays blocking.
_PARTIAL_TERMS = (r"one\s+leg", r"both\s+legs?", "only", r"but\s+not", r"partial\w*",
                  "first", "second", r"each\s+leg", r"either\s+leg", "timeout",
                  "fallback", "branch", "scenario", r"error\s+path", r"happy\s+path")
_PARTIAL_RE = re.compile(
    r"\b(" + _term_alternation(_PARTIAL_TERMS, "partial_claim_terms") + r")\b", re.I)
# Security / compliance / regulatory findings are NEVER auto-downgraded — they stay blocking regardless.
_SENSITIVE_TERMS = (r"secur\w*", r"auth\w*", r"inject\w*", "credential", "secret", "token",
                    r"encrypt\w*", r"regulat\w*", r"complian\w*", "pii", "signature",
                    "hmac", "tls", "mtls", r"tamper\w*")
_SENSITIVE_RE = re.compile(
    r"\b(" + _term_alternation(_SENSITIVE_TERMS, "sensitive_finding_terms") + r")\b", re.I)
