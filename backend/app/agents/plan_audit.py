...
# The central body the plan is written against. `central_switch_label` is the
# original (payments-shaped) key name; `authority_platform_label` is the neutral
# one. Read new-first so packs declaring either keep working — see
# `agents/agentic_subagents.py`, which resolves the same pair.
_SWITCH_LABEL = _PB("authority_platform_label",
                    _PB("central_switch_label", "the platform"))

_SYSTEM = (
    "You audit a ratified-candidate change PLAN for " + _SWITCH_LABEL + " for ENFORCEMENT SOUNDNESS — and "
