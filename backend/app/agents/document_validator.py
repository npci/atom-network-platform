...
of the domain's error codes, vague obligation language, placeholder text, missing FR
...
# The active domain's error-code CLASSIFICATION tokens. Resolved per call for
# the same reason as the pattern above. Empty means the domain declares none,
# and the check SKIPS rather than asserting another ecosystem's labels.
def _domain_error_class_tokens():
    from app.core.domain.contract import error_class_tokens_of
    from app.core.domain.registry import get_active_pack

    return error_class_tokens_of(get_active_pack())


def _obligation_example() -> str:
    """'(e.g. 'Authority shall', 'Partner must')' built from the active pack's
    participants instead of restating one ecosystem's actors."""
...
# network convention for masking sensitive values (account numbers, card numbers,
...
# Invented API names — the network's canonical form is Req<Pascal> / Resp<Pascal>. SCREAMING_
...
# Currency patterns — PACK DATA, because "the wrong way to write money" is a
# per-domain fact.
#
# This was `\bRs\.?\s*\d` with a message reading "Use 'INR'", which made a
# generic document validator enforce one country's currency: a euro-denominated
# deployment got no check at all, and any deployment that legitimately writes
# "Rs" got told to use a code its documents never mention. A pack that declares
# no currency conventions simply runs no currency check, which is the honest
# outcome — silence beats a confidently wrong warning.
def _bad_currency_re():
    from app.core.domain.registry import prompt_block

    pattern = prompt_block("doc_bad_currency_pattern", "")
...
def _currency_issues(content: str) -> list[Issue]:
    ...
    if rx is None:
        return []
    code = prompt_block("currency_code", "") or "the declared currency code"
    out: list[Issue] = []
    for m in rx.finditer(content):
        ...
        out.append({
            ...
            "message": (
                f"Use {code!r} (not '{m.group().strip()}') for amounts outside "
                "customer-facing display mockups."
            ),
            "evidence": content[start:end].strip(),
        })
    return out
...
def canonicalize_api_names(content: str) -> str:
    """Convert SCREAMING_SNAKE network API placeholders (REQ_FOO_BAR / RESP_FOO_BAR) to
...
def _validate_tech_spec(content: str) -> list[Issue]:
    ...
    # Error-code classification. The FIELD is `td_bd` in every pack (a pinned
    # key, which is why the rule name stays), but the VALUES are
    # domain-specific: the network pack declares TD/BD, adcn and nlln declare
    # SE/PD. This used to hardcode `\b(TD|BD)\b`, correct for exactly one of
    # the three packs — on the other two it both failed to enforce the tokens
    # the domain requires AND demanded tokens the domain forbids. That is the
    # same failure the error-code accessor above exists to prevent, one check
    # later. Empty token set => the domain declares no classification => skip.
    tokens = _domain_error_class_tokens()
    if tokens and "error" in content.lower():
        if not re.search(r"\b(%s)\b" % "|".join(sorted(tokens)), content):
            issues.append({
                "severity": "warning",
                "rule": "missing_td_bd",
                "message": ("Error section has no error-code classification "
                            f"({'/'.join(sorted(tokens))}) for this domain."),
                "evidence": "",
            })
