...
async def upload_policy(db: DbDep, current: AdminUser, file: UploadFile = File(...)):
    ...
    # Read the ceiling + 1 byte rather than the whole body then checking:
    # `await file.read()` first pulls the ENTIRE upload into memory, so the
    # cap rejected an oversized file only after already paying its full memory
    # cost. change_requests.py and rag.py use this form; these did not.
...
def reset_to_seed(db: DbDep, current: AdminUser):
    """Restore content from the seed file. Useful when an edit needs to be
    reverted to the shipped baseline.

    Resolves the seed through the same helper the first-boot seeder uses, so a
    reset restores exactly what was seeded. Reading
    `settings.authority_policy_path` directly here would have restored the
    operator's file even on a deployment that seeded from its pack's."""
    from app.core.authority_policy_seed import authority_policy_seed_path

    seed_path = authority_policy_seed_path()
    if seed_path is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=(
                f"No seed file found at {settings.authority_policy_path} or in "
                f"the active domain pack's data/ directory"
            ),
        )
