...
from app.core.change_access import require_change_access, require_workflow_role
...
from app.models.phase_c import (
    ...
    ORIGINATOR_AUTHORITY, ORIGINATOR_PARTNER,
)
...
# Object-level authorization for every "{change_id}" route on this router.
# Router-level (not per-route) so a route added later cannot silently skip it —
# that drift is exactly how this router came to serve change-scoped data with
# only an existence check. See app/core/change_access.py.
router = APIRouter(tags=["phase-c"], dependencies=[Depends(require_change_access)])
...
class ShipKitRequest(BaseModel):
    # Partners to ship to. Any not yet assigned to the change are assigned first.
    partner_ids: list[str]
...
# Privileged workflow decision — PM/PO/admin only. Was gated on
# authentication alone; see app/core/change_access.require_workflow_role.
@router.post("/changes/{change_id}/phase-c/ship-kit", dependencies=[Depends(require_workflow_role)])
async def ship_kit(change_id: str, body: ShipKitRequest, db: DbDep, user: CurrentUser, background: BackgroundTasks):
    """Unified communication: assign the selected partners (if needed) and ship a
    ...
    also preflight the gate before dispatching to selected partners.
    """
    ...
    if not body.partner_ids:
        raise HTTPException(status_code=400, detail="Select at least one partner to ship to")
    ...
    for pid in body.partner_ids:
        ...
        # A failed send must NOT advance the partner's state — the same rule
        # change_dispatch.py:790 already enforces on the communicate path, which
        # this path did not inherit. Without the `delivered` gate a partner whose
        # edge refused the RPC was recorded as RECEIVED, which both blocks the
        # re-ship (the ASSIGNED guard below stops matching) and starts the
        # negotiation-round clock against a partner holding nothing. Operators
        # reach for ship-kit precisely to re-drive a partner that had trouble,
        # so this path sees the failing population more often, not less.
        delivered = bool(message and message.delivered)
        # already-communicated partner leaves the lifecycle untouched (re-acceptance
        # is gated partner-side on negotiation_version_accepted).
        if delivered and assignment.status == AssignmentStatus.ASSIGNED:
            set_status(
    ...
    # Mark the revision plan shipped once its version has actually gone out —
    # `results` alone only says we tried, so a ship where every partner refused
    # used to flip the plan to SHIPPED and strand it there.
    if rev_plan and rev_plan.status != RP_STATUS_SHIPPED and any(r["delivered"] for r in results):
    ...
    # Delivered, not attempted — see the note on the same counter in
    # change_dispatch.py. `results` carries every partner we tried.
    delivered_count = sum(1 for r in results if r["delivered"])
    logger.info(
        "Ship-kit: change=%s v=%d notified=%d failed=%d skipped=%d",
        change_id, version, delivered_count, len(results) - delivered_count, len(skipped),
    )
    return {
        "change_id": change_id,
        "negotiation_version": version,
        "partners_notified": delivered_count,
        "partners_failed": len(results) - delivered_count,
...
def list_a2a_messages(change_id: str, db: DbDep, _: CurrentUser, limit: int = Query(100, ge=1, le=200)):
...
class NegotiationRespondRequest(BaseModel):
    ...
    # one question silently accepted every other outstanding ask from that partner, each
        ...
        if m.event_kind == "proposal" and m.counter_proposal_id:
            ...
            if cp is not None:
                ...
                entry.update({
                    ...
                    "originator":          (ORIGINATOR_AUTHORITY
                                            if cp.originator == ORIGINATOR_PARTNER
                                            else ORIGINATOR_PARTNER),
                    "status":              status,
                    "round":               cp.negotiation_round,
                    "counter_proposal_id": cp.counter_proposal_id,
...
# Privileged workflow decision — PM/PO/admin only. Was gated on
# authentication alone; see app/core/change_access.require_workflow_role.
@router.post("/changes/{change_id}/partners/{partner_id}/counter-proposals/{cp_id}/accept", dependencies=[Depends(require_workflow_role)])
...
async def counter_back_proposal(
    ...
    row with `originator=ORIGINATOR_AUTHORITY` that's now waiting for partner
    action.
    ...
    return [
        {
            ...
            "requested_action_from_authority": b.requested_action_from_authority,
...
# Privileged workflow decision — PM/PO/admin only. Was gated on
# authentication alone; see app/core/change_access.require_workflow_role.
@router.post("/changes/{change_id}/partners/{partner_id}/cert-waivers/{waiver_id}/decide", dependencies=[Depends(require_workflow_role)])
...
def get_progress_grid(change_id: str, db: DbDep, _: CurrentUser):
    ...
    for a in assignments:
        ...
        # When this partner last moved. The grid has always had a "Last Updated"
        # column, but the SPA read it from `assignment.updated_at` — a column
        # that does not exist on `change_partner_assignments`. So the cell
        # rendered "—" for every partner, always, and no amount of partner
        # activity could change it. The reported milestones carry the answer.
        last_reported = max((p.reported_at for p in progress if p.reported_at),
                            default=None)
        ...
        grid.append({
            ...
            "last_updated": last_reported.isoformat() if last_reported else None,
...
# partner — they never touch other partners, the change, or partner provisioning.
...
async def demo_run_certification(
    ...
    # Configured role, else the active pack's FIRST declared cert role. The
    # literal fallback here was `"REMITTER"` — one ecosystem's role key, handed
    # to a real cert run on any deployment that had not set the setting, which
    # is every deployment that is not that ecosystem.
    from app.core.domain.contract import cert_vocabulary_of
    from app.core.domain.registry import get_active_pack

    role = (getattr(settings, "precert_engine_certgroup", None) or "").strip()
    if not role:
        role = next(iter(cert_vocabulary_of(get_active_pack()).role_prefixes), "")
    if not role:
        raise HTTPException(
            status_code=status.HTTP_412_PRECONDITION_FAILED,
            detail=(
                "No certification role to run against: set "
                "PRECERT_ENGINE_CERTGROUP, or use a domain pack that declares "
                "cert_vocabulary.role_prefixes."
            ),
        )
...
def demo_reset_certification(change_id: str, partner_id: str, db: DbDep, _: CurrentUser):
    ...
    partners, the change, and partner provisioning untouched."""
...
def cert_txns(change_id: str, partner_id: str, db: DbDep, _: CurrentUser):
    ...
    for this cert run, read from the simulator's transaction log table
    (`PRECERT_ENGINE_TXN_LOG_TABLE` in precertdb): per case, the request the
    certification-agent switch (precert) sent and the response the partner switch
    (bank-sim) returned. Round 1 is the A2A agent<->agent conversation; this is the
    on-the-wire network exchange underneath it."""
    ...
    # Named in the simulator's schema, which this platform does not own — so the
    # table name is deployment config, validated as a bare identifier before it
    # reaches the query text.
    txn_log = require_external_table(
        settings.precert_engine_txn_log_table,
        "PRECERT_ENGINE_TXN_LOG_TABLE", "the certification transaction view")
    ...
    try:
        with con.cursor() as cur:
            ...
            cur.execute(
                ...
                f"FROM {txn_log} WHERE testid IN ({ph}) ORDER BY id DESC", tcs)
            seen, out = set(), []
...
async def dispatch_certification(change_id: str, partner_id: str, db: DbDep,
    ...
    Body (all optional):
        ...
        advance  — walk the assignment to CERTIFYING first (for a change whose
                   ...
                   These rows are FORCED: no partner message caused them. They
                   are attributed to the calling operator (actor_user_id), NOT
                   to the partner, and their reason is prefixed FORCED so a
                   reader can exclude them from any "the partner did X" claim.
                   Note the walk parks the assignment at CERTIFYING, which
                   outranks every negotiation state — the forward-only guard in
                   assignment_status.set_status will then DISCARD the partner's
                   own later acknowledgement/milestones. Do not use this on a
                   change whose partner is still expected to negotiate.
    ...
    if body.get("advance"):
        # Attribution rule lives in the service (F-30): these rows name the
        # operator, never the partner, which sent nothing.
        from app.services.assignment_status import force_advance_to_certifying

        force_advance_to_certifying(
            assignment, db,
            operator_user_id=user.id,
            operator_username=getattr(user, "username", "operator"),
        )
        db.commit()
...
# Privileged workflow decision — PM/PO/admin only. Was gated on
# authentication alone; see app/core/change_access.require_workflow_role.
@router.post("/changes/{change_id}/partners/{partner_id}/cert/start", dependencies=[Depends(require_workflow_role)])
...
def change_cert_summary(change_id: str, db: DbDep, _: CurrentUser):
    ...
    for a in assignments:
        ...
        status_val = a.status.value if hasattr(a.status, "value") else a.status
        # `[]`, not a default role — see the note on the same shape in
        # api/a2a.py. `"bank"` has not been a PartnerType member since migration
        # 0108 remapped it to `remitter`, so this emitted a token the SPA's
        # partnerTypeLabel() could only echo back verbatim.
        partner_types = (
            partner.partner_type if isinstance(partner.partner_type, list)
            else [partner.partner_type] if partner.partner_type
            else []
        )
...
def certification_agent_messages(db: DbDep, _: CurrentUser, limit: int = Query(100, ge=1, le=200)):
