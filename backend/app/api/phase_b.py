...
router = APIRouter(tags=["phase-b"], dependencies=[Depends(require_change_access)])
...
def _iteration_history_to_messages(iterations: list[CodeIteration]) -> list[dict]:
    """Convert stored code iterations into LLM conversation turns."""
    messages: list[dict] = []
...
def ingest_codebase_endpoint(change_id: str, body: IngestCodebaseRequest, db: DbDep, current_user: CurrentUser):
    ...
    try:
        ...
        # Do not echo the raw exception text to the caller
...
async def trigger_build(
    ...
    logger.info("Build+deploy queued: change=%s mode=%s core=%s app=%s script=%s build_run=%s",
