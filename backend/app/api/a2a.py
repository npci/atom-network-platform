...
The unified A2A SDK refactor TRIMMED this module:
...
def get_agent_card() -> dict:
    ...
    dash) via routes registered in the SDK mount.
    """
        ...
        "skills": [
            {
                ...
                "description": "Receive network feature change notifications with full Product Kit (BRD, Tech Spec, XSD, test cases, etc.)",
            },
            {
            ...
            },
            {
                ...
                "description": "Participate in bidirectional certification testing (Authority→Partner and Partner→Authority)",
            },
        ],
...
def authenticate_partner(body: AuthRequest, db: DbDep):
    ...
    A2A security hardening: access tokens are now 15 min,
...
class RefreshRequest(BaseModel):
    """Body for POST /a2a/auth/refresh."""
    refresh_token: str
...
def list_all_threads(db: DbDep, _: CurrentUser, kind: str | None = None):
    ...
    # of N. Reference: P6 "avoidance of chatty access and N+1 queries".
    ...
    for t in threads:
        ...
        # authority-reply row (`AUTHORITY_REPLY_ROLES`, defined once in
        # models/phase_c.py and mirrored by the SPA's copy of the same name).
        def _role(m):
        ...
        for i, m in enumerate(msgs):
            if _role(m) != "partner":
                continue
            has_reply_after = any(
                _role(n) in AUTHORITY_REPLY_ROLES for n in msgs[i + 1:])
        ...
        result.append({
            ...
            "partner_name": partner.name,
            # Empty, NOT a default role. `"bank"` sat here long after migration
            # 0108 remapped it away (bank->remitter), so it was not even a
            # member of PartnerType — and the SPA's partnerTypeLabel() returns
            # unknown keys verbatim, so a null-typed partner rendered the literal
            # string "bank" in the thread list. Substituting a VALID role would
            # be the same bug with better spelling: the honest answer to "what
            # role is this partner?" when none is recorded is none.
            "partner_types": partner.partner_type or [],
