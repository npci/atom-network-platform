...
def _safe_step_error(step: str, exc: BaseException) -> str:
    ...
    Error-message exposure (CWE-209). That list is not diagnostic-only: the endpoint ends with
...
def admin_delete_change(
    ...
    if sessions_dir.exists():
        try:
            ...
            # stays — but in the LOG, not in the response body (the
    ...
    try:
        ...
        # Error-message exposure (CWE-209): a SQLAlchemy commit failure renders table names, column names
    ...
    if not cr:
        ...
        try:
            ...
            # Error-message exposure (CWE-209): see note on the sibling handler above — a FK-violation
