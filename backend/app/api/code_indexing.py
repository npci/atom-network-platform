...
# an `on_progress` callback — that's a follow-up; R-6 v1 reports:
...
class AddRepoRequest(BaseModel):
    ...
    @classmethod
    def _check_gitlab_url(cls, v: str | None) -> str | None:
        """Validate the URL at the write boundary, like `gitlab_branch` above.

        This field was the unvalidated sibling of a field that has a careful
        validator, and it is the more dangerous of the two: `build_clone_url`
        embeds the platform's GitLab PAT into the netloc —
        ``https://oauth2:<token>@<host>/...`` — before handing it to `git clone`,
        and `ingest_repo` also passes it to `gitlab.Gitlab(url, private_token=...)`.
        So an attacker-chosen host does not merely fetch from the wrong place, it
        TRANSMITS the platform's credential there. `http://169.254.169.254` gives
        metadata-service SSRF through the same path.

        Guarded here rather than at the fetch so a bad value is never persisted;
        the second-order read path is then clean by construction.
        """
        if v is None or not v.strip():
            return v
        v = v.strip()
        if len(v) > 2048:
            raise ValueError("gitlab_url is unreasonably long")
        from urllib.parse import urlparse
        parsed = urlparse(v)
        if parsed.scheme not in ("http", "https"):
            raise ValueError(
                f"gitlab_url must use http:// or https:// — got {parsed.scheme or 'no'} scheme")
        if not parsed.hostname:
            raise ValueError("gitlab_url has no host")

        from app.core.ssrf_guard import SsrfBlocked, guard_outbound
        try:
            guard_outbound(v, context="gitlab_url")
        except SsrfBlocked as exc:
            raise ValueError(
                f"gitlab_url refused: {exc.reason}. If this is a legitimate internal "
                "GitLab, add its host to SSRF_ALLOWED_INTERNAL_HOSTS."
            ) from exc
        return v
