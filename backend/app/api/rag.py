...
DEFAULT_CATEGORIES = [
    {"value": "authority_circular",     "label": "Authority Circulars"},
    # VALUE PINNED, label pack-driven. `rbi_guideline` is a persisted
    # DocCategory member — it is in the enum created by migration 0001, widened
    # by 0006, stored on every ingested chunk, and mapped from the
    # `rbi_guidelines/` corpus folder by rag/ingestion.py. Renaming it needs a
    # migration and a backfill, not an edit here.
    #
    # The LABEL is pack-driven: every sibling entry reads authority_*/network_*,
    # and a hardcoded label here would display one country's regulator to every
    # deployment. The pack already declares `regulatory_body`.
    {"value": "rbi_guideline",
     "label": f"{prompt_block('regulatory_body', 'Regulator')} Guidelines"},
    {"value": "network_product_doc", "label": "Network Product Docs"},
    {"value": "past_brd",        "label": "Past BRDs"},
    {"value": "authority_tsd",        "label": "Past TSDs"},
    {"value": "authority_product_note", "label": "Product Notes"},
    {"value": "authority_cert_pack",  "label": "Certification Testcases"},
    {"value": "authority_error_code", "label": "Error Codes"},
    {"value": "authority_faq",        "label": "FAQs"},
    {"value": "authority_product_deck", "label": "Product Decks"},
