...
def _authority_label() -> str:
    """The authority's DISPLAY name for the timeline.

    A label, not the wire token: this string is rendered to a human, so it
    takes the pack's `initiator_authority_label` and falls back to the role
    token only when a pack declares no label.
    """
    from app.core.domain.registry import prompt_block
    from app.services.cert_roles import authority_role
    return prompt_block("initiator_authority_label", "") or authority_role()
...
def cert_timeline(
    ...
    for m in db.scalars(qm).all():
        ...
        if kind == "kit_communicated":
            ...
            # partner as having received a kit it never got.
            ...
            title  = f"{_authority_label()} responded to {pname}'s query"
            desc   = ((m.payload or {}).get("response_text") or "")[:120]
            actor_kind = "user"
            actor_name = f"{_authority_label()} PO"
            ...
            actor_name = _authority_label() if direction == "outbound" else pname
