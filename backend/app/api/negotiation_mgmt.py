...
#
# `require_workflow_role` is router-level too, because the module header already
# states the rule for the WHOLE router — "negotiation outcomes are PM workflow
# data; the product-kit-scoped review teams must not read them" — yet
# `_NEGOTIATION_ROLES` was only checked on 6 of its 22 routes. The unguarded
# ones included BRD-requirement create/update/delete, where flipping
# `is_mandatory` changes which partner counter-proposals get auto-accepted.
# The six inline checks are now redundant but left in place: they return
# route-specific messages and removing them buys nothing.
router = APIRouter(
    tags=["negotiation-mgmt"],
    dependencies=[Depends(require_change_access), Depends(require_workflow_role)],
)
...
async def new_version_and_ship(change_id: str, db: DbDep, current_user: CurrentUser):
    ...
    #    then generate the partner-facing change summary.
...
def round_close_summary(change_id: str, db: DbDep, current_user: CurrentUser):
    """Consolidated round outcomes for the PM.
