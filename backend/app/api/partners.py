...
class CreatePartnerRequest(BaseModel):
    name: str
    # Participant-role keys from the active domain pack (see
    # `_selectable_partner_types`), e.g. ["remitter"] or ["psp", "remitter"].
    # Legacy stored spellings are still accepted so an existing registration
...
class UpdatePartnerRequest(BaseModel):
    ...
    # Cert-agent's short partner code. Required for the readiness orchestrator
    # to trigger cert runs against this partner. Empty string clears it.
    #
    # DUAL-ACCEPT: the column became `cert_agent_partner_id` in alembic 0141,
    # but a cached SPA bundle or an operator's script may still send the old
    # key, so both are accepted. `cert_agent_code()` resolves them, the new
    # spelling winning. Drop `cert_agent_bank_id` once no client sends it.
    cert_agent_partner_id: str | None = None
    cert_agent_bank_id: str | None = None

    def cert_agent_code(self) -> str | None:
        """The submitted cert-agent code, under whichever key was used."""
        if self.cert_agent_partner_id is not None:
            return self.cert_agent_partner_id
        return self.cert_agent_bank_id


class UpdateProtocolRequest(BaseModel):
    """Body for `PATCH /admin/partners/{id}/protocol`.
...
class UpdateTlsTierRequest(BaseModel):
    """Body for `PATCH /admin/partners/{id}/tls-tier` (A2A security
    hardening). Switches the partner between :443 (jwt) and
    :8443 (mtls) ingresses."""
    tls_tier: str  # 'jwt' | 'mtls'


class UpdateCertFingerprintRequest(BaseModel):
    """Body for `PATCH /admin/partners/{id}/cert-fingerprint`.

    SHA-256 hex of the partner's client cert. Set to `null` to clear (use
...
class UpdateAllowedCidrsRequest(BaseModel):
    """Body for `PATCH /admin/partners/{id}/allowed-cidrs`.
...
class UpdateRateLimitRequest(BaseModel):
    """Body for `PATCH /admin/partners/{id}/rate-limit`.
    ...
    to everyone today); a follow-up may wire an njs-driven dynamic
...
def _selectable_partner_types() -> list[str]:
    """The participant roles an operator may register, from the ACTIVE PACK.

    Which roles exist is a domain question, so the pack answers it — the same
    way `cert_roles` already does for the certification exchange. The pack's
    keys come FIRST because they are the current vocabulary; a deployment that
    swaps `DOMAIN_PACK` gets its own roles here with nothing to change in code.

    The `PartnerType` members are then unioned in as a BACK-COMPAT accept-list.
    They are not the source of truth any more, but they are what is written on
    `partner_agents.partner_type` for every partner registered so far, and an
    update that silently dropped a stored role would blank a live registration.

    `cert_engine` is excluded: it addresses an authority-internal component, not
    a participant, and has always been hidden from the registry UI.
    """
    from app.core.domain.contract import participants_of
    from app.core.domain.registry import get_active_pack

    out: list[str] = []
    for key in ([p.key for p in participants_of(get_active_pack())]
                + [t.value for t in PartnerType]):
        if key != PartnerType.CERT_ENGINE.value and key not in out:
            out.append(key)
    return out


def _default_partner_type() -> str:
    """The role assumed when a caller names none the registry recognises.

    The first non-authority role the pack declares — the pack's own idea of
    "an ordinary participant" — falling back to the legacy stored default so a
    pack that declares no participants still writes a value the UI can render.
    """
    from app.core.domain.contract import participants_of
    from app.core.domain.registry import get_active_pack

    for participant in participants_of(get_active_pack()):
        if not participant.is_authority:
            return participant.key
    return PartnerType.PAYER_PSP.value


def _requires_mutual_tls(types) -> bool:
    """True when any of `types` is a role the pack marks high-assurance.

    Replaces a hardcoded "remitter/beneficiary → mtls, payer_psp/payee_psp →
    jwt" rule. Which roles warrant a mutually-authenticated channel is domain
    judgement; a pack that declares none forces nobody onto mTLS by role, which
    leaves the operator's explicit per-partner choice as the only input.
    """
    from app.core.domain.contract import high_assurance_participants_of
    from app.core.domain.registry import get_active_pack

    high = high_assurance_participants_of(get_active_pack())
    return bool(high) and any(t in high for t in _normalize_types(types))
...
def _partner_response(p: PartnerAgent) -> dict:
    return {
        ...
        # A2A wire selector — alembic 0033, part of the SDK refactor.
        ...
        # Partner-tier mTLS state. tls_tier determines which
        ...
        # Cert-agent short partner code used by the readiness orchestrator.
        # DUAL-EMIT alongside the superseded key (alembic 0141) so a cached SPA
        # bundle reading the old field keeps working. Drop `cert_agent_bank_id`
        # once no client reads it.
        "cert_agent_partner_id": getattr(p, "cert_agent_partner_id", None),
        "cert_agent_bank_id": getattr(p, "cert_agent_partner_id", None),
...
def create_partner(body: CreatePartnerRequest, db: DbDep, _: AdminUser):
    ...
    that the A2A security stack consumes:
      ...
      * jwt_signing_secret  — per-partner HS256 secret for
                              outbound Bearer JWT identity proof.
      * signing_secret      — per-partner HMAC secret for the
    ...
    values must be shipped to the partner out-of-band and installed on
    the partner's side under its own settings keys — historically
    spelled `partner_settings.npci_jwt_secret` and
    `partner_settings.npci_hmac_secret`, which remain the accepted
    names on that side of the wire."""
    ...
    valid_types = _selectable_partner_types()
    types = [t for t in body.partner_types if t in valid_types] or [_default_partner_type()]
...
def update_partner(partner_id: str, body: UpdatePartnerRequest, db: DbDep, _: AdminUser):
    ...
    _cert_code = body.cert_agent_code()
    if _cert_code is not None:
        # Empty string clears the mapping; a non-empty trimmed string writes
        # the cert-agent short code.
        partner.cert_agent_partner_id = _cert_code.strip() or None
...
def rotate_jwt_signing_secret(partner_id: str, db: DbDep, _: AdminUser):
    ...
    Part of A2A security hardening. The new secret is returned ONCE
...
def rotate_hmac_signing_secret(partner_id: str, request: Request, db: DbDep, admin: AdminUser):
    ...
    Part of A2A security hardening. The HMAC secret signs the
    payload-signature header (payload non-repudiation), distinct from
    the JWT signing secret (identity). The header is emitted as
    `X-Auth-Signature`; the superseded `X-NPCI-Signature` spelling is
    still accepted inbound. Returned ONCE; the admin must ship it
    out-of-band so the partner installs the matching secret (that
    setting is spelled `npci_hmac_secret` on the partner side).
    ...
    partner.secret_rotated_at = utcnow()
    partner.updated_at = utcnow()
    # T2 — monotonic version counter, so every
    ...
    # T8 — admin action audit. Never the secret VALUES
...
def _connect_hint(cause: BaseException) -> str:
    ...
    Error-message exposure (CWE-209). The endpoint used to interpolate the raw `__cause__` into its
...
def test_partner_connectivity(partner_id: str, db: DbDep, _: AdminUser):
    ...
    try:
        ...
        # Error-message exposure (CWE-209): this dict is the response body of POST /{partner_id}/test.
        ...
        logger.warning("partner_test: partner=%s FAILED for %s — %r",
                       partner.id, agent_card_url, e, exc_info=True)
        # Error-message exposure (CWE-209): `type(e).__name__` is a bare class name and is safe to keep —
...
# call. This endpoint just makes the flip clickable from the UI.
#
...
def update_tls_tier(
    ...
    Which participant roles warrant a mutually-authenticated channel is
    declared by the active domain pack as `high_assurance_participants`, not
    hardcoded here — a partner holding one of those roles belongs on 'mtls',
    every other role stays on 'jwt'. A pack that declares none puts nobody on
    mTLS by role, leaving this endpoint's explicit choice as the only input.

    The operator's choice still wins; the pack supplies the EXPECTATION, and a
    downgrade that contradicts it is logged as a security event rather than
    refused (an operator may legitimately be staging a certificate rotation).
    ...
    if body.tls_tier != "mtls" and _requires_mutual_tls(partner.partner_type):
        logger.warning(
            "SECURITY_EVENT event=tls_tier_below_pack_expectation severity=medium "
            "partner_id=%s roles=%s tls_tier=%s decision=allowed",
            partner.id, _normalize_types(partner.partner_type), body.tls_tier,
        )
    return _partner_response(partner)
...
def update_cert_fingerprint(
    ...
    """Register / update the SHA-256 fingerprint of the partner's pinned
    client cert (A2A security hardening). nginx forwards
...
def upload_partner_ca_cert(
    ...
    if pem:
        ...
        try:
            ...
            # Error-message exposure (CWE-209): `ssl.SSLError` is a third-party type on no allowlist, and
...
# ── CIDR allowlist + rate-limit override ─────────────────────────────────────
...
def update_allowed_cidrs(
    ...
    """Replace the partner's allowed_cidrs list (A2A security
    hardening). Each entry must parse as a CIDR via the
...
def update_rate_limit(
    ...
    enforcement at nginx level is a flat zone today (a follow-up may
    wire a per-partner override). The column makes the per-partner cap
    visible in the admin UI and unblocks that dynamic-zone work."""
    ...
    # T8 — admin action audit for a representative
...
# ── A2A session revocation (security hardening) ──────────────────────────────
...
def partner_stats(db: DbDep, _: AdminUser):
    ...
    # Seeded from the pack's roles PLUS the legacy stored ones, so a partner
    # registered under an older vocabulary still appears in the breakdown.
    by_type = {t: 0 for t in _selectable_partner_types()}
    by_type[PartnerType.CERT_ENGINE.value] = 0
