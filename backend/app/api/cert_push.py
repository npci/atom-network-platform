...
from app.services.cert_roles import (
    authority_role, authority_wire, both_roles, normalize_role, partner_role,
    partner_wire, pick_counts, zero_counts,
)


def _csv_block(name: str) -> list[str]:
    """A comma-separated pack block as a list; empty when undeclared."""
...
# `push_to_cert` previously took only `CurrentUser` and an existence check, so
# any authenticated user could push ANOTHER user's test cases into the shared
# cert-agent environment — a state-changing action visible outside the platform.
# See app/core/change_access.py.
router = APIRouter(tags=["cert-push"], dependencies=[Depends(require_change_access)])
...
def _parse_md_table(content: str) -> list[dict]:
    ...
    for ln in content.splitlines():
        ...
        initiated_by = normalize_role(init_raw) or ""
        ...
        # Fields the markdown table does not carry come from the active pack,
        # or stay EMPTY — never from a literal.
        #
        # The parser above reads four columns (id, scenario, expected,
        # initiated-by). Everything else in the stub was invented here, and was
        # invented in one ecosystem's vocabulary: api_type "Pay", entities
        # "Payer PSP"/"Payee PSP", handles "VPA", scope "v2.0", psp_as "Payer".
        # A library-network deployment pushed all six to its cert-agent on
        # every markdown-sourced upload.
        #
        # Empty is a supported value throughout: TestCaseStub defaults
        # scope/psp_as/payer_handle/payee_handle to "" for exactly this reason,
        # and the workbook renderer resolves the display value from the pack at
        # render time (testcase_sheet._display_scope). `initiated_by` a few
        # lines below already worked this way.
        out.append({
            "test_id": tid,
            "apis": [],
            "api_type": prompt_block("cert_default_api_type", ""),
            "entities": _csv_block("cert_default_entities"),
            "approval_type": "Non-Pre Approved",
            "payer_handle": prompt_block("cert_default_handle_kind", ""),
            "payee_handle": prompt_block("cert_default_handle_kind", ""),
            "scenario_summary": scenario,
            "expected_status": status,
            "response_code": rc.upper() if rc else "",
            "coverage_tag": coverage,
            # "" means "resolve from the pack at render time" — see
            # TestCaseStub.scope, whose default carries the same note.
            "scope": "",
            # The workbook spelling, NOT the canonical token — cert-agent
            # matches the pack's wire roles here exactly. See
            # cert_roles.authority_wire.
            "txn_initiated_by": (
                partner_wire() if initiated_by == partner_role()
                else authority_wire() if initiated_by == authority_role()
                else ""
            ),
            "psp_as": "",
...
def _normalise_initiated_by(cases: list[dict]) -> None:
    ...
    for c in cases:
        if c.get("initiated_by"):
            continue
        role = normalize_role(c.get("txn_initiated_by"))
        if role:
            c["initiated_by"] = role
...
async def push_to_cert(
    ...
    db: DbDep,
    _: CurrentUser,
    default_initiated_by: str = "",
):
    """Push this change's test cases to the cert-agent environment.

    Query params:
      default_initiated_by  the authority or partner role token from the active
                            pack; blank means the authority. Applied by cert-agent
    ...
    di = normalize_role(default_initiated_by) or authority_role()
    try:
        ...
        # Error-message exposure (CWE-209): the previous message interpolated BOTH the internal
    ...
    summary = {
        ...
        "tc_results": [], "flow": "",
        "initiated_by_counts": zero_counts(),
        "created_counts":      zero_counts(),
    }
    if resp.status_code == 200:
        for line in resp.text.splitlines():
            ...
            if etype == "flow_check":
                ...
                summary["initiated_by_counts"] = pick_counts(ib)
                ...
                summary["created_counts"] = pick_counts(cc)

    feature_name = artifact["feature_name"]
    flow = summary.get("flow") or re.sub(r"[^A-Z0-9]+", "_", feature_name.upper()).strip("_")

    # Orchestrator enabler: stamp every assignment for this change with the
...
def _persist_feature_mapping(
    ...
    surface zero. The orchestrator reads
...
async def ws_cert_push(websocket: WebSocket, change_id: str):
    ...
    default_initiated_by = (
        normalize_role(websocket.query_params.get("default_initiated_by"))
        or authority_role()
    )
    ...
    try:
        ...
        while True:
            ...
            # Persist feature mapping for orchestrator
            _persist_feature_mapping(db, change_id, feature_name, flow)
