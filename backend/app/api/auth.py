...
def login(payload: LoginRequest, request: Request, response: Response, db: DbDep):
    ...
    # closing T7, for ADMIN accounts specifically
    ...
    # Keyed on the roles the user is ASSIGNED, not the one currently active.
    #
    # `user.role` is the ACTIVE role, and /auth/switch-role lets a user promote
    # it to any assigned role at will. Testing the active role therefore let a
    # user assigned {product_manager, admin} log in as PM — no OTP — then switch
    # to admin and hold a full admin session whose `amr` is only "pwd".
    # `require_admin` would refuse that, but the many routes that check the role
    # inline (governance.py, jobs.py, eval.py, emergency_issues.py, phase_b.py)
    # never consult `amr`, so they accepted a password-only admin. That defeats
    # T7.
    #
    # `user.roles` always includes the active role, so this is strictly wider
    # than the old test — no login that was previously challenged stops being.
    _admin_mfa_required = (
        UserRole.ADMIN in user.roles and getattr(settings, "admin_mfa_required", True)
    )
...
def mfa_setup(user: EnrollingUser, db: DbDep):
    ...
    # mfa_secret is EncryptedSecret (see models/user.py);
...
def mfa_activate(body: MfaActivateRequest, request: Request, user: EnrollingUser,
    ...
    try:
        ...
        raise
    # user.mfa_secret is already the plaintext TOTP seed
...
def mfa_verify(body: MfaVerifyRequest, request: Request, response: Response, db: DbDep):
    ...
    # HMAC-SHA-1 code (RFC 6238's mandated default);
    ...
    # user.mfa_secret is already the plaintext TOTP seed
    # here; EncryptedSecret decrypts transparently on column read.
    # Which SECOND FACTOR was used is recorded, not just "mfa". A consumed
    # backup code is an account-recovery signal — the user has lost their
    # authenticator, or someone with the codes is using them — and the durable
    # trail previously said `detail="mfa"` identically for both, so that signal
    # existed only in the rotating app log. `remaining` matters too: a run of
    # burns is what a compromise looks like.
    factor = "totp"
    if mfa.verify_totp(user.mfa_secret, body.code):
        ...
        db.commit()
        factor = f"backup_code (remaining={len(remaining)})"
    ...
    auth_audit.record(db, auth_audit.LOGIN_SUCCESS, username=user.username, user_id=user.id,
                      ip=client_ip, detail=f"mfa factor={factor}")
    ...
    # core.security.create_access_token's docstring / T7).
    session_token = create_access_token(user.id, amr="pwd+mfa")
    set_session_cookie(response, session_token)
...
def mfa_disable(body: MfaDisableRequest, request: Request, current_user: CurrentUser, db: DbDep):
    ...
    if current_user.mfa_enabled and current_user.mfa_secret:
        ...
        try:
            ...
            raise
        # current_user.mfa_secret is already the plaintext
...
def switch_role(body: SwitchRoleRequest, request: Request, response: Response,
    ...
    The session identifier is ROTATED on every actual switch: a new token is
    issued and the old one revoked. Without that, one token spanned both the
    before and after of a privilege change, so a copy captured while the user
    was a product_manager silently gained admin authority the moment they
    switched — and stayed valid for the rest of the 8h TTL. Rotating on
    privilege change is the same rule already applied at login.
    """
    ...
    if body.role != current_user.role:
        ...
        # Preserve the ORIGINAL amr: rotation must not launder a password-only
        # session into one that claims MFA, nor demote a genuine pwd+mfa
        # session into re-authentication. Login now challenges anyone assigned
        # admin, so an admin-capable session already carries "pwd+mfa".
        old_token = extract_token(request)
        amr = (decode_access_token_amr(old_token) if old_token else None) or "pwd"
        new_token = create_access_token(current_user.id, amr=amr)
        ...
        auth_audit.record(db, auth_audit.ROLE_SWITCHED, username=current_user.username,
                          user_id=current_user.id, ip=client_ip,
                          detail=f"{previous.value}->{body.role.value} (session rotated)")
    return UserResponse.model_validate(current_user)
...
def auth_audit_log(
    ...
    limit: int = Query(100, ge=1, le=500), event: str | None = None, username: str | None = None,
):
