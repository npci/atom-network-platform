...
Internal testing tool. Two modes, both AdminUser-gated:
  ...
  * **send**             — the Authority originates a cert message. Target is
                           either a registered partner (uses its stored endpoint
                           + secrets, so JWT + HMAC are exercised) or an
                           arbitrary base URL for a locally-run cert stack.
  * **simulate-inbound** — the Authority plays the *partner* and posts a
                           Partner→Authority message at its own
                           `/a2a-rpc/rpc`. Goes through the real ingress
...
# own role/prefix here, not a payments network's.
...
# The `initiator`/`reporter` tokens below are PINNED wire values, mapped in
# exactly one place (`services/cert_agent/execution.py::WIRE_INITIATOR`).
# Imported rather than restated so these templates cannot drift from the wire.
from app.services.cert_agent.execution import WIRE_INITIATOR as _WIRE_INITIATOR

_WIRE_AUTHORITY = _WIRE_INITIATOR["authority"]
...
_TEMPLATE_ARGS: dict[str, dict[str, Any]] = {
    ...
    T.CERT_SETUP_NOTIFICATION: {
        ...
        "case_list": [{"case_id": _EX_CASE, "sheet": "REPLACE",
                       "initiator": _WIRE_AUTHORITY,
    ...
    T.CERT_CONFIG_SUBMISSION: {
        # PINNED WIRE FIELD NAMES — not renameable vocabulary. `bank_identity`
        # and its `nbin`/`ifsc` members are the A2A cert protocol's own keys:
        # a2a_common/cert_tasks.py lists them as redaction paths,
        # cert_config_submission() takes `bank_identity` as a parameter,
        # cert_agent/setup.py reads it back, and protocol.py defines a
        # BANK_IDENTITY_MISMATCH error against it. A partner platform sends this
        # shape across a trust boundary, so renaming it here would break
        # interop, not just wording. The VALUES are already placeholders.
    ...
    T.CERT_VERDICT_DISPUTE: {
        ...
        # Pinned wire field name — see the note on `bank_identity` above.
        # cert_tasks.cert_verdict_dispute() takes `bank_position` by that name.
    ...
    T.CERT_CASE_RESULT: {"case_id": _EX_CASE, "attempt": 1, "reporter": _WIRE_AUTHORITY, "status": "passed"},
...
def cert_templates(_: AdminUser):
    ...
    for tt in sorted(T.ALL_CERT_TASKS):
        ...
        out.append({
            "task_type": tt,
            "direction": direction,
            # The Authority can originate everything except the partner-only
            # messages; those are reachable through simulate-inbound instead.
            # ("npci_to_bank" is the value on the wire, not a brand name.)
...
async def cert_send(body: CertSendRequest, db: DbDep, _: AdminUser):
    ...
    if body.task_type in T.PARTNER_TO_AUTHORITY:
        raise HTTPException(
            status_code=400,
            detail=f"'{body.task_type}' is Partner→Authority — use /simulate-inbound instead.",
        )
    ...
    for _label, _target in (("endpoint_url", base_url), ("rpc_url", body.rpc_url)):
        ...
        # expose live test transaction data on a link we do not control. Unlike
...
class CertSimulateInboundRequest(BaseModel):
    ...
    cert_attempt: int = 1
    payload: dict
    partner_id: str            # the partner we impersonate
    change_id: Optional[str] = None


@router.post("/simulate-inbound")
async def cert_simulate_inbound(body: CertSimulateInboundRequest, db: DbDep, _: AdminUser):
    """Post a Partner→Authority cert message at our own ingress, as the named partner.
    ...
    if body.task_type in T.AUTHORITY_TO_PARTNER:
        raise HTTPException(
            status_code=400,
            detail=f"'{body.task_type}' is Authority→Partner — use /send instead.",
        )
