...
from app.core.change_access import require_change_access
...
router = APIRouter(tags=["agents"], dependencies=[Depends(require_change_access)])


def _docgen_audience(doc_type: str) -> str:
    """Who a generated document is addressed to, per the ACTIVE pack.

    The addressee list must come from the pack's `docgen_circular_addressees`,
    never from here. When it was hardcoded, an aviation-domain airworthiness
    circular was generated addressed to "All Member Banks, Payment Service
    Providers (PSPs) and Third-Party Application Providers (TPAPs)" — the quote
    is kept because it is the acceptance criterion: a domain term in a generated
    artifact came from the platform, not from the pack.

    Non-circular documents keep their internal-reader audience, which carries no
    domain vocabulary and needs no pack lookup.
    """
    if doc_type != "circular":
        return "Product Managers, Tech Leads, Operations"

    from app.core.domain.registry import prompt_block

    return prompt_block(
        "docgen_circular_addressees",
        "Every participant category in the ecosystem",
    )
...
def _load_conversation(change_id: str, module: ConversationModule, db: Session) -> list[dict]:
    """Load conversation history for a module as provider-agnostic message dicts."""
    rows = (
    ...
    try:
        ...
        if ta.get("data_model_changes_superseded_by_approach_decision"):
            ...
            lines.append("- WIRE/API & SCHEMA SURFACE: the pre-gate surface was SUPERSEDED by the human's "
                         ...
                         + " — describe ONLY what that approach requires; introduce NO new wire message "
                           "or authority API the chosen approach does not, and give NO Req/Resp name to an "
                           "internal operation.")
        elif dmc:
            lines.append("- WIRE/API & SCHEMA SURFACE (the COMPLETE set — introduce NO wire message, "
                         "schema, or authority API beyond this; an internal DB/cache/Kafka/config operation is "
                         "NOT a new API and must never be given a Req/Resp name): " + str(dmc)[:1400])
        else:
            lines.append("- WIRE/API & SCHEMA SURFACE: NONE. This change adds NO new wire message, NO "
                         "schema/XSD change, and NO new authority API — it is PARTICIPANT-INTERNAL. Every doc must say "
...
def _tech_design_block(change_id: str, db: Session) -> str:
    ...
    try:
        ...
        parts: list[str] = [
            ...
            "wire/authority error codes the design did not define, and do NOT contradict it across sections."]
...
def get_ui_config():
    ...
    # screens derive their rule from this instead of hardcoding the network's
    # core+app pair. EMPTY IS MEANINGFUL, not a failure: it means the domain declares no
    ...
    # The two INITIATOR tokens — distinct from `cert_roles` above, which is the
    # party list a partner certifies FOR. These are the strings that arrive as
    # JSON object KEYS in the cert-push summary (`initiated_by_counts`), so the
    # SPA has to read them rather than hardcode a pair: it used to compare
    # `v === 'NPCI'` and key its colour map on `NPCI`/`BANK`, which would render
    # a silently-zero breakdown under any pack with different tokens.
    # `token` is the wire value; `label` is the human-facing name.
    try:
        from app.core.domain.registry import prompt_block
        from app.services.cert_roles import authority_role, partner_role

        _auth, _part = authority_role(), partner_role()
        cert_initiators = {
            "authority": {
                "token": _auth,
                "label": prompt_block("initiator_authority_label", "") or _auth,
            },
            "partner": {
                "token": _part,
                "label": prompt_block("initiator_participant_label", "") or _part,
            },
            # How the domain phrases "this party acting as <role>" — e.g. a
            # pack may declare `role_as_label` as "Operator as". The SPA
            # rendered one pack's literal label in SyncDiffModal's initiator
            # chip tooltip, which is the same hardcode the comment above
            # describes for the tokens, one field over. "" means the deployment
            # declares none, and the chip drops the prefix rather than
            # inventing one.
            "role_as_label": prompt_block("role_as_label", ""),
        }
    except Exception:  # noqa: BLE001 — same rule as repo_roles above
        logger.exception("config/ui: could not resolve cert_initiators; sending {}")
        cert_initiators = {}

    # Test-id prefix -> role-sheet name, for engine-generated packs whose JSON is
    # a flat test_cases list with no sheet grouping.
    #
    # The SPA rebuilt this map itself, hardcoding one ecosystem's prefixes
    # (e.g. "PR_" -> "ROLE_A"). The backend has sourced these from the
    # pack (`cert_vocabulary.role_prefixes`) for as long as the pack has
    # existed — `services/cert_catalogue.py` and `tc_store_sync.py` both do —
    # so the SPA was the one place still asserting one ecosystem's convention,
    # and a pack with different prefixes rendered every case under "Other".
    #
    # `cert_extra_sheet_prefixes` carries the non-role sheets (whatever extra
    # tabs a pack declares, e.g. META-API / UAT) in "PREFIX|Sheet" lines.
    try:
        from app.core.domain.registry import prompt_block

        _cv = cert_vocabulary_of(get_active_pack())
        cert_sheet_prefixes = [
            {"prefix": pfx, "sheet": _cv.role_labels.get(role, role)}
            for role, pfx in _cv.role_prefixes.items() if pfx
        ]
        for line in prompt_block("cert_extra_sheet_prefixes", "").splitlines():
            pfx, _, sheet = line.partition("|")
            if pfx.strip() and sheet.strip():
                cert_sheet_prefixes.append(
                    {"prefix": pfx.strip(), "sheet": sheet.strip()}
                )
    except Exception:  # noqa: BLE001 — same rule as repo_roles above
        logger.exception("config/ui: could not resolve cert_sheet_prefixes; sending []")
        cert_sheet_prefixes = []

    return {
        "dev_mode": (_app_settings.app_env or "").lower() != "production",
        "app_env":  _app_settings.app_env or "development",
        # Configurable external link for the sidebar's "Authority Simulator"
        ...
        "cert_initiators": cert_initiators,
        "cert_sheet_prefixes": cert_sheet_prefixes,
    }
...
def download_canvas_docx(change_id: str, db: DbDep, _: CurrentUser):
    ...
    title = cr.title or "Network Feature"
    ...
    try:
        ...
        while True:
            ...
            existing = (db.query(TechSpec)
                        .filter(TechSpec.change_request_id == change_id)
                        .order_by(TechSpec.version.desc()).first())
            # ADR-0005 — auto-approve on generate. No explicit
    ...
    try:
        ...
        while True:
            ...
            doc_labels = {
                ...
                "circular": "authority technical circular",
            ...
            if docgen_eligible:
                ...
                if is_revise:
                    ...
                    state = build_initial_state(
                        ...
                        audience=_docgen_audience(doc_type),
                        desired_outcome=f"Approved {docgen_doc_type}",
                        research_report=research_report,
                        canvas_content=canvas_content,
                        additional_context=additional_context,
                        # Circular blueprint expects no diagrams (the Authority's
                        # operating-circular format).
            ...
            # Authority-format .xlsx + .md + .docx. Gated on
...
async def docgen_edit_section(
    ...
    try:
        ...
        # Error-message exposure (CWE-209): broad handler — `e` here can be a SQLAlchemy error carrying
