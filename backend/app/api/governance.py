...
async def upload_skill_bundle(stype: str, db: DbDep, current: CurrentUser,
    ...
    _stype_or_404(stype)
    _require_skill_owner(stype, current)
    # Cap the COMPRESSED upload before it is buffered. `parse_bundle` enforces
    # per-member, member-count and total-extracted ceilings, but all of those
    # run after the archive is already fully in memory — an unbounded
    # `await file.read()` here meant a multi-GB body was buffered before any of
    # them could fire. Reuses the archive ceiling the parser itself declares, so
    # the two cannot drift.
    raw = await file.read(GB.MAX_ARCHIVE_BYTES + 1)
    if len(raw) > GB.MAX_ARCHIVE_BYTES:
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail=f"Bundle exceeds {GB.MAX_ARCHIVE_BYTES // (1024 * 1024)} MB ceiling")
    try:
