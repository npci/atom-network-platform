...
"""REST + WebSocket surface for the agentic codegen pipeline.
...
def agentic_health(db: DbDep, current_user: AgenticUser):
    ...
    return {
        ...
        # Resolve through get_model() — settings.claude_model is only the Anthropic
        # path's model, so reporting it directly lies under ollama/openai/gemini.
        "model": _get_model(),
    }


@router.get("/agentic/runs")
def list_runs(db: DbDep, current_user: AgenticUser, limit: int = Query(50, ge=1, le=200)):
...
def decide_tsd_approval(run_id: str, db: DbDep, current_user: CurrentUser):
    """Resume a run parked at AWAITING_TSD_APPROVAL (ADR-0005).
...
def decide_plan(run_id: str, body: DecidePlanRequest, db: DbDep, current_user: CurrentUser):
    ...
    if getattr(settings, "agentic_require_critical_decisions", True):
        ...
        if not isinstance(_cds, list) or not _cds:
            # The dimensions are PACK content: `agentic_subagents` prompts the
            # planner from this same block. This operator-facing message used to
            # name one ecosystem's dimensions while the prompt named the pack's,
            # so the gate and the prompt disagreed about what was required.
            from app.core.domain.registry import prompt_block as _prompt_block

            _dims = _prompt_block(
                "critical_decision_dimensions",
                "state_changing_legs (enumerate every leg that changes state "
                "and who is affected), ordering_model")
            _gaps.append("plan has no critical_decisions block — re-run the analysis "
                         f"(the planner must decide {_dims} explicitly, or ask a "
                         "clarification)")
        else:
            for cd in _cds:
