...
class CodePlanStatus(str, enum.Enum):
    """HITL gate #3 status for a structured code plan."""
    DRAFT    = "draft"
    APPROVED = "approved"
    REJECTED = "rejected"
...
class BuildRun(Base):
    ...
    app_branch: Mapped[str | None] = mapped_column(String(200), nullable=True)
...
class CodePlan(Base):
    """Structured code-change plan produced by the code_planner agent.
