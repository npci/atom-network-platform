...
class DocCategory:
    ...
    NETWORK_PRODUCT_DOC = "network_product_doc"
    PAST_BRD = "past_brd"
    API_SPEC = "api_spec"
    XSD = "xsd"
    JAVA_SOURCE = "java_source"
    # Polyglot code source categories. Strings match the values
    ...
    AUTHORITY_CIRCULAR     = "authority_circular"      # The Authority operational circulars (OCs)
    AUTHORITY_ERROR_CODE   = "authority_error_code"    # error/response code reference tables
    AUTHORITY_TSD          = "authority_tsd"           # technical specification documents
    AUTHORITY_PRODUCT_NOTE = "authority_product_note"  # product guideline PDFs
    AUTHORITY_FAQ          = "authority_faq"           # FAQ docs
    AUTHORITY_PRODUCT_DECK = "authority_product_deck"  # product slide decks
    AUTHORITY_CERT_PACK    = "authority_cert_pack"     # cert testcase reference packs (.xlsx)
    AUTHORITY_XML_SPEC     = "authority_xml_spec"      # network wire-format reference XML samples 

    # Distilled Authority design guidance (API/flow, test-case, error-code design) used
...
# All code-source categories. Code-change retrieval (and any
...
class DocumentChunk(Base, TimestampMixin):
    ...
    # Tree-sitter-derived symbol metadata (nullable; populated only for
    ...
    # 3-view embedding grouping. When USE_CODE_MULTIVIEW_EMBEDDING is
    ...
    # Doc metadata + hierarchical-chunking linkage (all nullable).
    ...
    # Symbol-graph edges extracted via tree-sitter for Java (and
    # later Python/JS/TS via the LSP resolvers). Cross-file edges require a
    # global symbol index — not yet built. All nullable; populated only by
    ...
    # Cross-file call resolution via Python LSP. Each entry is
    ...
    # (and TS via its own LSP resolver) reuse this same column.
