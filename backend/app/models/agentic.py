...
This is the persistent spine for the agentic Phase-B replacement. The legacy
single-shot Phase-B path (``phase_b.py``) is untouched and runs when the
feature flag is off.
...
class AgenticPhase(str, enum.Enum):
    ...
    # ADR-0005 — the TSD approval + version-lock gate (only
...
class AgenticRun(Base):
    ...
    # Phase A/B split: a run is "full" (legacy single run, xsd→code
    ...
    # Structured failure category for ops triage (e.g. llm_rate_limit,
    ...
    # ADR-0005 — the TechSpec.version that was APPROVED at the
...
class VerificationRun(Base):
    """One verification round — raw result + LLM reasoning + the gate verdict (§9)."""
...
class ReviewFinding(Base):
    ...
    # A claude id, or an AiNxt-recognized OpenAI id (gpt/o1/o3/o4) when a
    # different-vendor reviewer is configured — validated in run_review.
