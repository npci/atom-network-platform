...
Tables:
  partner_agents             — Partner registry (the domain's participant roles;
                               + internal cert_engine)
...
class PartnerType(str, enum.Enum):
    """LEGACY stored vocabulary — no longer the source of truth.

    Which participant roles an operator may register is a DOMAIN question, and
    the answer now comes from the active pack: `participants_of(pack)` supplies
    the selectable keys, and `high_assurance_participants_of(pack)` decides
    which of them belong on the mTLS ingress. `api/partners.py` resolves both.

    These four values stay because they SIT IN DATABASE ROWS —
    `partner_agents.partner_type` is a JSON array of these strings on every
    partner registered so far, and the network pack's participant keys use the
    same spelling for the two that matter to the TLS rule (`remitter`,
    `beneficiary`). Deleting a member would orphan those rows; the values are
    therefore pinned and the enum is kept as the back-compat accept-list that
    is UNIONed with the pack's own keys.

    `cert_engine` is different in kind: it is an authority-internal address, not
    a domain role, so it is not pack data and is never operator-selectable.
    """

    PAYER_PSP    = "payer_psp"     # legacy: initiating-side intermediary
    PAYEE_PSP    = "payee_psp"     # legacy: receiving-side intermediary
    REMITTER     = "remitter"      # legacy: initiating-side account-holding participant
    BENEFICIARY  = "beneficiary"   # legacy: receiving-side account-holding participant
...
class CertDirection(str, enum.Enum):
    AUTHORITY_TO_PARTNER = "authority_to_partner"
    PARTNER_TO_AUTHORITY = "partner_to_authority"


# Pre-rename spellings of the values above, still arriving on the wire.
#
# Unlike the tables and columns migration 0139 renamed, these are values the
# PARTNER platform sends us. It deploys separately and has not been updated, so
# until it has, both spellings reach `_normalize_direction`.
#
# This needs an explicit alias table rather than a "close enough" match because
# the normalisers fall back to AUTHORITY_TO_PARTNER for anything they do not
# recognise. An unmatched legacy value is therefore not a loud failure but a
# silently INVERTED one: every partner-initiated result attributed to the wrong
# side of the trust boundary, with no exception and no log line. The HTTP
# headers got dual-accept treatment for exactly this reason
# (`a2a_common.hmac_signer`); this payload value was missed because, unlike a
# header mismatch, nothing about it fails loudly.
#
# Safe to delete once the partner platform emits only the new spellings AND no
# stored payload replays the old ones.
LEGACY_CERT_DIRECTION_ALIASES: dict[str, CertDirection] = {
    "partner_to_npci": CertDirection.PARTNER_TO_AUTHORITY,
    "npci_to_partner": CertDirection.AUTHORITY_TO_PARTNER,
}


# Payload keys for the blocker's "what do you need from the authority" field,
# current spelling first.
#
# Same class of problem as the direction aliases, with one extra twist:
# migration 0139 renamed the blockers COLUMN, but nothing rewrites JSON payload
# keys, so `blockers.payload` and `a2a_messages.payload` keep the old spelling
# on historical rows permanently — the partner deploying is not enough to retire
# this. Reading only the new key drops the partner's actual ask silently: the
# blocker stores NULL and the auto-resolver triages without knowing what was
# asked for.
_REQUESTED_ACTION_KEYS: tuple[str, ...] = (
    "requested_action_from_authority",
    "requested_action_from_npci",
)


# ── Negotiation originator / reply-role tokens ───────────────────────────────
#
# LEGACY WIRE SPELLINGS. `"npci"` is what `counter_proposals.originator` holds
# on every authority-originated row already written, what the SQL that finds
# those rows compares against, what the partner payload's `"from"` field
# carries, and one of the `negotiation_messages.role` values the unread-count
# walk treats as "the authority has replied". It cannot be renamed from this
# side without a data migration AND a coordinated partner deploy.
#
# What IS fixed here is the scattering: the same literal was written out
# independently in six places across three modules, which is how a rename sweep
# gets one of them and breaks the query silently. Naming it once means the next
# person changes one line, and the frontend already did the same thing
# (`lib/certStatus.js::AUTHORITY_REPLY_ROLES`).
#
# A future migration that retires the legacy spelling has to do all of:
#   1. `UPDATE counter_proposals SET originator='authority' WHERE originator='npci'`
#   2. accept BOTH spellings on the inbound `"from"` field for a release
#      (the `LEGACY_CERT_DIRECTION_ALIASES` pattern above), and
#   3. `UPDATE negotiation_messages SET role='authority' WHERE role='npci'`,
#      plus the same dual-accept in the SPA's role→label map.
# Until all three land together, this is the spelling.
ORIGINATOR_AUTHORITY = "npci"
ORIGINATOR_PARTNER   = "partner"

# `negotiation_messages.role` values that count as "the authority has answered"
# — a partner message with none of these after it is unread. Mirrors the SPA's
# `AUTHORITY_REPLY_ROLES`; the two must stay in step or the badge count and the
# thread rendering disagree.
AUTHORITY_REPLY_ROLES: frozenset[str] = frozenset(
    {"po_approved", ORIGINATOR_AUTHORITY, "approved"})


def requested_action_from(payload: dict):
    """The blocker's requested action, under whichever key the payload uses."""
    for key in _REQUESTED_ACTION_KEYS:
...
class PartnerAgent(Base, TimestampMixin):
    ...
    # Participant roles. The API resolves the selectable set from the active
    # pack (`api/partners.py::_selectable_partner_types`); this column-level
    # default is a legacy STORED value, kept so a direct insert that names no
    # role still writes something the existing rows agree with. Every API path
    # passes an explicit list, so it is a floor, not the policy.
    ...
    # Cert-agent's short partner code for this partner. Used by the readiness
    # orchestrator to address the right partner-simulator endpoint when
    # triggering an LLM cert run. The VALUE is operator-supplied and opaque —
    # it must match whatever the external cert-agent has configured.
    # Column renamed from `cert_agent_bank_id` in alembic 0141; the partners
    # API still accepts and emits the old payload key for one release.
    cert_agent_partner_id: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    # ── Per-partner JWT signing secret ─────────────────────────────────
    ...
    # ── HMAC envelope secret ─────────────────────────────────────────
    # Distinct from `jwt_signing_secret`: that one signs the IDENTITY
    # (Bearer JWT), this one signs the PAYLOAD (the signature header
    # over body sha256 + ts + nonce — emitted as `X-Auth-Signature`, with
    # the superseded `X-NPCI-Signature` spelling still accepted inbound;
    # see a2a_common/hmac_signer.py). Both NULL means "back-compat" —
    ...
    # account-holding participants are the tier-mtls
    # callers today; the intermediary-tier partners and cert_engine stay on jwt.
    # `client_cert_fingerprint` is the SHA-256 hex of the
    # partner's client cert; nginx forwards `X-Client-Cert-Fingerprint`
    ...
    # zone, this column is the per-partner override hook.
    ...
    # ── T2 — "HMAC signature/key-version not persisted
    ...
    def generate_jwt_signing_secret() -> str:
        """32-byte hex secret for HS256 outbound JWT signing."""
        return secrets.token_hex(32)

    @staticmethod
    def generate_signing_secret() -> str:
        """32-byte hex secret for HMAC envelope signing."""
        return secrets.token_hex(32)
...
class A2AMessage(Base):
    ...
    # Response artifact (admin A2A logs UI) persisted alongside
    ...
    # ── Audit columns (alembic 0036) — regulator evidence trail ────────────
    ...
    # side. The mTLS fingerprint lives on `client_cert_fingerprint`.
    ...
    # ── T1/T2 — "No at-rest integrity check on
    ...
    # hmac_signature: the verified payload-signature header value for this
    # message, under whichever spelling the sender used (inbound only — outbound rows leave this NULL, since the
...
class CounterProposal(Base):
    ...
    # Which side proposed: `ORIGINATOR_PARTNER` (default — the partner counters
    # the Authority's rollout proposal) or `ORIGINATOR_AUTHORITY` (PM counters
    # back the partner's terms). Each round of a multi-round negotiation
    # alternates. Both tokens are legacy wire spellings — see the constants.
    originator: Mapped[str] = mapped_column(
        String(20), nullable=False, default=ORIGINATOR_PARTNER)
...
class Blocker(Base):
    ...
    requested_action_from_authority: Mapped[str | None] = mapped_column(Text, nullable=True)
    payload: Mapped[dict | None] = mapped_column(JSON, nullable=True)  # full inbound payload for audit
...
class CertRun(Base):
    ...
    authority_mode: Mapped[str | None] = mapped_column(String(20), nullable=True)
...
class CertTestResult(Base):
    ...
    # may mix modes (partner-initiated legs) and §3.4 wants the contract ON the
