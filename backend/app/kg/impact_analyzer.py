...
Standalone module, no production wiring yet.

Contrast with `graph_retriever`:
...
Target resolution accepts (in priority order):
  ...
  - `change_description` — seeds extracted via the graph retriever's extractor, each
...
SELECT from hitting InFailedSqlTransaction — same guard as the graph retriever.
"""
