...
This is a standalone **post-ingest** pass — not wired into
...
silently drop the edge (logged, counted) — acceptable for v0; the graph
retriever tolerates sparse edges.
...
Not populated here (no data yet, or future work):
...
def build_inherits_edge_within_file(
    ...
    index). Language-specific LSPs are expected to backfill.
    """
    return build_merge_edge_cypher(
        src_label="Class",
...
def build_cross_file_call_edge(
    ...
    Projects each entry of `chunk.cross_file_calls` (set by the
    Python LSP resolver) as a graph edge. Uses the same `CALLS` label as
...
def plan_ingest_from_chunks(
    ...
    class_chunks: list[dict] = []      # kept for the second pass (edges)
    ...
    # Cross-file CALLS from LSP-resolved entries. Same
    ...
    *,
    ...
    for kind, cypher in plan.operations:
        try:
            ...
            # Error-message exposure (CWE-209): `report.failures` is returned verbatim by
...
def _chunk_to_dict(c: Any) -> dict:
    ...
    return {
        ...
        # Cross-file calls from Python LSP. Same JSON shape
