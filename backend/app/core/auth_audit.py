...
# A privileged route refused an authenticated caller.
#
# Previously recorded nowhere: `deps.require_admin` / `require_role` raised 403
# with no audit row and no SECURITY_EVENT, so someone probing routes for one
# that would let them through left no durable trace. The admin rate limiter
# fires on THROTTLE, not on denial, so it did not cover this either — which is
# precisely the shape of privilege-escalation probing going unseen.
