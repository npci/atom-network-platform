...
Closes S5 ("Wrap direct OS process
...
execution time") and implements ADR-0003 and the rule that "Direct OS
process execution from application business flows is prohibited... MUST
be routed through a tightly controlled, approved, auditable service
interface".
...
def _allowlist_from_settings() -> frozenset[str]:
    ...
    fail-safe, per the 'fail fast instead of starting insecurely' rule: an
    EMPTY effective allowlist would
    ...
    async def run(self, req: ProcessExecutionRequest) -> ProcessResult:
        ...
        if not req.timeout_s or req.timeout_s <= 0:
            raise MissingTimeoutError(
                ...
                f"supply a bounded execution time."
            )
