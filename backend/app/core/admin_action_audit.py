...
T8 ("No comprehensive admin ACTION audit log (vs. auth-event audit)").
...
def redact_secret_fields(fields: dict | None) -> dict | None:
    ...
    RECURSES into nested dicts and lists. This used to walk only the top level,
    so a secret one level down — ``{"partner": {"signing_secret": "..."}}``, or
    any list-of-rows payload — reached the audit table verbatim. Audit rows are
    long-lived and broadly readable, which makes an unredacted secret there
    worse than the same value in a rotating log.
    """
    if not fields:
        return fields
    return _redact_value(fields)


def _redact_value(value):
    if isinstance(value, dict):
        return {
            k: ("<redacted>" if k in _SECRET_FIELD_NAMES else _redact_value(v))
