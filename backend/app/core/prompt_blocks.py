...
"""Centralised prompt blocks.

Single source of truth for the long, stable prose blocks that several
agents share. Centralising them is the prerequisite for
...
Usage:
    from app.core.prompt_blocks import (
        ...
        assemble_system_prompt, segments_for_prompt_cache,
    )
...
parts list to `segments_for_prompt_cache` which marks the long stable
...
def active_pack_terms() -> dict[str, str]:
    ...
    The network pack's blocks are byte-identical to DEFAULT_TERMS, so the default
...
# system prompts). Under the default network pack the pack blocks are byte-identical
...
def assemble_system_prompt(parts: Iterable[str]) -> str:
    ...
    Used by agents whose prompt-cache wiring isn't on yet, or by providers
    where the system prompt is a single string.
    """
    blocks = [p.strip() for p in parts if p and p.strip()]
    return _BLOCK_SEPARATOR.join(blocks)


def segments_for_prompt_cache(
    parts: list[tuple[str, bool]],
) -> list[dict]:
    """Produce segmented system-prompt blocks with selective
    `cache_control` markers.

    Prompt caching over a stable prefix is not vendor-specific — the wire
    encoding here is the Anthropic one, and `llm._coerce_system_to_str`
    flattens it harmlessly for providers that take a single string, so every
    one of this function's callers is provider-agnostic. It was named
    `segments_for_anthropic_cache`, which read as though the whole call path
    were Anthropic-only; that alias is kept below for compatibility.
...
# prefer `segments_for_prompt_cache`.
segments_for_anthropic_cache = segments_for_prompt_cache
