...
# Use of hardcoded credentials (CWE-798) — this
...
class Settings(BaseSettings):
    ...
    # AI Provider: "anthropic", "openai", "ainxt", "ollama", or "gemini".
    # "claude" remains accepted as a backward-compatible alias.
    ...
    # Apply pending alembic migrations from the app's own startup event.
    #
    # OFF by default, and deliberately so: compose already runs
    # `alembic upgrade head` before uvicorn, that is the correct place for it,
    # and DDL issued from an app process races itself when more than one worker
    # or replica boots at once (prod runs --workers 2).
    #
    # It exists for the DEV hot-reload stack, which has a gap the compose step
    # cannot cover: docker-compose.override.yml bind-mounts backend/app AND
    # backend/alembic and runs uvicorn --reload, so editing a model reloads the
    # code and adding a revision drops it into the container, but neither
    # re-runs alembic. The schema and the models then drift apart until the
    # container is restarted — which is how alembic 0141 came to be written,
    # mounted, and never applied on 2026-09-06. Turn this on in that stack and a
    # reload applies what it needs; leave it off anywhere that boots more than
    ...
    # External Authority Simulator UI. Surfaced by the sidebar's "Authority
    # Simulator" link. Override via env or the admin Config UI for non-localhost
    # deployments (Ubuntu host-mode, staging, production).
    authority_simulator_url: str = "http://localhost:5173"

    # Tree-sitter AST code chunker feature flag.
    ...
    # 3-view code embedding feature flag. Requires the tree-sitter chunker
    # above to be on to have any effect. When on, each tree-sitter symbol
    ...
    # Query understanding (rewriter + HyDE + NER) feature flag.
    ...
    # but unused for retrieval until the knowledge graph lands.
    # Default off — enables measurable A/B against the multi-view baseline.
    use_query_understanding: bool = False

    # Cross-encoder reranker. When on, retrieve() over-samples
    ...
    # Hierarchical markdown chunker. When on, .md files are chunked
    ...
    # Context compression. When on AND a query is supplied to
    ...
    # Citation enforcement (pattern: deep_researcher only; the same pattern
    # is replicated for BRD/TSD/Canvas). When on, the RAG context is
    ...
    # ADR contradiction check. When on, BRD and Tech Spec system
    ...
    # Sandboxed code execution (Docker-in-Docker via docker SDK).
    # `run_in_sandbox` uses these as defaults when callers don't override.
    # No production caller yet — the self-correction loop below is the first
    ...
    # Self-correction loop. When enabled, the editor's output is
    ...
    # AST editor (aider-style SEARCH/REPLACE patches). When on,
    ...
    # Symbol-graph extractor (tree-sitter-based, Java first;
    # Python/JS/TS followed later). When on, code
    # ingestion enriches each chunk with imports / inherits / implements /
    # calls / called_by (within-file). Plan §4.2.2. Does nothing when
    # USE_TREE_SITTER_CHUNKER is off — needs the tree-sitter
    ...
    # Doc↔code linker. Runs as a standalone post-ingest pass
    ...
    # Apache AGE knowledge graph hosted alongside pgvector in the
    ...
    #
    # UPGRADE NOTE: an AGE graph cannot be renamed in place. A deployment that
    # ingested under a different graph name and then takes this default gets a
    # retriever that returns nothing.
    #
    # AGE itself is not the quiet part: querying a graph that does not exist
    # raises `graph "x" does not exist` — it does NOT create one. The silence is
    # ours. `kg.client.is_age_available` swallows the error and every caller
    # degrades to an empty result, so the symptom is missing graph-hop evidence
    # with all health checks green. `_warn_if_graph_missing` now logs a WARNING
    # naming this setting when AGE is present but the graph is not; that log line
    # is the only thing that distinguishes this from AGE being uninstalled.
    #
    # Set KG_GRAPH_NAME to the existing graph, or re-ingest under this one.
    # Re-ingesting requires the graph to exist first: migration 0020 is the only
    # thing that calls `create_graph`, and it runs once.
    kg_graph_name: str = "atom_kg"

    # Projection of existing RAG data into the AGE graph.
    ...
    # Idempotent (MERGE-based). No production caller yet.
    use_kg_ingestion: bool = False

    # Graph-traversal retrieval pass. When on, `graph_retrieve(query)`
    ...
    # Python LSP cross-file call resolution. When on, the
    ...
    # TypeScript LSP cross-file call resolution. Same plumbing as
    # the Python LSP above — populates `cross_file_calls` JSON column for TS/JS
    # chunks and projects into AGE via the existing CALLS edge
    ...
    # Java LSP cross-file call resolution. Same plumbing as the
    # Python/TypeScript LSPs — populates `cross_file_calls` JSON column for Java
    # chunks, projects into AGE via the CALLS edge builder.
    ...
    # Event-sourced incremental ingest. When on, polyglot
    ...
    # Celery-backed periodic incremental
    ...
    # ── Agentic XSD-driven code change ────────────────────────────────────────
    ...
    # memory between rounds, which is the root non-convergence gap. When True,
    ...
    # Index-time API FLOW-MAP generation for the reuse-first gate. When on, the
    ...
    # of failing the run — so a network blip does not terminate the run. Capped so
    ...
    # (compact and continue) instead of capping total spend. Kept only for
    ...
    # the agent stays directed and can re-read for the rest (preserving agent direction). Fail-open:
    ...
    # Dedicated reviewer model. A claude id, or an AiNxt-recognized OpenAI id
    # (gpt/o1/o3/o4) for different-vendor eyes — run_review validates both the id
    # and that the provider route can actually carry it.
    ...
    # OpenAI upstreams. The id must start with gpt/o1/o3/o4:
    # AiNxt silently routes unrecognized ids to its default model, so run_review rejects those loudly.
    agentic_reviewer_model: str = ""
    ...
    # Reuse-first "approach decision" gate: before creating any API/XSD
    ...
    # ENFORCING. The shadow rollout (agentic_acceptance_predicates=True,
    # measuring since its introduction) found no false-unmet signal against
    # the standard the flag's own comment set as the bar, so this shadow gate
    # was flipped to enforcing (alongside pointing agentic_reviewer_model away
    # from the author's own model), and is now a real blocker.
    # Fail-open + the LLM-advisory review and the
    ...
    # ENFORCE — the standing recommendation is to flip every
    ...
    # longer. Tracked as a scoped follow-up: widen contract_gate.py's
    ...
    # ENFORCING — flipped out of shadow mode. Unlike the contract
    ...
    # ADR-0005 — stops the TSD being treated as a prose document rather than a
    # binding contract. When True, CODE_CHANGE phase entry (kind='code' runs, and the
    ...
    # Mandatory cross-module analysis gate
    ...
    # TSD-derived test coverage gate
    ...
    # Tool-policy gate: the code agent must gather structural blast-radius
    ...
    # blind, and the loop runs on a reliable-tool-call provider (llm_provider=anthropic /
    # ainxt anthropic-compat — call_messages_api_tools refuses the finish-reason-blind path).
    ...
    # Per-agent LLM routing. Maps a `Purpose` (defined in
    ...
    # Model IDs are deliberately empty by default: a provider-neutral switch
    # must not send Anthropic model IDs to OpenAI, AiNxt, Gemini, or Ollama.
    # Configure an ID supported by the active provider to opt in per purpose.
    routing_model_routing: str = ""    # Purpose.ROUTING — inherit provider default
    routing_model_utility: str = ""    # Purpose.UTILITY — inherit provider default
    routing_model_reasoning: str = ""                            # Purpose.REASONING — empty = use global default (frontier model)

    # Per-agent model overrides — win over the `routing_model_*` settings
    ...
    # Critic grounding on the product / code knowledge base.
    ...
    # Observability traces. When on, every `call_llm` / `stream_llm`
    ...
    # Datadog via standard log shipping (no hard SDK dep here —
    # `use_langfuse` below wires Langfuse directly when a backend is chosen).
    ...
    # through AiNxt's /ask/cached endpoint cannot help.
    ...
    # Dedicated rotating JSONL file for LLM call traces.
    ...
    # Langfuse SDK forwarding. When True AND `langfuse`
    ...
    # ── Pre-call context budget assertion ────────────────────────────────────
    ...
    # "tsvector"  : Postgres content_tsv GIN index + ts_rank_cd.
    ...
    # ── Per-window storage for oversized chunks ──────────────────────────────
    ...
    #             for new installs.
    # "llm"     — original path (per-chunk LLM filter). Kept for
    ...
    # ── Reranker score cache + early-exit ────────────────────────────────────
    ...
    # ── Graph 1-hop expansion of top-K ───────────────────────────────────────
    ...
    # ── Text-search configuration for tsvector queries ───────────────────────
    # "english" (default) — stems + drops English stopwords. Good for prose,
    #   lossy for code (collapses `tokens`/`tokenize`, drops `is`/`as`/`in`).
    # "simple_code" — copy of pg_catalog.simple, no stemming
    #   and no stoplist. Better for code. Requires migration 0054 to have
    #   created the config on the target DB.
    # Hot-switchable: the tsvector GIN index serves any per-query config.
    bm25_text_search_config: str = "english"

    # PM-vocab → domain-vocab query rewriter (teammate's
    # `app/rag/query_rewriter.py`). Conceptually overlaps with
    ...
    # Shared token authenticating backend → cert-agent calls.
    # This used to default to a literal, published-in-source value
    ...
    # Partner-agent URL — required by the two-phase orchestrator to
    # fire the PARTNER-initiated batch from the partner side. Same docker
    # network as cert-agent; override via PARTNER_AGENT_URL when the
    # partner-side agent runs on host/remote.
    #
    # `BANK_AGENT_URL` stays accepted as a deprecated alias: the env var is set
    # in deployed .env files and compose overrides this repo cannot see, and a
    # rename that silently fell back to the default would point the orchestrator
    # at the wrong host with no error.
    #
    # The DEFAULT keeps the `bank-agent` hostname on purpose — it is the name of
    # a real service in the deployed compose topology, so it is a wire value,
    # not vocabulary. Renaming the service is a deployment change, not this one.
    partner_agent_url: str = Field(
        default="http://bank-agent:8003",
        validation_alias=AliasChoices("PARTNER_AGENT_URL", "BANK_AGENT_URL"),
    )

    # ── Integration-testing tunnel ──────────────────────────────────────────
    ...
    # ── The timeout budget. It MUST shrink inward ───────────────────────────
    ...
    # asserted at startup because a deployment that gets it wrong only
    ...
    # Per-alias egress gates: a bulkhead so one saturated target cannot
    ...
    # The ALIAS the partner's stack calls back through
    # for partner-initiated cases (`simulator.endpoint = a2a://<this>`). It must
    # name an entry in THIS platform's tunnel allowlist pointing at the
    # simulator's callback API. Deliberately NOT named `authority_simulator` —
    # the UI setting `authority_simulator_url` (config.py, External Authority
    # Simulator UI) is an unrelated knob and the near-collision caused confusion once
    # (COMBINED plan §Phase-3 3.5).
    integration_testing_simulator_alias: str = "cert_simulator"
    # The OTHER catalogue entry. Mode selects between these
    ...
    # The C-6 loop refuses to AUTO-dispatch a round in
    ...
    cert_harness: str = ""
    # The SUITE deadline — how long a run waits for partner-reported
    ...
    # (no schema change; 0133–0135 stay reserved for simulator/tunnel columns).
    ...
    # grades the response code the partner's switch actually returned.
    ...
    # (CWE-295, improper certificate validation). A misconfigured path fails
    # startup loudly rather than silently reverting to the insecure default.
    ...
    # ── Table names in databases this platform does NOT own ──────────────────
    #
    # These name tables inside the precert simulator's database and the cert
    # agent's catalogue. Renaming them here renames nothing there — it only
    # makes the query miss — so they are configuration rather than hardcoded
    # SQL literals, where a vocabulary sweep could silently rewrite a query
    # against someone else's schema.
    #
    # They are deployment configuration, not domain vocabulary: two deployments
    # of the same domain can point at differently-named tables. Empty by
    # default and validated at the point of use, so a deployment that turns the
    # precert engine or the cert agent on gets a named error instead of a
    # confusing SQL failure. See `require_external_table`.
    precert_engine_txn_log_table: str = ""    # was hardcoded "upihosttxnlog"
    cert_agent_testcase_table: str = ""       # was hardcoded "tbl_upi_testcases"
    # First-cut scope: the single precert scope + participant every readiness is
    # certified against. Per-(change, partner) scope mapping is the next step.
    #
    # The org id and the role are EMPTY by default, for the same reason as the
    # two table names above: they were a real-looking org id and one ecosystem's
    # role key, shipped as this platform's answer for every deployment. Both now
    # resolve at the point of use — the org id from the partner's own
    # `cert_agent_partner_id`, the role from the active pack's declared cert
    # roles — and a deployment supplying neither gets a named 412 rather than a
    # cert run conducted against somebody else's vocabulary.
    precert_engine_psp_org_id: str = ""       # was hardcoded "OLV101"
    precert_engine_subset: str = "Subset-A2A-BIDI"
    precert_engine_cert_name: str = "Cert1"
    precert_engine_certgroup: str = ""        # was hardcoded "REMITTER"
    ...
    # cfg/precert.cer, once seeded into tbl_cert_file.hsm_file on partner onboarding)
    # was retired here. Nothing in this repository reads tbl_cert_file.hsm_file
    ...
    # ── A8 (architecture review Critical — the "gateway-only security"
    # prohibition promotes this from the original review's "Important") — application-
    ...
    # `client_max_body_size 50M` mitigates this at the edge, but relying on the
    # gateway alone is not enough ("weak inner layers become attack paths"), and the prod
    ...
    # window). `PartnerAgent.rate_limit_rps` existed long before this but
    ...
    # calls out as "the override hook". Same §16 promotion as A8.
    ...
    # ── T3/T4 (partner outbound boundary) ───────────────────────────────
    ...
    # ── T10 ("No secret-scrubbing pass on workspace
    ...
    # ── T6 ("No PII redaction before sending content to
    # external LLM providers") — PII MUST be minimized in downstream flows.
    # See core/pii_redaction.py.
    #
    ...
    #    an operator can validate against the golden/eval suite first, the
    #    same discipline established for prompt changes.
    pii_redaction_freetext_enabled: bool = True
    pii_redaction_docs_enabled: bool = False

    # ── T8 ("No comprehensive admin ACTION audit log") — every important
    # control decision must stay traceable.
    # See core/admin_action_audit_middleware.py.
    ...
    # ── T7 ("MFA not confirmed mandatory specifically for
    # admin-privileged sessions") — human administrative access MUST
    # require MFA. Enforced in
    ...
    # ── S4 follow-up ("Move secrets from
    ...
    # ADR-0002 Phase 2.
    # Never silently falls back to a different backend than requested: it
    # fails fast instead.
    secrets_provider_backend: str = "db_fernet"

    # ── S5 ("Wrap direct OS process
    ...
    # worker process. Operators run a SEPARATE worker per queue (which
    # implies two-worker docker-compose commands) — this setting only
    ...
    # start rejecting legitimate partner traffic — exactly the mistake
    ...
    # traffic — a phased cutover plan is what this flag gates.
    a2a_strict_envelope_validation_enforce: bool = False

    # ── A7 (architecture review Critical #13, "HMAC Fail-Open Toggle Exists")
    # — startup validation: applications MUST validate hostility-tier
    # configuration at startup and fail fast instead of starting
    # insecurely. An ACTIVE partner with no
    ...
    # `core/database.py` (the "hardcoded infrastructure values" anti-pattern)
    # and there was no `pool_recycle`/`pool_timeout`, so a MySQL/
    ...
    # P4 ("no hardcoded infrastructure values"): circuit breaker thresholds
    # and bulkhead limits are mandatory per-interface config.
    #
    ...
    # agent card (the SDK posts here verbatim, so a bare path won't work —
    # httpx requires the protocol).
    #
    # The default is the HOST-deployment answer, not the docker one. It used to
    # be `http://atom_backend:8000`, a container name that resolves only on a
    # compose network — so a host install that did not set this advertised an
    # address no partner could reach, and the failure reads as "the partner is
    # down" rather than "discovery is advertising a bad address". Compose sets
    # AUTHORITY_PUBLIC_URL explicitly, so docker is unaffected by the flip; the
    # mode that needs no configuration is now the one that cannot resolve a
    # container name.
    authority_public_url: str = "http://localhost:8000"
    ...
    def _normalize_app_env(cls, v):
        """Strip and lower-case `app_env` at the boundary, once.

        Roughly a dozen sites branch on `app_env == "production"` (and
        `!= "development"`) to decide whether to expose OpenAPI docs, emit HSTS,
        require `config_encryption_key`, enforce the `secret_key` floor, and
        block cleartext service URLs. Those were exact-match, while
        `startup_validation._is_production` already normalized — so `APP_ENV=
        "Production"` passed every startup validator and then silently took the
        NON-production branch in all of them. Normalizing the field itself keeps
        the two readings from ever diverging again.
        """
    ...
    def _check_internal_token_not_default(self) -> "Settings":
        ...
        cert-agent calls. It used to have a hardcoded,
    ...
    def _check_integration_testing_allowlist(self) -> "Settings":
        ...
        # The timeout budget must SHRINK INWARD (ingress > A2A send >
...
_SQL_IDENTIFIER = __import__("re").compile(r"^[A-Za-z_][A-Za-z0-9_]*$")


def require_external_table(value: str, setting: str, feature: str) -> str:
    """Return a table name that lives in a database this platform does not own.

    Two guarantees, both of which used to come free from a hardcoded literal
    and stop being free the moment the name is configurable:

    1. **Set.** An unset name previously fell back to the original ecosystem's
       table. Now it raises here, naming the setting and the feature, instead
       of producing a SQL error against whatever `FROM ''` parses as.
    2. **An identifier.** These names are interpolated into SQL — a table name
       cannot be a bind parameter — so a value from the environment reaches the
       query text directly. Anything that is not a bare identifier is refused
       rather than concatenated.
    """
    name = (value or "").strip()
    if not name:
        raise RuntimeError(
            f"{feature} is enabled but {setting} is not set. It names a table "
            f"in a database this platform does not own, so there is no default "
            f"that could be correct — set it to the table your deployment "
            f"actually reads."
        )
    if not _SQL_IDENTIFIER.match(name):
        raise RuntimeError(
            f"{setting}={name!r} is not a bare SQL identifier. It is "
            f"interpolated into a query, so it must match "
