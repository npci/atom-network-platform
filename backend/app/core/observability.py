...
bucket, provider, model, latency, prompt/response sizes, and
...
(slicing cost-per-purpose now that the router splits 7 workloads). A
follow-up wires a real backend SDK once operators pick one.
...
# ── Dedicated rotating file for LLM call traces ──────────────────────────────
#
...
def _setup_llm_call_logger() -> None:
    ...
    try:
        ...
        # Defence-in-depth: scrub secrets per record, exactly as core/diag.py
        # does for the identical handler shape.
        #
        # This was the one file sink without it, and `propagate = False` above
        # means these records never reach the root handlers that carry the
        # filter — so nothing downstream could catch them either. It matters
        # because `record_llm_call` writes `error_message=str(error)[:500]`:
        # raw provider-exception text, which routinely carries a
        # credential-bearing URL or the bearer token the gateway just rejected.
        # Those were scrubbed in app.jsonl and written verbatim here.
...
class LlmCallTrace:
    ...
    cache_read_tokens:  int  | None = None    # provider prompt-cache hit (Anthropic, Gemini)
    cache_write_tokens: int  | None = None    # provider prompt-cache write (Anthropic, Gemini)
...
def record_llm_call(trace: LlmCallTrace) -> None:
    ...
    When `settings.use_langfuse` is also True AND the
    ...
    try:
        ...
        # Also tee into the dedicated rotating file. Lazy
    ...
    # Langfuse forward (best-effort).
    # Gated by `langfuse_sample_rate`. Local INFO logs above
...
def _should_sample_for_langfuse(trace: "LlmCallTrace") -> bool:
    """Return True if this trace should be forwarded.
...
def record_llm_call_kwargs(
    *,
    ...
    job_id: str | None = None,
    # Size/usage/cost (all optional; legacy callers
    ...
    """
    # S6 ("validate outbound dependency responses before use... applies to
    # LLM providers"). Sanity-checks the
