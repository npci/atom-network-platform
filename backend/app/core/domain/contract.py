...
lifecycle for one ecosystem — OCPP, an aviation airworthiness network, an
internal API deprecation, a payments rail. The core supplies the machinery
(retrieval, agent loop, rendering, lifecycle); the pack
...
first domain that has no certifier.

────────────────────────────────────────────────────────────────────────────
CORRECTION to the original target-architecture sketch
...
__all__ = [
    ...
    "message_flows_of", "party_aliases_of", "primary_effect_operations_of",
    "high_assurance_participants_of", "pii_patterns_of",
    "domain_acronyms_of", "schema_namespaces_of",
    "certification_harness_of",
    "WireTemplates", "wire_templates_of",
...
class Participant(BaseModel):
    """An actor in the ecosystem. OCPP: CSMS vendor / charge-point operator /
    the standards body. A payments network: participant / issuer / authority.
...
class LabeledOption(BaseModel):
    ...
    a change operation (a library domain: reserve/issue/renew; a payments
    network: init/auth/debit/credit), a risk tier, a compliance tier. Distinct from
    `Participant` (an ecosystem ACTOR) and `ChangeType` (a kind of CHANGE): this
    is vocabulary for a PM-facing multiple-choice question, and absence is
    legitimate — a domain with no operation-level scoping declares none rather
    than reusing another domain's list.
    """
...
class FeatureBucket(BaseModel):
    ...
    vocabulary (a library network: loan lifecycle, reservations; a payments
    network: payment initiation, mandates) and therefore pack content, not
    engine content.
...
def cert_test_defaults_of(pack: DomainPack) -> Mapping[str, str]:
    """Fallback test data stamped onto a generated case that supplies none.

    These values CROSS THE WIRE — they are rendered into the request payload and
    sent to the counterparty's simulator, which resolves them against its own
    seeded directory. An address the simulator does not recognise turns a
    nominal happy-path case into an unknown-address failure, so they belong to
    the deployment (pack data), not to this repository.
    """
    fn = getattr(pack, "cert_test_defaults", None)
    return dict(fn()) if callable(fn) else {}


def wire_envelope_of(pack: DomainPack) -> Mapping[str, str]:
    """The identity stamped on a generated certification payload: the root
    element name, its namespace URI, and the originator id.

    THIS IS A WIRE CONTRACT, and it is pack DATA precisely because it is one.
    A partner or simulator validates the payload against a schema, so the root
    QName and namespace must be the ones THAT deployment's counterparty
    expects — not whatever this repository happens to ship. Hardcoding them
    would let a rename sweep silently change the contract, and every generated
    request would be rejected with no code path looking wrong.

    A certifying pack MUST declare `wire_envelope:` — the loader refuses one
    that does not, rather than substituting a placeholder that would look like
    it worked until the counterparty rejected the payload.
    """
    fn = getattr(pack, "wire_envelope", None)
    return dict(fn()) if callable(fn) else {}


class WireTemplates(BaseModel):
    """The domain's built-in request-template catalogue, keyed by flow code.

    Every entry is a message body of THAT ecosystem's message set — its element
    names, its parties, its credential blocks. That makes it pack content by the
    same argument as `wire_envelope_of`: a template shipped by this repository
    is one ecosystem's contract asserted as every deployment's, and a
    counterparty schema-validating the result rejects it.

    Both fields default EMPTY, and empty is a true statement rather than a
    broken pack: a domain that declares no catalogue simply has every flow fall
    through to the LLM template generator, which is the existing behaviour for
    any flow code the catalogue never covered.

      placeholders — the Mustache names a GENERATED template may use. Callers
                     fall back to a domain-neutral set when a pack declares
                     none, since every domain has a correlation id and a
                     timestamp but only some have an addressing handle.
      catalog      — flow code -> Mustache XML body.
    """
...
def wire_templates_of(pack: DomainPack) -> WireTemplates:
    """The domain's request-template catalogue; EMPTY when it declares none."""
    fn = getattr(pack, "wire_templates", None)
    value = fn() if callable(fn) else None
    if not value:
        return WireTemplates()
    if isinstance(value, WireTemplates):
        return value
    return WireTemplates(
        placeholders=[str(p) for p in (value.get("placeholders") or ())],
        catalog={str(k): str(v) for k, v in (value.get("catalog") or {}).items()},
    )


def cert_roles_of(pack: DomainPack) -> Mapping[str, str]:
    """The token naming each side of a certification exchange —
    ``{"authority": ..., "partner": ...}``.

    These cross the wire and key a datastore: the value lands in the
    `initiated_by` column, is the key of each side's counts in the push
    summary the SPA renders, and is compared against the counterparty's own
    test-case catalogue. They were repository constants until it became clear
    that made them reachable by a vocabulary sweep — which is exactly the kind
    of edit that cannot be caught by a compiler or a test in this repo.

    Required for a certifying pack; `{}` for one that does not certify.
    """
    fn = getattr(pack, "cert_roles", None)
    return dict(fn()) if callable(fn) else {}


def change_sections_of(pack: DomainPack) -> Mapping[str, str]:
    """Deliberately NOT a pack key.

    The generated document's `section_key` values look like a counterparty
    contract but are not one: nothing outside this repository reads them, they
    are absent from every model and migration, and all ten consumers
    (document_validator, the tier filter, the writer pipeline, the docx
    builder…) are in-repo Python. They are internal identifiers, so the
    project's own rule applies — rename them, don't pin them and don't make
    every deployment declare them. Kept as a stub so the reasoning survives the
    next time someone reaches for `change_sections:` in a pack.
    """
    raise NotImplementedError(
        "section keys are internal identifiers, not pack data — rename them "
        "in app/docgen/document_guides.py"
    )
...
def error_code_pattern_of(pack: DomainPack) -> "re.Pattern[str] | None":
    ...
    the failure this accessor exists to prevent: one ecosystem's code shape
    applied to another's spec either rejects every valid document, or — worse —
...
def error_class_tokens_of(pack: DomainPack) -> "frozenset[str]":
    """The tokens this ecosystem uses to classify an error code.

    The FIELD name is `td_bd` in every pack — a pinned wire/prompt key. The
    VALUES are domain-specific and must not be shared: the network pack
    declares TD (Technical Decline) / BD (Business Decline), while adcn and
    nlln declare SE (System Error) / PD (Policy Decline). A checker that
    hardcodes one domain's pair is wrong for the others in BOTH directions —
    it fails to enforce the tokens the domain actually requires, and demands
    tokens the domain forbids.

    Empty means the domain declares no classification, and a checker must then
    SKIP rather than fall back to another domain's alphabet — the same
    contract as `error_code_pattern_of`.

    Read off `prompt_blocks['proposals_error_class_rule']`, which every pack
    already ships in the form `td_bd = "XX" (Long Name) or "YY" (Long Name)`.
    Parsed rather than given a dedicated key so existing packs need no edit.
    """
    try:
        blocks = pack.prompt_blocks() or {}
        rule = blocks.get("proposals_error_class_rule") or ""
    except Exception:  # noqa: BLE001 - a pack that cannot describe itself gets silence
        return frozenset()
    # Quoted 2-4 char all-caps tokens only. Long names in parentheses are prose
    # and are deliberately not matched.
    return frozenset(re.findall(r'"([A-Z]{2,4})"', rule))
...
def message_name_pattern_of(pack: DomainPack) -> "re.Pattern[str] | None":
    ...
    (OCPP: `\\b[A-Z][A-Za-z]+(?:Request|Response)\\b`; the network pack:
    `\\b(?:Req|Resp)[A-Z]…\\b|\\bAck\\b`).
...
def message_flows_of(pack: DomainPack) -> Mapping[str, str]:
    """wire message name -> flow family code (OCPP: `StartTransaction ->
    SESSION`; the network pack: `ReqTransfer -> PAY`).
    Empty when the pack declares none — a message that maps to no flow is surfaced as
...
def party_aliases_of(pack: DomainPack) -> Mapping[str, str]:
    """loose party spelling -> canonical party key (OCPP: `CHARGEPOINT ->
    CHARGE_POINT_OPERATOR`; the network pack: `ISSUER -> REMITTER_BANK`).
    Empty when undeclared — an unknown spelling is dropped by
...
def primary_effect_operations_of(pack: DomainPack) -> "frozenset[str]":
    """Change-operation keys that carry the domain's PRIMARY EFFECT — the flows
    that do the consequential thing, as against the metadata/initiation/status
    flows around them. A charging network: energy delivery. A clinical-records
    domain: the write that amends a patient record. A payments network: the
    ones that move money.

    Empty when the pack declares none — every flow then classifies as "meta",
    which is a true statement about a domain with no such distinction.

    Accepts `financial_operations` as the DEPRECATED accessor name so a Python
    pack written before the rename still resolves.
    """
    fn = getattr(pack, "primary_effect_operations", None)
    if not callable(fn):
        fn = getattr(pack, "financial_operations", None)   # deprecated spelling
    return frozenset(str(v) for v in (fn() if callable(fn) else ()) or ())


def pii_patterns_of(pack: DomainPack) -> "dict[str, re.Pattern[str]]":
    """redaction kind -> compiled regex for a PII class this domain knows about.

    ADDITIVE to `core/pii_redaction.py`'s country-neutral built-ins, never a
    replacement for them: a pack that declares none (or one whose patterns do
    not compile) still gets full built-in filtering. `_compiled_pattern_map`
    logs and skips an individual bad pattern rather than raising, so one typo
    cannot take the whole redaction pass down with it.

    Declaration order is preserved, and it matters — these run BEFORE the broad
    built-ins so a domain-specific match is not first swallowed by the generic
    account-number rule.
    """
    return _compiled_pattern_map(pack, "pii_patterns")


def high_assurance_participants_of(pack: DomainPack) -> "frozenset[str]":
    """Participant keys whose channel must be mutually authenticated.

    Empty when the pack declares none, and empty is a real answer: it means no
    role by itself forces a partner onto the mTLS ingress. An operator can
    still put any individual partner there — this only expresses the domain's
    default, and a domain with no such default should not inherit another's.
    """
    fn = getattr(pack, "high_assurance_participants", None)
    return frozenset(str(v) for v in (fn() if callable(fn) else ()) or ())


def domain_acronyms_of(pack: DomainPack) -> Sequence[str]:
    """The domain's own short uppercase acronyms, whatever the pack declares
    (e.g. "IATA", "ICAO") — tokens an error-code-shaped scan must NOT mistake
    for codes. Empty when undeclared."""
...
class CertVocabulary(BaseModel):
    ...
    # Display label per role key (e.g. "ROLE_A" -> "Role A"). Consolidates what
    ...
    # role -> the test-data field names that role legitimately supplies, so the
    # initiating participant's payload cannot overwrite the receiving
    # participant's identifiers. The FIELD NAMES are ecosystem vocabulary —
    # each pack declares its own — which is why the map is pack content.
...
def change_operations_of(pack: DomainPack) -> Sequence[LabeledOption]:
    """The domain's change-scoped operations (a library domain:
    reserve/issue/renew/return; the network pack: init/auth/debit/credit/...).
    ...
    than inventing one ecosystem's list for another.
    """
...
def compliance_levels_of(pack: DomainPack) -> Sequence[LabeledOption]:
    """The domain's compliance-sensitivity vocabulary (an aviation network:
    regulator-mandated, safety-critical; the network pack: RBI-mandated,
...
def default_assumptions_of(pack: DomainPack) -> Mapping[str, str]:
    ...
    The values are DOMAIN FACTS ("30 seconds (standard network SLA)"), which is
