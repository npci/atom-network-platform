...
Selection: `DOMAIN_PACK`, a file path, defaulting to `packs/generic/generic.yaml`
...
DEFAULT_PACK = str(Path(__file__).resolve().parents[2] / "packs" / "generic" / "generic.yaml")
...
def pack_data_file(name: str) -> Path | None:
    """A data file shipped alongside the active pack's YAML, or None.

    Resolved as `<dir holding the pack YAML>/data/<name>`. Convention rather
    than declaration because `DOMAIN_PACK` is already a path — an out-of-tree
    pack (the ADCN fixture repo) gets this for free by dropping a `data/`
    directory next to its YAML, with nothing to register anywhere.

    Exists for seed CONTENT that a domain supplies and the platform must not.
    The policy brief the feasibility resolver reads used to ship from
    `backend/data/`, which meant every deployment of a domain-general platform
    first-booted with one payments network's rulebook — named banks, regulator
    circular numbers and all — installed as its default policy.

    `name` must be a bare filename. It is a module constant at every call site
    today, but the guard is cheap and this function's whole contract is "a file
    belonging to THIS pack" — a `name` carrying `..` would quietly make that
    false.
    """
    if not name or name != Path(name).name:
        return None
    try:
        candidate = Path(active_pack_key()).resolve().parent / "data" / name
