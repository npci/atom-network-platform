...
_SUPPORTED_PROVIDERS = frozenset({"anthropic", "openai", "ainxt", "ollama", "gemini"})
...
# ── Per-call usage capture via ContextVar ────────────────────────────────────
#
# Inner provider helpers (`_call_messages_api`, `_stream_messages_api`, etc.) set this
...
def normalize_provider(provider: str | None) -> str:
    ...
    canonical = _PROVIDER_ALIASES.get(p, p)
    if canonical not in _SUPPORTED_PROVIDERS:
        choices = ", ".join(sorted(_SUPPORTED_PROVIDERS))
        raise ValueError(f"Unknown LLM provider: {p!r}. Expected one of: {choices}.")
    return canonical


def get_provider() -> str:
    return normalize_provider(settings.llm_provider or "anthropic")
...
class UnsupportedProviderCapability(ValueError):
    """Raised when the active provider cannot serve a capability.

    Subclasses ValueError so callers that already fail open on a ValueError
    keep working. The message is written for an OPERATOR, not a developer: it
    names the capability, the configured provider, and what to change — the
    previous text told the reader to "route through call_llm", which is not
    something the person who set LLM_PROVIDER can act on.
    """
    ...
    def __init__(self, capability: str, provider: str):
        ...
        super().__init__(
            f"{capability} requires a provider that speaks the Anthropic "
            f"/v1/messages protocol: 'anthropic', or 'ainxt' with "
            f"AINXT_COMPAT_MODE=anthropic. LLM_PROVIDER is currently "
            f"'{provider}'. Basic completion, streaming and structured output "
            f"work on every supported provider; see the provider capability "
            f"matrix in CONFIGURATION.md."
        )


#
# Not every provider reaches every entry point. `call_llm`, `stream_llm` and
# `call_llm_structured` work everywhere; the native tool loop and vision speak
# the Anthropic /v1/messages protocol and are reachable only on providers that
# implement it. These predicates exist so a caller can FAIL FAST with an
# actionable message instead of doing expensive setup and then dying on the
# first turn — and so the docs can state the matrix rather than imply that
# "multi-provider" means "every feature, every provider".

def supports_native_tool_loop(provider: str | None = None) -> bool:
    """True when `provider` can drive `call_messages_api_tools`.

    The agentic runtime (plan/code/review) is built on the Anthropic tool-use
    protocol. `ainxt` qualifies only in its anthropic compat mode; in its
    default openai mode it does not.
    """
    prov = normalize_provider(provider or settings.llm_provider)
    if prov == "anthropic":
        return True
    if prov == "ainxt":
        return _ainxt_uses_anthropic()
    return False


def supports_vision(provider: str | None = None) -> bool:
    """True when `provider` can drive `call_llm_vision`.

    Same protocol constraint as the tool loop. OpenAI and Gemini both ship
    capable vision APIs; this platform does not yet call them, so an operator
    on those providers gets no image understanding and must be told so rather
    than silently receiving zero captions.
    """
    return supports_native_tool_loop(provider)


def provider_capabilities(provider: str | None = None) -> dict[str, bool]:
    """Capability map for the active (or named) provider, for docs and preflight."""
    prov = normalize_provider(provider or settings.llm_provider)
    return {
        "completion":       True,
        "streaming":        True,
        "structured_output": True,   # forced-tool on Anthropic, prose-JSON elsewhere
        "native_tool_loop": supports_native_tool_loop(prov),
        "vision":           supports_vision(prov),
    }


def reset_clients() -> None:
    """Drop every cached provider client so the next call rebuilds them.

    The cached async clients bind their httpx transports to the event loop
    they were first created on; reusing one inside a fresh loop raises
    `RuntimeError: unable to perform operation on <TCPTransport closed=True>`
    at cleanup. Callers that spin their own loop (docgen's `_run_sync`) must
    clear the caches first.

    Iterates every `_get_*_client` factory rather than naming them: the
    hand-written list this replaced covered 2 of the 5 and so only ever fixed
    the bug on Anthropic and OpenAI — the two providers where the failing path
    is hardest to reach.
    """
    for name in list(globals()):
        if not (name.startswith("_get_") and name.endswith("_client")):
            continue
        fn = globals().get(name)
...
def _clamp_max_tokens(max_tokens: int, model: str | None,
                      provider: str | None = None) -> int:
    """Clamp an agent's requested output budget to what `model` actually accepts.

    Agents request an ambitious budget (32k, 64k) written against a
    large-output model. Passing that straight through is a hard 400 on
    providers with a lower cap, which turns a portable agent into an
    Anthropic-only one. Clamping keeps the agent portable; the DEBUG line is
    there so a surprisingly short answer is explainable.
    """
    try:
        from app.core.tokens import model_max_output_tokens
        cap = model_max_output_tokens(model, provider)
    except Exception:                                   # pragma: no cover - defensive
        return max_tokens
    if max_tokens > cap:
        logger.debug(
            "max_tokens %d exceeds the cap for model %r (%d) — clamping.",
            max_tokens, model, cap,
        )
        return cap
    return max_tokens


# ── System-prompt coercion helper ────────────────────────────────────────────
#
...
#
# Callers should use `app.core.prompt_blocks.segments_for_prompt_cache` to
# build the list rather than hand-writing the dicts.

def _coerce_system_to_str(system) -> str:
    """Collapse a segmented system prompt into a single string for non-Anthropic-protocol
...
def _system_for_messages_api(system):
...
def get_model(provider: str | None = None) -> str:
    ...
    long-output agent direct to Anthropic while global is AiNxt). None falls
...
def _sdk_client_timeout():
    """Shared explicit timeout for the Anthropic/OpenAI SDK clients — see
    Settings.llm_client_*_timeout_s. Closes "No Explicit Timeout on
    Claude/OpenAI SDK Calls": both SDKs previously
...
async def call_llm(
    ...
    Optional `model` override lets per-agent routing send
    ...
    Optional `agent_name` tags the call for the observability
    ...
    (planner, writer, etc.) can pass provider="anthropic" to route direct
    ...
    max_tokens = _clamp_max_tokens(max_tokens, chosen_model, provider)

    logger.info("LLM call: provider=%s model=%s messages=%d", provider, chosen_model, len(messages))
    model = chosen_model

    # `system` may be a segmented list for Anthropic cache_control.
    # For observability / non-Anthropic-protocol providers / budget checks we need the
    # collapsed string form.
    system_str = _coerce_system_to_str(system)

    # Wrap the dispatch with timing + structured trace emit.
    ...
    # Pre-call context-budget assertion. Raises ContextOverflowError
    ...
    try:
        ...
        async with guarded_call(provider):
            if provider == "anthropic":
                response = await _call_messages_api(system, messages, model, max_tokens, agent_name=agent_name, provider=provider)
            elif provider == "gemini":
                # _call_gemini takes a flat str; pass system_str like the other
                # non-Anthropic-protocol providers (raw `system` may be a segmented list).
                response = await _call_gemini(system_str, messages, model, max_tokens, agent_name=agent_name)
            elif provider == "ainxt":
                if _ainxt_uses_anthropic():
                    response = await _call_messages_api(system, messages, model, max_tokens, agent_name=agent_name, client=_get_ainxt_anthropic_client(), provider=provider)
                else:
        ...
        # Usage was stashed by the inner provider helper
...
async def stream_llm(
    ...
    Optional `model` override; see `call_llm` docstring.
    Optional `agent_name` for observability trace tagging.
    Truncation-fix — Optional `provider` override; see `call_llm` docstring.
    `system` may be `str` or a list of Anthropic system segments
    with `cache_control` markers. Non-Anthropic-protocol providers see the collapsed
    string form.

    `temperature` — optional, Claude/ainxt-anthropic only (see `_stream_messages_api`).
    None leaves it unset, which is the API default (1.0) — unchanged behaviour
    for every caller that doesn't pass it. Non-Anthropic-protocol providers ignore it.
    """
    provider = normalize_provider(provider or get_provider())
    chosen_model = model or get_model(provider=provider)
    max_tokens = _clamp_max_tokens(max_tokens, chosen_model, provider)
    ...
    # See call_llm for rationale.
    system_str = _coerce_system_to_str(system)

    # Accumulate stream stats for the trace. Emit at end of
    ...
    # Pre-stream context-budget assertion. See call_llm above.
    try:
        from app.agents._context_packing import (assert_within_context, ContextOverflowError,
                                                 trim_messages_to_fit)
        if getattr(settings, "use_context_budget_check", True):
    ...
    try:
        ...
        async with guarded_call(provider):
            if provider == "anthropic":
                async for chunk in _stream_messages_api(system, messages, model, max_tokens, agent_name=agent_name, temperature=temperature, provider=provider):
                ...
                if _ainxt_uses_anthropic():
                    async for chunk in _stream_messages_api(system, messages, model, max_tokens, agent_name=agent_name, client=_get_ainxt_anthropic_client(), temperature=temperature, provider=provider):
                        response_chars += len(chunk or "")
                        response_chunks += 1
                        _resp_parts.append(chunk or "")
                        if _dl_buf is not None:
        ...
        log_cache_probe(system, agent_name)   # flag-gated cache-viability probe (log-only)
        # Read usage stashed by inner _stream_messages_api /
...
# Kept module-private dispatch helpers (_call_messages_api / _stream_messages_api /
...
async def _legacy_stream_dispatch_unused(system, messages, max_tokens, model):  # pragma: no cover
    ...
    if provider == "anthropic":
        async for chunk in _stream_messages_api(system, messages, model, max_tokens):
            yield chunk
    elif provider == "gemini":
        ...
        if _ainxt_uses_anthropic():
            async for chunk in _stream_messages_api(system, messages, model, max_tokens, client=_get_ainxt_anthropic_client()):
                yield chunk
        else:
...
def _warn_if_truncated(stop_reason: str | None, model: str, max_tokens: int,
    ...
    """Emit a WARNING log when the model reports stop_reason='max_tokens'.
    ...
    if stop_reason == "max_tokens":
        logger.warning(
            "TRUNCATED at max_tokens=%d — model=%s agent=%s output_chars=%d. "
...
def _is_gateway_error_text(text: str) -> bool:
    ...
    `delta.content="[LLM error: …]"` (F15/caveat 6). Accepting
...
def _truncated_by_budget(stop_reason: str | None, output_tokens: int | None, max_tokens: int) -> bool:
    ...
    native `max_tokens` alike (B8b/caveat 1) — so truncation is
...
async def _call_messages_api(system, messages: list[dict], model: str, max_tokens: int,
                       agent_name: str | None = None, client=None, provider: str | None = None) -> str:
    ...
    # The Anthropic SDK is the WIRE here, not necessarily the vendor: the AiNxt
    # gateway speaks the same /v1/messages protocol and arrives via the `client`
    # override above. Log the configured provider, not the protocol — these lines
    # used to hard-code the direct vendor unconditionally, so an AiNxt rate-limit read
    # as an Anthropic one in the operator's logs.
    provider = normalize_provider(provider or get_provider())
    corr_id = f"llm-{_uuid.uuid4().hex[:12]}"
    started = _time.perf_counter()

    # Segment list with cache_control markers, OR string (legacy).
    messages_api_system = _system_for_messages_api(system)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt >= max_retries:
                elapsed_ms = int((_time.perf_counter() - started) * 1000)
                logger.error(
                    "RATE_LIMIT_EXHAUSTED | corr_id=%s provider=%s model=%s "
                    "agent=%s elapsed_ms=%d attempts=%d exc=%s",
                    corr_id, provider, model, agent_name or "?",
            ...
            wait_s = min(server_wait or backoff, 60.0)
            logger.warning(
                "RATE_LIMIT_RETRY | corr_id=%s provider=%s model=%s agent=%s "
    ...
    if not text:
        ...
        raise RuntimeError(
            f"LLM returned no text content (provider={provider} model={model} "
    ...
    # Capture provider-reported usage for the trace.
...
# is a separate, Anthropic-protocol-only entry point (Claude direct, or AiNxt anthropic-compat).
#
# Prompt caching: the system prompt already routes cache_control segments
# (_system_for_messages_api). Here we ALSO cache the tools block — large and static
...
def _trim_system_cache_breakpoints(messages_api_system, keep: int):
    ...
    if not isinstance(messages_api_system, list) or keep < 0:
        return messages_api_system
    marked = [i for i, seg in enumerate(messages_api_system)
    ...
    return [({k: v for k, v in seg.items() if k != "cache_control"} if i in drop else seg)
            for i, seg in enumerate(messages_api_system)]


@lru_cache(maxsize=1)
def _messages_api_supports_thinking() -> bool:
...
def _count_system_cache_breakpoints(messages_api_system) -> int:
    """Number of cache_control markers already on the system prompt (segments)."""
    if isinstance(messages_api_system, list):
        return sum(1 for seg in messages_api_system if isinstance(seg, dict) and seg.get("cache_control"))
    return 0
...
async def _messages_api_create_with_retry(make_call, *, model, agent_name, corr_id, provider):
    """Run `make_call()` (an Anthropic messages.create coroutine factory) with the
    same full-jitter 429 backoff as `_call_messages_api`. Kept separate so the legacy
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt >= max_retries:
                # `provider` is the CONFIGURED provider, not the wire protocol — the
                # AiNxt gateway also reaches this loop over the Anthropic /v1/messages
                # shape, and used to be logged as the direct vendor.
                logger.error("TRANSIENT_EXHAUSTED | corr_id=%s provider=%s model=%s agent=%s attempts=%d err=%s",
                             corr_id, provider, model, agent_name or "?", attempt + 1, type(e).__name__)
                raise
...
async def call_llm_structured(
    ...
    *,
    ...
    On providers with native Anthropic tool support (anthropic / ainxt anthropic-compat) the
    ...
    if provider == "anthropic" or (provider == "ainxt" and _ainxt_uses_anthropic()):
        tool = {"name": tool_name, "description": tool_description, "input_schema": schema}
        turn = await call_messages_api_tools(
...
async def call_messages_api_tools(
    *,
    system,
    messages: list[dict],
    tools: list[dict],
    ...
    Requires provider='anthropic' or 'ainxt' with ainxt_compat_mode='anthropic'.
    ...
    if effective_provider == "ainxt" and _ainxt_uses_anthropic():
        ...
        chosen_model = model or get_model(provider="anthropic")
        client = _get_anthropic_client()
    else:
        raise UnsupportedProviderCapability("native tool loop", effective_provider)
    max_tokens = _clamp_max_tokens(max_tokens, chosen_model, effective_provider)
    messages_api_system = _system_for_messages_api(system)
    ...
    if cache_on and settings.agentic_cache_message_tail:
        messages_api_system = _trim_system_cache_breakpoints(messages_api_system, keep=2)
    ...
    use_tool_cache = cache_on and cache_tools and _count_system_cache_breakpoints(messages_api_system) < 4
    ...
    used_breakpoints = _count_system_cache_breakpoints(messages_api_system) + (1 if use_tool_cache else 0)
    ...
    create_kwargs: dict = dict(
        ...
        system=messages_api_system, messages=send_messages, tools=send_tools,
    )
    ...
    if thinking_budget and thinking_budget > 0 and _messages_api_supports_thinking():
    ...
    response = await _messages_api_create_with_retry(
        lambda: client.messages.create(**create_kwargs),
        model=chosen_model, agent_name=agent_name, corr_id=corr_id, provider=effective_provider,
    )
    ...
    if _truncated_by_budget(stop_reason, _out0, create_kwargs["max_tokens"]):
        logger.warning("call_messages_api_tools: output consumed the FULL budget (%s/%s) with stop_reason=%r "
    ...
    if stop_reason == "max_tokens":
        ...
        logger.warning("call_messages_api_tools truncated by max_tokens=%d model=%s tool_uses=%d",
    ...
    return MessagesToolTurn(text=text, tool_uses=tool_uses, stop_reason=stop_reason,
...
async def call_llm_vision(system: str, prompt: str, image_bytes: bytes,
    ...
    AiNxt forwards image blocks is UNVERIFIED; callers must be fail-open).
    Used by image_understanding to turn
    ...
    if effective_provider == "ainxt" and _ainxt_uses_anthropic():
        ...
        raise UnsupportedProviderCapability("vision", effective_provider)
    max_tokens = _clamp_max_tokens(max_tokens, chosen_model, effective_provider)
...
async def _stream_messages_api(system, messages: list[dict], model: str, max_tokens: int,
                         agent_name: str | None = None, client=None,
                         temperature: float | None = None, provider: str | None = None) -> AsyncGenerator[str, None]:
    import asyncio as _asyncio
    client = client or _get_anthropic_client()
    provider = normalize_provider(provider or get_provider())
    total_chars = 0
    # Pass segmented list verbatim when supplied so the SDK
    ...
    async with client.messages.stream(**stream_kwargs) as stream:
        ...
        while True:
            ...
            try:
                ...
                raise RuntimeError(
                    f"LLM stream stalled — no chunk for {_timeout}s (provider={provider} "
        ...
        try:
            ...
            # Capture usage + stop_reason from the final
            # assembled message.
            ...
            logger.debug("Anthropic-protocol stream final-message fetch failed (truncation check skipped): %s", e)
    ...
    if total_chars == 0:
        raise RuntimeError(
            f"LLM stream ended with no text (provider={provider} model={model} "
...
async def _do_call_openai_compat(client, system: str, messages: list[dict], model: str, max_tokens: int,
    ...
    # matching block in _call_messages_api for the rationale on dropping the
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # the matching block in _call_messages_api for the rationale.
            server_wait = _retry_after_seconds(e)
            backoff = _random.uniform(0, 2 ** attempt)
    ...
    if isinstance(response, str):
        ...
        # Stash stop_reason from the SSE-shim path. No usage
    ...
    if finish_reason == "length":
        ...
        # Same typed-at-the-text-level signal as the Anthropic-protocol path: partial output must
        ...
        text += TRUNCATION_MARKER
    # Capture provider-reported usage. OpenAI-compat returns
...
async def _stream_openai_compat(client, system: str, messages: list[dict], model: str, max_tokens: int,
    ...
    Behaviour:
      - Chunks are buffered in-memory until accumulated output exceeds
        ...
        invisible to the consumer for normal Anthropic-protocol streams (which produce
    ...
    total_chars = 0
    truncated_reason: str | None = None
    # Capture the provider-native stop reason (whichever
    ...
    # Usage from the final OpenAI / AiNxt chunk. OpenAI
    ...
    # Streaming guard (parity with _stream_messages_api — this IS the prod path when
            ...
            try:
                ...
                if obj.get("type") == "message_delta":
                    ...
                    if sr:
                        ...
                        if sr == "max_tokens":
                            truncated_reason = "max_tokens"
                # Usage in SSE-form (covers both OpenAI's
    ...
    # Stash captured usage + stop_reason on the contextvar
    # so the outer stream_llm finally block picks it up alongside the Anthropic-protocol path
