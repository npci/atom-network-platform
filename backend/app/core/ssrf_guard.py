...
def guard_outbound(url: str, *, context: str) -> Verdict:
    """`check_outbound_url` with every knob read from settings.

    The six-argument form is correct but long, and each call site repeating it
    is a chance to omit `block_on_resolution_failure` (which fails OPEN if left
    at a caller's ad-hoc default) or to forget the allowlist (which turns a
    legitimate internal host into a hard block). Several outbound calls simply
    skipped the guard entirely rather than write the boilerplate again.

    Import inside the function body, not at module scope: `app.core.config`
    imports this module transitively, so a top-level import is circular.
    """
