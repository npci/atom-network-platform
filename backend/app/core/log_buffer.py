...
_LOG_REDACTIONS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    # `x-api-key` / `api-key` / `x-internal-token` added: only `authorization`
    # and `private-token` were matched, so the platform's OWN internal-token
    # header and the api-key header partners present both logged in the clear.
    (re.compile(r"(?i)\b(private-token|authorization|x-api-key|api-key"
                r"|x-internal-token|x-auth-token)\s*[:=]\s*.+"), r"\1: [REDACTED]"),
    ...
    # ── JSON / dict form ────────────────────────────────────────────────────
    # The assignment pattern above anchors on `=`, so it only ever caught
    # `KEY=value` shapes. Structured logging and `repr()` of a request body
    # produce `{"password": "..."}` / `{'api_key': '...'}`, which passed through
    # untouched — and those are the commoner shapes here, since payloads reach
    # the log as dicts rather than env-style strings. Both quote styles, because
    # a Python dict repr uses single quotes.
    (re.compile(
        r"""(?i)(["']([A-Za-z0-9_\-]*(?:password|passwd|secret|token|api[_-]?key"""
        r"""|credential|signing[_-]?key))["']\s*:\s*)["'][^"']*["']"""),
     r"\1'[REDACTED]'"),

    # ── Cookies ─────────────────────────────────────────────────────────────
    # There was no cookie pattern at all, so `Set-Cookie: atom_session=<jwt>` —
    # or a logged `cookies={'atom_session': ...}` — wrote a live session
    # credential to disk verbatim. That is precisely the material an operator
    # would have to rotate after a log exposure.
    # Plural accepted: the header is `Set-Cookie`/`Cookie`, but httpx and
    # requests expose the same values as `cookies={...}`, and a kwarg repr is
    # how they most often reach a log line here.
    (re.compile(r"(?i)\b(set-cookies?|cookies?)\s*[:=]\s*.+"), r"\1: [REDACTED]"),

    # ── Bare high-entropy credentials (nothing to anchor a name on) ────────
    # Every pattern above needs a NAME beside the value. A JWT or vendor key
    # logged on its own — `logger.info("token %s", tok)`, or an exception
    # message quoting it — matched none of them. These match the value's own
    # shape; the prefixes are specific enough not to fire on ordinary prose.
    (re.compile(r"\beyJ[A-Za-z0-9_\-]{8,}\.[A-Za-z0-9_\-]{8,}\.[A-Za-z0-9_\-]+"),
     "[REDACTED-JWT]"),
    (re.compile(
        r"\b(?:sk-ant-[A-Za-z0-9_\-]{16,}|sk-proj-[A-Za-z0-9_\-]{16,}"
        r"|sk-[A-Za-z0-9]{20,}"
        # Google AI Studio key. The platform supports gemini, so this shape is
        # exactly as reachable as the others and was this table's one gap.
        r"|AIza[0-9A-Za-z_\-]{35}"
        r"|glpat-[A-Za-z0-9_\-]{16,}|gh[pousr]_[A-Za-z0-9]{20,}"
        r"|github_pat_[A-Za-z0-9_]{20,}|xox[baprs]-[A-Za-z0-9\-]{10,}"
        r"|AKIA[0-9A-Z]{16})"),
     "[REDACTED]"),

    # ── Key / certificate blocks ────────────────────────────────────────────
    # Migrated up from the second copy of this table in agents/agentic_events.py
    # (see `redact` there). That copy had these three and this one did not,
    # while this one had patterns that copy lacked — the "keep them in sync"
    # comment beside it had drifted in BOTH directions. There is now one table.
...
def _ensure_log_dir():
    # 0700, not the default 0755. The default LOG_DIR is under /tmp, which is
    # world-readable and shared with every other local user and container
    # process — and app.jsonl holds redacted-but-still-sensitive operational
    # detail, with redaction being defence-in-depth rather than a guarantee.
    # Ownership is the control that does not depend on a regex being complete.
    _LOG_DIR.mkdir(parents=True, exist_ok=True, mode=0o700)
    try:
        # mkdir's `mode` is ignored when the directory already exists, so tighten
        # an existing one too. Best-effort: a dir owned by another uid (a shared
        # bind mount) must not crash logging.
...
def install(level: int = logging.DEBUG) -> None:
    ...
    # (Also closes the "Filtering Sensitive Logs" static-analysis rule, which
