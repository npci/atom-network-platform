...
def offload_write(path: Path, record: dict[str, Any]) -> None:
    ...
    def _job() -> None:
        try:
            ...
            # FAIL CLOSED. This previously fell back to writing the RAW record
            # when redaction raised — but a record pathological enough to break
            # the redactor is exactly the one most worth scrubbing, and that
            # fallback wrote it to disk in full. A transcript is a debugging
            # convenience; losing one beats writing an unredacted credential.
            # Logged, so the failure is visible rather than silent.
            logger.warning("transcript redaction failed (%s) — record dropped: %s",
                           path, exc)
            return
        try:
