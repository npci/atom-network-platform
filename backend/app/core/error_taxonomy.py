...
Closes S2 ("948 `broad except Exception` sites: an error taxonomy with safe
propagation and telemetry"), and implements the review rules of a "clear
business vs technical error taxonomy" with "no swallowed exceptions":
applications MUST NOT swallow errors; errors MUST be categorized, mapped to
taxonomy, emitted with correlation IDs, safe for callers, free of sensitive
data leakage, and linked to telemetry and alerting where relevant.
...
resumption"). Rewriting all 948 in one pass would itself violate the
"smallest change that fully implements the intent" guidance and risks
turning deliberate resilience into accidental brittleness.

The remediation strategy this module enables is:
...
3. **The hygiene ratchet holds the line going forward**: CI enforcement
   fails a PR that INCREASES the
   bare-`except Exception`-with-no-taxonomy-reference count
...
class ErrorCategory(str, Enum):
    ...
    Mirrors the review's "clear business vs technical error taxonomy"
    and the `error_taxonomy` boundary-contract block of categorized errors
    (business_errors / technical_errors / resource_access_errors /
    security_errors)."""
    ...
    # from BUSINESS/TECHNICAL per P10 ("separate resource-access error
    # category... structured exception details for
    ...
    # structured security telemetry — never merely logged at INFO/DEBUG
    # and never silently swallowed, even in a "best-effort" code path.
...
# authored messages like "Select at least one partner to ship to",
# and English sentences such as "cannot delete from an
...
def client_safe_detail(exc: BaseException, *, fallback: str | None = None) -> str:
    ...
    Static analysis for "Information Exposure Through an Error Message"
    (CWE-209) fires on `HTTPException(500, detail=f"...: {exc}")`. Most of those
    results are false positives — the detail goes to `logger.*`, which is what
    the rule itself recommends. A handful were genuine: a broad `except Exception`
...
def client_safe_message(text: str | None, *, fallback: str = "an internal processing error occurred") -> str:
    ...
    failed", "partner has not declared this case ready") intact while replacing
...
def safe_call(
    ...
    **kwargs: Any,
    ...
    - `reraise=True` re-raises AFTER logging — use this for TECHNICAL/
      ...
      re-raised exception has a searchable trail (errors stay "safe for
      callers... linked to telemetry").
    - Every invocation logs one structured line
      ...
      time — the auditability P9 and the security review require.
    ...
    logging anti-pattern the reviews flag.
    """
    try:
