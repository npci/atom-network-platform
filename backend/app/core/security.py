...
def create_access_token(subject: Any, *, amr: str = "pwd") -> str:
    ...
    Closes T7 ("MFA not confirmed mandatory specifically
    for admin-privileged sessions") — `require_admin` (core/deps.py) reads
    this claim to enforce the rule that "Human
...
# A2A security hardening: short-TTL access tokens + long-TTL
...
A2A_ACCESS_TOKEN_TTL_S  = 15 * 60      # 15 minutes (reduced from 1 hour)
A2A_REFRESH_TOKEN_TTL_S = 24 * 60 * 60 # 24 hours


def create_partner_token(partner_id: str) -> str:
    """Create a short-lived A2A access JWT (15 min).
...
def create_partner_refresh_token(partner_id: str) -> str:
    ...
    Part of A2A security hardening. Used by /a2a/auth/refresh to
...
# ── Outbound JWT minted with per-partner signing secret ──────────────────────
