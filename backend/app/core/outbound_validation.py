...
Closes S6 ("Validate outbound dependency
...
implements the rule that "Every outbound dependency response MUST be
validated before use", along with the anti-corruption adapter's
responsibilities: "validate inbound response shape... normalize data...
reject malformed or unexpected responses".
...
def validate_partner_response(response: dict | None, *, expected_task_type: str | None = None) -> ValidationResult:
    ...
    the same rigour is required for
    responses as for requests — "every dependency output may be malformed,
    corrupted, delayed, replayed, or hostile" applies to a partner's
