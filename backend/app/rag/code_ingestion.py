...
# Was 16. With EMBED_BATCH_SIZE=32 and EMBED_HTTP_CONCURRENCY=8,
...
# Polyglot extension → language map. Used by the polyglot
...
LANGUAGE_EXTENSIONS: dict[str, str] = {
    ".java": "java",
    ".py":   "python",
    # TypeScript / JSX / JavaScript share the same grammar via
...
# ── Commit-SHA tracking + git-diff fetch ─────────────────────────────────────
...
def _fetch_changed_paths_via_compare(
    ...
    """Use the GitLab Compare API to learn which paths changed
...
# ── 3-view multiview expansion ───────────────────────────────────────────────

def _attach_java_symbol_graph(file_chunks: list[dict], content: str) -> None:
    """Enrich Java chunks in-place with symbol-graph edges.
    ...
    symbol-graph extractors land separately).
    """
    ...
    for chunk in file_chunks:
        ...
        # via chunk["class_name"] (set by the tree-sitter chunker).
...
# ── Python symbol-graph attach ───────────────────────────────────────────────

def _attach_python_symbol_graph(file_chunks: list[dict], content: str) -> None:
    """Enrich Python chunks in-place with symbol-graph edges.
...
def _attach_typescript_symbol_graph(
    ...
    """Enrich TS/JS chunks in-place with symbol-graph edges.
...
def _attach_symbol_graph_for_language(
    ...
    *, file_path: str | None = None,
    ...
    """Dispatch on `language` so the right extractor runs.
    ...
    # else: no-op (other languages → punt to the LSP resolvers)
...
def ingest_codebase(
    ...
    for file in java_files:
        ...
        # Attach symbol-graph edges when flag on. Runs per-file so
    ...
    # Optional 3-view expansion (body / signature / nl_summary).
    ...
    for batch_start in range(0, len(all_chunks), BATCH_SIZE):
        ...
        # Consult the content-hash embedding cache before
        ...
        for chunk, embedding in zip(batch, embeddings):
            record = DocumentChunk(
                ...
                # Tree-sitter fields (absent on regex chunker output)
                symbol_kind=chunk.get("symbol_kind"),
                ...
                # Multiview fields (absent when flag off)
                view_kind=chunk.get("view_kind"),
                parent_symbol_id=chunk.get("parent_symbol_id"),
                # Symbol-graph edges (absent when flag off)
...
def ingest_polyglot_repo(
    ...
    *,
    ...
    filter (deliberate — keeps Phase A retrievals unchanged).
    ...
    # Python LSP cross-file resolution. Runs AFTER per-file symbol
    ...
    # TypeScript / JavaScript LSP cross-file resolution. Same
    ...
    # Java LSP cross-file resolution. eclipse-jdt is JVM-
    ...
    for batch_start in range(0, len(all_chunks), BATCH_SIZE):
        ...
        for chunk, embedding in zip(batch, embeddings):
            ...
            record = DocumentChunk(
                ...
                calls=chunk.get("calls"),
                called_by=chunk.get("called_by"),
                # Cross-file calls from Python LSP (only set when
...
def _persist_file_state(
    ...
    """Upsert one CodeRepoFileState row per file using a
...
# ── Incremental ingest entry point ────────────────────────────────────────────
...
def ingest_polyglot_repo_incremental(
    ...
    *,
    ...
    # 1. Fetch.
    #
    # Fast path via GitLab Compare API:
    ...
    if not delta_files and not deleted:
        # Idempotent re-run — nothing changed. Still record the
    ...
    # Record the post-ingest HEAD SHA so the next run can use
