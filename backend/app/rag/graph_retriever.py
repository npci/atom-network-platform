...
"""Graph-traversal retrieval pass.
...
`graph_retrieve(query)`:
  ...
  2. For each seed, runs 3 Cypher queries against the AGE graph named by
     `settings.kg_graph_name`:
...
No production caller yet. Consumers will wrap this as an extra
pass inside `retrieval._multi_pass_retrieve` in a follow-up change.
"""
...
def _domain_acronym_stopwords() -> frozenset[str]:
    """Domain acronyms from the ACTIVE pack, not hardcoded (one domain's
    own acronyms used to sit in the list above): the pack key plus every
    all-uppercase word in a participant label ("ACME Partner" → "acme"). Fail-open — this module never
...
# another uppercase boundary later. Accepts "Foo", "FooBar", "XMLService".
...
def _extract_query_seeds(query: str, *, max_seeds: int | None = None) -> list[str]:
    ...
    Returns an ordered, deduplicated list. Seeds come from three patterns:
      ...
      - CamelCase words (Foo, FooBar, XMLService)
...
def _graph_retrieve_via_sql(
    ...
    *,
    ...
    """Pure-SQL alternative to the AGE Cypher traversal.
...
def graph_retrieve(
    ...
    *,
    ...
    When `settings.graph_backend == "sql"`, dispatches to the
