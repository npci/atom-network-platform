...
"""Synthetic natural-language code summarizer.
...
Model selection (3-layer override, most-specific first):
  ...
  2. ROUTING_MODEL_UTILITY env var (via the model router) — applies to
     utility-purpose agents in general
  3. the active provider's own model setting (CLAUDE_MODEL, OPENAI_MODEL,
     GEMINI_MODEL, AINXT_MODEL, OLLAMA_CHAT_MODEL) — provider default
...
# When set, wins over model routing for this one agent. Useful when
...
async def synthesize(code: str, language: str, symbol_kind: str) -> str:
    ...
    try:
        ...
        # Per-agent override (env CODE_SUMMARIZER_MODEL) wins over model
