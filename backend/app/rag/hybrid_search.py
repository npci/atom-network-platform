...
def _dense_search(
    ...
    # HNSW `ef_search` is per-session in pgvector. Set it before
...
def _dedup_by_parent_symbol(chunks: list[dict]) -> list[dict]:
    """After RRF fusion, collapse multiple views of the same symbol
    (body / signature / nl_summary) to a single representative row — the one
    with the highest fused score. Rows with no `parent_symbol_id` (pre-multiview
...
def hybrid_retrieve(
    ...
    *,
    ...
    # 5. Sort by RRF score, over-sample, post-filter, dedup, truncate.
    ...
    # Collapse multiple views of the same symbol to one best-scored row.
