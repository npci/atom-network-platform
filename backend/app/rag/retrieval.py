...
runs a multi-pass enriched retrieval: original query + HyDE
...
def retrieve(
    ...
    """
    # When the reranker is on, over-sample the retrieval pool so the
    ...
    # When the graph-retrieval flag is on, run graph_retrieve
    ...
    # When MMR is on, ask the reranker / slicer for a wider
    ...
    # Augment the final top-K with 1-hop graph neighbours.
...
def _multi_pass_retrieve(
    ...
    When `settings.use_parallel_multi_pass` is on, the per-variant
...
def _run_variants_parallel(
    ...
    try:
        ...
        logger.warning(
            "Parallel retrieval: SessionLocal unavailable (%s), falling back to serial loop", e,
        )
...
def _rrf_fuse_passes(rankings: list[list[dict]], top_k: int) -> list[dict]:
    ...
    Preserves the parent_symbol_id dedup so we don't return multiple
...
def _expand_top_k_neighbors(
    ...
    *,
    ...
    """Fetch 1-hop CALLS/INHERITS/IMPLEMENTS neighbours of the
    ...
    the token-budget code can drop them first on overflow.
...
def _diversify_with_mmr(
    ...
    *,
    ...
    """Apply MMR to the (already reranked) candidate set.
...
def format_sources(chunks: list[dict]) -> tuple[str, list[dict]]:
    """Produce a numbered-source context + a parallel source table.
    ...
    for i, chunk in enumerate(chunks, start=1):
        ...
        sources.append({
            ...
            "title_breadcrumb": chunk.get("title_breadcrumb"),  # may be None
        })
    context_text = "\n---\n".join(parts)
...
def build_context(
    ...
    *,
    ...
    Accounting uses `app.core.tokens.count_tokens` against
    ...
    When `settings.use_context_compression` is True AND a `query`
    ...
    """
    # Optional compression before budgeting.
    ...
    # Resolve target model lazily so import-time cycles are
