...
# Dynamic concurrency default. On a 16-core box this
...
# ── Observable zero-vector failures ──────────────────────────────────────────
...
def embed_texts(texts: list[str]) -> list[list[float]]:
    ...
    for count in per_input_window_counts:
        if count == 0:
            # Empty/whitespace inputs surface here. Track
...
def embed_query(text: str) -> list[float]:
    ...
    Bounded LRU cache. Cache hits skip the Ollama HTTP round-
    trip; cache disabled when `settings.query_embedding_cache_size <= 0`.

    Retry parity with `embed_texts`. Earlier versions of
