...
"""Per-chunk LLM context compressor.
...
Notes for follow-up:
  ...
  - Sentence splitting is naive (regex on `. ` followed by uppercase). Real
    NLP tokenization (spaCy / nltk) is overkill here; we revisit if
...
# abbreviations ("Dr.", "etc.") occasionally — acceptable here.
...
async def _compress_one(query: str, content: str) -> str:
    ...
    try:
        # Lazy imports so this module is importable without a provider SDK at rest.
        from app.core.llm import call_llm
        from app.core.llm_router import pick_model_for_agent
        from app.core.json_recovery import parse_llm_json

        # context_compressor is Purpose.UTILITY (per-chunk
...
def compress_chunks_sync(
    ...
    *,
    ...
    Three modes:
      ...
      - "llm"      → original path (per-chunk LLM filter).
    ...
    # mode == "llm" (or anything unrecognised) → legacy LLM path.
    try:
...
# ── Deterministic line-overlap compression ───────────────────────────────────
...
def compress_chunks_overlap(query: str, chunks: list[dict]) -> list[dict]:
    """Pure-Python compression. Same return contract as
