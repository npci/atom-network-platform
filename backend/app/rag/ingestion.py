...
FOLDER_TO_CATEGORY: dict[str, DocCategory] = {
    ...
    "network_product_docs":  DocCategory.NETWORK_PRODUCT_DOC,
    ...
    # Authority-specific DocCategory constants added in
    ...
    # Distilled Authority design guidance. Consulted broadly by Deep Researcher,
    # and scoped explicitly by the Tech-Spec agent + Excel testcase engine.
    "api_design_knowledge": DocCategory.API_DESIGN_KNOWLEDGE,
}

# Superseded knowledge_base/ subfolder names, still present on operators' disks.
#
# A folder name keys a datastore this platform does not own — it lives in a bind
# mount, not in the repo — so it falls under "pin values", not "rename
# identifiers". Renaming the key alone made an existing `upi_product_docs/`
# invisible to ingestion at the same moment migration 0139 rewrote its rows to
# the NEW category value. That combination is what armed the orphan sweep below
# to delete the entire corpus: the rows became known-to-the-sweep and
# absent-from-disk in one step.
#
# Accepting the old name keeps that corpus ingesting until the operator renames
# the directory, which the warning below asks them to do.
LEGACY_FOLDER_ALIASES: dict[str, str] = {
    "upi_product_docs": "network_product_docs",
}
...
def ingest_all(db: Session, force: bool = False) -> dict:
    ...
    scan_folders: list[tuple[str, DocCategory]] = list(FOLDER_TO_CATEGORY.items())
    for legacy, canonical in LEGACY_FOLDER_ALIASES.items():
        if (kb_dir / legacy).is_dir():
            logger.warning(
                "knowledge_base/%s/ uses a superseded folder name; ingesting it as "
                "%s. Rename the directory to knowledge_base/%s/ — support for the "
                "old name is a migration aid, not a permanent alias.",
                legacy, FOLDER_TO_CATEGORY[canonical], canonical,
            )
            scan_folders.append((legacy, FOLDER_TO_CATEGORY[canonical]))

    for folder, category in scan_folders:
    ...
    for folder, category, file_path, is_update in candidates:
        ...
        # Check BEFORE deleting. This delete replaces a file's chunks with the
        # freshly-computed ones, so with nothing to put back it is not a
        # replacement — it is a deletion. A file that becomes unchunkable
        # (truncated, corrupted, or a parser regression) would otherwise lose
        # the chunks it already had, reported as a bare `errors` counter with
        # the document silently gone from retrieval.
        if not chunk_rows:
            summary["errors"] += 1
            continue
    ...
    # Sweep ONLY categories whose folder is actually on disk. A missing folder is
    # not evidence that its documents were deleted — an unmounted volume or a
    # directory renamed out from under us looks identical — and treating it as
    # "every file removed" turns a mount failure into silent, committed data
    # loss. This is the generalised guard for the `upi_product_docs` trap above:
    # whatever makes a folder disappear, the rows outlive it.
    present_categories = {
        category for folder, category in scan_folders if (kb_dir / folder).is_dir()
    }
    unswept = set(FOLDER_TO_CATEGORY.values()) - present_categories
    if unswept:
        logger.info(
            "orphan sweep skipping %d category(ies) with no folder on disk: %s",
            len(unswept), sorted(unswept),
        )
    known_categories = present_categories
    db_paths = {
        row[0]
        for row in (
