...
Two backends, selected via `settings.bm25_backend`:
  ...
  - "tsvector" (default): runs a single Postgres query using
...
# ── Postgres tsvector backend (default) ───────────────────────────────────────
...
def _tsv_config_name() -> str:
    """Return the text-search config to use for tsquery.
...
def _search_tsvector(db: Session, query: str, top_k: int) -> list[tuple[str, float]]:
    ...
    used depends on `settings.bm25_text_search_config`:

      - 'english' (default) — generated-column-compatible, prose-friendly.
      - 'simple_code' — preserves code identifiers verbatim
