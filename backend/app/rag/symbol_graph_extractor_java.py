...
"""Java symbol-graph extractor.

Parses a Java source file using the tree-sitter Java grammar (already
installed by the chunker) and returns a structured view of:
...
no type resolution). That's enough for this module's scope — building the
symbol graph for RAG retrieval boosting. A future revision can swap in real
