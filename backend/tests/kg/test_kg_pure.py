...
"""Pure unit tests for the KG client + schema constants.
...
class TestBuildCypherSql:
    ...
    def test_default_return_col_produces_result_agtype(self):
        sql = build_cypher_sql("MATCH (n) RETURN n", graph_name="test_kg")
        assert "SELECT * FROM cypher('test_kg'" in sql
    ...
    def test_string_return_cols_default_to_agtype(self):
        sql = build_cypher_sql(
            "RETURN 1", graph_name="test_kg", return_cols=["v"],
        )
        assert "AS (v agtype)" in sql

    def test_empty_cypher_rejected(self):
        with pytest.raises(ValueError, match="non-empty"):
            build_cypher_sql("", graph_name="test_kg")
        with pytest.raises(ValueError, match="non-empty"):
            build_cypher_sql("   ", graph_name="test_kg")
    ...
    def test_dollar_quote_in_cypher_rejected(self):
        ...
        with pytest.raises(ValueError, match="\\$\\$"):
            build_cypher_sql("MATCH (n) $$ bad", graph_name="test_kg")
    ...
    def test_body_preserves_newlines_and_spacing(self):
        ...
        sql = build_cypher_sql(body, graph_name="test_kg")
        assert body in sql
...
def test_kg_graph_name_configured():
    """`settings.kg_graph_name` carries the expected default.

    Changing this default is an upgrade hazard: an AGE graph cannot be renamed
    in place, and a graph name that does not exist yet is CREATED rather than
    refused — so a deployment that ingested under a different name and then
    takes this default gets a new, EMPTY graph and a retriever that returns
    nothing while looking healthy. The assertion is kept rather than deleted so
    the default stays a stated fact; the upgrade note lives on the setting.

    Asserts the SHIPPED default, so it builds a fresh Settings with the env
    override cleared. Reading the process-wide `settings` singleton instead
    would make this test fail on any machine whose `.env` pins the graph — and
    pinning it is exactly what an existing deployment is told to do, so the
    test would have failed precisely where the setting was configured
    correctly.
    """
    import os
    ...
    with mock.patch.dict(os.environ, {"KG_GRAPH_NAME": ""}, clear=False):
        os.environ.pop("KG_GRAPH_NAME", None)
        fresh = Settings(_env_file=None)
    assert fresh.kg_graph_name == "atom_kg"
...
class TestIsAgeAvailableRollsBack:
    ...
    def test_probe_failure_rolls_back_and_returns_false(self, monkeypatch):
        ...
        # AT LEAST once — the invariant is "no poisoned transaction escapes",
        # not an exact count. `_warn_if_graph_missing` then probes
        # `ag_catalog.ag_graph` to tell "AGE is absent" apart from "the
        # configured graph is absent", and that probe rolls back too when
        # ag_catalog is unreadable — precisely the case this test simulates.
