...
ADMIN = SimpleNamespace(id="admin-1", email="admin@example")
...
def _file(data: bytes, name="skill.md"):
    """Stand-in for Starlette's UploadFile.

    `read` takes an optional size, mirroring the real signature. This stub took
    none, so it only ever supported `await file.read()` — and the handlers now
    call `read(MAX + 1)` so an oversized upload is never buffered in full before
    the ceiling is checked. Honouring `size` also lets a test exercise that bound.
    """
...
def test_bundle_upload_roles_and_lifecycle(db, monkeypatch):
    ...
    infosec_user = SimpleNamespace(id="u-is", email="is@example", role=UserRole.INFOSEC_REVIEWER)
    tech_lead = SimpleNamespace(id="u-tl", email="tl@example", role=UserRole.TECH_LEAD)
    admin = SimpleNamespace(id="u-adm", email="a@example", role=UserRole.ADMIN)
...
def test_bundle_contract_from_skill_md_no_manifest_field(db, monkeypatch):
    ...
    from app.models.user import UserRole
    admin = _NS(id="adm-role", email="a@example", role=UserRole.ADMIN)
