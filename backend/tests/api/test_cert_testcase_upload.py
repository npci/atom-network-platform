...
class TestValidateXlsxBytes:
    def test_accepts_authority_shaped_xlsx_with_zero_warnings(self):
    ...
    def test_warns_on_non_authority_shape_but_does_not_reject(self):
        """A user uploading a workbook whose sheets don't look Authority-shaped
...
class TestDownloadAccessControl:
    ...
    pack can carry sensitive network flow detail, so any authenticated user must
