...
ADMIN = SimpleNamespace(email="admin@example", id="admin-id")
...
class TestEvalRegexParity:
    ...
    `message_name_pattern`; the shape assertions below pin the default network pack."""

    @staticmethod
    def _network_pattern(monkeypatch):
        import pathlib
    ...
    def test_ack_matched(self, monkeypatch):
        pat = self._network_pattern(monkeypatch)
