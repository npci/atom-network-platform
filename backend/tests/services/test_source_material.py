...
def test_block_is_wrapped_untrusted_and_named():
    out = source_block(_cr(text="Limit is 5000 units per request.", name="sample_brd.docx"))
    assert "SOURCE_DOCUMENT" in out and "untrusted" in out          # injection defense wrapper
    assert "sample_brd.docx" in out
    assert "Limit is 5000 units per request." in out
