...
def test_party_gap_map_is_complete():
    """Every pack-scoped party key must have a gap-key mapping (network pack: 4)."""
...
def test_operation_gap_map_is_complete():
    """Every pack change-operation must have a gap-key mapping (network pack: 6)."""
...
#
# The v2 fixtures below use the network pack's per-party/per-operation gap keys, which are
# pack data — pin the default network pack for them rather than assume it.

import pytest as _pytest
from pathlib import Path as _Path

_NETWORK_PACK = str(_Path(__file__).resolve().parents[2] / "app" / "packs" / "network" / "network.yaml")
