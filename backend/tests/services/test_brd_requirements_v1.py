...
The tagging regexes and party vocabulary under test are network pack data
(`operation_patterns`/`party_patterns`/`financial_operations`), so the module
pins DOMAIN_PACK to the default network pack rather than assuming it. (The criteria-
coercion tests additionally depend on brd_extractor's import-time vocabulary,
which only matches when the whole process runs under the default network pack.)
"""
