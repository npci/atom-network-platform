...
CERT_TASK_TYPES = {
    ...
    # Real A2ATaskType members (protocol.py:113 and :154) emitted by
    # cert_pack_run when a round starts. They were missing here, so genuine
    # certification sends read as change-communication offenders.
    "CERT_SETUP_NOTIFICATION", "CERT_EXECUTION_START",
}
...
ALLOWED_DIRECT = {
    ...
    # The integration-testing tunnel carries an HTTP exchange for a cert round,
    # not a change communication. It passes A2ATaskType.HTTP_EXCHANGE_REQUEST
    # positionally, so the task_type regex below reads "?" and it can only be
    # cleared by path.
    "services/integration_testing/ingress.py",
}
...
def _direct_sends() -> list[tuple[str, int, str]]:
    ...
    for path in APP.rglob("*.py"):
        ...
        for i, ln in enumerate(lines):
            # Skip comments and docstring prose. Two files DISCUSS
            # `send_task_to_partner(` in explanatory comments; matching those
            # made the guard report a migration offence for a line that emits
            # nothing. A guard that cries wolf about prose gets muted.
            stripped = ln.lstrip()
