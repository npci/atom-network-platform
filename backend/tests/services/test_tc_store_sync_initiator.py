...
"""Direction (initiated_by + psp_as) and per-TC test_data are
...
The prefixes/flows under test (PR_/MT_, ReqTransfer→PAY) are network pack data
(`cert_vocabulary.role_prefixes`, `authority_case_prefix`, `message_flows`),
so the module pins DOMAIN_PACK to the default network pack rather than assuming it.
"""
...
_NETWORK_PACK = str(Path(__file__).resolve().parents[2] / "app" / "packs" / "network" / "network.yaml")


@pytest.fixture(autouse=True)
def _pin_network_pack(monkeypatch):
    monkeypatch.setenv("DOMAIN_PACK", _NETWORK_PACK)
...
class TestResolveInitiatedBy:
    ...
    def test_explicit_authority_uppercased(self):
...
class TestResolvePspAs:
    ...
    def test_normalization(self, raw, expected):
        assert _resolve_role_as({"psp_as": raw}) == expected
...
class TestResolveTestData:
    def test_defaults_when_no_overrides(self):
        ...
        assert td["payer_vpa"] == "test@example"
    ...
    def test_stub_overrides_win_over_handle_defaults(self):
        td = _resolve_test_data({
            "payer_handle": "VPA",
            "payee_handle": "VPA",
            "test_data": {"amount": "742.50", "payer_vpa": "alice@example"},
        })
        assert td["amount"]    == "742.50"
        assert td["payer_vpa"] == "alice@example"
        # untouched handle defaults remain
        assert td["payee_vpa"] == "merchant@example"
        assert td["currency"]  == "INR"
    ...
    def test_handle_specific_extras_still_apply(self):
        # An account-addressed handle still pulls in the pack's account
        # extras. The IFSC value is pack data now (a documentation-range
        # placeholder), not a literal in tc_store_sync — it used to be a real
        # institution's prefix hardcoded in the service.
        td = _resolve_test_data({"payer_handle": "A/c+IFSC"})
        assert td["ifsc"] == "EXMP0000001"
