...
This file originally tested both the legacy POST wire and the SDK wire.
The legacy path has since been deleted, so it now covers the
SDK-only behaviour:
...
def _make_partner(db_session, *, is_cert_engine: bool = False, with_endpoint: bool = True):
    ...
    protocol_version is no longer a behavioural switch (always
...
def test_sdk_send_succeeds(db_session):
    ...
    # is persisted onto response_body (a JSON column). A bare AsyncMock()
