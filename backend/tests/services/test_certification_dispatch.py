...
# A pack that declares `certification_harness:` must also declare its
# certification wire identity — the loader refuses one that doesn't, so a
# deployment discovers a missing counterparty value at boot rather than midway
# through a certification run (see `config_pack._cert_mapping`). These tests
# are about harness SELECTION, so they carry the minimum that makes a
# certifying pack loadable; none of these values is sent anywhere here.
_CERT_IDENTITY = (
    "cert_roles: {authority: AUTH, partner: PARTNER}\n"
...
def test_the_network_pack_now_declares_certification(monkeypatch):
