...
def _p(ssl_verify=None, ca_cert_pem=None):
    # `name` is nullable=False on PartnerAgent, so a real row always carries it.
    # a2a_client logs `partner.id, partner.name` on the ssl_verify=off paths;
    # a stub without it raised AttributeError before the assertions ran.
    return SimpleNamespace(id="p1", name="p1", ssl_verify=ssl_verify,
...
def test_global_ca_bundle_used_when_no_partner_cert(monkeypatch):
    monkeypatch.setattr(settings, "partner_tls_verify", True)
    monkeypatch.setattr(settings, "partner_ca_bundle", "/etc/ssl/partner-ca.pem")
    assert partner_verify(_p()) == "/etc/ssl/partner-ca.pem"
