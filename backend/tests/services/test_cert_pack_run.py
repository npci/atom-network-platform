...
def test_a_clean_round_certifies_and_records_everything(db_session):
    ...
    assert (run.pack_ref, run.authority_mode, run.partner_mode) == \
    ...
    for row in db_session.query(CertTestResult).all():
        ...
        assert (row.authority_mode, row.partner_mode) == ("simulator", "simulator")
...
def _enable_tunnel(db, monkeypatch, *, onboard=True, sent=None,
    ...
    if onboard:
        db.add(PartnerAgent(id=PARTNER, name="Partner One"))
        db.commit()

    async def _send(**kw):
        if sent is not None:
            sent.append(kw)
        # `status` is REQUIRED to model the real contract: send_task_to_partner
        # ALWAYS returns the persisted A2AMessage row and signals failure by
        # status ('delivered' | 'delivery_failed' | 'sent'), never by raising
        # (a2a_client.py:427). This stub omitted it while nothing read it;
        # `_announce` now does, so a status-less stub reads as UNDELIVERED and
        # the partner's class gets dropped — which is why these tests started
        # returning 'certified' instead of 'awaiting_partner'.
        # The undelivered path is covered in test_cert_announce_delivery.py.
        return SimpleNamespace(id="msg-1", status=send_status)
...
def test_the_partners_class_is_announced_and_awaited(db_session, monkeypatch):
    ...
    assert payload["cert_context"]["authority_mode"] == "simulator"
...
def test_application_mode_publishes_the_application_alias(db_session, monkeypatch):
    ...
    assert payload["simulator"]["endpoint"].startswith("a2a://cert_simulator"), \
        "the authority side is still a simulator — the sides are independent"
...
def test_application_mode_triggers_the_app_and_awaits_its_outbound_call(
    ...
    summary = _run(db_session, test_data={
        "case_catalogue": CATALOGUE, "modes": {"authority_mode": "application"}})

    assert summary["status"] == "awaiting_partner" and summary["passed"] is None
    assert summary["authority_mode"] == "application"
    ...
    assert kw["cert_context"]["authority_mode"] == "application"

    run = db_session.query(CertRun).one()
    assert run.status is CertRunStatus.RUNNING
    assert run.authority_mode == "application"
    row = db_session.query(CertTestResult).one()
...
def test_a_trigger_that_could_not_fire_says_so_on_the_row(db_session, monkeypatch):
    ...
    _run(db_session, test_data={"case_catalogue": CATALOGUE,
                                "modes": {"authority_mode": "application"}})
    row = db_session.query(CertTestResult).one()
