...
"""Tree-sitter AST code chunker tests.
...
def test_typescript_specific_symbols_found(monkeypatch):
    ...
    # noise filter drops on purpose — so extraction is asserted with the
...
def test_typescript_trivial_delegate_dropped_by_noise_filter(monkeypatch):
    """The other half of the above: with the filter on (the default), the one-line
