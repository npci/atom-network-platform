...
"""Tests for the hierarchical markdown chunker.
...
The network is a real-time exchange system. It enables instant requests between
participant systems through client devices, 24x7.

## Request Flow

1. User initiates a request through a provider app.
2. The provider forwards the request to the Authority.
3. The Authority routes the request to the receiving participant.
4. The receiving participant validates and fulfils it.

## Security

Two-factor authentication is mandatory. Device binding ties each network ID to a
...
The per-request limit is 100,000 units for most participants. Bulk requests may have
separate ceilings.
"""
...
def test_emits_parent_per_heading_section():
    ...
    assert any("Request Flow" in (t or "") for t in parent_titles)
