...
def test_split_sentences_basic():
    text = "the network is an exchange system. It was launched by the Authority. It handles millions of requests."
    out = context_compressor._split_sentences(text)
    assert out == [
        "the network is an exchange system.",
...
def test_split_sentences_handles_numbers_and_punctuation():
    text = "Limit is 100,000 units. Can this be raised? Yes! Contact your provider."
    out = context_compressor._split_sentences(text)
    assert len(out) == 4
...
async def test_compress_keeps_only_selected_sentences(monkeypatch):
    ...
    chunk = {
        "content": (
            "the network has request limits. It supports code-scan initiation. "
            "The default per-request limit is one hundred thousand units. "
            "Participants can opt for higher limits. There are also daily caps."
        ),
    }
    ...
    assert "the network has request limits." in compressed
    assert "one hundred thousand units" in compressed
