...
These tests lock *current* behavior of retrieval
constants, `build_context` output format, the network taxonomy schema, BM25
...
def test_build_context_header_format():
    ...
    chunks = [
        {
            ...
            "source_file": "regulator/network_master_direction.pdf",
            "content": "Payment Systems Operator shall ensure...",
        },
        {
            "doc_category": "network_product_doc",
            "source_file": "authority/network_handbook_v3.md",
    ...
    assert "[rbi_guideline | regulator/network_master_direction.pdf]" in ctx
    assert "[network_product_doc | authority/network_handbook_v3.md]" in ctx
    assert ctx.index("[rbi_guideline") < ctx.index("[network_product_doc")
...
def test_domain_taxonomy_has_ten_buckets_with_required_keys(monkeypatch):
    ...
    DOMAIN_PACK to the default network pack and lock its shape: 10 buckets, each carrying
    ...
    network_pack = Path(registry.__file__).resolve().parents[2] / "packs" / "network" / "network.yaml"
    monkeypatch.setenv("DOMAIN_PACK", str(network_pack))
    ...
    try:
        ...
        assert len(taxonomy) == 10, (
            f"The network pack must declare exactly 10 buckets; got {len(taxonomy)}. "
