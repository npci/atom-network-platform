...
# Tests that assert NETWORK-pack content must name that pack rather than rely
# on the platform default, which is now the domain-neutral `generic` pack.
_PACKS_DIR = pathlib.Path(__file__).resolve().parents[2] / "app" / "packs"


def _network_pack_path() -> str:
    return str(_PACKS_DIR / "network" / "network.yaml")


def test_default_pack_is_generic(monkeypatch):
    """An UNCONFIGURED deployment gets the domain-neutral pack.

    This used to assert the default was the network pack, which was the right
    assertion while the platform shipped exactly one domain. It is now the
    wrong one, and quietly so: a platform that defaults to a payments pack
    tells every unconfigured deployment — a library network, an aviation-data
    network — that it IS a payments network, in generated prose that reads as
    fact rather than as a misconfiguration. A neutral default is the whole
    point of the `generic` pack, so that is what this test guards.

    DOMAIN_PACK is a file path (the registered-key/`_PACKS` resolution is
    dormant — see registry.py's module docstring), so the raw selector is not
    the bare word "generic"; what must hold is that the DEFAULT resolves to a
    pack whose `.key` is "generic".
    ...
    try:
        assert active_pack_key().endswith("generic.yaml")
        assert get_active_pack().key == "generic"
    finally:
        registry._load.cache_clear()
...
def test_pack_supplies_the_exact_rules_the_agents_used_to_import(monkeypatch):
    ...
    Pinned to the network pack BY NAME: the byte-identity claim is about its
    constants, and asserting them against whatever pack the shell exports made
    this fail on every non-network host (the F-1 shape).

    It used to pin by DELETING DOMAIN_PACK and letting the default apply, which
    worked only while the default happened to be this pack. Now that the
    default is the neutral `generic` pack, deleting the variable pins the wrong
    pack — so the name is stated."""
    from app.core.domain import registry

    monkeypatch.setenv("DOMAIN_PACK", _network_pack_path())
    registry._load.cache_clear()
    try:
