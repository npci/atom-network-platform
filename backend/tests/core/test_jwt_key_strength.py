...
and a deployment's VEX statement cites it by name. These tests are what make
that citation trustworthy: if the validator is ever weakened or deleted, the
VEX statement becomes a formally signed false assurance, and that must fail
loudly here.
...
def test_validator_is_registered_in_run_all():
    """The VEX statement cites this validator as the control that makes
    ...
    assert "validate_jwt_key_strength()" in source, (
        ...
        "statement for CVE-2025-45768 depends on it "
        "actually running"
    )
