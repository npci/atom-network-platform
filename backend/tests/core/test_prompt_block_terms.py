...
# Frozen, copied from the rendered default. Do not regenerate from the module —
# update deliberately, and expect a prompt-cache miss when you do.
...
def test_network_pack_terms_are_byte_identical_to_the_defaults():
    """The prompt-cache guarantee: the default deployment (network pack) must keep
    ...
    pack_yaml = (Path(__file__).resolve().parents[2]
                / "app" / "packs" / "network" / "network.yaml")
    blocks = load_config_pack(str(pack_yaml)).prompt_blocks()
    pack_terms = {k: blocks.get(k, v) for k, v in DEFAULT_TERMS.items()}
    assert pack_terms == DEFAULT_TERMS
    assert citation_rules(pack_terms) == FROZEN_CITATION_RULES
