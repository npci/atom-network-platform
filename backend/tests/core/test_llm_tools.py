...
from app.core.llm import (
    call_messages_api_tools, _tools_with_cache, _assistant_blocks_to_dicts,
    tool_result_block, ToolUseRequest, MessagesToolTurn,
)
...
def test_wants_tools_predicate():
    assert MessagesToolTurn("", [], "tool_use", []).wants_tools is True
    assert MessagesToolTurn("done", [], "end_turn", []).wants_tools is False


# ── call_messages_api_tools (mocked SDK) ────────────────────────────────────────────
...
def test_call_claude_tools_end_to_end(monkeypatch):
    ...
    turn = asyncio.run(call_messages_api_tools(
...
def test_tool_cache_skipped_when_system_already_at_breakpoint_budget(monkeypatch):
    ...
    asyncio.run(call_messages_api_tools(
