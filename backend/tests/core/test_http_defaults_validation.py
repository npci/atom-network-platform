...
These tests pin the exact URL fields a sweep of `Settings` turned up
(cert_agent_url, partner_agent_url, authority_public_url, frontend_url
[localhost-exempt CORS origin, deliberately excluded], authority_simulator_url,
ollama_url) plus the additional ones already covered
...
# The fields that sweep confirmed default to http:// outbound URLs
# used for real network calls. frontend_url is deliberately
