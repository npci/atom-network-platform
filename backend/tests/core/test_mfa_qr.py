...
can read the symbol:
  ...
  1. **Pixel dimensions.** qrcode.make() defaults to box_size=10, border=4 and
     error-correction M. The equivalence was MEASURED by running both libraries
     side by side, not assumed — at the time, against an issuer string of
     "UnifiedAgentNxt" (a 95-byte URI):

         qrcode 8.2 : version 6, 41 modules, box_size 10, border 4 -> 490x490
         segno 1.6.6: error="m" -> version 6, 41 modules -> 490x490  (match)

     The URI has since been shortened to a 73-byte one (see `_URI`), which fits
     at version 5 = 37 modules -> 450x450. What the measurement established is
     that the two libraries AGREE for a given URI; the constants below track
     whatever URI this file currently uses.
...
# A realistic otpauth:// URI. THIS STRING IS LENGTH-CALIBRATED, NOT BRANDING:
# the URI's byte length picks the QR version, which sets the module count, which
# sets the pixel geometry asserted below. Editing the issuer/label — including a
# rename sweep — silently changes the symbol and fails the two assertions with a
# message about module counts that reads like a segno bug.
#
# It has already happened once: the issuer was "UnifiedAgentNxt" (a 95-byte URI)
# and a product-rename pass shortened it to "atom", dropping the symbol from
# version 6 to version 5. If you must change it, RE-MEASURE both constants with
# `segno.make(_URI, error="m")` rather than adjusting them until green.
_URI = "otpauth://totp/atom:alice@example.com?secret=JBSWY3DPEHPK3PXP&issuer=atom"

# Re-measured for the URI above: QR version 5 -> 37 modules per side, with
# box_size=10 and border=4 -> (37 + 2*4) * 10 = 450 pixels. The invariant under
# test is that segno reproduces what qrcode 8.2 produced FOR THE SAME URI, not
# any particular number.
_EXPECTED_MODULES = 37
_EXPECTED_PX = (_EXPECTED_MODULES + 2 * 4) * 10  # == 450
...
def test_qr_geometry_matches_the_qrcode_defaults_it_replaced():
    """450x450 — the exact output qrcode 8.2 produces for this URI.
    ...
    version 5 (37 modules) and therefore (37 + 8) * 10 = 450 pixels. mfa.py
...
def test_module_count_matches_qrcode_version_5():
    """37 modules per side — the scanner-relevant quantity behind the 450px.
    ...
    assert len(qr.matrix) == _EXPECTED_MODULES, (
        f"expected {_EXPECTED_MODULES} modules (QR version 5, matching qrcode "
        f"8.2), got {len(qr.matrix)}"
    )
    assert qr.version == 5, f"expected QR version 5, got {qr.version}"
...
def test_error_correction_level_is_m():
    ...
    Not arbitrary: the level fixes both the module count (hence the 450x450
