...
class TestPayloadRedaction:
    def test_redacts_nested_pii_in_a_pii_bearing_payload(self):
        payload = {
            "payload": {
                "test_case_id": "TC-1",
                "request": {"payerVpa": "ram.kumar@okexamplebank",
                            "mobile": "9876543210"},
        ...
        assert count == 3
        flat = str(out)
        for leaked in ("ram.kumar@okexamplebank", "9876543210", "123456789012"):
    ...
    def test_does_not_mutate_the_input(self):
        payload = {"payload": {"vpa": "ram.kumar@okexamplebank"}}
    ...
    def test_walks_lists(self):
        payload = {"cases": [{"vpa": "a.b@okexampleone"}, {"vpa": "c.d@okexampletwo"}]}
        out, count = redact_a2a_payload_for_llm(payload, A2ATaskType.CERT_CASE_RESULT)
        assert count == 2
        assert "okexampleone" not in str(out) and "okexampletwo" not in str(out)
