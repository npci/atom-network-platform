...
"""Regression cover for information exposure through an error message (CWE-209).
...
is what makes each one hard to see:
  ...
  * `TestStoreThenReturn`  — exception text accumulated into a container that a
    ...
    the two, which is why an AST classifier that works one handler at a time
    reports these handlers as LOG_ONLY.
...
AGE_CREATE_VLABEL = (
    ...
    "[SQL: SELECT create_vlabel('atom_graph', 'Document');]"
)
...
class TestStoreThenReturn:
    ...
    def test_kg_schema_init_failure_hides_sql(self):
        ...
        for secret in ("create_vlabel", "atom_graph", "[SQL:", "psycopg2"):
...
class TestEnvironmentDisclosure:
    ...
    @pytest.mark.parametrize("text,label", [
        ...
        ("postgresql://agentnxt:hunter2@db:5432/atom down", "dsn credentials"),
