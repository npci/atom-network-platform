...
"""Regression cover for `client_safe_detail` — CWE-209, information exposure
through an error message.

A static sweep for this pattern flags most `except` blocks in the codebase, and
the overwhelming majority are false positives: the exception detail goes to
`logger.*`, which is precisely what the recommendation for the pattern asks for.
A small number were genuine — a broad `except Exception` echoing `str(exc)` into
an HTTP response body, where the exception is machine-generated and quotes
