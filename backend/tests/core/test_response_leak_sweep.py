...
"""Regression cover for the two code defects found by re-reviewing the
static-analysis categories OTHER than information exposure, at the same
token-level standard.

Both sit inside categories that are otherwise entirely false positives, which is
exactly why they were worth isolating: dismissing a category wholesale is how a
real finding inside it gets missed.

  * `TestConfigCoercionNeverLogsValues` — CWE-532, sensitive data written to a
    log. `coerce_setting` used to log the rejected value with `%r`, and `value`
    on that path can be a DECRYPTED SECRET. Unreachable today only because all
    eight `is_secret` keys happen to be typed `str`; a single non-`str` secret
    key would have made it live.
  * `TestImageHandleIsReleased` — CWE-404, improper resource shutdown.
