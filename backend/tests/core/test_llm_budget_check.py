...
"""Integration — `call_llm` and `stream_llm` must raise
...
def stub_claude(monkeypatch):
    ...
    monkeypatch.setattr(llm_mod, "_call_messages_api", fake_call)
    monkeypatch.setattr(llm_mod, "_stream_messages_api", fake_stream)
