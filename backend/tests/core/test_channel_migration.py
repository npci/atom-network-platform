...
def test_network_pack_now_supplies_a_channel():
...
def test_config_pack_supplies_the_channel_it_NAMES(network_pack):
    ...
    A pack that declares `partner_channel: a2a` therefore yields the real A2A
    channel rather than None.

    Pinned to the network pack because that is *a pack which names a channel*.
    This used to read the default and assert "the default pack, which declares
    partner_channel: a2a" — true only while the network pack was the default.
    The neutral `generic` default names no channel, and the case for that is
    the next test down."""
