...
"""Sensitive data written to a log file (CWE-532).
...
# Added while triaging a batch of sensitive-data-in-logs findings (CWE-532). The
...
# Second pass over the same batch, after asking "what if the URL is
...
# Third pass over the same batch. The first two passes only ever asked what
