...
"""Byte-identity guard for prompt text, for the prompt-externalisation work.
...
`core/prompt_blocks.segments_for_prompt_cache` marks system-prompt segments
...
changed. Deliberate rewording (the domain-vocabulary sweep) will fail
    ...
    for path in sorted(APP.rglob("*.py")):
        ...
        for node in tree.body:
            ...
            if _is_binop_string(value):
                ...
                # neither a Constant nor a JoinedStr. Constants were migrated
                # first and JoinedStrs after, and this scan originally saw only
...
def _pack_identity() -> str:
    """The active pack's DECLARED key, not the path it was loaded from.

    This used to record `active_pack_key()`, which is an absolute filesystem
    path. Since the comparison below skips when the recorded value differs from
    the live one, any checkout at a different path — every git worktree —
    skipped this test silently. That is not hypothetical: a change series
    validated from a worktree left this test green while five prompts drifted
    from the snapshot.

    A pack's `key:` is its identity and is stable across checkouts, so keying on
    it makes the test actually run wherever the repository lives.
    """
    from app.core.domain.registry import active_pack_key, get_active_pack

    return str(getattr(get_active_pack(), "key", "") or active_pack_key())
...
def test_prompt_text_is_unchanged():
    ...
    if os.environ.get("UPDATE_PROMPT_SNAPSHOT"):
        SNAPSHOT.write_text(
            json.dumps(
                {"_meta": {"domain_pack": _pack_identity(),
    ...
    # Compared on the pack's declared KEY — see `_pack_identity` for why a path
    # here made this skip in every worktree.
    if meta.get("domain_pack") != _pack_identity():
        pytest.skip(
            f"snapshot was taken under pack {meta.get('domain_pack')!r}, "
            f"active pack is {_pack_identity()!r} — hashes are pack-specific"
        )
