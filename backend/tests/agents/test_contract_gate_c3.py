...
def test_undeclared_credit_leg_warns(network_pack):
    findings = CG.check_declared_legs(CREDIT_CALL, "[D1] atomicity: all-or-nothing")
    assert len(findings) == 1
    f = findings[0]
    assert f.severity == "warning"
    assert f.key == "buildMerchantCreditReqTransfer"
    assert "not traceable to a declared leg" in f.detail
...
def test_declared_credit_leg_not_flagged():
    ...
    assert CG.check_declared_legs(CREDIT_CALL, directives) == []


def test_generic_credit_token_passes_on_credit_mention(network_pack):
    blob = _new_file("src/A.java", "svc.createReqTransferCredit(txn);\n")
    assert CG.check_declared_legs(blob, "[D1] credit the payee bank") == []
    assert len(CG.check_declared_legs(blob, "[D1] debit only")) == 1


def test_symbols_the_pack_does_not_declare_are_never_flagged():
    # The network pack declares only its CREDIT symbols — the debit ones it
    # used to list were matched and then discarded by an in-loop filter.
    blob = _new_file(
        "src/A.java",
        "svc.createReqpayDebit(txn);\n"
        "type = REQPAY_DEBIT_REVERSAL;\n"
        "kind = BANK_REQPAY_DEBIT;\n")
    assert CG.check_declared_legs(blob, "") == []


def test_duplicate_credit_sites_deduped_per_file(network_pack):
    blob = _new_file(
        "src/A.java",
        "svc.createReqTransferCredit(a);\n"
        "svc.createReqTransferCredit(b);\n")
    assert len(CG.check_declared_legs(blob, "")) == 1


def test_check_skips_when_the_pack_declares_no_leg_symbols(monkeypatch):
    """A pack with no `contract_gate_consequential_leg_patterns` gets NO
    findings — this is an always-on gate that emits an operator-visible
    warning, so it must not fall back to another domain's symbol alphabet."""
    import app.core.domain.registry as reg

    monkeypatch.setattr(
        reg, "prompt_block",
        lambda name, default="": (
            "" if name == "contract_gate_consequential_leg_patterns" else default))
    assert CG.check_declared_legs(CREDIT_CALL, "") == []
...
def test_gate_aggregates_c3_findings_and_blocker_flag(network_pack):
    blob = (MANIFEST + "\n" + PUBLISH_THEN_PERSIST + "\n\n" + CREDIT_CALL
            + "\n\n" + UNPLANNED_DELETION)
    ...
    assert {"publish_before_persist", "shared_file_behavior_edit",
            "declared_leg", "config_key_declared"} <= checks
...
# `entity: "NPCI"` is the network pack's spelling, so every test below that
# uses this fixture pins that pack. Two of them assert `== []`, which the
# neutral `generic` default satisfies VACUOUSLY — it recognises no authority
# entity, so nothing is ever required to be emitted and "no findings" stops
# meaning "the gate looked and was satisfied".
_U16 = [{"code": "U16", "entity": "NPCI", "td_bd": "TD", "description": "txn not found"}]


def test_error_code_direct_literal_in_diff_not_flagged(network_pack):
...
def test_error_code_via_named_constant_needs_corpus(network_pack):
...
def test_error_code_never_emitted_still_flagged_even_with_corpus(network_pack):
...
def test_error_code_corpus_threaded_through_run_gate(network_pack):
    diff = _new_file("src/main/java/com/example/x/H.java",
                     "return buildError(req, UdirErrorCodes.TXN_NOT_FOUND, msg);\n")
    corpus = 'public static final String TXN_NOT_FOUND = "U16";'
