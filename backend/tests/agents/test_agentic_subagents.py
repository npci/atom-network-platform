...
from app.core.llm import ToolUseRequest, MessagesToolTurn
...
# ── Network-pack pin ──────────────────────────────────────────────────────────
# This file asserts the network pack's payments vocabulary, and agentic_subagents freezes
# that vocabulary into module-level constants AT IMPORT. A function-scoped
# `network_pack` fixture runs too late — collection has already imported the
# module against the default (generic) pack. Pin for the module's duration and
# reload both ways, the same shape as test_party_inference.
_NETWORK_PACK = str(Path(S.__file__).resolve().parents[1]
...
def test_rectification_clause_is_comply_first_and_party_flows():
    ...
    # states many network flows involve only 2-3 parties and must say so "rather than padding
...
def test_system_segments_carry_context_and_bounded_cache():
    ...
    # (network: "integer paise"; NLLN: loan periods), so assert the invariant
...
def _turn(tool_uses, text="", stop="tool_use"):
    ...
    return MessagesToolTurn(text=text, tool_uses=tool_uses, stop_reason=stop, assistant_content=ac)
...
def _script(monkeypatch, turns):
    ...
    monkeypatch.setattr(RT, "call_messages_api_tools", fake)
