...
"""Turn-level recovery: a max_tokens-truncated turn is discarded and
...
from app.core.llm import MessagesToolTurn, ToolUseRequest
...
def _turn(text="", stop="end_turn", tool_uses=()):
    ...
    return MessagesToolTurn(text=text, tool_uses=list(tool_uses), stop_reason=stop,
...
def _capture_runner(monkeypatch, script):
    ...
    monkeypatch.setattr(R, "call_messages_api_tools", fake)
    return calls
...
def test_empty_turn_retried_once_then_completes(ws, monkeypatch):
    calls = _capture_runner(monkeypatch, [
        MessagesToolTurn(text="", tool_uses=[], stop_reason="tool_use", assistant_content=[]),
        _turn(text="done", stop="end_turn"),
    ])
    res = asyncio.run(R.run_agent_loop(
...
def test_persistently_empty_turn_still_ends_cleanly(ws, monkeypatch):
    empty = MessagesToolTurn(text="", tool_uses=[], stop_reason="tool_use", assistant_content=[])
...
def test_summary_ladder_retries_on_overflow(monkeypatch):
    ...
    async def fake(**kwargs):
        ...
        return MessagesToolTurn(text="1. GOAL — finish", tool_uses=[], stop_reason="end_turn",
