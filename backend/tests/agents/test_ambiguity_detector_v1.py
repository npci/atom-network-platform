...
# `network_pack` now comes from tests/conftest.py. The local copy that lived
# here was NAMED for the network pack but PINNED `registry.DEFAULT_PACK` — the
# same string only for as long as the network pack was the default. Once the
# default became the neutral `generic` pack, this fixture silently began
# selecting a pack that declares no taxonomy at all, and the tests below went
# from asserting the network buckets to asserting against an empty dict.
...
class TestTaxonomyCarriesNoScopeSignals:
    ...
    def test_no_bucket_lists_a_scope_signal_key(self, network_pack):
        offenders = {
...
class TestFloorMechanismStillWorks:
    ...
    def test_get_required_fields_excludes_signals(self, network_pack):
