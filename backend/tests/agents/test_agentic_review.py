...
"""Read-only review subagent. Parsing is pure; the runner uses a scripted
...
def test_extra_tools_reach_the_model_alongside_the_read_only_set(ws, monkeypatch):
    ...
    async def fake(**kwargs):
        captured.update(kwargs)
        return MessagesToolTurn(text="[]", tool_uses=[], stop_reason="end_turn",
...
def test_gpt_reviewer_rejected_when_provider_cannot_route_it(ws, monkeypatch):
    # A gpt id under llm_provider=anthropic would pass the prefix guard and then hard-fail
