...
These tests assert network vocabulary (four parties, six operations), and the
module under test resolves that vocabulary from the ACTIVE pack at import
time. So this file pins the default network pack for its own duration: the module is
reloaded under the network pack before the tests and reloaded under the ambient pack
...
_NETWORK_PACK = str(Path(_qg_module.__file__).resolve().parents[1]
