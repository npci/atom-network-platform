...
These tests pin the containment fix: each validator only ever checks a document
against the schema that generated it. They will need revisiting if the pack
contract is ever extended to declare which of the two document schemas is
authoritative, at which point the containment can collapse into one — that is
the point of pinning it now rather than leaving the coupling implicit.
"""
import pytest
