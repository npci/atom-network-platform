...
The equivalence groups are pack data (`schema_namespaces`), so the split-namespace
assertions pin DOMAIN_PACK to the default network pack explicitly instead of assuming it;
...
def test_unknown_namespace_untouched_no_false_equivalence(network_pack):
    ...
    # a different product of the same Authority is NOT merged with the network
    assert not same_namespace(CANON, "http://example.org/netc/schema/")


def test_sibling_spellings(network_pack):
...
def test_none_and_empty_safe(network_pack):
...
def test_pack_without_groups_matches_raw_strings(nlln_pack):
    ...
    ecosystem's equivalences — the network's split is two different namespaces here."""
