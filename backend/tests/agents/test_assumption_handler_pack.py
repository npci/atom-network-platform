...
def test_network_pack_blocks_its_curated_keywords_and_keeps_its_defaults(monkeypatch):
...
def test_nlln_pack_gets_library_blockers_not_network_ones(monkeypatch):
    _activate(monkeypatch, _PACKS / "nlln" / "nlln.yaml")
    try:
        blocking, assumed = apply([
            {"key": "mandate_type_change"},    # network blocker — must NOT block here
            {"key": "loan_period_extension"},  # NLLN blocker
            {"key": "response_timeout"},       # network default must NOT be assumed
        ])
        assert blocking == ["loan_period_extension"]
        by_key = {g["key"]: g["default"] for g in assumed}
        # No invented network SLA for a library domain — generic fallback instead.
