...
LOG = """
[INFO] Building igw 1.0
[ERROR] /app/clone/igw/src/main/java/com/example/Pay.java:[42,10] cannot find symbol
[ERROR] /app/clone/igw/src/main/java/com/example/Pay.java:[42,10] cannot find symbol
[INFO] Building legacy-mod 1.0
[ERROR] /app/clone/legacy-mod/src/main/java/com/example/Old.java:[7,5] package x does not exist
"""
IGW = "/app/clone/igw/src/main/java/com/example/Pay.java"
LEGACY = "/app/clone/legacy-mod/src/main/java/com/example/Old.java"
CHANGED = ["igw/src/main/java/com/example/Pay.java"]   # repo-relative, touches module "igw"


def test_parse_mvn_errors_dedups():
    errs = bt.parse_mvn_errors(LOG)
    assert len(errs) == 2                       # the duplicate igw line is collapsed
...
def test_module_name_stable_across_absolute_and_relative():
    assert bt.module_name(IGW) == "igw"
    assert bt.module_name("igw/src/main/java/com/example/Pay.java") == "igw"
...
def test_annotate_marks_touched_module():
    ...
    assert by_file[IGW].touched is True and by_file[IGW].module == "igw"
