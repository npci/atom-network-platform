...
def test_error_code_classifies_every_provider_as_llm_unavailable():
    """An LLM outage must triage as `llm_unavailable` on ALL supported providers.
    The classifier used to match the vendor token "anthropic" alone, so an
    OpenAI/Gemini/Ollama/AiNxt outage silently landed in "internal_error" and an
    operator reading the run could not tell an LLM outage from a platform bug."""
    from app.agents.agentic_orchestrator import _error_code
    for vendor in ("anthropic", "openai", "gemini", "googleapis", "ollama", "ainxt"):
        assert _error_code(Exception(f"{vendor} gateway returned 503")) == "llm_unavailable", vendor
    # Still discriminating — a non-LLM failure must not be swept into the bucket.
    assert _error_code(Exception("verify step produced no changeset")) == "internal_error"
