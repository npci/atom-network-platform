...
Network transaction volumes have grown 40x since 2018, crossing 18 billion per month in late 2025 [1]. That puts this network well ahead of any comparable real-time network globally [2].

## 2. Product & Ecosystem

The central settlement operator role is held by the Authority, which coordinates participants and providers [1]. The three largest independent providers now process more than 85% of total network volume [3].

## Sources

- [1] regulator_guidelines/master_direction.pdf
- [2] network_product_docs/Network_Complete_Guide.md
...
The per-transaction limit on the network applies to most participants [1].

## Sources

- [1] network_product_docs/limits.md
"""
    report = citation_validator.validate(md, num_sources=1)
