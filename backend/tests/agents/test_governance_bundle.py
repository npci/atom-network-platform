...
GOOD = [
    ...
    ("is-skill/rules/sast_rule_catalog.csv", b"id,name\n1,SQLi\n"),
]
...
def test_single_root_bundle_parses_and_classifies():
    ...
    assert cls == {"SKILL.md": "skill_manifest",
                   ...
                   "rules/sast_rule_catalog.csv": "scanner_rule_config"}
...
def test_option_shaped_member_names_reject():
    """A member name must never be readable as a command-line OPTION.

    The name becomes the script operand of ``python3 <path> <target>``, which
    `governance_sandbox._render_invocation` shlex-splits into argv. A member
    called ``-c<code>.py`` therefore makes argv[1] a `python3 -c` payload that
    runs in the backend container whenever Docker is unavailable.

    Traversal rejection does not cover this (no ``..``, not absolute), and
    neither does `_validated_invocation`'s "is it a declared script?" test —
    such a member IS declared. Only charset containment at extraction closes it,
    which is why this is pinned separately from the traversal test above.
    """
    for evil in ("-c__import__(chr(111)).system(chr(105))#.py",
                 "scripts/-rf.py",
                 "--upload-pack=id.py"):
        with pytest.raises(BundleError, match="unsafe"):
            parse_bundle(tgz([("SKILL.md", b"x"), (evil, b"")]), "a.tar.gz")
    # Shell metacharacters and whitespace go the same way.
    for evil in ("a b.py", "foo;id.py", "$(id).py", "x|y.py"):
        with pytest.raises(BundleError, match="unsafe"):
            parse_bundle(tgz([("SKILL.md", b"x"), (evil, b"")]), "a.tar.gz")
    # Guard against the matcher going vacuous: ordinary names still parse.
    parse_bundle(tgz(GOOD), "a.tar.gz")
