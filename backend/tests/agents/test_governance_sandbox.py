...
# ── XML scanner output (SAST results XML, generic issue XML) ──────────────────

_SAST_XML_EMITTER = '''
import sys
# Emit a SAST-shaped results report to the {output} file path passed as argv[2],
...
def _xml_contract(**over):
    c = {"path": "sast_scan.py", "role": "validator", "output_format": "xml",
         "invocation": "python3 sast_scan.py {target} {output}",
...
def test_xml_output_counts_result_elements(tmp_path):
    ...
    (bundle / "sast_scan.py").write_text(_SAST_XML_EMITTER)
...
def test_xml_entity_expansion_is_rejected_not_parsed(tmp_path):
    ...
    evil = ('import sys\nprint("""<?xml version="1.0"?>'
            ...
            '<ScanResults><Result>&e;</Result></ScanResults>""")\n')
    bundle = tmp_path / "b"; bundle.mkdir()
    (bundle / "sast_scan.py").write_text(evil)
    r = run_script(_xml_contract(invocation="python3 sast_scan.py {target}"),
                   bundle_dir=bundle, target_dir=tmp_path / "t", scratch_dir=tmp_path / "s")
...
def test_xml_empty_output_is_error_not_pass(tmp_path):
    bundle = tmp_path / "b"; bundle.mkdir()
    (bundle / "sast_scan.py").write_text("import sys; sys.exit(0)\n")   # writes nothing
    r = run_script(_xml_contract(invocation="python3 sast_scan.py {target}"),
                   bundle_dir=bundle, target_dir=tmp_path / "t", scratch_dir=tmp_path / "s")
