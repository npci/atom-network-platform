...
# These tests assert the network pack's four-party vocabulary, and the module resolves its
# canonical set from the ACTIVE pack at import time — so pin the default network pack for
# this file's duration and reload the module both ways (see
# test_question_generator_v1 for the full reasoning; same F-1 shape).
_NETWORK_PACK = str(Path(PI.__file__).resolve().parents[1]
                / "packs" / "network" / "network.yaml")


@pytest.fixture(scope="module", autouse=True)
def _pin_network_pack_for_this_module():
    prior = os.environ.get("DOMAIN_PACK")
    os.environ["DOMAIN_PACK"] = _NETWORK_PACK
...
@pytest.mark.asyncio
async def test_llm_error_falls_back_to_all_parties(monkeypatch):
...
def test_legacy_fallback_source_still_validates():
    """A `change_request_context` row written before the rename carries
    `source="fallback_all_four"`. It must still load, and normalise to the
    current spelling — dual-accept, single-emit."""
    result = PI.PartyInferenceResult.model_validate({
        "parties_in_scope": ["PAYER_PSP"],
        "rationale": "stored row",
        "confidence": "low",
