...
WHY THIS EXISTS. The prompts were tuned by pushing AWAY from generality —
`agents/canvas.py` literally instructs "Be specific to the network / the
Authority — avoid generic product management boilerplate." Moving that
specificity into a domain pack therefore removes quality by construction (the
prompt can no longer name the domain it is writing about), and the loss is
...
def _headings(text: str) -> set[str]:
    ...
    "## A. The Authority — Changes" alike: the two live document schemas
    (`app.agents.blueprints` and `app.docgen.document_guides`) number their
    sections differently — arabic in one, roman or alphabetic in the other —
    so a heading set compared verbatim would report every section missing.
    """
    out: set[str] = set()
...
def deterministic_findings(candidate: str, doc_type: str) -> list[str]:
    ...
    # what the suite measures. Excluded deliberately, not silently. Whether the
    # mandatory-section list should instead be pack-owned is still open.
