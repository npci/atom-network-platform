<!-- NETWORK-PACK TEST DATA. The accepted reference capture for case_001,
     produced by the canvas agent on 2026-08-10 with the model pinned in
     case_001.json.

     The prompt is synthetic — an invented feature, no real change request
     and no customer or institution data — so the capture is safe to
     publish. It was copy-edited before being accepted; no edit touches a
     scored term (see case_001.json `domain_terms`) or a heading. Part of
     that copy-edit was grammar: an earlier plain-substitution rename left
     article collisions ("the ... the network ..."). Grammar matters more
     here than in ordinary prose, because this file is the REFERENCE the
     semantic judge grades candidates against, so ungrammatical reference
     text penalises correct output.

     The payments vocabulary (RBI, TPAP, PPI, NACH, MDR, INR, SGF/FRM, PSP,
     Remitter Bank) is deliberately NOT neutralised. This capture was taken
     under app/packs/network/network.yaml, which declares exactly that
     domain — `regulatory_body: RBI`, `ecosystem_actors: banks, PSPs,
     TPAPs`, `market_context: the Indian digital payments context`, INR
     amounts, and the SGF/FRM bullets injected by `product_operating_extra`.
     Neutralising the text would make the golden diverge from what this pack
     actually produces, so the next real capture would score WORSE against
     it and the harness would report fixture drift as a regression. A
     domain-neutral golden comes from capturing a case under a
     domain-neutral pack, not from hand-editing this one. -->
## 1. Feature

This feature lets a customer set a minimum balance rule on their network-linked wallet (e.g., "reload with INR 500 whenever balance drops below INR 100"). Once the customer sets up this rule one time — authenticating with their PIN only during this setup — every future top-up happens automatically in the background by debiting their bank account, with no PIN entry needed at the time of reload. The customer gets a notification after each auto-reload and can pause, modify, or cancel the rule anytime from their PSP app.

## 2. Need

- **Why should we do this?**
  - Removes reload friction that currently causes wallet balance depletion and transaction failures at point-of-sale/QR scan, directly hurting network-linked PPI wallet usage (transit, small-ticket merchant QR, gig-worker payouts).
  - Extends the existing network Autopay e-mandate rail (used today for subscriptions/SIPs/EMIs) to a new "balance-triggered" use case instead of only calendar-triggered debits.
- **Differentiation** — Incremental from a rails perspective (reuses the Authority Mandate Management System, e-mandate registration, and "as-and-when-presented" mandate category already live under the network Autopay), but exponential from a UX perspective: it is the first network mandate type triggered by an account-state event (balance threshold) rather than a fixed date/frequency.
...
- **Ecosystem efforts (costs to make this work)**
  ...
  - Remitter/Issuer Bank must support "as-and-when-presented" mandate execution without additional OTP/PIN step, consistent with the existing network Autopay mandate execution flow.
...
- **Impact opportunity**
  - Assumption: ~350-400 million active network-linked wallet users nationally could be addressable; even 10-15% opt-in reduces manual reload transactions by a similar order, freeing that volume for merchant-side network growth instead.
...
- **FAQs + trained LLM** — FAQ topics: how to set/change threshold, why no PIN is needed, how to cancel, what happens on failed reload, how refunds/reversals work. LLM trained on these FAQs plus the existing network Autopay mandate FAQ corpus for consistency.
- **Circular** — New Authority product circular defining "Balance-Threshold Triggered Mandate" as a sub-category under the network Autopay "as-and-when-presented" mandates, including cap, notification, and FRM tagging requirements.
...
- **Market view to pay the price** — PPI issuers are highly incentivized to adopt at zero/near-zero authority fee given existing infra reuse; unlikely to accept a new per-mandate fee given wallet-load is a zero-MDR category by regulation.
...
- **Must have compliances in the Authority's product circular for ecosystem**
  ...
  - Mandatory grievance routing through the existing network Autopay dispute resolution flow with defined reversal SLA.