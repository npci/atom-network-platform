<!-- Captured from the real xsd agent (see case_002.json `generation`).
     Verbatim except for ONE edit: the two schema filenames the model
     invented carried the old brand (`upi_refund_*.xsd`). Renamed to the
     network spelling. Safe because they are MODEL-INVENTED, not corpus
     facts — the shipped XSD corpus (knowledge_base/existing_xsds) holds
     one file, `adcn-v1.xsd`, and neither name appears anywhere else in
     the tree. The edit touches no scored term (case_002.json
     `domain_terms`) and no heading. -->
...
### Affected Schemas
- `network_refund_request.xsd` (governs the `ReqRefund` message)
...
- `network_refund_response.xsd` (governs `RespRefund`) — *Assumption: no new fields are echoed back in the response per the tech spec; only referenced for completeness of validation error codes (e.g., BD/TD for exceeded refund amount or invalid reason code). No structural change required here unless the Authority mandates echoing `refundAmount`.*
