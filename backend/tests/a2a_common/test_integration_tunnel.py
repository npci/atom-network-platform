...
"""The tunnel contract and allowlist. Pure; no containers, no wire.

The bar the tunnel has to clear: round-trip a request with duplicate headers, a
binary body, an empty body and no body; hop-by-hop headers dropped; unknown
alias rejected; path-prefix enforcement; a malformed allowlist fails startup;
and **the query string survives byte-identically** — duplicate keys, an encoded
`%40`, and an unrecognised parameter — because contract selection rides on it.
...
def test_cert_context_rides_along_and_is_optional():
    ctx = {"cflow_id": "CFLOW-ab12", "cert_attempt": 2, "test_case_id": "TC-7",
           "initiator": "bank", "authority_mode": "simulator", "partner_mode": "application"}
