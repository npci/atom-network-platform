...
"""One row per hop, and the row alone must diagnose a failure.
...
def test_authority_ingress_records_its_hop(db_session, monkeypatch):
    ...
    partner = PartnerAgent(id="p1", name="Partner One")
...
def test_authority_ingress_records_the_disabled_refusal_too(db_session, monkeypatch):
    from app.services.integration_testing.ingress import forward_exchange
