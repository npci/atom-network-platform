...
"""Contract guard for the A2A protocol v1.
...
def test_ext_types_excluded_from_frozen():
    ...
    assert {tt.value for tt in ext} == {
        ...
        "round_opened",
        "round_closed",
        # The integration-testing tunnel pair.
        "http_exchange_request",
        "http_exchange_response",
        # The start signal for the partner-initiated suite half.
...
def test_round_lifecycle_ext_are_authority_to_partner_change_id():
    """round_opened / round_closed must correlate on change_id and flow Authority→bank —
