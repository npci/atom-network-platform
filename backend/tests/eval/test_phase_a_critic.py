...
class TestReadCriticConfig:
    def test_default_enabled_with_cross_provider_when_blank(self, monkeypatch):
        ...
        # A cross-provider target is only chosen when its key is CONFIGURED.
        monkeypatch.setattr(critic_mod.settings, "openai_api_key", "sk-test")
    ...
    def test_cross_provider_falls_back_when_no_other_key_is_configured(self, monkeypatch):
        """A deployment with only one provider's key must not be routed off it.

        The preference table used to be consulted blind, so an ainxt-only or
        openai-only deployment was sent to `anthropic` for every critic call —
        a provider it has no key for, and one whose API the network may not
        even resolve. Grading with the generator's own provider is a weaker
        check than independent eyes, but it is a check that runs.
        """
        monkeypatch.setattr(critic_mod.settings, "eval_critic_enabled_by_default", True)
        monkeypatch.setattr(critic_mod.settings, "eval_critic_default_provider", "")
        monkeypatch.setattr(critic_mod.settings, "eval_critic_default_model", "")
        monkeypatch.setattr(critic_mod.settings, "eval_critic_cross_provider", True)
        monkeypatch.setattr(critic_mod.settings, "llm_provider", "ainxt")
        for key in ("anthropic_api_key", "openai_api_key", "gemini_api_key"):
            monkeypatch.setattr(critic_mod.settings, key, "")

        cfg = critic_mod.read_critic_config(_FakeDb(), CheckpointId.BRD_TO_TECH_SPEC)
        assert cfg["provider"] == "ainxt"
...
class TestGroundingSeam:
    ...
    def test_success_collects_snippets_and_provenance(self, monkeypatch):
        ...
        def _fake_retrieve(query, db, top_k=6, **_k):
            return [
                {"source_file": "network_error_codes.md", "doc_category": "authority_error_code",
                ...
                {"source_file": "empty.md", "doc_category": "network_product_doc",
        ...
        prov = res.provenance()
        assert prov[0]["source_file"] == "network_error_codes.md"
        block = grounding_mod.format_grounding_block(res)
        assert "network_error_codes.md" in block and "[1]" in block
    ...
    def test_build_prompt_injects_kb_block_when_grounded(self):
        ...
        assert "network_error_codes.md" in prompt["user"]
