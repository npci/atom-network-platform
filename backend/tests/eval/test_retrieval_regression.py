...
asserts against `Network_Complete_Guide`, deleted on purpose (commit 647b246)
because its provenance could not be established and it may reproduce regulator
circular text verbatim. Restoring that corpus to turn this green would
reintroduce precisely what a legal review removed. Making this a real gate again
means writing NEW gold cases against a corpus that legitimately ships — the
resolution recorded at the time was that adopters bring their own corpus, so
this is deployment-side, not repo-side.
...
def test_retrieval_no_regression_vs_baseline():
    ...
    # `retrieval_gold.jsonl` asserts against `Network_Complete_Guide`, which was
    # DELETED on purpose (commit 647b246) because its provenance could not be
    # established and it may reproduce regulator circular text verbatim.
    # Re-ingesting it would reintroduce exactly what a legal review
