...
# Resolved against the ACTIVE domain pack: a pack shipping
# `eval/retrieval_gold.jsonl` is graded on its own corpus and vocabulary, and
# everything else gets the platform's domain-neutral cases. A retrieval gold
# set is inseparable from the corpus it queries, so a single shipped set could
# only ever be right for one industry.
from tests.eval.gold_paths import resolve_gold
GOLD_PATH = resolve_gold("retrieval_gold.jsonl")
