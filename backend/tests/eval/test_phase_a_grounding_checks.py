...
"""Phase A cross-artifact grounding checks.
...
def network_pack(monkeypatch):
    """Pin the default network pack for tests whose FIXTURES are network codes.

    These assertions are only true under the network pack, and used to pass by inheriting
...
class TestDomainErrorCodesAreValid:
    def test_clean_when_at_least_one_domain_code_present(self, network_pack):
        c = _combined({
            "tech_spec_document": {
    ...
    def test_clean_when_multiple_domain_codes_present(self, network_pack):
        c = _combined({
            "tech_spec_document": {
    ...
    def test_flags_error_section_without_codes(self):
        ...
        # rather than the network's — pinning the old "no recognised error"
        # wording would re-hardcode the thing this check was genericised to stop.
    ...
    def test_code_shape_follows_the_active_domain_pack(self, monkeypatch):
        """The check must grade against the ACTIVE domain's codes, not the network's.

        Regression for the genericisation sweep. Before this, the network's code alphabet
        ...
        # An NLLN error section: real NLLN codes, zero network codes, and a
...
class TestNoHttpCodesAsNetworkErrors:
    def test_clean_when_only_domain_codes_in_error_section(self):
        c = _combined({
