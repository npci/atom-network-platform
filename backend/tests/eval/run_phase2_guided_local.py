...
def _login() -> str:
    # No default password: this script talks to a running instance, so a
    # baked-in credential would either be a real one in the repo or a silent
    # failure against every deployment that does not use it.
    username = os.getenv("EVAL_HARNESS_USERNAME", "admin")
    password = os.getenv("EVAL_HARNESS_PASSWORD")
    if not password:
        raise RuntimeError(
            "set EVAL_HARNESS_PASSWORD (and optionally EVAL_HARNESS_USERNAME) "
            "to the credentials of the local instance under test"
        )
