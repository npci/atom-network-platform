...
# Overridable per pack like the other gold sets, though the shipped cases are
# deliberately domain-free: they test a LANGUAGE capability (add a decorator,
# add a route, write a validator), not a domain, so they are meaningful for
# every deployment as they stand.
from tests.eval.gold_paths import resolve_gold
GOLD_PATH = resolve_gold("code_change_gold.jsonl")
