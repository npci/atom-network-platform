...
WHY THIS SERVICE EXISTS. `torch` and `sentence-transformers` carry six SBOM
policy findings between them, including a CVSS 9.8. They were installed in the
backend image while the reranker defaulted to OFF — so the platform carried
the risk without using the feature. Rather than drop reranking (worth +5-15pp
recall@10), the model stack moved out of the backend into this one-endpoint
container. The backend sets `reranker_backend=remote` and gets identical
results over HTTP.
