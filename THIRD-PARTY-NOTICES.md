...
> amd64 lock has dropped. `scripts/ci/hygiene-check.sh` reports the arm64 lock as
> having pins absent while amd64 is in sync — that failure is this, and it is
> known.
>
...
An SCA report predating that date will still name it — reconcile against
`backend/requirements.txt`, not against the older report.
...
use it. Moving them behind an optional extra is known outstanding work; until
then, an adopter who does not enable `USE_RERANKER` is still paying the download
and image size for them.
