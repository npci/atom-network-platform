...
// Reusable agentic-run panel for the complete-flow stages (Phase A/B split).
...
export default function AgenticPhasePanel({ changeId, kind, repoIds = [], intent = '', onApproved, variant = 'console', startBlockedReason = '' }) {
  ...
  // ADR-0005 — TSD approval gate (shadow/enforce controlled by
  ...
  // hung. The banner replaces the silence so a slow model call (provider side, network, retry)
  ...
  // ADR-0005 — re-checks the TSD's status and resumes if it's now
  ...
  return (
    <div>
      ...
      {/* ADR-0005 — TSD approval gate. Approving/regenerating the TSD
        ...
        <section style={{ marginTop: 16, padding: '12px 14px', borderRadius: 8,
          ...
          {pushTargets.map(t => (
            <div key={t.repo_id} style={{ fontSize: 13, marginTop: 8, color: 'var(--text-secondary)' }}>
              ...
              {t.mr_url && <> · <a href={safeHref(t.mr_url)} target="_blank" rel="noopener noreferrer" style={{ color: '#60a5fa' }}>Open merge request →</a></>}
            </div>
          ))}
        </section>
      )}
