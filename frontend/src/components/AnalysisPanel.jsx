...
export default function AnalysisPanel({ changeId, onStatus, hideActions = false }) {
  ...
  if (!run) {
    ...
    // Rule comes from the domain pack, not from any one domain's core+app
    // pair. Undeclared topology → single-repo default (any one repo) + a notice.
    // Mirrors the server gate in repo_scope.validate_selection.
        ...
        <section style={card}>
          ...
          <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid var(--border-subtle)' }}>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--text-muted)', marginTop: 2 }}>
              ...
              for a new API, who initiates it and how it routes through the declared party model.
            </div>
      ...
      {phase === 'awaiting_plan_approval' && plan?.exists && (
        <section style={card}>
          ...
          <div style={{ marginTop: 10, fontSize: 11.5, color: 'var(--text-muted)' }}>
            ...
            how it routes through the declared party model.
          </div>
