...
// Code Indexing role dropdown). That is one ecosystem's topology, not the
// platform's — a single-repo domain could never start a run, because the UI
// refused to submit a one-repo selection even though the BACKEND accepts it
// happily.
//
...
// need a topology (a multi-repo domain) isn't silently allowed to start a
// half-scoped run.
 ...
 *  historical network vocabulary when the domain declares none, so the admin screen
