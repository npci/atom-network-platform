...
import {
  ...
  MessageSquare, Sparkles, X, MessagesSquare,
  Inbox, Archive,
} from 'lucide-react'
...
import { ASSIGNMENT_STATUS, AUTHORITY_REPLY_ROLES, relativeTime, formatDateTime } from '../../lib/certStatus'
...
// Label, colour AND icon all come from lib/partnerTypes (the backend
// PartnerType enum) so every surface agrees.
const typeColor = t => partnerTypeColor(primaryPartnerType(t), '#58a6ff')
const typeIcon  = t => partnerTypeIcon(primaryPartnerType(t))

const fmtTime = ts => formatDateTime(ts)
...
// Effective open-thread state derivation: a thread is "awaiting an authority
// reply" when its latest_role is 'partner' (last message was from the partner).
// "Awaiting partner" when latest_role is one of AUTHORITY_REPLY_ROLES.
...
function threadState(t) {
  ...
  if (AUTHORITY_REPLY_ROLES.includes(lr)) return 'awaiting_partner'
  return 'open'
}
...
// ─── Message bubble ───────────────────────────────────────────────────────────
function Bubble({ msg, partnerName, partnerType }) {
  // The authority side's display name for this domain (pack-derived).
  const { authorityLabel } = useCertInitiators()
  const tc   = typeColor(partnerType)
  ...
  // ── Received: partner query — LEFT side ───────────────────────────────────
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-start', gap: 10, margin: '10px 0' }}>
      ...
      <div style={{ maxWidth: '68%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
          <span style={{ fontSize: 10, fontWeight: 600, color: tc }}>{partnerName}</span>
  ...
  // ── Sent: the authority's response — RIGHT side ──────────────────────────
  if (AUTHORITY_REPLY_ROLES.includes(role)) return (
    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, margin: '10px 0' }}>
      <div style={{ maxWidth: '68%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4, justifyContent: 'flex-end' }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 600, color: '#2563eb' }}>{authorityLabel}</span>
        </div>
        <div style={{
          padding: '11px 15px',
      ...
      {/* Authority avatar — initial of the pack's own label, exactly as the
          counter-proposal bubble below does it. This was a literal 'N'. */}
      <div style={{
        width: 32, height: 32, borderRadius: '50%',
        background: '#2563eb',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
  ...
  // authority=right) with a tag describing the round / severity / status so
  ...
  if (role === 'counter_proposal' || role === 'counter_resolution'
    ...
    // The authority's side takes its name from the active domain pack. This
    // was the literal 'NPCI', which mislabels every message on any other domain.
    const avatarText = fromPartner ? (partnerName || 'B')[0] : (authorityLabel || 'A')[0]
    const senderLabel = fromPartner ? partnerName : authorityLabel
...
export default function AgentMessaging() {
  // Pack-derived side names for the thread-list and composer copy.
  ...
  const lastReplyIdx    = messages.reduce((acc, m, i) => AUTHORITY_REPLY_ROLES.includes(m.role) ? i : acc, -1)
  ...
  const partnerColor = activeThread ? typeColor(activeThread.partnerType) : '#2563eb'
  const partnerCfg   = activeThread ? statusCfg(activeThread.status) : null
  // Conversation-header glyph. Was a hardcoded bank building; now the role's
  // own icon, and Globe when the role is unknown (see lib/partnerTypes).
  const PartnerIcon  = typeIcon(activeThread?.partnerType)
    ...
    <div style={{
        ...
        <div style={{ padding: '14px 14px 10px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              placeholder={t('ph.messaging.search')}
        ...
        <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
          ...
          {filtered.map(t => (
            <ThreadItem
              ...
              onClick={() => {
                setActiveThread({ threadId: t.thread_id, kind: t.kind || kindFilter, changeId: t.change_id, partnerId: t.partner_id, crId: t.change_id, crTitle: t.change_title, partnerName: t.partner_name, partnerType: t.partner_types?.[0] || '', status: t.thread_status })
      ...
      {activeThread ? (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--bg-card)', flexShrink: 0 }}>
            <div style={{ width: 40, height: 40, borderRadius: 11, background: `${partnerColor}12`, border: `1.5px solid ${partnerColor}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <PartnerIcon size={18} color={partnerColor} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>{activeThread.partnerName}</span>
                <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 8, color: partnerColor, background: `${partnerColor}12`, border: `1px solid ${partnerColor}25`, fontWeight: 600, }}>{partnerTypeLabel(activeThread.partnerType)}</span>
                {partnerCfg && (
          ...
          <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '16px 20px 8px', display: 'flex', flexDirection: 'column' }}>
            ...
            {!loadingMsgs && messages.length === 0 && (
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', gap: 8 }}>
                ...
                <p style={{ margin: 0, fontSize: 11 }}>Messages appear when {activeThread.partnerName} sends a query via A2A</p>
              </div>
            )}
            {messages.map((msg, i) => (
              <Bubble key={msg.id || i} msg={msg} partnerName={activeThread.partnerName} partnerType={activeThread.partnerType} />
            ))}
            {sendMut.isPending && (
          ...
          <div style={{ padding: '10px 16px 14px', borderTop: '1px solid var(--border)', background: 'var(--bg-card)', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
              <textarea
                ...
                placeholder={messages.some(m => m.role === 'partner') ? `Reply to ${activeThread.partnerName}'s query…` : `Message ${activeThread.partnerName}…`}
                rows={2}
                style={{
            ...
            <p style={{ margin: '6px 0 0', fontSize: 10, color: 'var(--text-muted)' }}>
              {authorityLabel} → {activeThread.partnerName} · <span style={{ fontFamily: 'monospace' }}>TASK_SEND · query_response</span> · Google A2A · auto-refreshes every 5s
            </p>
          </div>
        </div>
      ) : (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', gap: 12 }}>
          ...
          <p style={{ margin: 0, fontSize: 12 }}>Choose a {partnerLabel.toLowerCase()} thread from the left to view the A2A conversation</p>
        </div>
      )}
