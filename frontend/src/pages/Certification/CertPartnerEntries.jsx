...
import { partnerTypeLabel, partnerTypeColor, partnerTypeIcon, primaryPartnerType, SELECTABLE_PARTNER_TYPES } from '../../lib/partnerTypes'
...
export default function CertPartnerEntries() {
  ...
  const partnerRows = useMemo(() => {
    ...
    for (const p of partners) {
      const types = p.partner_types || (p.partner_type ? [p.partner_type] : ['payer_psp'])
    ...
    <div style={{ padding: 'var(--space-7) var(--space-7)', maxWidth: '1280px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-3)', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-1)' }}>
          {[{ v: 'all', l: 'All' },
            ...SELECTABLE_PARTNER_TYPES.map(v => ({ v, l: partnerTypeLabel(v) }))].map(t => (
            <button
              key={t.v}
...
function PartnerCell({ row }) {
  const primary = primaryPartnerType(row.types)
