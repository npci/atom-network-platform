...
const typeIcon = t => partnerTypeIcon(t)
...
function TestResultsPanel({ changeId, partnerId, partnerName }) {
  // Both direction labels come from the pack. They were the literals
  // "the Authority" and "Bank" — the second names a payments participant,
  // and the first only looked neutral: a deployment whose authority is the
  // NLLC still read "the Authority" here while every other surface said NLLC.
  ...
  const dirBreakdown = (runDetail?.results || []).reduce((acc, r) => {
    // Purely local accumulator keys — nothing reads them off this object
    // except the two breakdown chips below. The partner side was keyed
    // 'bank' while its label already came from the pack.
    const key = r.direction === 'partner_to_authority' ? 'partner' : 'authority'
      ...
      {runDetail && (
        <>
          <div style={{
            ...
            {(dirBreakdown.authority || dirBreakdown.partner) && (
              <span style={{ display: 'inline-flex', gap: 'var(--space-3)', fontSize: '11px' }}>
                {dirBreakdown.authority && (
                  <span title={`TCs the ${authorityLabel} side initiated`} style={{ color: 'var(--text-secondary)' }}>
                    <strong style={{ color: '#58a6ff' }}>{authorityLabel}</strong>
                    {' '}
                    <span style={{ color: '#3fb950' }}>{dirBreakdown.authority.pass}</span>
                    <span style={{ color: 'var(--text-muted)' }}>/{dirBreakdown.authority.total}</span>
                    {dirBreakdown.authority.fail > 0 && <span style={{ color: '#e06c6c' }}> · {dirBreakdown.authority.fail} fail</span>}
                  </span>
                )}
                {dirBreakdown.partner && (
                  <span title={`TCs the ${partnerLabel} side initiated`} style={{ color: 'var(--text-secondary)' }}>
                    <strong style={{ color: '#d29922' }}>{partnerLabel}</strong>
                    {' '}
                    <span style={{ color: '#3fb950' }}>{dirBreakdown.partner.pass}</span>
                    <span style={{ color: 'var(--text-muted)' }}>/{dirBreakdown.partner.total}</span>
                    {dirBreakdown.partner.fail > 0 && <span style={{ color: '#e06c6c' }}> · {dirBreakdown.partner.fail} fail</span>}
                  </span>
                )}
              </span>
            )}
                ...
                {results.map((r, idx) => {
                  ...
                  return (
                    <tr key={r.id} style={{
                      ...
                      <td style={td()}>
                        <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                          {r.direction === 'partner_to_authority' ? 'Partner → Authority' : 'Authority → Partner'}
                        </span>
                      </td>
                      <td style={td()}>
    ...
    <div style={{ padding: 'var(--space-7) var(--space-7)', maxWidth: '1200px' }}>
      ...
      {partners.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
          {partners.map(p => {
            const types = p.partner_types || ['payer_psp']
            const primary = primaryPartnerType(types)
            const meta = { color: partnerTypeColor(primary) }
            const Icon = typeIcon(primary)
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                        ...
                        {types.map(t => (
                          <span key={t} style={{
                            ...
                            padding: '1px 7px',
                            borderRadius: 999,
                            background: `${partnerTypeColor(t, meta.color)}1A`,
                            color: partnerTypeColor(t, meta.color),
                            border: `1px solid ${partnerTypeColor(t, meta.color)}40`,
                            textTransform: 'uppercase',
                            fontWeight: 600,
                            letterSpacing: '0.04em',
                          }}>
