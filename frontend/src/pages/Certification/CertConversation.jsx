...
// Cert A2A Conversation — the full certification lifecycle for one (feature, partner)
...
// mirrored in CertChangeDetail): blue = authority, amber = partner. The "soft" feel is
// carried by the *treatment* — faint lane tints + accent bars, never a fill.
const AUTHORITY = { key: 'authority', name: 'Authority Certification Agent', color: '#58a6ff', tint: 'rgba(88,166,255,0.07)', icon: Cpu }
const PARTNER = { key: 'partner', color: '#d29922', tint: 'rgba(210,153,34,0.07)', icon: Building2 }
// Pack labels are title-case for headings; sentences need the lower form.
...
// Round 2 parties — the two endpoints on the wire.
//
// "Switch" is the payments word for an interbank router. What this actually
// names is whichever endpoint each side terminates the wire on, which every
// domain has and only one calls a switch.
//
// `short` is the same identity at tab/subtitle size — the round selectors and
// section titles below all render one of these two rather than spelling the
// endpoint out again, which is how "Switch" survived the first pass.
const CERT_ENDPOINT = { name: 'Certification Endpoint', short: 'Cert endpoint', color: '#58a6ff', icon: Cpu }
const PARTNER_ENDPOINT = { name: 'Partner Endpoint', short: 'Partner endpoint', color: '#d29922', icon: Server }
...
function buildTurns(items) {
  ...
  for (const m of items) {
    ...
    const primaryParty = m.direction === 'inbound' ? PARTNER : AUTHORITY
    ...
    if (repMeaningful) {
      ...
      turns.push({
        key: m.id + '-r',
        party: primaryParty.key === AUTHORITY.key ? PARTNER.key : AUTHORITY.key,
        taskType: rt,
...
function Avatar({ party, name }) {
  const P = party === PARTNER.key ? PARTNER : AUTHORITY
  const Icon = P.icon
  return (
...
function MessageCard({ turn, partnerName, txnIds, onViewTxn, partnerLabel }) {
  const [raw, setRaw] = useState(false)
  const isAuthority = turn.party === AUTHORITY.key
  const P = isAuthority ? AUTHORITY : PARTNER
  const sender = isAuthority ? AUTHORITY.name : `${partnerName} Cert Agent`
  ...
  return (
    <div style={{ display: 'flex', justifyContent: isAuthority ? 'flex-end' : 'flex-start', margin: '12px 0' }}>
      <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, flexWrap: 'wrap' }}>
            ...
            {hasTxn && (
              <button
                ...
                title={`View the cert-endpoint ⇄ ${lc(partnerLabel)}-simulator transaction log for ${tcId}`}
                style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10.5, fontWeight: 600, color: CERT_ENDPOINT.color, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
              >
...
// ── Round 2 — online endpoint↔endpoint exchange (from precertdb) ───────────────
...
function NetworkExchange({ txn, highlight }) {
  ...
  return (
    <div id={`txn-${txn.test_case_id}`} style={{
      background: 'var(--bg-elevated)',
      border: `1px solid ${highlight ? CERT_ENDPOINT.color : 'var(--border-subtle)'}`,
      boxShadow: highlight ? `0 0 0 3px ${CERT_ENDPOINT.color}3d` : 'none',
      ...
      <div style={{ marginTop: 11, display: 'flex', flexDirection: 'column', gap: 9 }}>
        <XmlLeg dir="req" from={CERT_ENDPOINT} to={PARTNER_ENDPOINT} xml={txn.request_xml} />
        <XmlLeg dir="resp" from={PARTNER_ENDPOINT} to={CERT_ENDPOINT} xml={txn.response_xml} rc={txn.rc} rcColor={rcColor} />
      </div>
    </div>
  )
}

// ── Test case report modal — the per-case result set delivered to the partner ──
const RTh = ({ children }) => (
...
function ReportModal({ report, partnerName, cflow, onClose, partnerLabel }) {
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, width: '92vw', maxWidth: 880, maxHeight: '86vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '10px 20px', borderTop: '1px solid var(--border-subtle)', fontSize: 11, color: 'var(--text-muted)' }}>
          This result set is delivered to the {lc(partnerLabel)} in the certification sign-off (cert_test_response).
        </div>
      </div>
    </div>
  )
...
export default function CertConversation() {
  ...
  // Pack-configured counterparty label — 'Partner' unless the active domain
  // pack names its sides otherwise. Replaces the hardcoded 'Bank' copy.
  ...
  const [round, setRound] = useState(1)    // 1 = Round 1 (offline, cert endpoint ⇄ partner simulator), 2 = Round 2 (online, real partner endpoint)
  const [r1tab, setR1tab] = useState('convo')  // Round 1 sub-view: 'convo' (agent↔agent) | 'txn' (endpoint↔simulator log)
  ...
  // On cross-link, open Round 1's transaction-log view, scroll the matching transaction in + flash it.
  useEffect(() => {
    if (round !== 1 || r1tab !== 'txn' || !jumpTc) return
    const el = document.getElementById(`txn-${jumpTc}`)
  ...
  async function resetCert() {
    if (!window.confirm(
      `Reset this ${lc(partnerLabel)}'s certification conversation and run history?\n\n`
  ...
  // Test case report — the per-case result set the Authority delivers to the partner at sign-off.
  // Sourced from the cert_case_result messages, enriched with the API from the transaction log.
  ...
  // Round 1 (offline, cert endpoint ⇄ partner simulator) is what streams live — its two
  // facets are the A2A conversation (config/setup/case_result/verdict/sign-off) and
  // the endpoint↔simulator transaction log. Round 2 (online, cert endpoint ⇄ the
  // partner's real endpoint) is provisioned but not executed in this run.
    ...
    <div style={{ padding: 'var(--space-7)', maxWidth: 940 }}>
        ...
        actions={
          <>
            <button
              onClick={runCert}
              disabled={busy === 'run'}
              title={`Fire a fresh certification cycle for this ${lc(partnerLabel)} over signed A2A`}
            ...
            <button
              onClick={resetCert}
              disabled={busy === 'reset'}
              title={`Clear this ${lc(partnerLabel)}'s conversation + run history so you can demo from scratch`}
            ...
            <button
              ...
              title={`View the per-test-case certification report delivered to the ${lc(partnerLabel)}`}
      ...
      {!isLoading && model.empty && (
        <Section>
          <div style={{ textAlign: 'center', padding: 44, color: 'var(--text-muted)' }}>
            ...
            <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>No certification A2A messages yet for this {lc(partnerLabel)}.</p>
      ...
      {!isLoading && !model.empty && view && (
        <>
          ...
          {/* Round selector — Round 1 (offline: cert endpoint ⇄ partner simulator) / Round 2 (online: cert endpoint ⇄ real partner endpoint) */}
          <div style={{ display: 'flex', gap: 10, margin: '0 0 var(--space-3)', flexWrap: 'wrap' }}>
            {[
              { n: 1, kind: 'offline', label: 'Round 1 · Offline', sub: `${CERT_ENDPOINT.short} ⇄ ${lc(partnerLabel)} simulator`, icon: MessagesSquare, count: `${view.turnCount} messages · ${txns.length} transactions` },
              { n: 2, kind: 'online', label: 'Round 2 · Online', sub: `${CERT_ENDPOINT.short} ⇄ ${lc(partnerLabel)} endpoint (live)`, icon: Zap, count: '—' },
            ].map(r => {
              const on = round === r.n
          ...
          {/* Round 1 has two facets of the SAME offline round: the A2A conversation and the endpoint↔simulator transaction log */}
          {round === 1 && (
            <div style={{ display: 'flex', gap: 6, margin: '0 0 var(--space-4)', flexWrap: 'wrap' }}>
              {[
                ...
                { k: 'txn', label: 'Transaction log', sub: `${CERT_ENDPOINT.short} ↔ Simulator`, icon: ArrowRightLeft, count: txns.length },
              ].map(s => {
                const on = r1tab === s.k
                const Icon = s.icon
                return (
          ...
          {round === 1 && r1tab === 'convo' && (
            <Section
              ...
              subtitle={`${activeRun.cflow}  ·  the signed A2A lifecycle conversation  ·  a case result links to its endpoint↔simulator transaction`}
            >
              {view.byPhase.map(g => (
                <div key={g.phase.key}>
                  <PhaseDivider phase={g.phase} count={g.turns.length} />
                  {g.turns.map(t => <MessageCard key={t.key} turn={t} partnerName={model.partnerName} txnIds={txnIds} onViewTxn={onViewTxn} partnerLabel={partnerLabel} />)}
                </div>
              ))}
            </Section>
          )}
          ...
          {round === 1 && r1tab === 'txn' && (
            <Section
              title={`Round 1 · Offline — ${CERT_ENDPOINT.short} ↔ Simulator`}
              subtitle={`Network exchanges the Round 1 results were graded from — precert (certification endpoint) ⇄ sim (${lc(partnerLabel)} simulator)`}
            >
              {txns.length === 0
          ...
          {round === 2 && (
            <Section
              title={`Round 2 · Online — ${CERT_ENDPOINT.short} ↔ ${partnerLabel} endpoint`}
              subtitle={`live certification against the ${lc(partnerLabel)}'s real production endpoint`}
            >
              <div style={{ textAlign: 'center', padding: 44, color: 'var(--text-muted)' }}>
                ...
                <p style={{ margin: '6px auto 0', fontSize: 11.5, lineHeight: 1.55, maxWidth: 480 }}>
                  Round 1 (offline) certifies the {lc(partnerLabel)} against the simulator — the agent↔agent conversation and its {lc(CERT_ENDPOINT.short)} ⇄ simulator transaction log. Round 2 repeats the suite live, {lc(CERT_ENDPOINT.short)} ⇄ the {lc(partnerLabel)}'s real endpoint — provisioned here, pending enablement.
                </p>
              </div>
            </Section>
          )}
