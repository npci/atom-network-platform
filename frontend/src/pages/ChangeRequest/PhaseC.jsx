...
import { partnerTypeLabel, partnerTypeColor, partnerTypeIcon, primaryPartnerType } from '../../lib/partnerTypes'
import { AUTHORITY_REPLY_ROLES, formatDate } from '../../lib/certStatus'
// Aliased: this file uses `t` as an arrow parameter in a dozen places, and a
// bare `t` import would be shadowed by every one of them.
import { t as uiLabel } from '../../strings'
import { useCertInitiators } from '../../hooks/useUiConfig'
...
// Pack labels are title-case for headings; sentences need the lower form and
// counted phrases need a plural. `partnerLabel` defaults to 'Partner'.
//
// The plural is a heuristic, not a naive +"s": a pack is free to name its
// counterparty "Borrowing Library" or "Clearing House", and "librarys" /
// "clearing houss" would be visible nonsense on screen. Handles the -y and
// sibilant endings, which covers the labels a pack realistically ships.
const lc = (label) => (label || 'partner').toLowerCase()
...
// Title-case plural, for headings and sentence-initial use.
const titlePlural = (label, n) =>
  plural(label, n).replace(/\b[a-z]/g, c => c.toUpperCase())

const typeMetaOf = t => {
  const k = primaryPartnerType(t)
  return { label: partnerTypeLabel(k), color: partnerTypeColor(k, '#6ea8dc'), icon: partnerTypeIcon(k) }
}

// Assignment statuses at or past "this partner can be certified".
//
// Shared because it was duplicated four times as an inline array and the
// certification PANEL used a different list from the "Start cert" BUTTON that
// scrolls to it. `certifying` was in the button's list and missing from the
// panel's, so a partner in exactly the state you would certify from rendered a
// button that revealed nothing — no error, no console warning, just a dead
// control. The two must be one list.
const CERT_READY_STATUSES = ['ready_for_certification', 'ready', 'certifying']

// Statuses for which a certification panel is worth rendering: ready to start,
// mid-flight, or finished. Superset of the above — `communicated` and
// `acknowledged` are pre-migration-0031 vocabulary kept for back-compat.
const CERT_PANEL_STATUSES = [
  ...CERT_READY_STATUSES, 'certified', 'in_progress', 'communicated', 'acknowledged',
]
...
function CertPanel({ changeId, partnerId, partnerName, partnerStatus, blocked, blockedReason }) {
  ...
  const { authorityLabel, partnerLabel } = useCertInitiators()
                      ...
                      return (
                        <tr key={r.id} style={{ borderBottom: i < runDetail.results.length - 1 ? '1px solid var(--border-subtle)' : 'none' }}>
                          ...
                          <td style={{ padding: '6px 10px', color: 'var(--text-secondary)' }}>
                            {r.direction === 'authority_to_partner'
                              ? `${authorityLabel} → ${partnerLabel}`
                              : `${partnerLabel} → ${authorityLabel}`}
                          </td>
...
// ── Partner chat list (left rail) — built for 350+ partners ────────────────
//
...
function PartnerChatList({ partners, pendingByPartner, selectedPartnerId, onSelect, collapsed, onToggleCollapse }) {
  ...
  // Two-letter avatar from the partner name — handles one-word acronyms and
  // multi-word names alike.
  const avatarOf = (name) => {
...
function CounterActionCard({ changeId, partnerId, cp, onResolved }) {
  ...
  const headerLabel = isFromPartner ? `${partnerLabel} counter` : `${authorityLabel} counter (sent)`
...
function NegotiationPanel({ changeId, partnerId, partnerName, pendingMessages }) {
  const qc = useQueryClient()
  // The authority's side takes its name from the active domain pack. This was
  // the literal 'NPCI', which mislabels every authority-side bubble on any
  // other domain — the same miss AgentMessaging.jsx already fixed for its own.
  const { authorityLabel } = useCertInitiators()
  ...
  const answeredQueries = new Set(
    msgs.filter(m => AUTHORITY_REPLY_ROLES.includes(m.role) && m.correlation_id)
  ...
  // Hide the resolver card once the PM has sent an actual text reply (a role in
  // AUTHORITY_REPLY_ROLES) after the recommendation was generated. Using _recMs
  // as reference rather than _pendingMs avoids false-positives from
  // auto-generated messages (counter_decision, counter_resolution) that share
  // the same timestamp window.
  const pmRepliedAfterQuery = _recMs > 0 && visibleMsgs.some(m =>
    AUTHORITY_REPLY_ROLES.includes(m.role) && m.created_at && new Date(m.created_at).getTime() > _recMs
  )
          ...
          {openBlockers.map(b => (
            <BlockerActionCard
              ...
              onResolved={() => {
                ...
                // ALSO the partner rows. `CertPanel`'s "Blocked — forward
                // actions disabled until unblocked" banner, and the gating that
                // hides Start/Re-run behind it, read `blocked` /
                // `blocked_reason` off the partner from `phase-c-partners`.
                // Invalidating only the blocker queries left that banner
                // asserting the change was still blocked after the blocker was
                // resolved — the operator resolves, nothing visibly changes,
                // and certification looks stuck until a hard reload.
                qc.invalidateQueries({ queryKey: ['phase-c-partners', changeId] })
          ...
          for (const m of tabMsgs) {
            ...
            items.push(
              <Bubble
                ...
                name={fromPartner ? partnerName : authorityLabel}
...
const KIT_DOC_OPTIONS = [
  ['product_doc', 'Product Document'], ['faq', 'FAQ'], ['circular', uiLabel('artifact.circular')],
...
function RevisionPlanPanel({ changeId, stage = 'round_closed', openRounds = 0 }) {
  const qc = useQueryClient()
  const { partnerLabel } = useCertInitiators()
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {generated && (
                <div style={{ fontSize: 12, color: 'var(--success)', background: 'rgba(76,175,125,0.10)', border: '1px solid rgba(76,175,125,0.3)', borderRadius: 6, padding: '8px 10px' }}>
                  v{target} generated and ready. Select {plural(partnerLabel, 2)} + the latest version and ship it from the Communicate / Ship action.
                </div>
              )}
              {generating && (
...
function NegotiationHub({ changeId }) {
  ...
  const STAGE_CHIP = {
    negotiating:   { label: `Round open — ${plural(partnerLabel, 2)} responding`, bg: 'rgba(100,160,200,0.12)', fg: '#64a0c8' },
  ...
  const NEXT_STEP_HINT = {
    negotiating:   `${titlePlural(partnerLabel, 2)} are responding. Review their requests below, then Close the current round when you're ready to decide the next step.`,
    ...
    ready_to_ship: `The updated kit is ready. Send it to the ${plural(partnerLabel, 2)} with “Send kit to ${plural(partnerLabel, 2)}” at the top of the page.`,
      ...
      <div style={{ padding: '12px 20px', borderBottom: collapsed ? 'none' : '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        ...
        {stage === 'negotiating' && (
          <button
            ...
            disabled={finalizing}
            title={`Close the current round — lock the ${plural(partnerLabel, 2)}' decisions and end their response window so you can choose the next step.`}
                  ...
                  <div key={r.id} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 8px' }}>
                      ...
                      {r.status === 'open' && (
                        ...
                        >Close this {lc(partnerLabel)}'s round</button>
                      )}
                    </div>
...
export default function PhaseC() {
  const { partnerLabel, authorityLabel } = useCertInitiators()
  const { id } = useParams()
  ...
  const [shipPartnerIds, setShipPartnerIds] = useState([])
  ...
  const readyForCertCount   = (assignedPartners || []).filter(p => CERT_READY_STATUSES.includes(p.status)).length
  ...
  const openShipPanel = async () => {
    setShipResult(null)
    setShipPartnerIds((assignedPartners || []).map(p => p.partner_id))
    setShipItems({})
    setShowShipPanel(true)
    // Kick off both in parallel — partner list refresh + manifest load.
    loadShipManifest()
    try {
      const fresh = await refetchAssigned()
      if (fresh?.data) setShipPartnerIds(fresh.data.map(p => p.partner_id))
  ...
  const handleStartAllCert = async () => {
    const eligible = (assignedPartners || []).filter(
      p => CERT_READY_STATUSES.includes(p.status) && !p.blocked
    )
  ...
  const handleShipKit = async () => {
    if (!shipPartnerIds.length) return
    ...
    try {
      const res = await phaseCApi.shipKit(id, shipPartnerIds, shipVersion ?? currentVersion, checkedItems)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 0px)', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {(assignedPartners || []).length === 0 ? (
          ...
          <button
            ...
            title={`Select ${plural(partnerLabel, 2)} and a kit version, then communicate to all selected at once`}
            style={{
              display: 'flex', alignItems: 'center', gap: '6px',
      ...
      {totalAttentionItems > 0 && (
        <div style={{
          ...
          {negStage === 'negotiating' && (
            ...
            >
              <BarChart2 size={11} />
              Round open — {plural(partnerLabel, 2)} responding
            </button>
          )}
        </div>
      )}
      ...
      {/* ── Unified "Communicate Change" panel ─────────────────────────────────
          Pick {plural(partnerLabel, 2)} (any not yet assigned are assigned on ship) + an existing
          ...
          >
            ...
            <div style={{ marginBottom: '18px' }}>
              <label style={{ fontSize: '12px', fontWeight: '600', color: 'var(--text-muted)', display: 'block', marginBottom: '6px' }}>
                {titlePlural(partnerLabel, 2)} — {shipPartnerIds.length} selected · already-assigned marked ✓
              </label>
              {(allPartners || []).filter(p => p.status === 'active').length === 0 ? (
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {(allPartners || []).filter(p => p.status === 'active').map(p => {
                    const meta = typeMetaOf(p.partner_type)
                    const selected = shipPartnerIds.includes(p.id)
                    const isAssigned = assignedIds.has(p.id)
                    return (
                      <button
                        key={p.id}
                        onClick={() => setShipPartnerIds(prev => selected ? prev.filter(x => x !== p.id) : [...prev, p.id])}
                        style={{
            ...
            {shipResult && (
              <div style={{ fontSize: '12px', color: shipResult.partners_failed ? 'var(--danger)' : 'var(--success)', marginBottom: '12px', display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
                {shipResult.partners_failed ? <AlertCircle size={12} /> : <Check size={12} />} Shipped v{shipResult.negotiation_version} to {shipResult.partners_notified} {plural(partnerLabel, shipResult.partners_notified)}{shipResult.partners_failed ? ` · ${shipResult.partners_failed} failed` : ''}{shipResult.partners_skipped ? ` (${shipResult.partners_skipped} skipped)` : ''}.
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                disabled={shipping || !shipPartnerIds.length || !Object.values(shipItems).some(it => it.checked)}
                style={{
                  padding: '8px 18px', borderRadius: '6px', border: 'none', background: 'var(--accent)', color: 'white', fontSize: '13px', fontWeight: '600',
                  cursor: (shipping || !shipPartnerIds.length || !Object.values(shipItems).some(it => it.checked)) ? 'not-allowed' : 'pointer',
                  opacity: (shipping || !shipPartnerIds.length || !Object.values(shipItems).some(it => it.checked)) ? 0.5 : 1,
                  display: 'inline-flex', alignItems: 'center', gap: '6px',
                }}
              >
                {shipping ? <Loader size={13} style={{ animation: 'spin 1s linear infinite' }} /> : <Send size={13} />}
                Ship {Object.values(shipItems).filter(it => it.checked).length} item{Object.values(shipItems).filter(it => it.checked).length === 1 ? '' : 's'} to {shipPartnerIds.length} {plural(partnerLabel, shipPartnerIds.length)}
              </button>
            </div>
          </div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px' }}>
        ...
        {shipResult && (
          /* Failed deliveries are reported as failures, not as a smaller success.
             The banner used to read "shipped to N partners" off an attempt count,
             so a kit every partner refused rendered green. */
          <div style={{ padding: '12px 16px', borderRadius: '8px', marginBottom: '16px', background: shipResult.partners_failed ? 'rgba(224,108,108,0.08)' : 'rgba(76,175,125,0.08)', border: `1px solid ${shipResult.partners_failed ? 'rgba(224,108,108,0.25)' : 'rgba(76,175,125,0.25)'}`, display: 'flex', alignItems: 'center', gap: '8px' }}>
            {shipResult.partners_failed
              ? <AlertCircle size={14} style={{ color: 'var(--danger)' }} />
              : <CheckCircle size={14} style={{ color: 'var(--success)' }} />}
            <span style={{ fontSize: '13px', color: shipResult.partners_failed ? 'var(--danger)' : 'var(--success)' }}>
              Kit v{shipResult.negotiation_version} shipped to {shipResult.partners_notified} {plural(partnerLabel, shipResult.partners_notified)}
              {shipResult.partners_failed ? ` · ${shipResult.partners_failed} failed to deliver` : ''}
          ...
          {(!assignedPartners || assignedPartners.length === 0) && (
            <div style={{ padding: '32px', textAlign: 'center' }}>
              <p style={{ margin: 0, fontSize: '13px', color: 'var(--text-muted)' }}>
                No partners assigned yet. Click "Assign Partner" to add ecosystem {plural(partnerLabel, 2)}.
              </p>
            </div>
          )}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                ...
                <tbody>
                  {progressGrid.partners.flatMap((p, i) => {
                    ...
                    // `p.last_updated` (progress grid), NOT `ap.updated_at`:
                    // `change_partner_assignments` has no `updated_at` column,
                    // so the old read was always undefined and this cell showed
                    // "—" for every partner forever, however much they reported.
                    const updatedAt = p.last_updated
                    const updatedStr = updatedAt
                      ? formatDate(updatedAt, { day: 'numeric', month: 'short', year: '2-digit' })
                      : '—'
                    const counters = ap?.open_counters_count || 0
                    const blockers = ap?.open_blockers_count || 0
                    const isReadyForCert = CERT_READY_STATUSES.includes(statusKey)
        ...
        {(assignedPartners || []).filter(p => CERT_PANEL_STATUSES.includes(p.status)).length > 0 && (
          ...
          <div style={{ borderRadius: '8px', background: 'var(--bg-elevated)', border: '1px solid var(--border)', overflow: 'hidden' }}>
            ...
            {showMessageLog && messages.map((m, i) => (
              <div key={m.task_id} style={{
                ...
                }}>{m.direction === 'outbound' ? `${authorityLabel} →` : `→ ${authorityLabel}`}</span>
