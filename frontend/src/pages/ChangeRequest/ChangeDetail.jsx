...
import { partnerTypeLabel, partnerTypeColor } from '../../lib/partnerTypes'
import {
...
const PHASE_B_STEP_DESCRIPTIONS = {
  ...
  is_review:    'Information-security review against OWASP Top 10 + the ecosystem security standards',
...
// DETAILS labels that already have a dedicated column in the table below.
// Everything ELSE the document declares is a party column — see
// _detailPartyFields.
const _CERT_FIXED_DETAIL_LABELS = new Set([
  'API Involved', 'Type', 'Entity Involved', 'Approval Type',
])
// A malformed document must not be able to widen the table without bound.
const _CERT_MAX_PARTY_COLS = 4
// A `Label: value` line: the label is anchored at column 0 exactly as
// _detailField requires, starts with a letter, is short enough to be a field
// name rather than a sentence that happens to contain a colon, and is not
// followed by `//` — which is what stops a bare `https://...` line in the block
// from producing an "https" column.
const _CERT_DETAIL_LINE = /^([A-Za-z][A-Za-z0-9 ()/_-]{0,39}):(?!\/)(.*)$/

/**
 * The per-party columns this document declares, in first-appearance order.
 *
 * These USED to be two hardcoded reads of 'Payer Handle' and 'Payee Handle',
 * rendered under two hardcoded payments headers — on every deployment, whatever
 * its domain. The DETAILS block is generated from the active pack's own
 * certification vocabulary, so the labels IN THE DOCUMENT are already the right
 * words; reading them back out is what makes the table domain-neutral. A pack
 * that names no parties produces no such lines, and the columns disappear
 * rather than rendering a rail of em-dashes.
 *
 * (The cleaner source would be a `cert_vocabulary` party list on /config/ui —
 * the pack has one, as `summary_role_headers` — but that route does not expose
 * it today and the backend is outside this change.)
 */
function _detailPartyFields(detailsBlock) {
  const out = []
  for (const line of String(detailsBlock || '').split('\n')) {
    const m = line.match(_CERT_DETAIL_LINE)
    if (!m) continue
    const label = m[1].trim()
    if (!label || _CERT_FIXED_DETAIL_LABELS.has(label)) continue
    out.push({ label, value: m[2].replace(/\s+/g, ' ').trim() })
  }
  return out
}

// Union of the party labels across every case, capped, preserving the order
// the document introduced them in.
function _certPartyColumns(testCases) {
  const cols = []
  for (const tc of testCases || []) {
    for (const f of tc.party_fields || []) {
      if (!cols.includes(f.label)) cols.push(f.label)
      if (cols.length >= _CERT_MAX_PARTY_COLS) return cols
    }
  }
  return cols
}
...
function parseCertTestCasesMd(md) {
  ...
  for (const chunk of caseChunks) {
    ...
    cases.push({
      ...
      party_fields:   _detailPartyFields(details_block),
      details_block,
      description_block,
      steps_block,
      response_code,
...
function CertTestCaseRow({ tc, index, partyCols = [] }) {
  const [open, setOpen] = useState(false)
  const statusColor = CERT_STATUS_COLORS[tc.expected_status] || 'var(--text-muted)'
  const partyValue = (label) =>
    (tc.party_fields || []).find(f => f.label === label)?.value || ''
  return (
    <>
      <tr
        onClick={() => setOpen(v => !v)}
      ...
      >
        ...
        {partyCols.map(label => (
          <td key={label} style={{ ..._certTd, whiteSpace: 'nowrap' }}>{partyValue(label) || '—'}</td>
        ))}
      ...
      {open && (
        <tr style={{ background: 'var(--bg-base)' }}>
          {/* 8 fixed columns (chevron, TC ID, Status, APIs, Type, Entities,
              Approval, Resp) plus however many parties the pack declares. */}
          <td colSpan={8 + partyCols.length} style={{ padding: '12px 20px 16px 44px', borderBottom: '1px solid var(--border-subtle)' }}>
...
function CertTestCasesView({ markdown }) {
  ...
  // Computed before the early return so the hook order is unconditional.
  const partyCols = useMemo(() => _certPartyColumns(plan?.test_cases), [plan])
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              ...
              {partyCols.map(label => <th key={label} style={_certTh}>{label}</th>)}
              <th style={_certTh}>Resp</th>
            </tr>
          </thead>
          <tbody>
            {plan.test_cases.map((tc, i) => (
              <CertTestCaseRow key={tc.test_id || i} tc={tc} index={i} partyCols={partyCols} />
            ))}
          </tbody>
        </table>
      </div>
...
// True when this result was reported by the partner rather than the authority.
// The comparison is case-insensitive because the two ends disagree on case: the
// pack declares the token upper-cased (`cert_roles.partner`) and the emitter
// lower-cases it onto the payload.
function isPartnerReported(actualResponse, partnerToken) {
  const reporter = String((actualResponse || {}).reporter || '').trim().toLowerCase()
  const token = String(partnerToken || '').trim().toLowerCase()
  return Boolean(reporter) && Boolean(token) && reporter === token
}

function CertificationPanel({ changeId }) {
  const qc = useQueryClient()
  // Wire tokens for the two sides, from the active pack.
  const { partnerToken } = useCertInitiators()
                ...
                <div key={r.id} style={{ borderTop: '1px solid var(--border-subtle)', padding: '5px 0' }}>
                  <div onClick={() => setOpenCase(isOpen ? null : r.id)}
                    ...
                    <span style={{ fontSize: 10.5, color: 'var(--text-muted)' }}>
                      {/* `reporter` is the pack's PARTNER token, lower-cased by
                          the emitter. Compare against that token, not a literal:
                          on any pack whose partner token is not "bank" a literal
                          comparison silently falls through to the raw direction
                          string. */}
                      {isPartnerReported(r.actual_response, partnerToken) ? 'executed by partner' : r.direction}
              ...
              return (
                <div key={p.partner_id} style={{
                  ...
                  <span style={{
                    ...
                    background: `${partnerTypeColor(p.partner_type, '#b388e8')}1A`,
                    color: partnerTypeColor(p.partner_type, '#b388e8'),
                    textTransform: 'uppercase',
                  }}>{partnerTypeLabel(p.partner_type)}</span>
