...
import { formatDate, formatDateTime } from '../../lib/certStatus'
...
const DOC_TYPES = [
  ...
  },
  {
    ...
    desc:  '2–3 min step-by-step tutorial for partner developer teams',
    icon:  Video,
  },
  {
    key:   'faq',
    label: 'FAQ Document',
    desc:  'Q&A for end users, providers, and partners',
    icon:  HelpCircle,
  },
  {
    key:   'cert_test_cases',
  ...
  },
  {
    ...
    desc:  `Official technical ${t('artifact.circular').toLowerCase()} to ecosystem participants`,
    icon:  Bell,
  },
  {
    key:   'manifest',
  ...
  },
  {
    ...
    desc:  'HTML wireframe mockups for partner UI teams',
    icon:  Monitor,
  },
  {
...
function DocPanel({ changeId, docDef, onGenerated, autogen = false }) {
  ...
  // The certification body's own name for this deployment.
            ...
            <>
              ...
              {docDef.key === 'cert_test_cases' && (
                <button
                  ...
                  title={`Download the ${authorityLabel}-format Excel workbook`}
                >
                  {xlsxDownloading
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ margin: 0, fontSize: '13px', color: 'var(--text-muted)' }}>
                {readOnly
                  ? `${authorityLabel} hasn’t published this document yet.`
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              ...
              <th style={{ ...certTh(), width: 140 }}>INITIATED BY</th>
...
// Map test-ID prefix → role sheet name for engine-generated packs whose JSON is
// a flat test_cases list without sheet grouping.
//
// The prefixes are PACK DATA, served by /config/ui as `cert_sheet_prefixes`.
// This list used to be hardcoded here as PR_/PE_/RE_/BE_ → Payer PSP/Payee
// PSP/Remitter/Beneficiary, which is one ecosystem's convention: the backend
// has read these from `cert_vocabulary.role_prefixes` for as long as packs have
// existed (cert_catalogue.py, tc_store_sync.py), so this was the last copy
// asserting it, and any other pack's workbook rendered entirely under "Other".
//
// An empty list is a legitimate answer (a domain that declares no prefixes);
// everything then falls through to the generic bucket, which is what the old
// hardcoded map did for unrecognised ids anyway.
function inferSheetFromTestId(testId, sheetPrefixes) {
  ...
  // Structural, not domain vocabulary — these stay in code.
  ...
  for (const { prefix, sheet } of sheetPrefixes || []) {
    if (prefix && t.startsWith(String(prefix).toUpperCase())) return sheet
  }
  return 'Test cases'
}

function groupCasesBySheet(cases, sheetPrefixes) {
  ...
  for (const tc of cases) {
    ...
    const key = tc.sheet || inferSheetFromTestId(tc.test_id, sheetPrefixes)
    if (!key) continue
...
function CertTestCasesTable({ plan }) {
  ...
  const { certSheetPrefixes } = useCertInitiators()
  let sheets
  if (Array.isArray(plan?.sheets) && plan.sheets.length > 0) {
    ...
    sheets = groupCasesBySheet(plan.test_cases, certSheetPrefixes)
  } else {
    sheets = []
  }
  if (sheets.length === 0) {
