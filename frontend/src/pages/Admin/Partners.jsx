...
import { partnerTypeLabel, partnerTypeColor, partnerTypeIcon } from '../../lib/partnerTypes'
...
// The operator-selectable partner roles. cert_engine is the
...
// Label, colour and icon all come from lib/partnerTypes so every surface
// agrees; only the `internal` flag is registry-specific and stays here.
const roleMeta = (key, extra) => ({
  label: partnerTypeLabel(key),
  color: partnerTypeColor(key),
  icon:  partnerTypeIcon(key),
  ...extra,
})
const TYPE_META = {
  payer_psp:   roleMeta('payer_psp'),
  payee_psp:   roleMeta('payee_psp'),
  remitter:    roleMeta('remitter'),
  beneficiary: roleMeta('beneficiary'),
  cert_engine: roleMeta('cert_engine', { internal: true }),
}
...
export default function Partners() {
  ...
  // The create + each rotate endpoint may return any
  ...
  // Cert-Agent partner short-code. Required for the readiness
  ...
  // Rotate the per-partner JWT signing secret. Returned ONCE.
  ...
  // Rotate the per-partner HMAC envelope secret. Returned ONCE.
  ...
  // Emergency lever: revoke every active a2a_sessions row.
  ...
  const startEdit = (p) => {
    ...
    setEditPartnerCode(p.cert_agent_partner_id ?? p.cert_agent_bank_id ?? '')
  }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await partnersApi.update(editingId, {
        ...
        cert_agent_partner_id: editPartnerCode.trim(),
      })
      setEditingId(null)
      qc.invalidateQueries({ queryKey: ['partners'] })
      qc.invalidateQueries({ queryKey: ['partner-stats'] })
  ...
  // Flip A2A wire (legacy ↔ a2a_sdk). Backend audits the change
  ...
  const summary = {
    ...
    // Provider tier = the two provider-side roles; institution tier = the two
    // account-holding roles. Split kept because the registry counts them apart.
    providers:    partnerList.filter(p => _hasType(p, 'payer_psp', 'payee_psp')).length,
    institutions: partnerList.filter(p => _hasType(p, 'remitter', 'beneficiary')).length,
  }
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 1600, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: '13px', color: 'var(--text-muted)' }}>
            Manage ecosystem partners for A2A communication
          </p>
        </div>
      ...
      <StatTileRow>
        ...
        <StatTile label="Institutions"   value={summary.institutions} accent="#6ea8dc" />
        <StatTile label="Providers"      value={summary.providers}    accent="#b388e8" />
      </StatTileRow>
      ...
      {newSecrets && (
        <div style={{ padding: '14px 16px', borderRadius: '8px', marginBottom: '16px', background: 'rgba(76,175,125,0.08)', border: '1px solid rgba(76,175,125,0.25)' }}>
          ...
          {/* One row per secret the backend returned. Create returns
              api_key + jwt_signing_secret + signing_secret
        ...
        <div style={{ padding: '20px', marginBottom: '24px', borderRadius: '8px', background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '14px' }}>
            <div>
              ...
              <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Acme Institution"
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '24px' }}>
        {[
          ...
          { label: partnerTypeLabel('payer_psp'),   value: stats?.by_type?.payer_psp   || 0 },
          { label: partnerTypeLabel('payee_psp'),   value: stats?.by_type?.payee_psp   || 0 },
          { label: partnerTypeLabel('remitter'),    value: stats?.by_type?.remitter    || 0 },
          { label: partnerTypeLabel('beneficiary'), value: stats?.by_type?.beneficiary || 0 },
        ].map(s => (
      ...
      {partners && partners.length > 0 && (
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', alignItems: 'center' }}>
          ...
          <select value={filterType} onChange={e => setFilterType(e.target.value)} style={{
            ...
            {/* Values are the wire/DB enum; labels come from lib/partnerTypes
                so this filter reads the same as the chips it filters on. */}
            {Object.keys(TYPE_META).map(key => (
              <option key={key} value={key}>{partnerTypeLabel(key)}</option>
            ))}
          </select>
            ...
            return (
              <div key={p.id} style={{
                ...
                {editingId === p.id ? (
                  <div style={{ marginBottom: '14px' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.2fr 1.2fr 0.8fr', gap: '10px', marginBottom: '12px' }}>
                      ...
                      <div>
                        <label
                          title="Cert-Agent partner short-code. Must match a partner configured in cert-agent's partner config. Required before this partner can run cert tests."
                          style={{ display: 'block', fontSize: '10px', color: 'var(--text-muted)', marginBottom: '3px', textTransform: 'uppercase', letterSpacing: '0.04em' }}
                        >
                          Cert-Agent Partner ID
                        </label>
                        <input
                          value={editPartnerCode}
                          onChange={e => setEditPartnerCode(e.target.value.toUpperCase())}
                          placeholder={t('ph.partners.certAgentId')}
                          style={{
                            width: '100%', padding: '7px 10px', borderRadius: '5px', border: '1px solid var(--border)',
                            background: 'var(--bg-input)', color: 'var(--text-primary)', fontSize: '13px',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: testResult ? '10px' : 0, flexWrap: 'wrap' }}>
                  ...
                  {/* Wire selector. Default 'legacy' = hand-rolled
                      ...
                      {isExpanded && (
                        <div style={{
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr 180px', gap: 12, marginBottom: 12 }}>
                            ...
                            <div>
                              ...
                              <select
                                ...
                                <option value="mtls">mtls — :8443 (mTLS tier)</option>
                              </select>
                            </div>
                            {/* fingerprint */}
                            <div>
                          ...
                          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                            <button onClick={() => handleRotateJwtSecret(p.id)} title="Rotate JWT signing secret" style={{
                              padding: '6px 10px', background: 'transparent', border: '1px solid var(--border)',
                              borderRadius: '5px', fontSize: 11, color: 'var(--text-secondary)', cursor: 'pointer',
                              display: 'flex', alignItems: 'center', gap: 4,
                            }}><RotateCcw size={11} /> Rotate JWT Secret</button>
                            <button onClick={() => handleRotateHmacSecret(p.id)} title="Rotate HMAC envelope secret" style={{
                              padding: '6px 10px', background: 'transparent', border: '1px solid var(--border)',
                              borderRadius: '5px', fontSize: 11, color: 'var(--text-secondary)', cursor: 'pointer',
                              display: 'flex', alignItems: 'center', gap: 4,
                            }}><RotateCcw size={11} /> Rotate HMAC Secret</button>
                            <button onClick={() => handleRevokeSessions(p.id)} title="Revoke all active A2A sessions" style={{
