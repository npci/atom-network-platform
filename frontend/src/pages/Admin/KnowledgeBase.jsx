...
// Colour per category KEY. The keys are persisted DocCategory values, so they
// are stable and belong in code; only their display LABELS are domain wording,
// and those come from the API (see useCategoryLabels below).
const CATEGORY_COLORS = {
  rbi_guideline:   { bg: 'rgba(100,160,220,0.12)', color: '#6ea8dc' },
  network_product_doc: { bg: 'rgba(76,175,125,0.12)',  color: '#4caf7d' },
...
// Category labels, from /rag/categories.
//
// Do not keep a second copy of the label map here. A local copy drifts from the
// backend's DEFAULT_CATEGORIES and ends up rendering a hardcoded regulator name
// to every deployment. The backend resolves that label from the active pack's
// `regulatory_body`, so the only correct source is the API.
//
// react-query dedupes by key, so calling this from each Badge costs one request
// for the page regardless of how many badges render.
function useCategoryLabels() {
  ...
  return Object.fromEntries((list || []).map(c => [c.value, c.label]))
}
