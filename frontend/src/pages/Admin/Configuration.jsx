...
export default function Configuration() {
  ...
  // The AI section's connectivity test is labelled from the CONFIGURED provider
  // rather than from one vendor's name: this platform is not built for a single
  // LLM vendor, and an operator on any other one was previously shown a button
  // offering to test a product they do not run. `edits` holds the value
  // currently in the form, so the label tracks an unsaved provider change too.
  // Falls back to generic wording when the deployment declares no provider.
  const aiProvider = String(edits.llm_provider || '').trim()
  const aiTestLabel = aiProvider ? `Test ${aiProvider}` : 'Test connection'
  ...
  const handleTest = async (type) => {
    ...
    try {
      const fn = type === 'gitlab' ? configApi.testGitlab : configApi.testAiProvider
      const res = await fn()
        ...
        return (
          <div key={category} style={{
            ...
            <div style={{
              ...
              {category === 'ai' && (
                <button onClick={() => handleTest('ai')} disabled={testing.ai}
                  title={aiProvider
                    ? `Probe the AI host configured for provider "${aiProvider}"`
                    : 'Probe the configured AI host'}
                  style={{
                    display: 'flex', alignItems: 'center', gap: '5px',
                    padding: '5px 12px', background: 'transparent', border: '1px solid var(--border)',
                    borderRadius: '5px', fontSize: '11px', color: 'var(--text-muted)', cursor: testing.ai ? 'not-allowed' : 'pointer',
                  }}>
                  {testing.ai ? <Loader size={11} style={{ animation: 'spin 1s linear infinite' }} /> : <Wifi size={11} />}
                  {aiTestLabel}
                </button>
              )}
            </div>
            ...
            {(testResults.ai && category === 'ai') && (
              <div style={{
                padding: '8px 20px', borderBottom: '1px solid var(--border-subtle)',
                background: testResults.ai.status === 'ok' ? 'rgba(76,175,125,0.06)' : 'rgba(224,108,108,0.06)',
                display: 'flex', alignItems: 'center', gap: '8px',
              }}>
                {testResults.ai.status === 'ok'
                  ? <CheckCircle size={13} style={{ color: 'var(--success)' }} />
                  : <AlertCircle size={13} style={{ color: 'var(--danger)' }} />}
                <span style={{ fontSize: '12px', color: testResults.ai.status === 'ok' ? 'var(--success)' : 'var(--danger)' }}>
