                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}
                    ...
                    >
                      {/* Options come from the active domain pack's declared
                          topology; falls back to the historical network vocabulary
                ...
                {isIndexing && (
                  <div style={{
                    ...
                    <span style={{ fontSize: '12px', color: 'var(--accent)' }}>
                      Fetching files from GitLab, chunking, and generating embeddings…
                    </span>
                  </div>
                )}
...
function RepoContextPanel({ repoId }) {
  ...
  return (
    <div style={wrap}>
      {/* Reuse-first flow map: which APIs carry the domain's primary operation.
          BACKEND-COUPLED: the split below is `flow.transaction_apis` vs
          `flow.meta_apis`, named by the indexer, not by this component. Another
          worker is making that side pack-driven; until it is, the label here
          says what the field means in any domain rather than naming one
          domain's operation. */}
      {flow && (
        <div style={{ marginBottom: 14, padding: '10px 12px', borderRadius: 6,
          ...
          {flow.transaction_apis?.length > 0 && (
            <div style={muted}><strong style={{ color: '#16a34a' }}>primary operation:</strong>{' '}
