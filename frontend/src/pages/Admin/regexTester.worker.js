...
// Vite-bundled worker, so it satisfies the prod CSP — which now names
// `worker-src 'self' blob:` explicitly rather than relying on the script-src
// fallback. (When this line was written the prod CSP it cites did not yet
// exist: frontend/nginx.conf.template shipped with no add_header block at all.
// It does now, and that file is the authority for what is enforced.)
//
// ReDoS / client-side regex injection (CWE-1333) — the
