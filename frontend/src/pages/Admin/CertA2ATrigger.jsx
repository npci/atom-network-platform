...
//   inbound  — we play the partner and post at our OWN ingress, so the receive
...
// `direction` on a template is a WIRE value produced by the backend, and three
// spellings are in circulation:
//   - `authority_to_partner`, the neutral form the platform is migrating to;
//   - `<authority>_to_<partner>` built from the pack's own `cert_roles` tokens;
//   - `npci_to_bank`, the pre-migration literal, still emitted by deployments
//     running an older backend (models/phase_c.py keeps the same legacy alias).
// All three are ACCEPTED here on purpose. This predicate gates which templates
// an operator may fire in `inbound` mode, so a spelling it fails to recognise
// does not error — it silently leaves every template selectable, which is the
// guard not working at all.
const LEGACY_AUTHORITY_TO_PARTNER = 'npci_to_bank'

function isAuthorityOriginated(direction, authorityToken, partnerToken) {
  const d = String(direction || '').trim().toLowerCase()
  if (!d) return false
  if (d === 'authority_to_partner' || d === LEGACY_AUTHORITY_TO_PARTNER) return true
  const a = String(authorityToken || '').trim().toLowerCase()
  const p = String(partnerToken || '').trim().toLowerCase()
  return Boolean(a) && Boolean(p) && d === `${a}_to_${p}`
}
...
const MODES = [
  ...
  { id: 'inbound',  label: 'Simulate inbound',   icon: ArrowDownLeft, hint: 'We act as the partner, posting at our own ingress' },
]

export default function CertA2ATrigger() {
  // Partner-side label from the pack — this form said "Bank to impersonate".
  // The tokens drive the direction predicate below.
  const { partnerLabel, authorityToken, partnerToken } = useCertInitiators()
  ...
  // Which task types are legal for the chosen mode. `sendable` is the
  // backend's own boolean for "the authority can originate this"; there is no
  // matching one for the inbound direction, so that side is derived.
  const available = useMemo(() => templates.filter(t => (
    mode === 'inbound'
      ? !isAuthorityOriginated(t.direction, authorityToken, partnerToken)
      : t.sendable
  )), [templates, mode, authorityToken, partnerToken])
  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 1100 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
        {needsPartner ? (
          <Field label={mode === 'inbound' ? `${partnerLabel} to impersonate` : 'Partner'}>
