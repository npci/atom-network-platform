...
// Agentic XSD-driven codegen — chat-the-intent flow.
export const agenticApi = {
  ...
  // Reuse-first approach gate + XSD refine loop
  ...
  // ADR-0005 — resume a run parked at the TSD approval gate.
...
export const configApi = {
  ...
  // Connectivity probe for the AI category. The ROUTE is named for the only
  // host it can currently reach (the local model server that serves the
  // embedding model); the member is named for the category so the caller does
  // not have to spell one vendor to test its AI settings. Renaming the route
  // is a backend change and is not this file's to make.
  testAiProvider: ()   => api.post('/admin/config/test-ollama'),
}
...
export const evalApi = {
  ...
  // Re-run advisory eval on an existing change
...
export const partnersApi = {
  ...
  // A2A wire selector. body: {protocol_version: 'legacy'|'a2a_sdk'}
  setProtocol: (id, protocol_version) =>
    api.patch(`/admin/partners/${id}/protocol`, { protocol_version }),

  // ── A2A security hardening endpoints ──
  // Rotate the per-partner JWT signing secret (HS256, used
  ...
  // Rotate the per-partner HMAC envelope secret (signs the
  // signature header on every outbound body).
  rotateHmacSecret: (id) => api.post(`/admin/partners/${id}/rotate-hmac-secret`),
  // Partner-tier mTLS controls. tls_tier flips the partner
  ...
  // Network controls. allowed_cidrs is null/[] for "no
  ...
  // Revoke all active sessions for a partner (emergency lever).
...
// Clarifications — pre-generation gap questions for PM
...
// Artifact download — historically named "docxApi"; now also
